...
type CoreReviewListResponse struct {
	Data       []*CoreReview `json:"data"`
...
func NewCoreReview(name string) *CoreReview {
	now := time.Now()
	return &CoreReview{
		Name:      name,
		Status:    "pending",
...
func (m *CoreReview) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("core_review: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("core_review: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("core_review: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("core_review: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("core_review: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("core_review: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("core_review: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("core_review: version must be at least 1")
	}
	return nil
}

func (m *CoreReview) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("core_review: marshal failed: %w", err)
	}
	return data, nil
}

func (m *CoreReview) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("core_review: unmarshal failed: %w", err)
	}
	return nil
}

func (m *CoreReview) Clone() *CoreReview {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *CoreReview) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("core_review: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("core_review: transition from %q to %q not allowed", m.Status, status)
}

func (m *CoreReview) Sanitize() {
...
func (m *CoreReview) Merge(other *CoreReview) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *CoreReview) String() string {
	return fmt.Sprintf("CoreReview[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *CoreReview) MatchesFilter(f *CoreReviewFilter) bool {
...
func FilterCoreReviewList(items []*CoreReview, f *CoreReviewFilter) *CoreReviewListResponse {
	var filtered []*CoreReview
...
func CalculateCoreReviewStats(items []*CoreReview) *CoreReviewStats {
	stats := &CoreReviewStats{}
	if len(items) == 0 {
		return stats
	}
