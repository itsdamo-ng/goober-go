...
type ImportEventListResponse struct {
	Data       []*ImportEvent `json:"data"`
...
func NewImportEvent(name string) *ImportEvent {
	now := time.Now()
	return &ImportEvent{
		Name:      name,
		Status:    "pending",
...
func (m *ImportEvent) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("import_event: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("import_event: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("import_event: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("import_event: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("import_event: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("import_event: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("import_event: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("import_event: version must be at least 1")
	}
	return nil
}

func (m *ImportEvent) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("import_event: marshal failed: %w", err)
	}
	return data, nil
}

func (m *ImportEvent) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("import_event: unmarshal failed: %w", err)
	}
	return nil
}

func (m *ImportEvent) Clone() *ImportEvent {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *ImportEvent) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("import_event: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("import_event: transition from %q to %q not allowed", m.Status, status)
}

func (m *ImportEvent) Sanitize() {
...
func (m *ImportEvent) Merge(other *ImportEvent) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *ImportEvent) String() string {
	return fmt.Sprintf("ImportEvent[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *ImportEvent) MatchesFilter(f *ImportEventFilter) bool {
...
func FilterImportEventList(items []*ImportEvent, f *ImportEventFilter) *ImportEventListResponse {
	var filtered []*ImportEvent
	...
	return &ImportEventListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateImportEventStats(items []*ImportEvent) *ImportEventStats {
	stats := &ImportEventStats{}
	if len(items) == 0 {
		return stats
	}
