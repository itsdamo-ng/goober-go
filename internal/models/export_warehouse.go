...
type ExportWarehouseFilter struct {
	IDs       []int64   `json:"ids"`
...
type ExportWarehouseListResponse struct {
	Data       []*ExportWarehouse `json:"data"`
...
type ExportWarehouseStats struct {
...
func NewExportWarehouse(name string) *ExportWarehouse {
	now := time.Now()
...
func (m *ExportWarehouse) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("export_warehouse: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("export_warehouse: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("export_warehouse: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("export_warehouse: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("export_warehouse: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("export_warehouse: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("export_warehouse: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("export_warehouse: version must be at least 1")
	}
	return nil
}

func (m *ExportWarehouse) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("export_warehouse: marshal failed: %w", err)
	}
	return data, nil
}

func (m *ExportWarehouse) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("export_warehouse: unmarshal failed: %w", err)
	}
	return nil
}

func (m *ExportWarehouse) Clone() *ExportWarehouse {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *ExportWarehouse) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("export_warehouse: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("export_warehouse: transition from %q to %q not allowed", m.Status, status)
}

func (m *ExportWarehouse) Sanitize() {
...
func (m *ExportWarehouse) Merge(other *ExportWarehouse) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *ExportWarehouse) String() string {
	return fmt.Sprintf("ExportWarehouse[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *ExportWarehouse) MatchesFilter(f *ExportWarehouseFilter) bool {
...
func FilterExportWarehouseList(items []*ExportWarehouse, f *ExportWarehouseFilter) *ExportWarehouseListResponse {
	var filtered []*ExportWarehouse
	...
	return &ExportWarehouseListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateExportWarehouseStats(items []*ExportWarehouse) *ExportWarehouseStats {
