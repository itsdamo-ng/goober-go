...
type ShippingInvoiceFilter struct {
	IDs       []int64   `json:"ids"`
...
type ShippingInvoiceListResponse struct {
	Data       []*ShippingInvoice `json:"data"`
...
type ShippingInvoiceStats struct {
...
func NewShippingInvoice(name string) *ShippingInvoice {
	now := time.Now()
...
func (m *ShippingInvoice) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("shipping_invoice: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("shipping_invoice: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("shipping_invoice: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("shipping_invoice: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("shipping_invoice: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("shipping_invoice: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("shipping_invoice: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("shipping_invoice: version must be at least 1")
	}
	return nil
}

func (m *ShippingInvoice) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("shipping_invoice: marshal failed: %w", err)
	}
	return data, nil
}

func (m *ShippingInvoice) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("shipping_invoice: unmarshal failed: %w", err)
	}
	return nil
}

func (m *ShippingInvoice) Clone() *ShippingInvoice {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *ShippingInvoice) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("shipping_invoice: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("shipping_invoice: transition from %q to %q not allowed", m.Status, status)
}

func (m *ShippingInvoice) Sanitize() {
...
func (m *ShippingInvoice) Merge(other *ShippingInvoice) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *ShippingInvoice) String() string {
	return fmt.Sprintf("ShippingInvoice[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *ShippingInvoice) MatchesFilter(f *ShippingInvoiceFilter) bool {
...
func FilterShippingInvoiceList(items []*ShippingInvoice, f *ShippingInvoiceFilter) *ShippingInvoiceListResponse {
	var filtered []*ShippingInvoice
	...
	return &ShippingInvoiceListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateShippingInvoiceStats(items []*ShippingInvoice) *ShippingInvoiceStats {
