...
type AdminWarehouseListResponse struct {
	Data       []*AdminWarehouse `json:"data"`
...
type AdminWarehouseStats struct {
...
func NewAdminWarehouse(name string) *AdminWarehouse {
	now := time.Now()
...
func (m *AdminWarehouse) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("admin_warehouse: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_warehouse: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_warehouse: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_warehouse: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_warehouse: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_warehouse: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_warehouse: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_warehouse: version must be at least 1")
	}
	return nil
}

func (m *AdminWarehouse) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_warehouse: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminWarehouse) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_warehouse: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminWarehouse) Clone() *AdminWarehouse {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminWarehouse) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_warehouse: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_warehouse: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminWarehouse) Sanitize() {
...
func (m *AdminWarehouse) Merge(other *AdminWarehouse) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminWarehouse) String() string {
	return fmt.Sprintf("AdminWarehouse[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminWarehouse) MatchesFilter(f *AdminWarehouseFilter) bool {
...
func FilterAdminWarehouseList(items []*AdminWarehouse, f *AdminWarehouseFilter) *AdminWarehouseListResponse {
	var filtered []*AdminWarehouse
...
func CalculateAdminWarehouseStats(items []*AdminWarehouse) *AdminWarehouseStats {
