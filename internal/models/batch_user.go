...
type BatchUserListResponse struct {
	Data       []*BatchUser `json:"data"`
...
func NewBatchUser(name string) *BatchUser {
	now := time.Now()
	return &BatchUser{
		Name:      name,
		Status:    "pending",
...
func (m *BatchUser) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("batch_user: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("batch_user: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("batch_user: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("batch_user: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("batch_user: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("batch_user: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("batch_user: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("batch_user: version must be at least 1")
	}
	return nil
}

func (m *BatchUser) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("batch_user: marshal failed: %w", err)
	}
	return data, nil
}

func (m *BatchUser) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("batch_user: unmarshal failed: %w", err)
	}
	return nil
}

func (m *BatchUser) Clone() *BatchUser {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *BatchUser) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("batch_user: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("batch_user: transition from %q to %q not allowed", m.Status, status)
}

func (m *BatchUser) Sanitize() {
...
func (m *BatchUser) Merge(other *BatchUser) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *BatchUser) String() string {
	return fmt.Sprintf("BatchUser[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *BatchUser) MatchesFilter(f *BatchUserFilter) bool {
...
func FilterBatchUserList(items []*BatchUser, f *BatchUserFilter) *BatchUserListResponse {
	var filtered []*BatchUser
...
func CalculateBatchUserStats(items []*BatchUser) *BatchUserStats {
	stats := &BatchUserStats{}
	if len(items) == 0 {
		return stats
	}
