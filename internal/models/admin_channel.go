...
func NewAdminChannel(name string) *AdminChannel {
	now := time.Now()
	return &AdminChannel{
		Name:      name,
		Status:    "pending",
...
func (m *AdminChannel) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("admin_channel: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_channel: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_channel: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_channel: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_channel: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_channel: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_channel: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_channel: version must be at least 1")
	}
	return nil
}

func (m *AdminChannel) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_channel: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminChannel) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_channel: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminChannel) Clone() *AdminChannel {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminChannel) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_channel: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_channel: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminChannel) Sanitize() {
...
func (m *AdminChannel) Merge(other *AdminChannel) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminChannel) String() string {
	return fmt.Sprintf("AdminChannel[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminChannel) MatchesFilter(f *AdminChannelFilter) bool {
...
func FilterAdminChannelList(items []*AdminChannel, f *AdminChannelFilter) *AdminChannelListResponse {
	var filtered []*AdminChannel
...
func CalculateAdminChannelStats(items []*AdminChannel) *AdminChannelStats {
