...
type SearchReviewFilter struct {
	IDs       []int64   `json:"ids"`
...
type SearchReviewListResponse struct {
	Data       []*SearchReview `json:"data"`
...
func NewSearchReview(name string) *SearchReview {
	now := time.Now()
	return &SearchReview{
		Name:      name,
		Status:    "pending",
...
func (m *SearchReview) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("search_review: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("search_review: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("search_review: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("search_review: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("search_review: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("search_review: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("search_review: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("search_review: version must be at least 1")
	}
	return nil
}

func (m *SearchReview) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("search_review: marshal failed: %w", err)
	}
	return data, nil
}

func (m *SearchReview) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("search_review: unmarshal failed: %w", err)
	}
	return nil
}

func (m *SearchReview) Clone() *SearchReview {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *SearchReview) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("search_review: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("search_review: transition from %q to %q not allowed", m.Status, status)
}

func (m *SearchReview) Sanitize() {
...
func (m *SearchReview) Merge(other *SearchReview) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *SearchReview) String() string {
	return fmt.Sprintf("SearchReview[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *SearchReview) MatchesFilter(f *SearchReviewFilter) bool {
...
func FilterSearchReviewList(items []*SearchReview, f *SearchReviewFilter) *SearchReviewListResponse {
	var filtered []*SearchReview
	...
	return &SearchReviewListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateSearchReviewStats(items []*SearchReview) *SearchReviewStats {
