...
func NewShipment(name string) *Shipment {
	now := time.Now()
	return &Shipment{
		Name:      name,
		Status:    "pending",
...
func (m *Shipment) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("shipment: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("shipment: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("shipment: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("shipment: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("shipment: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("shipment: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("shipment: version must be at least 1")
	}
	return nil
}

func (m *Shipment) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("shipment: marshal failed: %w", err)
	}
	return data, nil
}

func (m *Shipment) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("shipment: unmarshal failed: %w", err)
	}
	return nil
}

func (m *Shipment) Clone() *Shipment {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *Shipment) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("shipment: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("shipment: transition from %q to %q not allowed", m.Status, status)
}

func (m *Shipment) Sanitize() {
...
func (m *Shipment) Merge(other *Shipment) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Shipment) String() string {
	return fmt.Sprintf("Shipment[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Shipment) MatchesFilter(f *ShipmentFilter) bool {
...
func FilterShipmentList(items []*Shipment, f *ShipmentFilter) *ShipmentListResponse {
...
func CalculateShipmentStats(items []*Shipment) *ShipmentStats {
	stats := &ShipmentStats{}
	if len(items) == 0 {
		return stats
	}
