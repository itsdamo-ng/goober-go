...
type BillingReviewFilter struct {
	IDs       []int64   `json:"ids"`
...
type BillingReviewListResponse struct {
	Data       []*BillingReview `json:"data"`
...
func NewBillingReview(name string) *BillingReview {
	now := time.Now()
...
func (m *BillingReview) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("billing_review: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("billing_review: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("billing_review: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("billing_review: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("billing_review: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("billing_review: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("billing_review: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("billing_review: version must be at least 1")
	}
	return nil
}

func (m *BillingReview) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("billing_review: marshal failed: %w", err)
	}
	return data, nil
}

func (m *BillingReview) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("billing_review: unmarshal failed: %w", err)
	}
	return nil
}

func (m *BillingReview) Clone() *BillingReview {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *BillingReview) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("billing_review: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("billing_review: transition from %q to %q not allowed", m.Status, status)
}

func (m *BillingReview) Sanitize() {
...
func (m *BillingReview) Merge(other *BillingReview) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *BillingReview) String() string {
	return fmt.Sprintf("BillingReview[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *BillingReview) MatchesFilter(f *BillingReviewFilter) bool {
...
func FilterBillingReviewList(items []*BillingReview, f *BillingReviewFilter) *BillingReviewListResponse {
	var filtered []*BillingReview
	...
	return &BillingReviewListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateBillingReviewStats(items []*BillingReview) *BillingReviewStats {
