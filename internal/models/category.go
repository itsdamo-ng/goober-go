...
func (m *Category) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("category: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("category: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("category: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("category: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("category: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("category: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("category: version must be at least 1")
	}
	return nil
}

func (m *Category) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("category: marshal failed: %w", err)
	}
	return data, nil
}

func (m *Category) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("category: unmarshal failed: %w", err)
	}
	return nil
}
...
func (m *Category) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("category: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("category: transition from %q to %q not allowed", m.Status, status)
}

func (m *Category) Sanitize() {
...
func (m *Category) Merge(other *Category) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Category) String() string {
	return fmt.Sprintf("Category[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Category) MatchesFilter(f *CategoryFilter) bool {
...
func FilterCategoryList(items []*Category, f *CategoryFilter) *CategoryListResponse {
...
func CalculateCategoryStats(items []*Category) *CategoryStats {
	stats := &CategoryStats{}
	if len(items) == 0 {
		return stats
	}
