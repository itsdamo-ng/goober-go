...
func (m *Session) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("session: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("session: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("session: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("session: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("session: priority must be between 0 and 10")
	}
...
func (m *Session) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("session: marshal failed: %w", err)
	}
	return data, nil
}

func (m *Session) FromJSON(data []byte) error {
...
func (m *Session) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("session: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("session: transition from %q to %q not allowed", m.Status, status)
}

func (m *Session) Sanitize() {
...
func (m *Session) Merge(other *Session) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Session) String() string {
	return fmt.Sprintf("Session[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Session) MatchesFilter(f *SessionFilter) bool {
...
func FilterSessionList(items []*Session, f *SessionFilter) *SessionListResponse {
...
func CalculateSessionStats(items []*Session) *SessionStats {
	stats := &SessionStats{}
	if len(items) == 0 {
		return stats
	}
