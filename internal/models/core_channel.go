...
type CoreChannelListResponse struct {
	Data       []*CoreChannel `json:"data"`
...
func NewCoreChannel(name string) *CoreChannel {
	now := time.Now()
	return &CoreChannel{
		Name:      name,
		Status:    "pending",
...
func (m *CoreChannel) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("core_channel: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("core_channel: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("core_channel: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("core_channel: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("core_channel: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("core_channel: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("core_channel: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("core_channel: version must be at least 1")
	}
	return nil
}

func (m *CoreChannel) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("core_channel: marshal failed: %w", err)
	}
	return data, nil
}

func (m *CoreChannel) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("core_channel: unmarshal failed: %w", err)
	}
	return nil
}

func (m *CoreChannel) Clone() *CoreChannel {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *CoreChannel) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("core_channel: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("core_channel: transition from %q to %q not allowed", m.Status, status)
}

func (m *CoreChannel) Sanitize() {
...
func (m *CoreChannel) Merge(other *CoreChannel) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *CoreChannel) String() string {
	return fmt.Sprintf("CoreChannel[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *CoreChannel) MatchesFilter(f *CoreChannelFilter) bool {
...
func FilterCoreChannelList(items []*CoreChannel, f *CoreChannelFilter) *CoreChannelListResponse {
	var filtered []*CoreChannel
	...
	return &CoreChannelListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateCoreChannelStats(items []*CoreChannel) *CoreChannelStats {
	stats := &CoreChannelStats{}
	if len(items) == 0 {
		return stats
	}
