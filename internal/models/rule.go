...
func (m *Rule) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("rule: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("rule: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("rule: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("rule: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("rule: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("rule: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("rule: version must be at least 1")
	}
	return nil
}

func (m *Rule) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("rule: marshal failed: %w", err)
	}
	return data, nil
}

func (m *Rule) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("rule: unmarshal failed: %w", err)
	}
	return nil
}

func (m *Rule) Clone() *Rule {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *Rule) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("rule: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("rule: transition from %q to %q not allowed", m.Status, status)
}

func (m *Rule) Sanitize() {
...
func (m *Rule) String() string {
	return fmt.Sprintf("Rule[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Rule) MatchesFilter(f *RuleFilter) bool {
...
func FilterRuleList(items []*Rule, f *RuleFilter) *RuleListResponse {
	var filtered []*Rule
...
func CalculateRuleStats(items []*Rule) *RuleStats {
	stats := &RuleStats{}
	if len(items) == 0 {
		return stats
	}
