...
type CoreShipmentFilter struct {
	IDs       []int64   `json:"ids"`
...
type CoreShipmentListResponse struct {
	Data       []*CoreShipment `json:"data"`
...
func NewCoreShipment(name string) *CoreShipment {
	now := time.Now()
	return &CoreShipment{
		Name:      name,
		Status:    "pending",
...
func (m *CoreShipment) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("core_shipment: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("core_shipment: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("core_shipment: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("core_shipment: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("core_shipment: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("core_shipment: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("core_shipment: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("core_shipment: version must be at least 1")
	}
	return nil
}

func (m *CoreShipment) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("core_shipment: marshal failed: %w", err)
	}
	return data, nil
}

func (m *CoreShipment) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("core_shipment: unmarshal failed: %w", err)
	}
	return nil
}

func (m *CoreShipment) Clone() *CoreShipment {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *CoreShipment) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("core_shipment: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("core_shipment: transition from %q to %q not allowed", m.Status, status)
}

func (m *CoreShipment) Sanitize() {
...
func (m *CoreShipment) Merge(other *CoreShipment) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *CoreShipment) String() string {
	return fmt.Sprintf("CoreShipment[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *CoreShipment) MatchesFilter(f *CoreShipmentFilter) bool {
...
func FilterCoreShipmentList(items []*CoreShipment, f *CoreShipmentFilter) *CoreShipmentListResponse {
	var filtered []*CoreShipment
	...
	return &CoreShipmentListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateCoreShipmentStats(items []*CoreShipment) *CoreShipmentStats {
	stats := &CoreShipmentStats{}
	if len(items) == 0 {
		return stats
	}
