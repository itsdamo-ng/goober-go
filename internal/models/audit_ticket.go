...
type AuditTicketListResponse struct {
	Data       []*AuditTicket `json:"data"`
...
func NewAuditTicket(name string) *AuditTicket {
	now := time.Now()
	return &AuditTicket{
		Name:      name,
		Status:    "pending",
...
func (m *AuditTicket) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("audit_ticket: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("audit_ticket: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("audit_ticket: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("audit_ticket: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("audit_ticket: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("audit_ticket: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("audit_ticket: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("audit_ticket: version must be at least 1")
	}
	return nil
}

func (m *AuditTicket) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("audit_ticket: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuditTicket) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("audit_ticket: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuditTicket) Clone() *AuditTicket {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuditTicket) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("audit_ticket: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("audit_ticket: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuditTicket) Sanitize() {
...
func (m *AuditTicket) Merge(other *AuditTicket) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuditTicket) String() string {
	return fmt.Sprintf("AuditTicket[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuditTicket) MatchesFilter(f *AuditTicketFilter) bool {
...
func FilterAuditTicketList(items []*AuditTicket, f *AuditTicketFilter) *AuditTicketListResponse {
	var filtered []*AuditTicket
	...
	return &AuditTicketListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateAuditTicketStats(items []*AuditTicket) *AuditTicketStats {
	stats := &AuditTicketStats{}
	if len(items) == 0 {
		return stats
	}
