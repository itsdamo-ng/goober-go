...
type BillingPaymentFilter struct {
	IDs       []int64   `json:"ids"`
...
type BillingPaymentStats struct {
...
func NewBillingPayment(name string) *BillingPayment {
	now := time.Now()
...
func (m *BillingPayment) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("billing_payment: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("billing_payment: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("billing_payment: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("billing_payment: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("billing_payment: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("billing_payment: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("billing_payment: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("billing_payment: version must be at least 1")
	}
	return nil
}

func (m *BillingPayment) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("billing_payment: marshal failed: %w", err)
	}
	return data, nil
}

func (m *BillingPayment) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("billing_payment: unmarshal failed: %w", err)
	}
	return nil
}

func (m *BillingPayment) Clone() *BillingPayment {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *BillingPayment) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("billing_payment: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("billing_payment: transition from %q to %q not allowed", m.Status, status)
}

func (m *BillingPayment) Sanitize() {
...
func (m *BillingPayment) Merge(other *BillingPayment) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *BillingPayment) String() string {
	return fmt.Sprintf("BillingPayment[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *BillingPayment) MatchesFilter(f *BillingPaymentFilter) bool {
...
func FilterBillingPaymentList(items []*BillingPayment, f *BillingPaymentFilter) *BillingPaymentListResponse {
	var filtered []*BillingPayment
...
func CalculateBillingPaymentStats(items []*BillingPayment) *BillingPaymentStats {
	stats := &BillingPaymentStats{}
	if len(items) == 0 {
		return stats
	}
