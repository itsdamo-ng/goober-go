...
type AuditDocumentFilter struct {
	IDs       []int64   `json:"ids"`
...
type AuditDocumentListResponse struct {
	Data       []*AuditDocument `json:"data"`
...
type AuditDocumentStats struct {
...
func NewAuditDocument(name string) *AuditDocument {
	now := time.Now()
...
func (m *AuditDocument) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("audit_document: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("audit_document: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("audit_document: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("audit_document: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("audit_document: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("audit_document: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("audit_document: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("audit_document: version must be at least 1")
	}
	return nil
}

func (m *AuditDocument) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("audit_document: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuditDocument) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("audit_document: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuditDocument) Clone() *AuditDocument {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuditDocument) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("audit_document: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("audit_document: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuditDocument) Sanitize() {
...
func (m *AuditDocument) Merge(other *AuditDocument) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuditDocument) String() string {
	return fmt.Sprintf("AuditDocument[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuditDocument) MatchesFilter(f *AuditDocumentFilter) bool {
...
func FilterAuditDocumentList(items []*AuditDocument, f *AuditDocumentFilter) *AuditDocumentListResponse {
	var filtered []*AuditDocument
	...
	return &AuditDocumentListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateAuditDocumentStats(items []*AuditDocument) *AuditDocumentStats {
	stats := &AuditDocumentStats{}
	if len(items) == 0 {
		return stats
	}
