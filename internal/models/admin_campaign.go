...
type AdminCampaignStats struct {
...
func NewAdminCampaign(name string) *AdminCampaign {
	now := time.Now()
...
func (m *AdminCampaign) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("admin_campaign: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_campaign: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_campaign: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_campaign: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_campaign: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_campaign: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_campaign: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_campaign: version must be at least 1")
	}
	return nil
}

func (m *AdminCampaign) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_campaign: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminCampaign) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_campaign: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminCampaign) Clone() *AdminCampaign {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminCampaign) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_campaign: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_campaign: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminCampaign) Sanitize() {
...
func (m *AdminCampaign) Merge(other *AdminCampaign) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminCampaign) String() string {
	return fmt.Sprintf("AdminCampaign[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminCampaign) MatchesFilter(f *AdminCampaignFilter) bool {
...
func FilterAdminCampaignList(items []*AdminCampaign, f *AdminCampaignFilter) *AdminCampaignListResponse {
	var filtered []*AdminCampaign
...
func CalculateAdminCampaignStats(items []*AdminCampaign) *AdminCampaignStats {
