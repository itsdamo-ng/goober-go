...
func (m *Contract) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("contract: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("contract: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("contract: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("contract: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("contract: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("contract: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("contract: version must be at least 1")
	}
	return nil
}
...
func (m *Contract) ToJSON() ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("contract: marshal failed: %w", err)
	}
	return data, nil
}
...
func (m *Contract) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("contract: unmarshal failed: %w", err)
	}
	return nil
}
...
func (m *Contract) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("contract: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("contract: transition from %q to %q not allowed", m.Status, status)
}

func (m *Contract) Sanitize() {
...
func (m *Contract) Merge(other *Contract) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Contract) String() string {
	return fmt.Sprintf("Contract[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Contract) MatchesFilter(f *ContractFilter) bool {
...
func FilterContractList(items []*Contract, f *ContractFilter) *ContractListResponse {
...
func CalculateContractStats(items []*Contract) *ContractStats {
	stats := &ContractStats{}
	if len(items) == 0 {
		return stats
	}
