...
type ImportMessageFilter struct {
	IDs       []int64   `json:"ids"`
...
type ImportMessageListResponse struct {
	Data       []*ImportMessage `json:"data"`
...
type ImportMessageStats struct {
...
func NewImportMessage(name string) *ImportMessage {
	now := time.Now()
...
func (m *ImportMessage) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("import_message: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("import_message: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("import_message: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("import_message: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("import_message: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("import_message: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("import_message: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("import_message: version must be at least 1")
	}
	return nil
}

func (m *ImportMessage) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("import_message: marshal failed: %w", err)
	}
	return data, nil
}

func (m *ImportMessage) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("import_message: unmarshal failed: %w", err)
	}
	return nil
}

func (m *ImportMessage) Clone() *ImportMessage {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *ImportMessage) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("import_message: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("import_message: transition from %q to %q not allowed", m.Status, status)
}

func (m *ImportMessage) Sanitize() {
...
func (m *ImportMessage) Merge(other *ImportMessage) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *ImportMessage) String() string {
	return fmt.Sprintf("ImportMessage[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *ImportMessage) MatchesFilter(f *ImportMessageFilter) bool {
...
func FilterImportMessageList(items []*ImportMessage, f *ImportMessageFilter) *ImportMessageListResponse {
	...
	return &ImportMessageListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateImportMessageStats(items []*ImportMessage) *ImportMessageStats {
	stats := &ImportMessageStats{}
	if len(items) == 0 {
		return stats
	}
