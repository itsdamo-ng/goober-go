...
type AuthLeadListResponse struct {
	Data       []*AuthLead `json:"data"`
...
func NewAuthLead(name string) *AuthLead {
	now := time.Now()
	return &AuthLead{
		Name:      name,
		Status:    "pending",
...
func (m *AuthLead) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("auth_lead: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("auth_lead: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("auth_lead: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("auth_lead: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("auth_lead: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("auth_lead: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("auth_lead: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("auth_lead: version must be at least 1")
	}
	return nil
}

func (m *AuthLead) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("auth_lead: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuthLead) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("auth_lead: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuthLead) Clone() *AuthLead {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuthLead) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("auth_lead: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("auth_lead: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuthLead) Sanitize() {
...
func (m *AuthLead) Merge(other *AuthLead) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuthLead) String() string {
	return fmt.Sprintf("AuthLead[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuthLead) MatchesFilter(f *AuthLeadFilter) bool {
...
func FilterAuthLeadList(items []*AuthLead, f *AuthLeadFilter) *AuthLeadListResponse {
	var filtered []*AuthLead
...
func CalculateAuthLeadStats(items []*AuthLead) *AuthLeadStats {
	stats := &AuthLeadStats{}
	if len(items) == 0 {
		return stats
	}
