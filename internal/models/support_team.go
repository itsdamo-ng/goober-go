...
type SupportTeamListResponse struct {
	Data       []*SupportTeam `json:"data"`
...
func NewSupportTeam(name string) *SupportTeam {
	now := time.Now()
	return &SupportTeam{
		Name:      name,
		Status:    "pending",
...
func (m *SupportTeam) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("support_team: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("support_team: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("support_team: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("support_team: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("support_team: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("support_team: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("support_team: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("support_team: version must be at least 1")
	}
	return nil
}

func (m *SupportTeam) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("support_team: marshal failed: %w", err)
	}
	return data, nil
}

func (m *SupportTeam) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("support_team: unmarshal failed: %w", err)
	}
	return nil
}

func (m *SupportTeam) Clone() *SupportTeam {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *SupportTeam) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("support_team: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("support_team: transition from %q to %q not allowed", m.Status, status)
}

func (m *SupportTeam) Sanitize() {
...
func (m *SupportTeam) Merge(other *SupportTeam) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *SupportTeam) String() string {
	return fmt.Sprintf("SupportTeam[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *SupportTeam) MatchesFilter(f *SupportTeamFilter) bool {
...
func FilterSupportTeamList(items []*SupportTeam, f *SupportTeamFilter) *SupportTeamListResponse {
	var filtered []*SupportTeam
	...
	return &SupportTeamListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateSupportTeamStats(items []*SupportTeam) *SupportTeamStats {
	stats := &SupportTeamStats{}
	if len(items) == 0 {
		return stats
	}
