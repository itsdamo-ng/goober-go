...
func (m *Workflow) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("workflow: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("workflow: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("workflow: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("workflow: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("workflow: priority must be between 0 and 10")
	}
...
func (m *Workflow) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("workflow: marshal failed: %w", err)
	}
	return data, nil
}

func (m *Workflow) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("workflow: unmarshal failed: %w", err)
	}
	return nil
}
...
func (m *Workflow) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("workflow: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("workflow: transition from %q to %q not allowed", m.Status, status)
}

func (m *Workflow) Sanitize() {
...
func (m *Workflow) Merge(other *Workflow) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Workflow) String() string {
	return fmt.Sprintf("Workflow[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Workflow) MatchesFilter(f *WorkflowFilter) bool {
...
func FilterWorkflowList(items []*Workflow, f *WorkflowFilter) *WorkflowListResponse {
...
func CalculateWorkflowStats(items []*Workflow) *WorkflowStats {
	stats := &WorkflowStats{}
	if len(items) == 0 {
		return stats
	}
