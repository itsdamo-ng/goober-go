...
func NewAuthRole(name string) *AuthRole {
	now := time.Now()
	return &AuthRole{
		Name:      name,
		Status:    "pending",
...
func (m *AuthRole) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("auth_role: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("auth_role: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("auth_role: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("auth_role: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("auth_role: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("auth_role: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("auth_role: version must be at least 1")
	}
	return nil
}

func (m *AuthRole) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("auth_role: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuthRole) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("auth_role: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuthRole) Clone() *AuthRole {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuthRole) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("auth_role: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("auth_role: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuthRole) Sanitize() {
...
func (m *AuthRole) Merge(other *AuthRole) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuthRole) String() string {
	return fmt.Sprintf("AuthRole[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuthRole) MatchesFilter(f *AuthRoleFilter) bool {
...
func FilterAuthRoleList(items []*AuthRole, f *AuthRoleFilter) *AuthRoleListResponse {
	var filtered []*AuthRole
...
func CalculateAuthRoleStats(items []*AuthRole) *AuthRoleStats {
	stats := &AuthRoleStats{}
	if len(items) == 0 {
		return stats
	}
