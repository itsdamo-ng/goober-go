...
type PlatformInvoiceFilter struct {
	IDs       []int64   `json:"ids"`
...
type PlatformInvoiceListResponse struct {
...
type PlatformInvoiceStats struct {
...
func NewPlatformInvoice(name string) *PlatformInvoice {
	now := time.Now()
...
func (m *PlatformInvoice) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("platform_invoice: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("platform_invoice: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("platform_invoice: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("platform_invoice: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("platform_invoice: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("platform_invoice: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("platform_invoice: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("platform_invoice: version must be at least 1")
	}
	return nil
}

func (m *PlatformInvoice) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("platform_invoice: marshal failed: %w", err)
	}
	return data, nil
}

func (m *PlatformInvoice) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("platform_invoice: unmarshal failed: %w", err)
	}
	return nil
}

func (m *PlatformInvoice) Clone() *PlatformInvoice {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *PlatformInvoice) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("platform_invoice: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("platform_invoice: transition from %q to %q not allowed", m.Status, status)
}

func (m *PlatformInvoice) Sanitize() {
...
func (m *PlatformInvoice) Merge(other *PlatformInvoice) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *PlatformInvoice) String() string {
	return fmt.Sprintf("PlatformInvoice[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *PlatformInvoice) MatchesFilter(f *PlatformInvoiceFilter) bool {
...
func FilterPlatformInvoiceList(items []*PlatformInvoice, f *PlatformInvoiceFilter) *PlatformInvoiceListResponse {
	var filtered []*PlatformInvoice
...
func CalculatePlatformInvoiceStats(items []*PlatformInvoice) *PlatformInvoiceStats {
	stats := &PlatformInvoiceStats{}
	if len(items) == 0 {
		return stats
	}
