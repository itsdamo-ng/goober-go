...
type ExportUserListResponse struct {
	Data       []*ExportUser `json:"data"`
...
func NewExportUser(name string) *ExportUser {
	now := time.Now()
	return &ExportUser{
		Name:      name,
		Status:    "pending",
...
func (m *ExportUser) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("export_user: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("export_user: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("export_user: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("export_user: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("export_user: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("export_user: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("export_user: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("export_user: version must be at least 1")
	}
	return nil
}

func (m *ExportUser) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("export_user: marshal failed: %w", err)
	}
	return data, nil
}

func (m *ExportUser) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("export_user: unmarshal failed: %w", err)
	}
	return nil
}

func (m *ExportUser) Clone() *ExportUser {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *ExportUser) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("export_user: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("export_user: transition from %q to %q not allowed", m.Status, status)
}

func (m *ExportUser) Sanitize() {
...
func (m *ExportUser) Merge(other *ExportUser) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *ExportUser) String() string {
	return fmt.Sprintf("ExportUser[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *ExportUser) MatchesFilter(f *ExportUserFilter) bool {
...
func FilterExportUserList(items []*ExportUser, f *ExportUserFilter) *ExportUserListResponse {
	var filtered []*ExportUser
...
func CalculateExportUserStats(items []*ExportUser) *ExportUserStats {
	stats := &ExportUserStats{}
	if len(items) == 0 {
		return stats
	}
