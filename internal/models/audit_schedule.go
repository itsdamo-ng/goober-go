...
type AuditScheduleFilter struct {
	IDs       []int64   `json:"ids"`
...
type AuditScheduleListResponse struct {
	Data       []*AuditSchedule `json:"data"`
...
type AuditScheduleStats struct {
...
func NewAuditSchedule(name string) *AuditSchedule {
	now := time.Now()
...
func (m *AuditSchedule) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("audit_schedule: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("audit_schedule: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("audit_schedule: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("audit_schedule: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("audit_schedule: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("audit_schedule: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("audit_schedule: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("audit_schedule: version must be at least 1")
	}
	return nil
}

func (m *AuditSchedule) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("audit_schedule: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuditSchedule) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("audit_schedule: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuditSchedule) Clone() *AuditSchedule {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuditSchedule) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("audit_schedule: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("audit_schedule: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuditSchedule) Sanitize() {
...
func (m *AuditSchedule) Merge(other *AuditSchedule) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuditSchedule) String() string {
	return fmt.Sprintf("AuditSchedule[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuditSchedule) MatchesFilter(f *AuditScheduleFilter) bool {
...
func FilterAuditScheduleList(items []*AuditSchedule, f *AuditScheduleFilter) *AuditScheduleListResponse {
	var filtered []*AuditSchedule
...
func CalculateAuditScheduleStats(items []*AuditSchedule) *AuditScheduleStats {
	stats := &AuditScheduleStats{}
	if len(items) == 0 {
		return stats
	}
