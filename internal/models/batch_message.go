...
type BatchMessageFilter struct {
	IDs       []int64   `json:"ids"`
...
type BatchMessageListResponse struct {
	Data       []*BatchMessage `json:"data"`
...
func NewBatchMessage(name string) *BatchMessage {
	now := time.Now()
	return &BatchMessage{
		Name:      name,
		Status:    "pending",
...
func (m *BatchMessage) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("batch_message: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("batch_message: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("batch_message: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("batch_message: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("batch_message: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("batch_message: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("batch_message: version must be at least 1")
	}
	return nil
}

func (m *BatchMessage) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("batch_message: marshal failed: %w", err)
	}
	return data, nil
}

func (m *BatchMessage) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("batch_message: unmarshal failed: %w", err)
	}
	return nil
}

func (m *BatchMessage) Clone() *BatchMessage {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *BatchMessage) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("batch_message: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("batch_message: transition from %q to %q not allowed", m.Status, status)
}

func (m *BatchMessage) Sanitize() {
...
func (m *BatchMessage) Merge(other *BatchMessage) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *BatchMessage) String() string {
	return fmt.Sprintf("BatchMessage[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *BatchMessage) MatchesFilter(f *BatchMessageFilter) bool {
...
func FilterBatchMessageList(items []*BatchMessage, f *BatchMessageFilter) *BatchMessageListResponse {
	var filtered []*BatchMessage
	...
	return &BatchMessageListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateBatchMessageStats(items []*BatchMessage) *BatchMessageStats {
	stats := &BatchMessageStats{}
	if len(items) == 0 {
		return stats
	}
