...
type BatchWorkflowFilter struct {
	IDs       []int64   `json:"ids"`
...
type BatchWorkflowListResponse struct {
	Data       []*BatchWorkflow `json:"data"`
...
func NewBatchWorkflow(name string) *BatchWorkflow {
	now := time.Now()
...
func (m *BatchWorkflow) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("batch_workflow: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("batch_workflow: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("batch_workflow: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("batch_workflow: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("batch_workflow: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("batch_workflow: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("batch_workflow: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("batch_workflow: version must be at least 1")
	}
	return nil
}

func (m *BatchWorkflow) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("batch_workflow: marshal failed: %w", err)
	}
	return data, nil
}

func (m *BatchWorkflow) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("batch_workflow: unmarshal failed: %w", err)
	}
	return nil
}

func (m *BatchWorkflow) Clone() *BatchWorkflow {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *BatchWorkflow) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("batch_workflow: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("batch_workflow: transition from %q to %q not allowed", m.Status, status)
}

func (m *BatchWorkflow) Sanitize() {
...
func (m *BatchWorkflow) Merge(other *BatchWorkflow) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *BatchWorkflow) String() string {
	return fmt.Sprintf("BatchWorkflow[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *BatchWorkflow) MatchesFilter(f *BatchWorkflowFilter) bool {
...
func FilterBatchWorkflowList(items []*BatchWorkflow, f *BatchWorkflowFilter) *BatchWorkflowListResponse {
	var filtered []*BatchWorkflow
	...
	return &BatchWorkflowListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateBatchWorkflowStats(items []*BatchWorkflow) *BatchWorkflowStats {
