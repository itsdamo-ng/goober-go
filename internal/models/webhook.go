...
func (m *Webhook) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("webhook: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("webhook: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("webhook: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("webhook: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("webhook: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("webhook: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("webhook: version must be at least 1")
	}
	return nil
}

func (m *Webhook) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("webhook: marshal failed: %w", err)
	}
	return data, nil
}

func (m *Webhook) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("webhook: unmarshal failed: %w", err)
	}
	return nil
}

func (m *Webhook) Clone() *Webhook {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *Webhook) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("webhook: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("webhook: transition from %q to %q not allowed", m.Status, status)
}

func (m *Webhook) Sanitize() {
...
func (m *Webhook) Merge(other *Webhook) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Webhook) String() string {
	return fmt.Sprintf("Webhook[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Webhook) MatchesFilter(f *WebhookFilter) bool {
...
func FilterWebhookList(items []*Webhook, f *WebhookFilter) *WebhookListResponse {
...
func CalculateWebhookStats(items []*Webhook) *WebhookStats {
	stats := &WebhookStats{}
	if len(items) == 0 {
		return stats
	}
