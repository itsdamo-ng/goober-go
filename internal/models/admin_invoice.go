...
func NewAdminInvoice(name string) *AdminInvoice {
	now := time.Now()
	return &AdminInvoice{
		Name:      name,
		Status:    "pending",
...
func (m *AdminInvoice) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("admin_invoice: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_invoice: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_invoice: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_invoice: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_invoice: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_invoice: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_invoice: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_invoice: version must be at least 1")
	}
	return nil
}

func (m *AdminInvoice) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_invoice: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminInvoice) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_invoice: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminInvoice) Clone() *AdminInvoice {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminInvoice) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_invoice: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_invoice: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminInvoice) Sanitize() {
...
func (m *AdminInvoice) Merge(other *AdminInvoice) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminInvoice) String() string {
	return fmt.Sprintf("AdminInvoice[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminInvoice) MatchesFilter(f *AdminInvoiceFilter) bool {
...
func FilterAdminInvoiceList(items []*AdminInvoice, f *AdminInvoiceFilter) *AdminInvoiceListResponse {
	var filtered []*AdminInvoice
...
func CalculateAdminInvoiceStats(items []*AdminInvoice) *AdminInvoiceStats {
