...
type CorePipelineFilter struct {
	IDs       []int64   `json:"ids"`
...
type CorePipelineListResponse struct {
	Data       []*CorePipeline `json:"data"`
...
func NewCorePipeline(name string) *CorePipeline {
	now := time.Now()
	return &CorePipeline{
		Name:      name,
		Status:    "pending",
...
func (m *CorePipeline) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("core_pipeline: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("core_pipeline: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("core_pipeline: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("core_pipeline: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("core_pipeline: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("core_pipeline: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("core_pipeline: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("core_pipeline: version must be at least 1")
	}
	return nil
}

func (m *CorePipeline) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("core_pipeline: marshal failed: %w", err)
	}
	return data, nil
}

func (m *CorePipeline) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("core_pipeline: unmarshal failed: %w", err)
	}
	return nil
}

func (m *CorePipeline) Clone() *CorePipeline {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *CorePipeline) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("core_pipeline: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("core_pipeline: transition from %q to %q not allowed", m.Status, status)
}

func (m *CorePipeline) Sanitize() {
...
func (m *CorePipeline) Merge(other *CorePipeline) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *CorePipeline) String() string {
	return fmt.Sprintf("CorePipeline[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *CorePipeline) MatchesFilter(f *CorePipelineFilter) bool {
...
func FilterCorePipelineList(items []*CorePipeline, f *CorePipelineFilter) *CorePipelineListResponse {
	var filtered []*CorePipeline
	...
	return &CorePipelineListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateCorePipelineStats(items []*CorePipeline) *CorePipelineStats {
