...
func (m *Budget) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("budget: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("budget: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("budget: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("budget: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("budget: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("budget: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("budget: version must be at least 1")
	}
	return nil
}

func (m *Budget) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("budget: marshal failed: %w", err)
	}
	return data, nil
}

func (m *Budget) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("budget: unmarshal failed: %w", err)
	}
	return nil
}

func (m *Budget) Clone() *Budget {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *Budget) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("budget: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("budget: transition from %q to %q not allowed", m.Status, status)
}

func (m *Budget) Sanitize() {
...
func (m *Budget) Merge(other *Budget) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Budget) String() string {
	return fmt.Sprintf("Budget[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Budget) MatchesFilter(f *BudgetFilter) bool {
...
func FilterBudgetList(items []*Budget, f *BudgetFilter) *BudgetListResponse {
	var filtered []*Budget
...
func CalculateBudgetStats(items []*Budget) *BudgetStats {
	stats := &BudgetStats{}
	if len(items) == 0 {
		return stats
	}
