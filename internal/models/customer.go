...
func (m *Customer) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("customer: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("customer: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("customer: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("customer: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("customer: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("customer: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("customer: version must be at least 1")
	}
	return nil
}

func (m *Customer) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("customer: marshal failed: %w", err)
	}
	return data, nil
}

func (m *Customer) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("customer: unmarshal failed: %w", err)
	}
	return nil
}

func (m *Customer) Clone() *Customer {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *Customer) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("customer: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("customer: transition from %q to %q not allowed", m.Status, status)
}
...
func (m *Customer) Merge(other *Customer) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Customer) String() string {
	return fmt.Sprintf("Customer[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Customer) MatchesFilter(f *CustomerFilter) bool {
...
func FilterCustomerList(items []*Customer, f *CustomerFilter) *CustomerListResponse {
...
func CalculateCustomerStats(items []*Customer) *CustomerStats {
	stats := &CustomerStats{}
	if len(items) == 0 {
		return stats
	}
