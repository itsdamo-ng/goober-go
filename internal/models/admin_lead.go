...
func NewAdminLead(name string) *AdminLead {
	now := time.Now()
	return &AdminLead{
		Name:      name,
		Status:    "pending",
...
func (m *AdminLead) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("admin_lead: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_lead: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_lead: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_lead: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_lead: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_lead: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_lead: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_lead: version must be at least 1")
	}
	return nil
}

func (m *AdminLead) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_lead: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminLead) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_lead: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminLead) Clone() *AdminLead {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminLead) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_lead: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_lead: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminLead) Sanitize() {
...
func (m *AdminLead) Merge(other *AdminLead) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminLead) String() string {
	return fmt.Sprintf("AdminLead[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminLead) MatchesFilter(f *AdminLeadFilter) bool {
...
func FilterAdminLeadList(items []*AdminLead, f *AdminLeadFilter) *AdminLeadListResponse {
	var filtered []*AdminLead
...
func CalculateAdminLeadStats(items []*AdminLead) *AdminLeadStats {
