...
type AdminFeatureFilter struct {
	IDs       []int64   `json:"ids"`
...
func NewAdminFeature(name string) *AdminFeature {
	now := time.Now()
	return &AdminFeature{
		Name:      name,
		Status:    "pending",
...
func (m *AdminFeature) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("admin_feature: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_feature: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_feature: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_feature: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_feature: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_feature: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_feature: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_feature: version must be at least 1")
	}
	return nil
}

func (m *AdminFeature) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_feature: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminFeature) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_feature: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminFeature) Clone() *AdminFeature {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminFeature) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_feature: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_feature: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminFeature) Sanitize() {
...
func (m *AdminFeature) Merge(other *AdminFeature) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminFeature) String() string {
	return fmt.Sprintf("AdminFeature[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminFeature) MatchesFilter(f *AdminFeatureFilter) bool {
...
func FilterAdminFeatureList(items []*AdminFeature, f *AdminFeatureFilter) *AdminFeatureListResponse {
	var filtered []*AdminFeature
...
func CalculateAdminFeatureStats(items []*AdminFeature) *AdminFeatureStats {
	stats := &AdminFeatureStats{}
	if len(items) == 0 {
		return stats
	}
