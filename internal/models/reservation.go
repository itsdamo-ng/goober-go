...
func (m *Reservation) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("reservation: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("reservation: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("reservation: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("reservation: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("reservation: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("reservation: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("reservation: version must be at least 1")
	}
	return nil
}

func (m *Reservation) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("reservation: marshal failed: %w", err)
	}
	return data, nil
}

func (m *Reservation) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("reservation: unmarshal failed: %w", err)
	}
	return nil
}

func (m *Reservation) Clone() *Reservation {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *Reservation) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("reservation: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("reservation: transition from %q to %q not allowed", m.Status, status)
}

func (m *Reservation) Sanitize() {
...
func (m *Reservation) Merge(other *Reservation) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Reservation) String() string {
	return fmt.Sprintf("Reservation[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Reservation) MatchesFilter(f *ReservationFilter) bool {
...
func FilterReservationList(items []*Reservation, f *ReservationFilter) *ReservationListResponse {
...
func CalculateReservationStats(items []*Reservation) *ReservationStats {
