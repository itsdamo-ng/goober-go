...
type BatchReviewListResponse struct {
	Data       []*BatchReview `json:"data"`
...
func NewBatchReview(name string) *BatchReview {
	now := time.Now()
	return &BatchReview{
		Name:      name,
		Status:    "pending",
...
func (m *BatchReview) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("batch_review: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("batch_review: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("batch_review: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("batch_review: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("batch_review: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("batch_review: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("batch_review: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("batch_review: version must be at least 1")
	}
	return nil
}

func (m *BatchReview) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("batch_review: marshal failed: %w", err)
	}
	return data, nil
}

func (m *BatchReview) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("batch_review: unmarshal failed: %w", err)
	}
	return nil
}

func (m *BatchReview) Clone() *BatchReview {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *BatchReview) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("batch_review: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("batch_review: transition from %q to %q not allowed", m.Status, status)
}

func (m *BatchReview) Sanitize() {
...
func (m *BatchReview) Merge(other *BatchReview) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *BatchReview) String() string {
	return fmt.Sprintf("BatchReview[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *BatchReview) MatchesFilter(f *BatchReviewFilter) bool {
...
func FilterBatchReviewList(items []*BatchReview, f *BatchReviewFilter) *BatchReviewListResponse {
	var filtered []*BatchReview
	...
	return &BatchReviewListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateBatchReviewStats(items []*BatchReview) *BatchReviewStats {
