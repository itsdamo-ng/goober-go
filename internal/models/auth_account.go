...
func NewAuthAccount(name string) *AuthAccount {
	now := time.Now()
	return &AuthAccount{
		Name:      name,
		Status:    "pending",
...
func (m *AuthAccount) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("auth_account: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("auth_account: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("auth_account: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("auth_account: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("auth_account: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("auth_account: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("auth_account: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("auth_account: version must be at least 1")
	}
	return nil
}

func (m *AuthAccount) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("auth_account: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuthAccount) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("auth_account: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuthAccount) Clone() *AuthAccount {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuthAccount) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("auth_account: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("auth_account: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuthAccount) Sanitize() {
...
func (m *AuthAccount) Merge(other *AuthAccount) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuthAccount) String() string {
	return fmt.Sprintf("AuthAccount[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuthAccount) MatchesFilter(f *AuthAccountFilter) bool {
...
func FilterAuthAccountList(items []*AuthAccount, f *AuthAccountFilter) *AuthAccountListResponse {
	var filtered []*AuthAccount
	...
	return &AuthAccountListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateAuthAccountStats(items []*AuthAccount) *AuthAccountStats {
	stats := &AuthAccountStats{}
	if len(items) == 0 {
		return stats
	}
