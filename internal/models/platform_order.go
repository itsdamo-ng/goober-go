...
type PlatformOrderListResponse struct {
	Data       []*PlatformOrder `json:"data"`
...
func NewPlatformOrder(name string) *PlatformOrder {
	now := time.Now()
...
func (m *PlatformOrder) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("platform_order: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("platform_order: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("platform_order: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("platform_order: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("platform_order: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("platform_order: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("platform_order: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("platform_order: version must be at least 1")
	}
	return nil
}

func (m *PlatformOrder) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("platform_order: marshal failed: %w", err)
	}
	return data, nil
}

func (m *PlatformOrder) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("platform_order: unmarshal failed: %w", err)
	}
	return nil
}

func (m *PlatformOrder) Clone() *PlatformOrder {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *PlatformOrder) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("platform_order: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("platform_order: transition from %q to %q not allowed", m.Status, status)
}

func (m *PlatformOrder) Sanitize() {
...
func (m *PlatformOrder) Merge(other *PlatformOrder) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *PlatformOrder) String() string {
	return fmt.Sprintf("PlatformOrder[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *PlatformOrder) MatchesFilter(f *PlatformOrderFilter) bool {
...
func FilterPlatformOrderList(items []*PlatformOrder, f *PlatformOrderFilter) *PlatformOrderListResponse {
	var filtered []*PlatformOrder
	...
	return &PlatformOrderListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculatePlatformOrderStats(items []*PlatformOrder) *PlatformOrderStats {
