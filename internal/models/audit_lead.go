...
type AuditLeadListResponse struct {
	Data       []*AuditLead `json:"data"`
...
func NewAuditLead(name string) *AuditLead {
	now := time.Now()
	return &AuditLead{
		Name:      name,
		Status:    "pending",
...
func (m *AuditLead) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("audit_lead: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("audit_lead: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("audit_lead: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("audit_lead: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("audit_lead: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("audit_lead: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("audit_lead: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("audit_lead: version must be at least 1")
	}
	return nil
}

func (m *AuditLead) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("audit_lead: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuditLead) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("audit_lead: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuditLead) Clone() *AuditLead {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuditLead) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("audit_lead: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("audit_lead: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuditLead) Sanitize() {
...
func (m *AuditLead) Merge(other *AuditLead) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuditLead) String() string {
	return fmt.Sprintf("AuditLead[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuditLead) MatchesFilter(f *AuditLeadFilter) bool {
...
func FilterAuditLeadList(items []*AuditLead, f *AuditLeadFilter) *AuditLeadListResponse {
	var filtered []*AuditLead
...
func CalculateAuditLeadStats(items []*AuditLead) *AuditLeadStats {
	stats := &AuditLeadStats{}
	if len(items) == 0 {
		return stats
	}
