...
type CoreTicketListResponse struct {
	Data       []*CoreTicket `json:"data"`
...
func NewCoreTicket(name string) *CoreTicket {
	now := time.Now()
	return &CoreTicket{
		Name:      name,
		Status:    "pending",
...
func (m *CoreTicket) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("core_ticket: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("core_ticket: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("core_ticket: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("core_ticket: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("core_ticket: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("core_ticket: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("core_ticket: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("core_ticket: version must be at least 1")
	}
	return nil
}

func (m *CoreTicket) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("core_ticket: marshal failed: %w", err)
	}
	return data, nil
}

func (m *CoreTicket) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("core_ticket: unmarshal failed: %w", err)
	}
	return nil
}

func (m *CoreTicket) Clone() *CoreTicket {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *CoreTicket) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("core_ticket: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("core_ticket: transition from %q to %q not allowed", m.Status, status)
}

func (m *CoreTicket) Sanitize() {
...
func (m *CoreTicket) Merge(other *CoreTicket) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *CoreTicket) String() string {
	return fmt.Sprintf("CoreTicket[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *CoreTicket) MatchesFilter(f *CoreTicketFilter) bool {
...
func FilterCoreTicketList(items []*CoreTicket, f *CoreTicketFilter) *CoreTicketListResponse {
	var filtered []*CoreTicket
...
func CalculateCoreTicketStats(items []*CoreTicket) *CoreTicketStats {
	stats := &CoreTicketStats{}
	if len(items) == 0 {
		return stats
	}
