...
type AuditRoleListResponse struct {
	Data       []*AuditRole `json:"data"`
...
func NewAuditRole(name string) *AuditRole {
	now := time.Now()
	return &AuditRole{
		Name:      name,
		Status:    "pending",
...
func (m *AuditRole) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("audit_role: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("audit_role: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("audit_role: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("audit_role: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("audit_role: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("audit_role: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("audit_role: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("audit_role: version must be at least 1")
	}
	return nil
}

func (m *AuditRole) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("audit_role: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuditRole) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("audit_role: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuditRole) Clone() *AuditRole {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuditRole) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("audit_role: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("audit_role: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuditRole) Sanitize() {
...
func (m *AuditRole) Merge(other *AuditRole) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuditRole) String() string {
	return fmt.Sprintf("AuditRole[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuditRole) MatchesFilter(f *AuditRoleFilter) bool {
...
func FilterAuditRoleList(items []*AuditRole, f *AuditRoleFilter) *AuditRoleListResponse {
	var filtered []*AuditRole
...
func CalculateAuditRoleStats(items []*AuditRole) *AuditRoleStats {
	stats := &AuditRoleStats{}
	if len(items) == 0 {
		return stats
	}
