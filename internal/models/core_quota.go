...
type CoreQuotaListResponse struct {
	Data       []*CoreQuota `json:"data"`
...
func NewCoreQuota(name string) *CoreQuota {
	now := time.Now()
	return &CoreQuota{
		Name:      name,
		Status:    "pending",
...
func (m *CoreQuota) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("core_quota: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("core_quota: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("core_quota: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("core_quota: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("core_quota: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("core_quota: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("core_quota: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("core_quota: version must be at least 1")
	}
	return nil
}

func (m *CoreQuota) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("core_quota: marshal failed: %w", err)
	}
	return data, nil
}

func (m *CoreQuota) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("core_quota: unmarshal failed: %w", err)
	}
	return nil
}

func (m *CoreQuota) Clone() *CoreQuota {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *CoreQuota) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("core_quota: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("core_quota: transition from %q to %q not allowed", m.Status, status)
}

func (m *CoreQuota) Sanitize() {
...
func (m *CoreQuota) Merge(other *CoreQuota) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *CoreQuota) String() string {
	return fmt.Sprintf("CoreQuota[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *CoreQuota) MatchesFilter(f *CoreQuotaFilter) bool {
...
func FilterCoreQuotaList(items []*CoreQuota, f *CoreQuotaFilter) *CoreQuotaListResponse {
	var filtered []*CoreQuota
...
func CalculateCoreQuotaStats(items []*CoreQuota) *CoreQuotaStats {
	stats := &CoreQuotaStats{}
	if len(items) == 0 {
		return stats
	}
