...
type SupportRuleListResponse struct {
	Data       []*SupportRule `json:"data"`
...
func NewSupportRule(name string) *SupportRule {
	now := time.Now()
	return &SupportRule{
		Name:      name,
		Status:    "pending",
...
func (m *SupportRule) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("support_rule: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("support_rule: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("support_rule: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("support_rule: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("support_rule: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("support_rule: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("support_rule: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("support_rule: version must be at least 1")
	}
	return nil
}

func (m *SupportRule) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("support_rule: marshal failed: %w", err)
	}
	return data, nil
}

func (m *SupportRule) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("support_rule: unmarshal failed: %w", err)
	}
	return nil
}

func (m *SupportRule) Clone() *SupportRule {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *SupportRule) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("support_rule: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("support_rule: transition from %q to %q not allowed", m.Status, status)
}

func (m *SupportRule) Sanitize() {
...
func (m *SupportRule) Merge(other *SupportRule) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *SupportRule) String() string {
	return fmt.Sprintf("SupportRule[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *SupportRule) MatchesFilter(f *SupportRuleFilter) bool {
...
func FilterSupportRuleList(items []*SupportRule, f *SupportRuleFilter) *SupportRuleListResponse {
	var filtered []*SupportRule
	...
	return &SupportRuleListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateSupportRuleStats(items []*SupportRule) *SupportRuleStats {
	stats := &SupportRuleStats{}
	if len(items) == 0 {
		return stats
	}
