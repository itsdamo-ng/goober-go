...
func (m *Pipeline) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("pipeline: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("pipeline: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("pipeline: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("pipeline: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("pipeline: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("pipeline: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("pipeline: version must be at least 1")
	}
	return nil
}

func (m *Pipeline) ToJSON() ([]byte, error) {
...
func (m *Pipeline) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("pipeline: unmarshal failed: %w", err)
	}
	return nil
}

func (m *Pipeline) Clone() *Pipeline {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *Pipeline) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("pipeline: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("pipeline: transition from %q to %q not allowed", m.Status, status)
}

func (m *Pipeline) Sanitize() {
...
func (m *Pipeline) String() string {
	return fmt.Sprintf("Pipeline[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Pipeline) MatchesFilter(f *PipelineFilter) bool {
...
func FilterPipelineList(items []*Pipeline, f *PipelineFilter) *PipelineListResponse {
...
func CalculatePipelineStats(items []*Pipeline) *PipelineStats {
	stats := &PipelineStats{}
	if len(items) == 0 {
		return stats
	}
