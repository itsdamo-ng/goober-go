...
func (m *Warehouse) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("warehouse: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("warehouse: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("warehouse: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("warehouse: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("warehouse: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("warehouse: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("warehouse: version must be at least 1")
	}
	return nil
}

func (m *Warehouse) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("warehouse: marshal failed: %w", err)
	}
	return data, nil
}

func (m *Warehouse) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("warehouse: unmarshal failed: %w", err)
	}
	return nil
}

func (m *Warehouse) Clone() *Warehouse {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *Warehouse) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("warehouse: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("warehouse: transition from %q to %q not allowed", m.Status, status)
}

func (m *Warehouse) Sanitize() {
...
func (m *Warehouse) Merge(other *Warehouse) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Warehouse) String() string {
	return fmt.Sprintf("Warehouse[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Warehouse) MatchesFilter(f *WarehouseFilter) bool {
...
func FilterWarehouseList(items []*Warehouse, f *WarehouseFilter) *WarehouseListResponse {
	var filtered []*Warehouse
...
func CalculateWarehouseStats(items []*Warehouse) *WarehouseStats {
