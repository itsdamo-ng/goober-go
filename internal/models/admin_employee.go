...
type AdminEmployeeFilter struct {
	IDs       []int64   `json:"ids"`
...
type AdminEmployeeListResponse struct {
...
type AdminEmployeeStats struct {
...
func NewAdminEmployee(name string) *AdminEmployee {
	now := time.Now()
...
func (m *AdminEmployee) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("admin_employee: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_employee: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_employee: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_employee: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_employee: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_employee: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_employee: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_employee: version must be at least 1")
	}
	return nil
}

func (m *AdminEmployee) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_employee: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminEmployee) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_employee: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminEmployee) Clone() *AdminEmployee {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminEmployee) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_employee: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_employee: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminEmployee) Sanitize() {
...
func (m *AdminEmployee) Merge(other *AdminEmployee) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminEmployee) String() string {
	return fmt.Sprintf("AdminEmployee[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminEmployee) MatchesFilter(f *AdminEmployeeFilter) bool {
...
func FilterAdminEmployeeList(items []*AdminEmployee, f *AdminEmployeeFilter) *AdminEmployeeListResponse {
	var filtered []*AdminEmployee
...
func CalculateAdminEmployeeStats(items []*AdminEmployee) *AdminEmployeeStats {
