...
type ImportRoleListResponse struct {
	Data       []*ImportRole `json:"data"`
...
func NewImportRole(name string) *ImportRole {
	now := time.Now()
	return &ImportRole{
		Name:      name,
		Status:    "pending",
...
func (m *ImportRole) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("import_role: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("import_role: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("import_role: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("import_role: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("import_role: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("import_role: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("import_role: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("import_role: version must be at least 1")
	}
	return nil
}

func (m *ImportRole) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("import_role: marshal failed: %w", err)
	}
	return data, nil
}

func (m *ImportRole) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("import_role: unmarshal failed: %w", err)
	}
	return nil
}

func (m *ImportRole) Clone() *ImportRole {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *ImportRole) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("import_role: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("import_role: transition from %q to %q not allowed", m.Status, status)
}

func (m *ImportRole) Sanitize() {
...
func (m *ImportRole) Merge(other *ImportRole) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *ImportRole) String() string {
	return fmt.Sprintf("ImportRole[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *ImportRole) MatchesFilter(f *ImportRoleFilter) bool {
...
func FilterImportRoleList(items []*ImportRole, f *ImportRoleFilter) *ImportRoleListResponse {
	var filtered []*ImportRole
...
func CalculateImportRoleStats(items []*ImportRole) *ImportRoleStats {
	stats := &ImportRoleStats{}
	if len(items) == 0 {
		return stats
	}
