...
type AdminSurveyListResponse struct {
	Data       []*AdminSurvey `json:"data"`
...
func NewAdminSurvey(name string) *AdminSurvey {
	now := time.Now()
	return &AdminSurvey{
		Name:      name,
		Status:    "pending",
...
func (m *AdminSurvey) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("admin_survey: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_survey: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_survey: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_survey: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_survey: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_survey: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_survey: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_survey: version must be at least 1")
	}
	return nil
}

func (m *AdminSurvey) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_survey: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminSurvey) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_survey: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminSurvey) Clone() *AdminSurvey {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminSurvey) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_survey: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_survey: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminSurvey) Sanitize() {
...
func (m *AdminSurvey) Merge(other *AdminSurvey) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminSurvey) String() string {
	return fmt.Sprintf("AdminSurvey[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminSurvey) MatchesFilter(f *AdminSurveyFilter) bool {
...
func FilterAdminSurveyList(items []*AdminSurvey, f *AdminSurveyFilter) *AdminSurveyListResponse {
	var filtered []*AdminSurvey
...
func CalculateAdminSurveyStats(items []*AdminSurvey) *AdminSurveyStats {
