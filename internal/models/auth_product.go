...
type AuthProductListResponse struct {
	Data       []*AuthProduct `json:"data"`
...
func NewAuthProduct(name string) *AuthProduct {
	now := time.Now()
	return &AuthProduct{
		Name:      name,
		Status:    "pending",
...
func (m *AuthProduct) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("auth_product: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("auth_product: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("auth_product: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("auth_product: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("auth_product: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("auth_product: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("auth_product: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("auth_product: version must be at least 1")
	}
	return nil
}

func (m *AuthProduct) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("auth_product: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuthProduct) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("auth_product: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuthProduct) Clone() *AuthProduct {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuthProduct) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("auth_product: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("auth_product: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuthProduct) Sanitize() {
...
func (m *AuthProduct) Merge(other *AuthProduct) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuthProduct) String() string {
	return fmt.Sprintf("AuthProduct[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuthProduct) MatchesFilter(f *AuthProductFilter) bool {
...
func FilterAuthProductList(items []*AuthProduct, f *AuthProductFilter) *AuthProductListResponse {
	var filtered []*AuthProduct
	...
	return &AuthProductListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateAuthProductStats(items []*AuthProduct) *AuthProductStats {
	stats := &AuthProductStats{}
	if len(items) == 0 {
		return stats
	}
