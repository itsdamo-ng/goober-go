...
type AuthTicketListResponse struct {
	Data       []*AuthTicket `json:"data"`
...
func NewAuthTicket(name string) *AuthTicket {
	now := time.Now()
	return &AuthTicket{
		Name:      name,
		Status:    "pending",
...
func (m *AuthTicket) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("auth_ticket: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("auth_ticket: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("auth_ticket: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("auth_ticket: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("auth_ticket: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("auth_ticket: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("auth_ticket: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("auth_ticket: version must be at least 1")
	}
	return nil
}

func (m *AuthTicket) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("auth_ticket: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuthTicket) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("auth_ticket: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuthTicket) Clone() *AuthTicket {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuthTicket) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("auth_ticket: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("auth_ticket: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuthTicket) Sanitize() {
...
func (m *AuthTicket) Merge(other *AuthTicket) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuthTicket) String() string {
	return fmt.Sprintf("AuthTicket[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuthTicket) MatchesFilter(f *AuthTicketFilter) bool {
...
func FilterAuthTicketList(items []*AuthTicket, f *AuthTicketFilter) *AuthTicketListResponse {
	var filtered []*AuthTicket
...
func CalculateAuthTicketStats(items []*AuthTicket) *AuthTicketStats {
	stats := &AuthTicketStats{}
	if len(items) == 0 {
		return stats
	}
