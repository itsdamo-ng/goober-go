...
type BatchContractFilter struct {
	IDs       []int64   `json:"ids"`
...
type BatchContractListResponse struct {
	Data       []*BatchContract `json:"data"`
...
type BatchContractStats struct {
...
func NewBatchContract(name string) *BatchContract {
	now := time.Now()
...
func (m *BatchContract) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("batch_contract: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("batch_contract: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("batch_contract: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("batch_contract: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("batch_contract: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("batch_contract: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("batch_contract: version must be at least 1")
	}
	return nil
}

func (m *BatchContract) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("batch_contract: marshal failed: %w", err)
	}
	return data, nil
}

func (m *BatchContract) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("batch_contract: unmarshal failed: %w", err)
	}
	return nil
}

func (m *BatchContract) Clone() *BatchContract {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *BatchContract) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("batch_contract: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("batch_contract: transition from %q to %q not allowed", m.Status, status)
}

func (m *BatchContract) Sanitize() {
...
func (m *BatchContract) Merge(other *BatchContract) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *BatchContract) String() string {
	return fmt.Sprintf("BatchContract[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *BatchContract) MatchesFilter(f *BatchContractFilter) bool {
...
func FilterBatchContractList(items []*BatchContract, f *BatchContractFilter) *BatchContractListResponse {
	var filtered []*BatchContract
	...
	return &BatchContractListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateBatchContractStats(items []*BatchContract) *BatchContractStats {
	stats := &BatchContractStats{}
	if len(items) == 0 {
		return stats
	}
