...
func (m *Metric) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("metric: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("metric: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("metric: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("metric: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("metric: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("metric: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("metric: version must be at least 1")
	}
	return nil
}
...
func (m *Metric) ToJSON() ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("metric: marshal failed: %w", err)
	}
	return data, nil
}

func (m *Metric) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("metric: unmarshal failed: %w", err)
	}
	return nil
}
...
func (m *Metric) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("metric: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("metric: transition from %q to %q not allowed", m.Status, status)
}
...
func (m *Metric) String() string {
	return fmt.Sprintf("Metric[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Metric) MatchesFilter(f *MetricFilter) bool {
...
func FilterMetricList(items []*Metric, f *MetricFilter) *MetricListResponse {
	var filtered []*Metric
...
func CalculateMetricStats(items []*Metric) *MetricStats {
	stats := &MetricStats{}
	if len(items) == 0 {
		return stats
	}
