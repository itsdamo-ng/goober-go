...
func NewReview(name string) *Review {
	now := time.Now()
	return &Review{
		Name:      name,
		Status:    "pending",
...
func (m *Review) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("review: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("review: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("review: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("review: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("review: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("review: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("review: version must be at least 1")
	}
	return nil
}

func (m *Review) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("review: marshal failed: %w", err)
	}
	return data, nil
}

func (m *Review) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("review: unmarshal failed: %w", err)
	}
	return nil
}

func (m *Review) Clone() *Review {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *Review) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("review: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("review: transition from %q to %q not allowed", m.Status, status)
}

func (m *Review) Sanitize() {
...
func (m *Review) Merge(other *Review) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Review) String() string {
	return fmt.Sprintf("Review[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Review) MatchesFilter(f *ReviewFilter) bool {
...
func FilterReviewList(items []*Review, f *ReviewFilter) *ReviewListResponse {
	var filtered []*Review
...
func CalculateReviewStats(items []*Review) *ReviewStats {
	stats := &ReviewStats{}
	if len(items) == 0 {
		return stats
	}
