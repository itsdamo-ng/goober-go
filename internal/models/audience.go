...
func (m *Audience) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("audience: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("audience: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("audience: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("audience: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("audience: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("audience: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("audience: version must be at least 1")
	}
	return nil
}

func (m *Audience) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("audience: marshal failed: %w", err)
	}
	return data, nil
}

func (m *Audience) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("audience: unmarshal failed: %w", err)
	}
	return nil
}

func (m *Audience) Clone() *Audience {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *Audience) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("audience: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("audience: transition from %q to %q not allowed", m.Status, status)
}

func (m *Audience) Sanitize() {
...
func (m *Audience) Merge(other *Audience) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Audience) String() string {
	return fmt.Sprintf("Audience[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Audience) MatchesFilter(f *AudienceFilter) bool {
...
func FilterAudienceList(items []*Audience, f *AudienceFilter) *AudienceListResponse {
...
func CalculateAudienceStats(items []*Audience) *AudienceStats {
	stats := &AudienceStats{}
	if len(items) == 0 {
		return stats
	}
