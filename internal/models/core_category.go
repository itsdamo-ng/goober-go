...
type CoreCategoryFilter struct {
	IDs       []int64   `json:"ids"`
...
type CoreCategoryListResponse struct {
	Data       []*CoreCategory `json:"data"`
...
func NewCoreCategory(name string) *CoreCategory {
	now := time.Now()
	return &CoreCategory{
		Name:      name,
		Status:    "pending",
...
func (m *CoreCategory) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("core_category: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("core_category: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("core_category: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("core_category: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("core_category: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("core_category: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("core_category: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("core_category: version must be at least 1")
	}
	return nil
}

func (m *CoreCategory) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("core_category: marshal failed: %w", err)
	}
	return data, nil
}

func (m *CoreCategory) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("core_category: unmarshal failed: %w", err)
	}
	return nil
}

func (m *CoreCategory) Clone() *CoreCategory {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *CoreCategory) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("core_category: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("core_category: transition from %q to %q not allowed", m.Status, status)
}

func (m *CoreCategory) Sanitize() {
...
func (m *CoreCategory) Merge(other *CoreCategory) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *CoreCategory) String() string {
	return fmt.Sprintf("CoreCategory[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *CoreCategory) MatchesFilter(f *CoreCategoryFilter) bool {
...
func FilterCoreCategoryList(items []*CoreCategory, f *CoreCategoryFilter) *CoreCategoryListResponse {
	var filtered []*CoreCategory
	...
	return &CoreCategoryListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateCoreCategoryStats(items []*CoreCategory) *CoreCategoryStats {
	stats := &CoreCategoryStats{}
	if len(items) == 0 {
		return stats
	}
