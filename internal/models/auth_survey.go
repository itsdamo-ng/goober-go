...
type AuthSurveyListResponse struct {
	Data       []*AuthSurvey `json:"data"`
...
func NewAuthSurvey(name string) *AuthSurvey {
	now := time.Now()
	return &AuthSurvey{
		Name:      name,
		Status:    "pending",
...
func (m *AuthSurvey) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("auth_survey: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("auth_survey: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("auth_survey: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("auth_survey: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("auth_survey: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("auth_survey: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("auth_survey: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("auth_survey: version must be at least 1")
	}
	return nil
}

func (m *AuthSurvey) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("auth_survey: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuthSurvey) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("auth_survey: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuthSurvey) Clone() *AuthSurvey {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuthSurvey) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("auth_survey: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("auth_survey: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuthSurvey) Sanitize() {
...
func (m *AuthSurvey) Merge(other *AuthSurvey) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuthSurvey) String() string {
	return fmt.Sprintf("AuthSurvey[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuthSurvey) MatchesFilter(f *AuthSurveyFilter) bool {
...
func FilterAuthSurveyList(items []*AuthSurvey, f *AuthSurveyFilter) *AuthSurveyListResponse {
	var filtered []*AuthSurvey
...
func CalculateAuthSurveyStats(items []*AuthSurvey) *AuthSurveyStats {
	stats := &AuthSurveyStats{}
	if len(items) == 0 {
		return stats
	}
