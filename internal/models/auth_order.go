...
type AuthOrderListResponse struct {
	Data       []*AuthOrder `json:"data"`
...
func NewAuthOrder(name string) *AuthOrder {
	now := time.Now()
	return &AuthOrder{
		Name:      name,
		Status:    "pending",
...
func (m *AuthOrder) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("auth_order: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("auth_order: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("auth_order: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("auth_order: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("auth_order: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("auth_order: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("auth_order: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("auth_order: version must be at least 1")
	}
	return nil
}

func (m *AuthOrder) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("auth_order: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuthOrder) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("auth_order: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuthOrder) Clone() *AuthOrder {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuthOrder) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("auth_order: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("auth_order: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuthOrder) Sanitize() {
...
func (m *AuthOrder) Merge(other *AuthOrder) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuthOrder) String() string {
	return fmt.Sprintf("AuthOrder[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuthOrder) MatchesFilter(f *AuthOrderFilter) bool {
...
func FilterAuthOrderList(items []*AuthOrder, f *AuthOrderFilter) *AuthOrderListResponse {
	var filtered []*AuthOrder
...
func CalculateAuthOrderStats(items []*AuthOrder) *AuthOrderStats {
	stats := &AuthOrderStats{}
	if len(items) == 0 {
		return stats
	}
