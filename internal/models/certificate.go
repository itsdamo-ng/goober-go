...
func (m *Certificate) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("certificate: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("certificate: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("certificate: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("certificate: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("certificate: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("certificate: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("certificate: version must be at least 1")
	}
	return nil
}

func (m *Certificate) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("certificate: marshal failed: %w", err)
	}
	return data, nil
}

func (m *Certificate) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("certificate: unmarshal failed: %w", err)
	}
	return nil
}

func (m *Certificate) Clone() *Certificate {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *Certificate) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("certificate: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("certificate: transition from %q to %q not allowed", m.Status, status)
}

func (m *Certificate) Sanitize() {
...
func (m *Certificate) Merge(other *Certificate) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Certificate) String() string {
	return fmt.Sprintf("Certificate[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Certificate) MatchesFilter(f *CertificateFilter) bool {
...
func FilterCertificateList(items []*Certificate, f *CertificateFilter) *CertificateListResponse {
...
func CalculateCertificateStats(items []*Certificate) *CertificateStats {
