...
type ImportRuleListResponse struct {
	Data       []*ImportRule `json:"data"`
...
func NewImportRule(name string) *ImportRule {
	now := time.Now()
	return &ImportRule{
		Name:      name,
		Status:    "pending",
...
func (m *ImportRule) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("import_rule: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("import_rule: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("import_rule: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("import_rule: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("import_rule: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("import_rule: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("import_rule: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("import_rule: version must be at least 1")
	}
	return nil
}

func (m *ImportRule) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("import_rule: marshal failed: %w", err)
	}
	return data, nil
}

func (m *ImportRule) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("import_rule: unmarshal failed: %w", err)
	}
	return nil
}

func (m *ImportRule) Clone() *ImportRule {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *ImportRule) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("import_rule: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("import_rule: transition from %q to %q not allowed", m.Status, status)
}

func (m *ImportRule) Sanitize() {
...
func (m *ImportRule) Merge(other *ImportRule) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *ImportRule) String() string {
	return fmt.Sprintf("ImportRule[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *ImportRule) MatchesFilter(f *ImportRuleFilter) bool {
...
func FilterImportRuleList(items []*ImportRule, f *ImportRuleFilter) *ImportRuleListResponse {
	var filtered []*ImportRule
...
func CalculateImportRuleStats(items []*ImportRule) *ImportRuleStats {
	stats := &ImportRuleStats{}
	if len(items) == 0 {
		return stats
	}
