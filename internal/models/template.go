...
func (m *Template) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("template: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("template: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("template: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("template: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("template: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("template: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("template: version must be at least 1")
	}
	return nil
}

func (m *Template) ToJSON() ([]byte, error) {
...
func (m *Template) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("template: unmarshal failed: %w", err)
	}
	return nil
}

func (m *Template) Clone() *Template {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *Template) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("template: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("template: transition from %q to %q not allowed", m.Status, status)
}

func (m *Template) Sanitize() {
...
func (m *Template) Merge(other *Template) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Template) String() string {
	return fmt.Sprintf("Template[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Template) MatchesFilter(f *TemplateFilter) bool {
...
func FilterTemplateList(items []*Template, f *TemplateFilter) *TemplateListResponse {
...
func CalculateTemplateStats(items []*Template) *TemplateStats {
	stats := &TemplateStats{}
	if len(items) == 0 {
		return stats
	}
