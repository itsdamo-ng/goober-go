...
type StreamingTaskFilter struct {
	IDs       []int64   `json:"ids"`
...
type StreamingTaskListResponse struct {
	Data       []*StreamingTask `json:"data"`
...
type StreamingTaskStats struct {
...
func NewStreamingTask(name string) *StreamingTask {
	now := time.Now()
...
func (m *StreamingTask) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("streaming_task: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("streaming_task: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("streaming_task: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("streaming_task: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("streaming_task: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("streaming_task: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("streaming_task: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("streaming_task: version must be at least 1")
	}
	return nil
}

func (m *StreamingTask) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("streaming_task: marshal failed: %w", err)
	}
	return data, nil
}

func (m *StreamingTask) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("streaming_task: unmarshal failed: %w", err)
	}
	return nil
}

func (m *StreamingTask) Clone() *StreamingTask {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *StreamingTask) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("streaming_task: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("streaming_task: transition from %q to %q not allowed", m.Status, status)
}

func (m *StreamingTask) Sanitize() {
...
func (m *StreamingTask) Merge(other *StreamingTask) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *StreamingTask) String() string {
	return fmt.Sprintf("StreamingTask[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *StreamingTask) MatchesFilter(f *StreamingTaskFilter) bool {
...
func FilterStreamingTaskList(items []*StreamingTask, f *StreamingTaskFilter) *StreamingTaskListResponse {
	var filtered []*StreamingTask
	...
	return &StreamingTaskListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateStreamingTaskStats(items []*StreamingTask) *StreamingTaskStats {
