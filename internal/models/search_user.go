...
func NewSearchUser(name string) *SearchUser {
	now := time.Now()
	return &SearchUser{
		Name:      name,
		Status:    "pending",
...
func (m *SearchUser) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("search_user: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("search_user: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("search_user: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("search_user: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("search_user: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("search_user: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("search_user: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("search_user: version must be at least 1")
	}
	return nil
}

func (m *SearchUser) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("search_user: marshal failed: %w", err)
	}
	return data, nil
}

func (m *SearchUser) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("search_user: unmarshal failed: %w", err)
	}
	return nil
}

func (m *SearchUser) Clone() *SearchUser {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *SearchUser) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("search_user: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("search_user: transition from %q to %q not allowed", m.Status, status)
}

func (m *SearchUser) Sanitize() {
...
func (m *SearchUser) Merge(other *SearchUser) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *SearchUser) String() string {
	return fmt.Sprintf("SearchUser[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *SearchUser) MatchesFilter(f *SearchUserFilter) bool {
...
func FilterSearchUserList(items []*SearchUser, f *SearchUserFilter) *SearchUserListResponse {
	var filtered []*SearchUser
...
func CalculateSearchUserStats(items []*SearchUser) *SearchUserStats {
	stats := &SearchUserStats{}
	if len(items) == 0 {
		return stats
	}
