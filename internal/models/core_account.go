...
type CoreAccountListResponse struct {
	Data       []*CoreAccount `json:"data"`
...
func NewCoreAccount(name string) *CoreAccount {
	now := time.Now()
	return &CoreAccount{
		Name:      name,
		Status:    "pending",
...
func (m *CoreAccount) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("core_account: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("core_account: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("core_account: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("core_account: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("core_account: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("core_account: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("core_account: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("core_account: version must be at least 1")
	}
	return nil
}

func (m *CoreAccount) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("core_account: marshal failed: %w", err)
	}
	return data, nil
}

func (m *CoreAccount) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("core_account: unmarshal failed: %w", err)
	}
	return nil
}

func (m *CoreAccount) Clone() *CoreAccount {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *CoreAccount) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("core_account: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("core_account: transition from %q to %q not allowed", m.Status, status)
}

func (m *CoreAccount) Sanitize() {
...
func (m *CoreAccount) Merge(other *CoreAccount) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *CoreAccount) String() string {
	return fmt.Sprintf("CoreAccount[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *CoreAccount) MatchesFilter(f *CoreAccountFilter) bool {
...
func FilterCoreAccountList(items []*CoreAccount, f *CoreAccountFilter) *CoreAccountListResponse {
	var filtered []*CoreAccount
	...
	return &CoreAccountListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateCoreAccountStats(items []*CoreAccount) *CoreAccountStats {
	stats := &CoreAccountStats{}
	if len(items) == 0 {
		return stats
	}
