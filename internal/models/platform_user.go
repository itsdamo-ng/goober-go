...
func NewPlatformUser(name string) *PlatformUser {
	now := time.Now()
	return &PlatformUser{
		Name:      name,
		Status:    "pending",
...
func (m *PlatformUser) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("platform_user: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("platform_user: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("platform_user: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("platform_user: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("platform_user: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("platform_user: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("platform_user: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("platform_user: version must be at least 1")
	}
	return nil
}

func (m *PlatformUser) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("platform_user: marshal failed: %w", err)
	}
	return data, nil
}

func (m *PlatformUser) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("platform_user: unmarshal failed: %w", err)
	}
	return nil
}

func (m *PlatformUser) Clone() *PlatformUser {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *PlatformUser) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("platform_user: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("platform_user: transition from %q to %q not allowed", m.Status, status)
}

func (m *PlatformUser) Sanitize() {
...
func (m *PlatformUser) Merge(other *PlatformUser) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *PlatformUser) String() string {
	return fmt.Sprintf("PlatformUser[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *PlatformUser) MatchesFilter(f *PlatformUserFilter) bool {
...
func FilterPlatformUserList(items []*PlatformUser, f *PlatformUserFilter) *PlatformUserListResponse {
	var filtered []*PlatformUser
...
func CalculatePlatformUserStats(items []*PlatformUser) *PlatformUserStats {
