...
type ExportOrderListResponse struct {
	Data       []*ExportOrder `json:"data"`
...
func NewExportOrder(name string) *ExportOrder {
	now := time.Now()
	return &ExportOrder{
		Name:      name,
		Status:    "pending",
...
func (m *ExportOrder) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("export_order: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("export_order: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("export_order: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("export_order: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("export_order: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("export_order: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("export_order: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("export_order: version must be at least 1")
	}
	return nil
}

func (m *ExportOrder) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("export_order: marshal failed: %w", err)
	}
	return data, nil
}

func (m *ExportOrder) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("export_order: unmarshal failed: %w", err)
	}
	return nil
}

func (m *ExportOrder) Clone() *ExportOrder {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *ExportOrder) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("export_order: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("export_order: transition from %q to %q not allowed", m.Status, status)
}

func (m *ExportOrder) Sanitize() {
...
func (m *ExportOrder) Merge(other *ExportOrder) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *ExportOrder) String() string {
	return fmt.Sprintf("ExportOrder[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *ExportOrder) MatchesFilter(f *ExportOrderFilter) bool {
...
func FilterExportOrderList(items []*ExportOrder, f *ExportOrderFilter) *ExportOrderListResponse {
	var filtered []*ExportOrder
...
func CalculateExportOrderStats(items []*ExportOrder) *ExportOrderStats {
	stats := &ExportOrderStats{}
	if len(items) == 0 {
		return stats
	}
