...
type CoreCommentListResponse struct {
	Data       []*CoreComment `json:"data"`
...
func NewCoreComment(name string) *CoreComment {
	now := time.Now()
	return &CoreComment{
		Name:      name,
		Status:    "pending",
...
func (m *CoreComment) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("core_comment: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("core_comment: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("core_comment: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("core_comment: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("core_comment: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("core_comment: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("core_comment: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("core_comment: version must be at least 1")
	}
	return nil
}

func (m *CoreComment) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("core_comment: marshal failed: %w", err)
	}
	return data, nil
}

func (m *CoreComment) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("core_comment: unmarshal failed: %w", err)
	}
	return nil
}

func (m *CoreComment) Clone() *CoreComment {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *CoreComment) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("core_comment: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("core_comment: transition from %q to %q not allowed", m.Status, status)
}

func (m *CoreComment) Sanitize() {
...
func (m *CoreComment) Merge(other *CoreComment) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *CoreComment) String() string {
	return fmt.Sprintf("CoreComment[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *CoreComment) MatchesFilter(f *CoreCommentFilter) bool {
...
func FilterCoreCommentList(items []*CoreComment, f *CoreCommentFilter) *CoreCommentListResponse {
	var filtered []*CoreComment
	...
	return &CoreCommentListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateCoreCommentStats(items []*CoreComment) *CoreCommentStats {
	stats := &CoreCommentStats{}
	if len(items) == 0 {
		return stats
	}
