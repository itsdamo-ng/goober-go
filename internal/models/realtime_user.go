...
type RealtimeUserFilter struct {
	IDs       []int64   `json:"ids"`
...
type RealtimeUserListResponse struct {
	Data       []*RealtimeUser `json:"data"`
...
func NewRealtimeUser(name string) *RealtimeUser {
	now := time.Now()
	return &RealtimeUser{
		Name:      name,
		Status:    "pending",
...
func (m *RealtimeUser) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("realtime_user: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("realtime_user: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("realtime_user: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("realtime_user: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("realtime_user: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("realtime_user: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("realtime_user: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("realtime_user: version must be at least 1")
	}
	return nil
}

func (m *RealtimeUser) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("realtime_user: marshal failed: %w", err)
	}
	return data, nil
}

func (m *RealtimeUser) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("realtime_user: unmarshal failed: %w", err)
	}
	return nil
}

func (m *RealtimeUser) Clone() *RealtimeUser {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *RealtimeUser) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("realtime_user: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("realtime_user: transition from %q to %q not allowed", m.Status, status)
}

func (m *RealtimeUser) Sanitize() {
...
func (m *RealtimeUser) Merge(other *RealtimeUser) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *RealtimeUser) String() string {
	return fmt.Sprintf("RealtimeUser[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *RealtimeUser) MatchesFilter(f *RealtimeUserFilter) bool {
...
func FilterRealtimeUserList(items []*RealtimeUser, f *RealtimeUserFilter) *RealtimeUserListResponse {
	var filtered []*RealtimeUser
	...
	return &RealtimeUserListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateRealtimeUserStats(items []*RealtimeUser) *RealtimeUserStats {
