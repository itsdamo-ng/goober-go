...
type StreamingEventListResponse struct {
	Data       []*StreamingEvent `json:"data"`
...
type StreamingEventStats struct {
...
func NewStreamingEvent(name string) *StreamingEvent {
	now := time.Now()
...
func (m *StreamingEvent) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("streaming_event: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("streaming_event: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("streaming_event: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("streaming_event: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("streaming_event: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("streaming_event: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("streaming_event: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("streaming_event: version must be at least 1")
	}
	return nil
}

func (m *StreamingEvent) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("streaming_event: marshal failed: %w", err)
	}
	return data, nil
}

func (m *StreamingEvent) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("streaming_event: unmarshal failed: %w", err)
	}
	return nil
}

func (m *StreamingEvent) Clone() *StreamingEvent {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *StreamingEvent) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("streaming_event: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("streaming_event: transition from %q to %q not allowed", m.Status, status)
}

func (m *StreamingEvent) Sanitize() {
...
func (m *StreamingEvent) Merge(other *StreamingEvent) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *StreamingEvent) String() string {
	return fmt.Sprintf("StreamingEvent[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *StreamingEvent) MatchesFilter(f *StreamingEventFilter) bool {
...
func FilterStreamingEventList(items []*StreamingEvent, f *StreamingEventFilter) *StreamingEventListResponse {
	var filtered []*StreamingEvent
...
func CalculateStreamingEventStats(items []*StreamingEvent) *StreamingEventStats {
	stats := &StreamingEventStats{}
	if len(items) == 0 {
		return stats
	}
