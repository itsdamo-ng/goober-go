...
type InventoryTaskFilter struct {
	IDs       []int64   `json:"ids"`
...
type InventoryTaskListResponse struct {
	Data       []*InventoryTask `json:"data"`
...
func NewInventoryTask(name string) *InventoryTask {
	now := time.Now()
...
func (m *InventoryTask) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("inventory_task: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("inventory_task: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("inventory_task: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("inventory_task: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("inventory_task: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("inventory_task: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("inventory_task: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("inventory_task: version must be at least 1")
	}
	return nil
}

func (m *InventoryTask) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("inventory_task: marshal failed: %w", err)
	}
	return data, nil
}

func (m *InventoryTask) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("inventory_task: unmarshal failed: %w", err)
	}
	return nil
}

func (m *InventoryTask) Clone() *InventoryTask {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *InventoryTask) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("inventory_task: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("inventory_task: transition from %q to %q not allowed", m.Status, status)
}

func (m *InventoryTask) Sanitize() {
...
func (m *InventoryTask) Merge(other *InventoryTask) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *InventoryTask) String() string {
	return fmt.Sprintf("InventoryTask[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *InventoryTask) MatchesFilter(f *InventoryTaskFilter) bool {
...
func FilterInventoryTaskList(items []*InventoryTask, f *InventoryTaskFilter) *InventoryTaskListResponse {
	var filtered []*InventoryTask
...
func CalculateInventoryTaskStats(items []*InventoryTask) *InventoryTaskStats {
