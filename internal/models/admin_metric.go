...
type AdminMetricListResponse struct {
	Data       []*AdminMetric `json:"data"`
...
func NewAdminMetric(name string) *AdminMetric {
	now := time.Now()
	return &AdminMetric{
		Name:      name,
		Status:    "pending",
...
func (m *AdminMetric) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("admin_metric: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_metric: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_metric: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_metric: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_metric: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_metric: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_metric: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_metric: version must be at least 1")
	}
	return nil
}

func (m *AdminMetric) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_metric: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminMetric) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_metric: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminMetric) Clone() *AdminMetric {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminMetric) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_metric: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_metric: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminMetric) Sanitize() {
...
func (m *AdminMetric) Merge(other *AdminMetric) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminMetric) String() string {
	return fmt.Sprintf("AdminMetric[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminMetric) MatchesFilter(f *AdminMetricFilter) bool {
...
func FilterAdminMetricList(items []*AdminMetric, f *AdminMetricFilter) *AdminMetricListResponse {
	var filtered []*AdminMetric
	...
	return &AdminMetricListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateAdminMetricStats(items []*AdminMetric) *AdminMetricStats {
