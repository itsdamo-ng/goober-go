...
func NewImportUser(name string) *ImportUser {
	now := time.Now()
	return &ImportUser{
		Name:      name,
		Status:    "pending",
...
func (m *ImportUser) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("import_user: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("import_user: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("import_user: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("import_user: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("import_user: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("import_user: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("import_user: version must be at least 1")
	}
	return nil
}

func (m *ImportUser) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("import_user: marshal failed: %w", err)
	}
	return data, nil
}

func (m *ImportUser) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("import_user: unmarshal failed: %w", err)
	}
	return nil
}

func (m *ImportUser) Clone() *ImportUser {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *ImportUser) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("import_user: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("import_user: transition from %q to %q not allowed", m.Status, status)
}

func (m *ImportUser) Sanitize() {
...
func (m *ImportUser) Merge(other *ImportUser) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *ImportUser) String() string {
	return fmt.Sprintf("ImportUser[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *ImportUser) MatchesFilter(f *ImportUserFilter) bool {
...
func FilterImportUserList(items []*ImportUser, f *ImportUserFilter) *ImportUserListResponse {
	var filtered []*ImportUser
...
func CalculateImportUserStats(items []*ImportUser) *ImportUserStats {
	stats := &ImportUserStats{}
	if len(items) == 0 {
		return stats
	}
