...
type CoreTaskListResponse struct {
	Data       []*CoreTask `json:"data"`
...
func NewCoreTask(name string) *CoreTask {
	now := time.Now()
	return &CoreTask{
		Name:      name,
		Status:    "pending",
...
func (m *CoreTask) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("core_task: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("core_task: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("core_task: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("core_task: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("core_task: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("core_task: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("core_task: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("core_task: version must be at least 1")
	}
	return nil
}

func (m *CoreTask) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("core_task: marshal failed: %w", err)
	}
	return data, nil
}

func (m *CoreTask) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("core_task: unmarshal failed: %w", err)
	}
	return nil
}

func (m *CoreTask) Clone() *CoreTask {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *CoreTask) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("core_task: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("core_task: transition from %q to %q not allowed", m.Status, status)
}

func (m *CoreTask) Sanitize() {
...
func (m *CoreTask) Merge(other *CoreTask) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *CoreTask) String() string {
	return fmt.Sprintf("CoreTask[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *CoreTask) MatchesFilter(f *CoreTaskFilter) bool {
...
func FilterCoreTaskList(items []*CoreTask, f *CoreTaskFilter) *CoreTaskListResponse {
	var filtered []*CoreTask
...
func CalculateCoreTaskStats(items []*CoreTask) *CoreTaskStats {
	stats := &CoreTaskStats{}
	if len(items) == 0 {
		return stats
	}
