...
type SearchRuleListResponse struct {
...
func NewSearchRule(name string) *SearchRule {
	now := time.Now()
	return &SearchRule{
		Name:      name,
		Status:    "pending",
...
func (m *SearchRule) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("search_rule: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("search_rule: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("search_rule: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("search_rule: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("search_rule: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("search_rule: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("search_rule: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("search_rule: version must be at least 1")
	}
	return nil
}

func (m *SearchRule) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("search_rule: marshal failed: %w", err)
	}
	return data, nil
}

func (m *SearchRule) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("search_rule: unmarshal failed: %w", err)
	}
	return nil
}

func (m *SearchRule) Clone() *SearchRule {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *SearchRule) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("search_rule: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("search_rule: transition from %q to %q not allowed", m.Status, status)
}

func (m *SearchRule) Sanitize() {
...
func (m *SearchRule) Merge(other *SearchRule) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *SearchRule) String() string {
	return fmt.Sprintf("SearchRule[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *SearchRule) MatchesFilter(f *SearchRuleFilter) bool {
...
func FilterSearchRuleList(items []*SearchRule, f *SearchRuleFilter) *SearchRuleListResponse {
	var filtered []*SearchRule
...
func CalculateSearchRuleStats(items []*SearchRule) *SearchRuleStats {
	stats := &SearchRuleStats{}
	if len(items) == 0 {
		return stats
	}
