...
type AuditQuotaListResponse struct {
	Data       []*AuditQuota `json:"data"`
...
func NewAuditQuota(name string) *AuditQuota {
	now := time.Now()
	return &AuditQuota{
		Name:      name,
		Status:    "pending",
...
func (m *AuditQuota) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("audit_quota: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("audit_quota: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("audit_quota: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("audit_quota: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("audit_quota: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("audit_quota: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("audit_quota: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("audit_quota: version must be at least 1")
	}
	return nil
}

func (m *AuditQuota) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("audit_quota: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuditQuota) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("audit_quota: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuditQuota) Clone() *AuditQuota {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuditQuota) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("audit_quota: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("audit_quota: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuditQuota) Sanitize() {
...
func (m *AuditQuota) Merge(other *AuditQuota) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuditQuota) String() string {
	return fmt.Sprintf("AuditQuota[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuditQuota) MatchesFilter(f *AuditQuotaFilter) bool {
...
func FilterAuditQuotaList(items []*AuditQuota, f *AuditQuotaFilter) *AuditQuotaListResponse {
	var filtered []*AuditQuota
...
func CalculateAuditQuotaStats(items []*AuditQuota) *AuditQuotaStats {
	stats := &AuditQuotaStats{}
	if len(items) == 0 {
		return stats
	}
