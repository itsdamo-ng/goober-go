...
func NewAdminPolicy(name string) *AdminPolicy {
	now := time.Now()
	return &AdminPolicy{
		Name:      name,
		Status:    "pending",
...
func (m *AdminPolicy) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("admin_policy: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_policy: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_policy: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_policy: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_policy: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_policy: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_policy: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_policy: version must be at least 1")
	}
	return nil
}

func (m *AdminPolicy) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_policy: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminPolicy) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_policy: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminPolicy) Clone() *AdminPolicy {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminPolicy) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_policy: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_policy: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminPolicy) Sanitize() {
...
func (m *AdminPolicy) Merge(other *AdminPolicy) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminPolicy) String() string {
	return fmt.Sprintf("AdminPolicy[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminPolicy) MatchesFilter(f *AdminPolicyFilter) bool {
...
func FilterAdminPolicyList(items []*AdminPolicy, f *AdminPolicyFilter) *AdminPolicyListResponse {
	var filtered []*AdminPolicy
...
func CalculateAdminPolicyStats(items []*AdminPolicy) *AdminPolicyStats {
	stats := &AdminPolicyStats{}
	if len(items) == 0 {
		return stats
	}
