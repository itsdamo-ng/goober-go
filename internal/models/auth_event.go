...
func NewAuthEvent(name string) *AuthEvent {
	now := time.Now()
	return &AuthEvent{
		Name:      name,
		Status:    "pending",
...
func (m *AuthEvent) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("auth_event: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("auth_event: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("auth_event: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("auth_event: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("auth_event: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("auth_event: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("auth_event: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("auth_event: version must be at least 1")
	}
	return nil
}

func (m *AuthEvent) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("auth_event: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuthEvent) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("auth_event: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuthEvent) Clone() *AuthEvent {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuthEvent) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("auth_event: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("auth_event: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuthEvent) Sanitize() {
...
func (m *AuthEvent) Merge(other *AuthEvent) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuthEvent) String() string {
	return fmt.Sprintf("AuthEvent[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuthEvent) MatchesFilter(f *AuthEventFilter) bool {
...
func FilterAuthEventList(items []*AuthEvent, f *AuthEventFilter) *AuthEventListResponse {
...
func CalculateAuthEventStats(items []*AuthEvent) *AuthEventStats {
