...
func NewSupportTicket(name string) *SupportTicket {
	now := time.Now()
...
func (m *SupportTicket) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("support_ticket: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("support_ticket: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("support_ticket: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("support_ticket: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("support_ticket: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("support_ticket: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("support_ticket: version must be at least 1")
	}
	return nil
}

func (m *SupportTicket) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("support_ticket: marshal failed: %w", err)
	}
	return data, nil
}

func (m *SupportTicket) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("support_ticket: unmarshal failed: %w", err)
	}
	return nil
}

func (m *SupportTicket) Clone() *SupportTicket {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *SupportTicket) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("support_ticket: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("support_ticket: transition from %q to %q not allowed", m.Status, status)
}

func (m *SupportTicket) Sanitize() {
...
func (m *SupportTicket) Merge(other *SupportTicket) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *SupportTicket) String() string {
	return fmt.Sprintf("SupportTicket[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *SupportTicket) MatchesFilter(f *SupportTicketFilter) bool {
...
func FilterSupportTicketList(items []*SupportTicket, f *SupportTicketFilter) *SupportTicketListResponse {
...
func CalculateSupportTicketStats(items []*SupportTicket) *SupportTicketStats {
