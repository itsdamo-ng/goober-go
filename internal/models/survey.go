...
func (m *Survey) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("survey: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("survey: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("survey: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("survey: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("survey: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("survey: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("survey: version must be at least 1")
	}
	return nil
}

func (m *Survey) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("survey: marshal failed: %w", err)
	}
	return data, nil
}

func (m *Survey) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("survey: unmarshal failed: %w", err)
	}
	return nil
}

func (m *Survey) Clone() *Survey {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *Survey) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("survey: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("survey: transition from %q to %q not allowed", m.Status, status)
}

func (m *Survey) Sanitize() {
...
func (m *Survey) Merge(other *Survey) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Survey) String() string {
	return fmt.Sprintf("Survey[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Survey) MatchesFilter(f *SurveyFilter) bool {
...
func FilterSurveyList(items []*Survey, f *SurveyFilter) *SurveyListResponse {
	var filtered []*Survey
...
func CalculateSurveyStats(items []*Survey) *SurveyStats {
	stats := &SurveyStats{}
	if len(items) == 0 {
		return stats
	}
