...
func (m *Expense) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("expense: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("expense: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("expense: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("expense: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("expense: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("expense: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("expense: version must be at least 1")
	}
	return nil
}

func (m *Expense) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("expense: marshal failed: %w", err)
	}
	return data, nil
}

func (m *Expense) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("expense: unmarshal failed: %w", err)
	}
	return nil
}

func (m *Expense) Clone() *Expense {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *Expense) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("expense: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("expense: transition from %q to %q not allowed", m.Status, status)
}

func (m *Expense) Sanitize() {
...
func (m *Expense) Merge(other *Expense) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Expense) String() string {
	return fmt.Sprintf("Expense[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Expense) MatchesFilter(f *ExpenseFilter) bool {
...
func FilterExpenseList(items []*Expense, f *ExpenseFilter) *ExpenseListResponse {
...
func CalculateExpenseStats(items []*Expense) *ExpenseStats {
	stats := &ExpenseStats{}
	if len(items) == 0 {
		return stats
	}
