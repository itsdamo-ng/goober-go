...
type ExportRoleListResponse struct {
	Data       []*ExportRole `json:"data"`
...
func NewExportRole(name string) *ExportRole {
	now := time.Now()
	return &ExportRole{
		Name:      name,
		Status:    "pending",
...
func (m *ExportRole) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("export_role: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("export_role: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("export_role: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("export_role: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("export_role: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("export_role: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("export_role: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("export_role: version must be at least 1")
	}
	return nil
}

func (m *ExportRole) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("export_role: marshal failed: %w", err)
	}
	return data, nil
}

func (m *ExportRole) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("export_role: unmarshal failed: %w", err)
	}
	return nil
}

func (m *ExportRole) Clone() *ExportRole {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *ExportRole) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("export_role: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("export_role: transition from %q to %q not allowed", m.Status, status)
}

func (m *ExportRole) Sanitize() {
...
func (m *ExportRole) Merge(other *ExportRole) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *ExportRole) String() string {
	return fmt.Sprintf("ExportRole[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *ExportRole) MatchesFilter(f *ExportRoleFilter) bool {
...
func FilterExportRoleList(items []*ExportRole, f *ExportRoleFilter) *ExportRoleListResponse {
	var filtered []*ExportRole
...
func CalculateExportRoleStats(items []*ExportRole) *ExportRoleStats {
	stats := &ExportRoleStats{}
	if len(items) == 0 {
		return stats
	}
