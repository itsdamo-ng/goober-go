...
type BatchSessionFilter struct {
	IDs       []int64   `json:"ids"`
...
type BatchSessionListResponse struct {
	Data       []*BatchSession `json:"data"`
...
func NewBatchSession(name string) *BatchSession {
	now := time.Now()
	return &BatchSession{
		Name:      name,
		Status:    "pending",
...
func (m *BatchSession) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("batch_session: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("batch_session: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("batch_session: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("batch_session: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("batch_session: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("batch_session: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("batch_session: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("batch_session: version must be at least 1")
	}
	return nil
}

func (m *BatchSession) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("batch_session: marshal failed: %w", err)
	}
	return data, nil
}

func (m *BatchSession) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("batch_session: unmarshal failed: %w", err)
	}
	return nil
}

func (m *BatchSession) Clone() *BatchSession {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *BatchSession) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("batch_session: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("batch_session: transition from %q to %q not allowed", m.Status, status)
}

func (m *BatchSession) Sanitize() {
...
func (m *BatchSession) Merge(other *BatchSession) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *BatchSession) String() string {
	return fmt.Sprintf("BatchSession[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *BatchSession) MatchesFilter(f *BatchSessionFilter) bool {
...
func FilterBatchSessionList(items []*BatchSession, f *BatchSessionFilter) *BatchSessionListResponse {
	var filtered []*BatchSession
...
func CalculateBatchSessionStats(items []*BatchSession) *BatchSessionStats {
	stats := &BatchSessionStats{}
	if len(items) == 0 {
		return stats
	}
