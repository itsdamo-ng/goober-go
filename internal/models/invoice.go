...
func (m *Invoice) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("invoice: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("invoice: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("invoice: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("invoice: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("invoice: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("invoice: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("invoice: version must be at least 1")
	}
	return nil
}
...
func (m *Invoice) ToJSON() ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("invoice: marshal failed: %w", err)
	}
	return data, nil
}
...
func (m *Invoice) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("invoice: unmarshal failed: %w", err)
	}
	return nil
}
...
func (m *Invoice) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("invoice: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("invoice: transition from %q to %q not allowed", m.Status, status)
}

func (m *Invoice) Sanitize() {
...
func (m *Invoice) Merge(other *Invoice) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Invoice) String() string {
	return fmt.Sprintf("Invoice[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Invoice) MatchesFilter(f *InvoiceFilter) bool {
...
func FilterInvoiceList(items []*Invoice, f *InvoiceFilter) *InvoiceListResponse {
...
func CalculateInvoiceStats(items []*Invoice) *InvoiceStats {
	stats := &InvoiceStats{}
	if len(items) == 0 {
		return stats
	}
