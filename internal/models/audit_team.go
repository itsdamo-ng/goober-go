...
type AuditTeamListResponse struct {
	Data       []*AuditTeam `json:"data"`
...
func NewAuditTeam(name string) *AuditTeam {
	now := time.Now()
	return &AuditTeam{
		Name:      name,
		Status:    "pending",
...
func (m *AuditTeam) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("audit_team: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("audit_team: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("audit_team: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("audit_team: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("audit_team: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("audit_team: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("audit_team: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("audit_team: version must be at least 1")
	}
	return nil
}

func (m *AuditTeam) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("audit_team: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuditTeam) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("audit_team: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuditTeam) Clone() *AuditTeam {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuditTeam) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("audit_team: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("audit_team: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuditTeam) Sanitize() {
...
func (m *AuditTeam) Merge(other *AuditTeam) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuditTeam) String() string {
	return fmt.Sprintf("AuditTeam[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuditTeam) MatchesFilter(f *AuditTeamFilter) bool {
...
func FilterAuditTeamList(items []*AuditTeam, f *AuditTeamFilter) *AuditTeamListResponse {
	var filtered []*AuditTeam
...
func CalculateAuditTeamStats(items []*AuditTeam) *AuditTeamStats {
	stats := &AuditTeamStats{}
	if len(items) == 0 {
		return stats
	}
