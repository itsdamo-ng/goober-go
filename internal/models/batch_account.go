...
type BatchAccountFilter struct {
	IDs       []int64   `json:"ids"`
...
func NewBatchAccount(name string) *BatchAccount {
	now := time.Now()
	return &BatchAccount{
		Name:      name,
		Status:    "pending",
...
func (m *BatchAccount) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("batch_account: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("batch_account: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("batch_account: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("batch_account: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("batch_account: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("batch_account: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("batch_account: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("batch_account: version must be at least 1")
	}
	return nil
}

func (m *BatchAccount) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("batch_account: marshal failed: %w", err)
	}
	return data, nil
}

func (m *BatchAccount) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("batch_account: unmarshal failed: %w", err)
	}
	return nil
}

func (m *BatchAccount) Clone() *BatchAccount {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *BatchAccount) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("batch_account: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("batch_account: transition from %q to %q not allowed", m.Status, status)
}

func (m *BatchAccount) Sanitize() {
...
func (m *BatchAccount) Merge(other *BatchAccount) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *BatchAccount) String() string {
	return fmt.Sprintf("BatchAccount[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *BatchAccount) MatchesFilter(f *BatchAccountFilter) bool {
...
func FilterBatchAccountList(items []*BatchAccount, f *BatchAccountFilter) *BatchAccountListResponse {
	var filtered []*BatchAccount
	...
	return &BatchAccountListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateBatchAccountStats(items []*BatchAccount) *BatchAccountStats {
	stats := &BatchAccountStats{}
	if len(items) == 0 {
		return stats
	}
