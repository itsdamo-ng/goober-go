...
type BillingTaskListResponse struct {
	Data       []*BillingTask `json:"data"`
...
func NewBillingTask(name string) *BillingTask {
	now := time.Now()
	return &BillingTask{
		Name:      name,
		Status:    "pending",
...
func (m *BillingTask) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("billing_task: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("billing_task: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("billing_task: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("billing_task: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("billing_task: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("billing_task: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("billing_task: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("billing_task: version must be at least 1")
	}
	return nil
}

func (m *BillingTask) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("billing_task: marshal failed: %w", err)
	}
	return data, nil
}

func (m *BillingTask) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("billing_task: unmarshal failed: %w", err)
	}
	return nil
}

func (m *BillingTask) Clone() *BillingTask {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *BillingTask) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("billing_task: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("billing_task: transition from %q to %q not allowed", m.Status, status)
}

func (m *BillingTask) Sanitize() {
...
func (m *BillingTask) Merge(other *BillingTask) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *BillingTask) String() string {
	return fmt.Sprintf("BillingTask[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *BillingTask) MatchesFilter(f *BillingTaskFilter) bool {
...
func FilterBillingTaskList(items []*BillingTask, f *BillingTaskFilter) *BillingTaskListResponse {
	var filtered []*BillingTask
	...
	return &BillingTaskListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateBillingTaskStats(items []*BillingTask) *BillingTaskStats {
	stats := &BillingTaskStats{}
	if len(items) == 0 {
		return stats
	}
