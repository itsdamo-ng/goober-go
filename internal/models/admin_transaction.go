...
func NewAdminTransaction(name string) *AdminTransaction {
	now := time.Now()
...
func (m *AdminTransaction) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("admin_transaction: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_transaction: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_transaction: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_transaction: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_transaction: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_transaction: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_transaction: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_transaction: version must be at least 1")
	}
	return nil
}

func (m *AdminTransaction) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_transaction: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminTransaction) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_transaction: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminTransaction) Clone() *AdminTransaction {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminTransaction) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_transaction: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_transaction: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminTransaction) Sanitize() {
...
func (m *AdminTransaction) Merge(other *AdminTransaction) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminTransaction) String() string {
	return fmt.Sprintf("AdminTransaction[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminTransaction) MatchesFilter(f *AdminTransactionFilter) bool {
...
func FilterAdminTransactionList(items []*AdminTransaction, f *AdminTransactionFilter) *AdminTransactionListResponse {
	var filtered []*AdminTransaction
...
func CalculateAdminTransactionStats(items []*AdminTransaction) *AdminTransactionStats {
