...
type AdminCategoryFilter struct {
	IDs       []int64   `json:"ids"`
...
type AdminCategoryStats struct {
...
func NewAdminCategory(name string) *AdminCategory {
	now := time.Now()
...
func (m *AdminCategory) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("admin_category: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_category: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_category: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_category: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_category: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_category: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_category: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_category: version must be at least 1")
	}
	return nil
}

func (m *AdminCategory) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_category: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminCategory) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_category: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminCategory) Clone() *AdminCategory {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminCategory) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_category: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_category: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminCategory) Sanitize() {
...
func (m *AdminCategory) Merge(other *AdminCategory) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminCategory) String() string {
	return fmt.Sprintf("AdminCategory[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminCategory) MatchesFilter(f *AdminCategoryFilter) bool {
...
func FilterAdminCategoryList(items []*AdminCategory, f *AdminCategoryFilter) *AdminCategoryListResponse {
	var filtered []*AdminCategory
...
func CalculateAdminCategoryStats(items []*AdminCategory) *AdminCategoryStats {
