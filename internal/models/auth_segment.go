...
type AuthSegmentListResponse struct {
	Data       []*AuthSegment `json:"data"`
...
func NewAuthSegment(name string) *AuthSegment {
	now := time.Now()
	return &AuthSegment{
		Name:      name,
		Status:    "pending",
...
func (m *AuthSegment) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("auth_segment: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("auth_segment: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("auth_segment: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("auth_segment: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("auth_segment: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("auth_segment: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("auth_segment: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("auth_segment: version must be at least 1")
	}
	return nil
}

func (m *AuthSegment) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("auth_segment: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuthSegment) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("auth_segment: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuthSegment) Clone() *AuthSegment {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuthSegment) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("auth_segment: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("auth_segment: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuthSegment) Sanitize() {
...
func (m *AuthSegment) Merge(other *AuthSegment) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuthSegment) String() string {
	return fmt.Sprintf("AuthSegment[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuthSegment) MatchesFilter(f *AuthSegmentFilter) bool {
...
func FilterAuthSegmentList(items []*AuthSegment, f *AuthSegmentFilter) *AuthSegmentListResponse {
	var filtered []*AuthSegment
	...
	return &AuthSegmentListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateAuthSegmentStats(items []*AuthSegment) *AuthSegmentStats {
	stats := &AuthSegmentStats{}
	if len(items) == 0 {
		return stats
	}
