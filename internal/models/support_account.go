...
type SupportAccountFilter struct {
	IDs       []int64   `json:"ids"`
...
type SupportAccountListResponse struct {
	Data       []*SupportAccount `json:"data"`
...
type SupportAccountStats struct {
...
func NewSupportAccount(name string) *SupportAccount {
	now := time.Now()
...
func (m *SupportAccount) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("support_account: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("support_account: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("support_account: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("support_account: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("support_account: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("support_account: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("support_account: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("support_account: version must be at least 1")
	}
	return nil
}

func (m *SupportAccount) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("support_account: marshal failed: %w", err)
	}
	return data, nil
}

func (m *SupportAccount) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("support_account: unmarshal failed: %w", err)
	}
	return nil
}

func (m *SupportAccount) Clone() *SupportAccount {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *SupportAccount) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("support_account: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("support_account: transition from %q to %q not allowed", m.Status, status)
}

func (m *SupportAccount) Sanitize() {
...
func (m *SupportAccount) Merge(other *SupportAccount) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *SupportAccount) String() string {
	return fmt.Sprintf("SupportAccount[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *SupportAccount) MatchesFilter(f *SupportAccountFilter) bool {
...
func FilterSupportAccountList(items []*SupportAccount, f *SupportAccountFilter) *SupportAccountListResponse {
	var filtered []*SupportAccount
...
func CalculateSupportAccountStats(items []*SupportAccount) *SupportAccountStats {
	stats := &SupportAccountStats{}
	if len(items) == 0 {
		return stats
	}
