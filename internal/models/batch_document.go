...
type BatchDocumentListResponse struct {
	Data       []*BatchDocument `json:"data"`
...
type BatchDocumentStats struct {
...
func NewBatchDocument(name string) *BatchDocument {
	now := time.Now()
...
func (m *BatchDocument) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("batch_document: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("batch_document: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("batch_document: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("batch_document: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("batch_document: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("batch_document: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("batch_document: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("batch_document: version must be at least 1")
	}
	return nil
}

func (m *BatchDocument) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("batch_document: marshal failed: %w", err)
	}
	return data, nil
}

func (m *BatchDocument) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("batch_document: unmarshal failed: %w", err)
	}
	return nil
}

func (m *BatchDocument) Clone() *BatchDocument {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *BatchDocument) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("batch_document: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("batch_document: transition from %q to %q not allowed", m.Status, status)
}

func (m *BatchDocument) Sanitize() {
...
func (m *BatchDocument) Merge(other *BatchDocument) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *BatchDocument) String() string {
	return fmt.Sprintf("BatchDocument[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *BatchDocument) MatchesFilter(f *BatchDocumentFilter) bool {
...
func FilterBatchDocumentList(items []*BatchDocument, f *BatchDocumentFilter) *BatchDocumentListResponse {
	var filtered []*BatchDocument
	...
	return &BatchDocumentListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateBatchDocumentStats(items []*BatchDocument) *BatchDocumentStats {
	stats := &BatchDocumentStats{}
	if len(items) == 0 {
		return stats
	}
