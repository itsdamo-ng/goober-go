...
type MarketingLeadFilter struct {
	IDs       []int64   `json:"ids"`
...
type MarketingLeadListResponse struct {
	Data       []*MarketingLead `json:"data"`
...
type MarketingLeadStats struct {
...
func NewMarketingLead(name string) *MarketingLead {
	now := time.Now()
...
func (m *MarketingLead) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("marketing_lead: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("marketing_lead: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("marketing_lead: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("marketing_lead: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("marketing_lead: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("marketing_lead: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("marketing_lead: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("marketing_lead: version must be at least 1")
	}
	return nil
}

func (m *MarketingLead) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("marketing_lead: marshal failed: %w", err)
	}
	return data, nil
}

func (m *MarketingLead) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("marketing_lead: unmarshal failed: %w", err)
	}
	return nil
}

func (m *MarketingLead) Clone() *MarketingLead {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *MarketingLead) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("marketing_lead: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("marketing_lead: transition from %q to %q not allowed", m.Status, status)
}

func (m *MarketingLead) Sanitize() {
...
func (m *MarketingLead) Merge(other *MarketingLead) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *MarketingLead) String() string {
	return fmt.Sprintf("MarketingLead[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *MarketingLead) MatchesFilter(f *MarketingLeadFilter) bool {
...
func FilterMarketingLeadList(items []*MarketingLead, f *MarketingLeadFilter) *MarketingLeadListResponse {
	var filtered []*MarketingLead
...
func CalculateMarketingLeadStats(items []*MarketingLead) *MarketingLeadStats {
	stats := &MarketingLeadStats{}
	if len(items) == 0 {
		return stats
	}
