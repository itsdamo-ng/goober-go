...
type BatchAlertListResponse struct {
	Data       []*BatchAlert `json:"data"`
...
func NewBatchAlert(name string) *BatchAlert {
	now := time.Now()
	return &BatchAlert{
		Name:      name,
		Status:    "pending",
...
func (m *BatchAlert) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("batch_alert: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("batch_alert: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("batch_alert: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("batch_alert: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("batch_alert: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("batch_alert: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("batch_alert: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("batch_alert: version must be at least 1")
	}
	return nil
}

func (m *BatchAlert) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("batch_alert: marshal failed: %w", err)
	}
	return data, nil
}

func (m *BatchAlert) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("batch_alert: unmarshal failed: %w", err)
	}
	return nil
}

func (m *BatchAlert) Clone() *BatchAlert {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *BatchAlert) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("batch_alert: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("batch_alert: transition from %q to %q not allowed", m.Status, status)
}

func (m *BatchAlert) Sanitize() {
...
func (m *BatchAlert) Merge(other *BatchAlert) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *BatchAlert) String() string {
	return fmt.Sprintf("BatchAlert[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *BatchAlert) MatchesFilter(f *BatchAlertFilter) bool {
...
func FilterBatchAlertList(items []*BatchAlert, f *BatchAlertFilter) *BatchAlertListResponse {
	var filtered []*BatchAlert
...
func CalculateBatchAlertStats(items []*BatchAlert) *BatchAlertStats {
	stats := &BatchAlertStats{}
	if len(items) == 0 {
		return stats
	}
