...
type ContentUserListResponse struct {
	Data       []*ContentUser `json:"data"`
...
func NewContentUser(name string) *ContentUser {
	now := time.Now()
	return &ContentUser{
		Name:      name,
		Status:    "pending",
...
func (m *ContentUser) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("content_user: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("content_user: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("content_user: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("content_user: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("content_user: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("content_user: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("content_user: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("content_user: version must be at least 1")
	}
	return nil
}

func (m *ContentUser) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("content_user: marshal failed: %w", err)
	}
	return data, nil
}

func (m *ContentUser) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("content_user: unmarshal failed: %w", err)
	}
	return nil
}

func (m *ContentUser) Clone() *ContentUser {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *ContentUser) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("content_user: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("content_user: transition from %q to %q not allowed", m.Status, status)
}

func (m *ContentUser) Sanitize() {
...
func (m *ContentUser) Merge(other *ContentUser) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *ContentUser) String() string {
	return fmt.Sprintf("ContentUser[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *ContentUser) MatchesFilter(f *ContentUserFilter) bool {
...
func FilterContentUserList(items []*ContentUser, f *ContentUserFilter) *ContentUserListResponse {
	var filtered []*ContentUser
	...
	return &ContentUserListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateContentUserStats(items []*ContentUser) *ContentUserStats {
	stats := &ContentUserStats{}
	if len(items) == 0 {
		return stats
	}
