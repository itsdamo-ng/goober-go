...
type AuthMessageListResponse struct {
	Data       []*AuthMessage `json:"data"`
...
func NewAuthMessage(name string) *AuthMessage {
	now := time.Now()
	return &AuthMessage{
		Name:      name,
		Status:    "pending",
...
func (m *AuthMessage) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("auth_message: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("auth_message: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("auth_message: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("auth_message: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("auth_message: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("auth_message: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("auth_message: version must be at least 1")
	}
	return nil
}

func (m *AuthMessage) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("auth_message: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuthMessage) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("auth_message: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuthMessage) Clone() *AuthMessage {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuthMessage) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("auth_message: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("auth_message: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuthMessage) Sanitize() {
...
func (m *AuthMessage) Merge(other *AuthMessage) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuthMessage) String() string {
	return fmt.Sprintf("AuthMessage[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuthMessage) MatchesFilter(f *AuthMessageFilter) bool {
...
func FilterAuthMessageList(items []*AuthMessage, f *AuthMessageFilter) *AuthMessageListResponse {
	var filtered []*AuthMessage
	...
	return &AuthMessageListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateAuthMessageStats(items []*AuthMessage) *AuthMessageStats {
	stats := &AuthMessageStats{}
	if len(items) == 0 {
		return stats
	}
