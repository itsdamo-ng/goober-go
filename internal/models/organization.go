...
func (m *Organization) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("organization: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("organization: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("organization: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("organization: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("organization: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("organization: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("organization: version must be at least 1")
	}
	return nil
}
...
func (m *Organization) ToJSON() ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("organization: marshal failed: %w", err)
	}
	return data, nil
}
...
func (m *Organization) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("organization: unmarshal failed: %w", err)
	}
	return nil
}
...
func (m *Organization) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("organization: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("organization: transition from %q to %q not allowed", m.Status, status)
}

func (m *Organization) Sanitize() {
...
func (m *Organization) Merge(other *Organization) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Organization) String() string {
	return fmt.Sprintf("Organization[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Organization) MatchesFilter(f *OrganizationFilter) bool {
...
func FilterOrganizationList(items []*Organization, f *OrganizationFilter) *OrganizationListResponse {
...
func CalculateOrganizationStats(items []*Organization) *OrganizationStats {
