...
func (m *Channel) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("channel: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("channel: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("channel: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("channel: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("channel: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("channel: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("channel: version must be at least 1")
	}
	return nil
}

func (m *Channel) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("channel: marshal failed: %w", err)
	}
	return data, nil
}

func (m *Channel) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("channel: unmarshal failed: %w", err)
	}
	return nil
}

func (m *Channel) Clone() *Channel {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *Channel) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("channel: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("channel: transition from %q to %q not allowed", m.Status, status)
}

func (m *Channel) Sanitize() {
...
func (m *Channel) Merge(other *Channel) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Channel) String() string {
	return fmt.Sprintf("Channel[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Channel) MatchesFilter(f *ChannelFilter) bool {
...
func FilterChannelList(items []*Channel, f *ChannelFilter) *ChannelListResponse {
...
func CalculateChannelStats(items []*Channel) *ChannelStats {
	stats := &ChannelStats{}
	if len(items) == 0 {
		return stats
	}
