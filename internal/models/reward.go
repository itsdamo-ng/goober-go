...
func NewReward(name string) *Reward {
	now := time.Now()
	return &Reward{
		Name:      name,
		Status:    "pending",
...
func (m *Reward) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("reward: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("reward: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("reward: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("reward: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("reward: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("reward: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("reward: version must be at least 1")
	}
	return nil
}

func (m *Reward) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("reward: marshal failed: %w", err)
	}
	return data, nil
}

func (m *Reward) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("reward: unmarshal failed: %w", err)
	}
	return nil
}
...
func (m *Reward) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("reward: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("reward: transition from %q to %q not allowed", m.Status, status)
}

func (m *Reward) Sanitize() {
...
func (m *Reward) Merge(other *Reward) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Reward) String() string {
	return fmt.Sprintf("Reward[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Reward) MatchesFilter(f *RewardFilter) bool {
...
func FilterRewardList(items []*Reward, f *RewardFilter) *RewardListResponse {
	var filtered []*Reward
...
func CalculateRewardStats(items []*Reward) *RewardStats {
	stats := &RewardStats{}
	if len(items) == 0 {
		return stats
	}
