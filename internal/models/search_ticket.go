...
type SearchTicketFilter struct {
	IDs       []int64   `json:"ids"`
...
func NewSearchTicket(name string) *SearchTicket {
	now := time.Now()
	return &SearchTicket{
		Name:      name,
		Status:    "pending",
...
func (m *SearchTicket) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("search_ticket: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("search_ticket: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("search_ticket: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("search_ticket: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("search_ticket: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("search_ticket: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("search_ticket: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("search_ticket: version must be at least 1")
	}
	return nil
}

func (m *SearchTicket) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("search_ticket: marshal failed: %w", err)
	}
	return data, nil
}

func (m *SearchTicket) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("search_ticket: unmarshal failed: %w", err)
	}
	return nil
}

func (m *SearchTicket) Clone() *SearchTicket {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *SearchTicket) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("search_ticket: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("search_ticket: transition from %q to %q not allowed", m.Status, status)
}

func (m *SearchTicket) Sanitize() {
...
func (m *SearchTicket) Merge(other *SearchTicket) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *SearchTicket) String() string {
	return fmt.Sprintf("SearchTicket[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *SearchTicket) MatchesFilter(f *SearchTicketFilter) bool {
...
func FilterSearchTicketList(items []*SearchTicket, f *SearchTicketFilter) *SearchTicketListResponse {
	var filtered []*SearchTicket
...
func CalculateSearchTicketStats(items []*SearchTicket) *SearchTicketStats {
	stats := &SearchTicketStats{}
	if len(items) == 0 {
		return stats
	}
