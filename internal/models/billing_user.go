...
type BillingUserListResponse struct {
	Data       []*BillingUser `json:"data"`
...
func NewBillingUser(name string) *BillingUser {
	now := time.Now()
	return &BillingUser{
		Name:      name,
		Status:    "pending",
...
func (m *BillingUser) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("billing_user: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("billing_user: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("billing_user: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("billing_user: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("billing_user: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("billing_user: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("billing_user: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("billing_user: version must be at least 1")
	}
	return nil
}

func (m *BillingUser) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("billing_user: marshal failed: %w", err)
	}
	return data, nil
}

func (m *BillingUser) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("billing_user: unmarshal failed: %w", err)
	}
	return nil
}

func (m *BillingUser) Clone() *BillingUser {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *BillingUser) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("billing_user: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("billing_user: transition from %q to %q not allowed", m.Status, status)
}

func (m *BillingUser) Sanitize() {
...
func (m *BillingUser) Merge(other *BillingUser) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *BillingUser) String() string {
	return fmt.Sprintf("BillingUser[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *BillingUser) MatchesFilter(f *BillingUserFilter) bool {
...
func FilterBillingUserList(items []*BillingUser, f *BillingUserFilter) *BillingUserListResponse {
	var filtered []*BillingUser
	...
	return &BillingUserListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateBillingUserStats(items []*BillingUser) *BillingUserStats {
	stats := &BillingUserStats{}
	if len(items) == 0 {
		return stats
	}
