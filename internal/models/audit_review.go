...
type AuditReviewListResponse struct {
	Data       []*AuditReview `json:"data"`
...
func NewAuditReview(name string) *AuditReview {
	now := time.Now()
	return &AuditReview{
		Name:      name,
		Status:    "pending",
...
func (m *AuditReview) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("audit_review: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("audit_review: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("audit_review: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("audit_review: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("audit_review: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("audit_review: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("audit_review: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("audit_review: version must be at least 1")
	}
	return nil
}

func (m *AuditReview) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("audit_review: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuditReview) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("audit_review: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuditReview) Clone() *AuditReview {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuditReview) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("audit_review: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("audit_review: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuditReview) Sanitize() {
...
func (m *AuditReview) Merge(other *AuditReview) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuditReview) String() string {
	return fmt.Sprintf("AuditReview[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuditReview) MatchesFilter(f *AuditReviewFilter) bool {
...
func FilterAuditReviewList(items []*AuditReview, f *AuditReviewFilter) *AuditReviewListResponse {
	var filtered []*AuditReview
...
func CalculateAuditReviewStats(items []*AuditReview) *AuditReviewStats {
