...
type SearchSessionListResponse struct {
	Data       []*SearchSession `json:"data"`
...
func NewSearchSession(name string) *SearchSession {
	now := time.Now()
...
func (m *SearchSession) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("search_session: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("search_session: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("search_session: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("search_session: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("search_session: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("search_session: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("search_session: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("search_session: version must be at least 1")
	}
	return nil
}

func (m *SearchSession) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("search_session: marshal failed: %w", err)
	}
	return data, nil
}

func (m *SearchSession) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("search_session: unmarshal failed: %w", err)
	}
	return nil
}

func (m *SearchSession) Clone() *SearchSession {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *SearchSession) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("search_session: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("search_session: transition from %q to %q not allowed", m.Status, status)
}

func (m *SearchSession) Sanitize() {
...
func (m *SearchSession) Merge(other *SearchSession) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *SearchSession) String() string {
	return fmt.Sprintf("SearchSession[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *SearchSession) MatchesFilter(f *SearchSessionFilter) bool {
...
func FilterSearchSessionList(items []*SearchSession, f *SearchSessionFilter) *SearchSessionListResponse {
	var filtered []*SearchSession
...
func CalculateSearchSessionStats(items []*SearchSession) *SearchSessionStats {
