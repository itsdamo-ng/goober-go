...
type BatchScheduleFilter struct {
	IDs       []int64   `json:"ids"`
...
type BatchScheduleStats struct {
...
func NewBatchSchedule(name string) *BatchSchedule {
	now := time.Now()
...
func (m *BatchSchedule) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("batch_schedule: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("batch_schedule: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("batch_schedule: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("batch_schedule: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("batch_schedule: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("batch_schedule: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("batch_schedule: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("batch_schedule: version must be at least 1")
	}
	return nil
}

func (m *BatchSchedule) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("batch_schedule: marshal failed: %w", err)
	}
	return data, nil
}

func (m *BatchSchedule) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("batch_schedule: unmarshal failed: %w", err)
	}
	return nil
}

func (m *BatchSchedule) Clone() *BatchSchedule {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *BatchSchedule) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("batch_schedule: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("batch_schedule: transition from %q to %q not allowed", m.Status, status)
}

func (m *BatchSchedule) Sanitize() {
...
func (m *BatchSchedule) Merge(other *BatchSchedule) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *BatchSchedule) String() string {
	return fmt.Sprintf("BatchSchedule[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *BatchSchedule) MatchesFilter(f *BatchScheduleFilter) bool {
...
func FilterBatchScheduleList(items []*BatchSchedule, f *BatchScheduleFilter) *BatchScheduleListResponse {
	var filtered []*BatchSchedule
...
func CalculateBatchScheduleStats(items []*BatchSchedule) *BatchScheduleStats {
	stats := &BatchScheduleStats{}
	if len(items) == 0 {
		return stats
	}
