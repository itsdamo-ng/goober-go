...
type AuthVenueListResponse struct {
	Data       []*AuthVenue `json:"data"`
...
func NewAuthVenue(name string) *AuthVenue {
	now := time.Now()
	return &AuthVenue{
		Name:      name,
		Status:    "pending",
...
func (m *AuthVenue) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("auth_venue: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("auth_venue: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("auth_venue: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("auth_venue: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("auth_venue: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("auth_venue: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("auth_venue: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("auth_venue: version must be at least 1")
	}
	return nil
}

func (m *AuthVenue) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("auth_venue: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuthVenue) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("auth_venue: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuthVenue) Clone() *AuthVenue {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuthVenue) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("auth_venue: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("auth_venue: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuthVenue) Sanitize() {
...
func (m *AuthVenue) Merge(other *AuthVenue) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuthVenue) String() string {
	return fmt.Sprintf("AuthVenue[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuthVenue) MatchesFilter(f *AuthVenueFilter) bool {
...
func FilterAuthVenueList(items []*AuthVenue, f *AuthVenueFilter) *AuthVenueListResponse {
	var filtered []*AuthVenue
...
func CalculateAuthVenueStats(items []*AuthVenue) *AuthVenueStats {
	stats := &AuthVenueStats{}
	if len(items) == 0 {
		return stats
	}
