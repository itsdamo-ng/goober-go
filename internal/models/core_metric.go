...
type CoreMetricListResponse struct {
	Data       []*CoreMetric `json:"data"`
...
func NewCoreMetric(name string) *CoreMetric {
	now := time.Now()
	return &CoreMetric{
		Name:      name,
		Status:    "pending",
...
func (m *CoreMetric) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("core_metric: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("core_metric: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("core_metric: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("core_metric: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("core_metric: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("core_metric: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("core_metric: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("core_metric: version must be at least 1")
	}
	return nil
}

func (m *CoreMetric) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("core_metric: marshal failed: %w", err)
	}
	return data, nil
}

func (m *CoreMetric) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("core_metric: unmarshal failed: %w", err)
	}
	return nil
}

func (m *CoreMetric) Clone() *CoreMetric {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *CoreMetric) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("core_metric: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("core_metric: transition from %q to %q not allowed", m.Status, status)
}

func (m *CoreMetric) Sanitize() {
...
func (m *CoreMetric) Merge(other *CoreMetric) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *CoreMetric) String() string {
	return fmt.Sprintf("CoreMetric[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *CoreMetric) MatchesFilter(f *CoreMetricFilter) bool {
...
func FilterCoreMetricList(items []*CoreMetric, f *CoreMetricFilter) *CoreMetricListResponse {
	var filtered []*CoreMetric
...
func CalculateCoreMetricStats(items []*CoreMetric) *CoreMetricStats {
	stats := &CoreMetricStats{}
	if len(items) == 0 {
		return stats
	}
