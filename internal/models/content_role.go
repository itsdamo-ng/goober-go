...
type ContentRoleListResponse struct {
	Data       []*ContentRole `json:"data"`
...
func NewContentRole(name string) *ContentRole {
	now := time.Now()
	return &ContentRole{
		Name:      name,
		Status:    "pending",
...
func (m *ContentRole) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("content_role: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("content_role: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("content_role: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("content_role: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("content_role: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("content_role: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("content_role: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("content_role: version must be at least 1")
	}
	return nil
}

func (m *ContentRole) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("content_role: marshal failed: %w", err)
	}
	return data, nil
}

func (m *ContentRole) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("content_role: unmarshal failed: %w", err)
	}
	return nil
}

func (m *ContentRole) Clone() *ContentRole {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *ContentRole) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("content_role: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("content_role: transition from %q to %q not allowed", m.Status, status)
}

func (m *ContentRole) Sanitize() {
...
func (m *ContentRole) Merge(other *ContentRole) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *ContentRole) String() string {
	return fmt.Sprintf("ContentRole[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *ContentRole) MatchesFilter(f *ContentRoleFilter) bool {
...
func FilterContentRoleList(items []*ContentRole, f *ContentRoleFilter) *ContentRoleListResponse {
	var filtered []*ContentRole
	...
	return &ContentRoleListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateContentRoleStats(items []*ContentRole) *ContentRoleStats {
	stats := &ContentRoleStats{}
	if len(items) == 0 {
		return stats
	}
