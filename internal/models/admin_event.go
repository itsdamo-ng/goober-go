...
func NewAdminEvent(name string) *AdminEvent {
	now := time.Now()
	return &AdminEvent{
		Name:      name,
		Status:    "pending",
...
func (m *AdminEvent) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_event: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_event: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_event: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_event: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_event: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_event: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_event: version must be at least 1")
	}
	return nil
}

func (m *AdminEvent) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_event: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminEvent) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_event: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminEvent) Clone() *AdminEvent {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminEvent) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_event: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_event: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminEvent) Sanitize() {
...
func (m *AdminEvent) Merge(other *AdminEvent) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminEvent) String() string {
	return fmt.Sprintf("AdminEvent[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminEvent) MatchesFilter(f *AdminEventFilter) bool {
...
func FilterAdminEventList(items []*AdminEvent, f *AdminEventFilter) *AdminEventListResponse {
	var filtered []*AdminEvent
...
func CalculateAdminEventStats(items []*AdminEvent) *AdminEventStats {
