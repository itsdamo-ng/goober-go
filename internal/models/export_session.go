...
type ExportSessionFilter struct {
	IDs       []int64   `json:"ids"`
...
type ExportSessionListResponse struct {
	Data       []*ExportSession `json:"data"`
...
func NewExportSession(name string) *ExportSession {
	now := time.Now()
...
func (m *ExportSession) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("export_session: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("export_session: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("export_session: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("export_session: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("export_session: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("export_session: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("export_session: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("export_session: version must be at least 1")
	}
	return nil
}

func (m *ExportSession) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("export_session: marshal failed: %w", err)
	}
	return data, nil
}

func (m *ExportSession) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("export_session: unmarshal failed: %w", err)
	}
	return nil
}

func (m *ExportSession) Clone() *ExportSession {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *ExportSession) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("export_session: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("export_session: transition from %q to %q not allowed", m.Status, status)
}

func (m *ExportSession) Sanitize() {
...
func (m *ExportSession) Merge(other *ExportSession) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *ExportSession) String() string {
	return fmt.Sprintf("ExportSession[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *ExportSession) MatchesFilter(f *ExportSessionFilter) bool {
...
func FilterExportSessionList(items []*ExportSession, f *ExportSessionFilter) *ExportSessionListResponse {
	var filtered []*ExportSession
	...
	return &ExportSessionListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateExportSessionStats(items []*ExportSession) *ExportSessionStats {
