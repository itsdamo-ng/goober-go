...
type BillingMetricFilter struct {
	IDs       []int64   `json:"ids"`
...
type BillingMetricListResponse struct {
...
type BillingMetricStats struct {
...
func NewBillingMetric(name string) *BillingMetric {
	now := time.Now()
...
func (m *BillingMetric) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("billing_metric: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("billing_metric: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("billing_metric: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("billing_metric: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("billing_metric: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("billing_metric: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("billing_metric: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("billing_metric: version must be at least 1")
	}
	return nil
}

func (m *BillingMetric) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("billing_metric: marshal failed: %w", err)
	}
	return data, nil
}

func (m *BillingMetric) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("billing_metric: unmarshal failed: %w", err)
	}
	return nil
}

func (m *BillingMetric) Clone() *BillingMetric {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *BillingMetric) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("billing_metric: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("billing_metric: transition from %q to %q not allowed", m.Status, status)
}

func (m *BillingMetric) Sanitize() {
...
func (m *BillingMetric) Merge(other *BillingMetric) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *BillingMetric) String() string {
	return fmt.Sprintf("BillingMetric[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *BillingMetric) MatchesFilter(f *BillingMetricFilter) bool {
...
func FilterBillingMetricList(items []*BillingMetric, f *BillingMetricFilter) *BillingMetricListResponse {
	var filtered []*BillingMetric
	...
	return &BillingMetricListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateBillingMetricStats(items []*BillingMetric) *BillingMetricStats {
	stats := &BillingMetricStats{}
	if len(items) == 0 {
		return stats
	}
