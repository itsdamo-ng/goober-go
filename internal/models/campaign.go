...
func (m *Campaign) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("campaign: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("campaign: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("campaign: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("campaign: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("campaign: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("campaign: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("campaign: version must be at least 1")
	}
	return nil
}

func (m *Campaign) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("campaign: marshal failed: %w", err)
	}
	return data, nil
}

func (m *Campaign) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("campaign: unmarshal failed: %w", err)
	}
	return nil
}

func (m *Campaign) Clone() *Campaign {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *Campaign) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("campaign: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("campaign: transition from %q to %q not allowed", m.Status, status)
}

func (m *Campaign) Sanitize() {
...
func (m *Campaign) Merge(other *Campaign) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Campaign) String() string {
	return fmt.Sprintf("Campaign[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Campaign) MatchesFilter(f *CampaignFilter) bool {
...
func FilterCampaignList(items []*Campaign, f *CampaignFilter) *CampaignListResponse {
...
func CalculateCampaignStats(items []*Campaign) *CampaignStats {
	stats := &CampaignStats{}
	if len(items) == 0 {
		return stats
	}
