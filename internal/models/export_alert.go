...
type ExportAlertListResponse struct {
	Data       []*ExportAlert `json:"data"`
...
func NewExportAlert(name string) *ExportAlert {
	now := time.Now()
	return &ExportAlert{
		Name:      name,
		Status:    "pending",
...
func (m *ExportAlert) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("export_alert: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("export_alert: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("export_alert: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("export_alert: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("export_alert: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("export_alert: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("export_alert: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("export_alert: version must be at least 1")
	}
	return nil
}

func (m *ExportAlert) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("export_alert: marshal failed: %w", err)
	}
	return data, nil
}

func (m *ExportAlert) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("export_alert: unmarshal failed: %w", err)
	}
	return nil
}

func (m *ExportAlert) Clone() *ExportAlert {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *ExportAlert) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("export_alert: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("export_alert: transition from %q to %q not allowed", m.Status, status)
}

func (m *ExportAlert) Sanitize() {
...
func (m *ExportAlert) Merge(other *ExportAlert) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *ExportAlert) String() string {
	return fmt.Sprintf("ExportAlert[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *ExportAlert) MatchesFilter(f *ExportAlertFilter) bool {
...
func FilterExportAlertList(items []*ExportAlert, f *ExportAlertFilter) *ExportAlertListResponse {
	var filtered []*ExportAlert
	...
	return &ExportAlertListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateExportAlertStats(items []*ExportAlert) *ExportAlertStats {
	stats := &ExportAlertStats{}
	if len(items) == 0 {
		return stats
	}
