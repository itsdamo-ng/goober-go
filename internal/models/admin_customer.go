...
func NewAdminCustomer(name string) *AdminCustomer {
	now := time.Now()
...
func (m *AdminCustomer) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("admin_customer: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_customer: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_customer: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_customer: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_customer: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_customer: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_customer: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_customer: version must be at least 1")
	}
	return nil
}

func (m *AdminCustomer) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_customer: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminCustomer) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_customer: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminCustomer) Clone() *AdminCustomer {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminCustomer) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_customer: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_customer: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminCustomer) Sanitize() {
...
func (m *AdminCustomer) Merge(other *AdminCustomer) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminCustomer) String() string {
	return fmt.Sprintf("AdminCustomer[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminCustomer) MatchesFilter(f *AdminCustomerFilter) bool {
...
func FilterAdminCustomerList(items []*AdminCustomer, f *AdminCustomerFilter) *AdminCustomerListResponse {
	var filtered []*AdminCustomer
...
func CalculateAdminCustomerStats(items []*AdminCustomer) *AdminCustomerStats {
