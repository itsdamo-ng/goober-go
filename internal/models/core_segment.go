...
type CoreSegmentListResponse struct {
	Data       []*CoreSegment `json:"data"`
...
func NewCoreSegment(name string) *CoreSegment {
	now := time.Now()
	return &CoreSegment{
		Name:      name,
		Status:    "pending",
...
func (m *CoreSegment) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("core_segment: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("core_segment: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("core_segment: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("core_segment: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("core_segment: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("core_segment: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("core_segment: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("core_segment: version must be at least 1")
	}
	return nil
}

func (m *CoreSegment) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("core_segment: marshal failed: %w", err)
	}
	return data, nil
}

func (m *CoreSegment) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("core_segment: unmarshal failed: %w", err)
	}
	return nil
}

func (m *CoreSegment) Clone() *CoreSegment {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *CoreSegment) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("core_segment: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("core_segment: transition from %q to %q not allowed", m.Status, status)
}

func (m *CoreSegment) Sanitize() {
...
func (m *CoreSegment) Merge(other *CoreSegment) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *CoreSegment) String() string {
	return fmt.Sprintf("CoreSegment[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *CoreSegment) MatchesFilter(f *CoreSegmentFilter) bool {
...
func FilterCoreSegmentList(items []*CoreSegment, f *CoreSegmentFilter) *CoreSegmentListResponse {
	var filtered []*CoreSegment
	...
	return &CoreSegmentListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateCoreSegmentStats(items []*CoreSegment) *CoreSegmentStats {
	stats := &CoreSegmentStats{}
	if len(items) == 0 {
		return stats
	}
