...
func NewAuditTask(name string) *AuditTask {
	now := time.Now()
	return &AuditTask{
		Name:      name,
		Status:    "pending",
...
func (m *AuditTask) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("audit_task: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("audit_task: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("audit_task: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("audit_task: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("audit_task: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("audit_task: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("audit_task: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("audit_task: version must be at least 1")
	}
	return nil
}

func (m *AuditTask) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("audit_task: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuditTask) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("audit_task: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuditTask) Clone() *AuditTask {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuditTask) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("audit_task: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("audit_task: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuditTask) Sanitize() {
...
func (m *AuditTask) Merge(other *AuditTask) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuditTask) String() string {
	return fmt.Sprintf("AuditTask[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuditTask) MatchesFilter(f *AuditTaskFilter) bool {
...
func FilterAuditTaskList(items []*AuditTask, f *AuditTaskFilter) *AuditTaskListResponse {
	var filtered []*AuditTask
...
func CalculateAuditTaskStats(items []*AuditTask) *AuditTaskStats {
	stats := &AuditTaskStats{}
	if len(items) == 0 {
		return stats
	}
