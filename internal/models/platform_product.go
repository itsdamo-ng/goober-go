...
type PlatformProductListResponse struct {
	Data       []*PlatformProduct `json:"data"`
...
type PlatformProductStats struct {
...
func NewPlatformProduct(name string) *PlatformProduct {
	now := time.Now()
...
func (m *PlatformProduct) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("platform_product: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("platform_product: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("platform_product: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("platform_product: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("platform_product: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("platform_product: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("platform_product: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("platform_product: version must be at least 1")
	}
	return nil
}

func (m *PlatformProduct) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("platform_product: marshal failed: %w", err)
	}
	return data, nil
}

func (m *PlatformProduct) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("platform_product: unmarshal failed: %w", err)
	}
	return nil
}

func (m *PlatformProduct) Clone() *PlatformProduct {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *PlatformProduct) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("platform_product: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("platform_product: transition from %q to %q not allowed", m.Status, status)
}

func (m *PlatformProduct) Sanitize() {
...
func (m *PlatformProduct) Merge(other *PlatformProduct) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *PlatformProduct) String() string {
	return fmt.Sprintf("PlatformProduct[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *PlatformProduct) MatchesFilter(f *PlatformProductFilter) bool {
...
func FilterPlatformProductList(items []*PlatformProduct, f *PlatformProductFilter) *PlatformProductListResponse {
...
func CalculatePlatformProductStats(items []*PlatformProduct) *PlatformProductStats {
	stats := &PlatformProductStats{}
	if len(items) == 0 {
		return stats
	}
