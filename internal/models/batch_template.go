...
type BatchTemplateFilter struct {
	IDs       []int64   `json:"ids"`
...
type BatchTemplateListResponse struct {
	Data       []*BatchTemplate `json:"data"`
...
type BatchTemplateStats struct {
...
func NewBatchTemplate(name string) *BatchTemplate {
	now := time.Now()
...
func (m *BatchTemplate) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("batch_template: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("batch_template: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("batch_template: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("batch_template: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("batch_template: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("batch_template: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("batch_template: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("batch_template: version must be at least 1")
	}
	return nil
}

func (m *BatchTemplate) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("batch_template: marshal failed: %w", err)
	}
	return data, nil
}

func (m *BatchTemplate) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("batch_template: unmarshal failed: %w", err)
	}
	return nil
}

func (m *BatchTemplate) Clone() *BatchTemplate {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *BatchTemplate) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("batch_template: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("batch_template: transition from %q to %q not allowed", m.Status, status)
}

func (m *BatchTemplate) Sanitize() {
...
func (m *BatchTemplate) Merge(other *BatchTemplate) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *BatchTemplate) String() string {
	return fmt.Sprintf("BatchTemplate[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *BatchTemplate) MatchesFilter(f *BatchTemplateFilter) bool {
...
func FilterBatchTemplateList(items []*BatchTemplate, f *BatchTemplateFilter) *BatchTemplateListResponse {
	var filtered []*BatchTemplate
...
func CalculateBatchTemplateStats(items []*BatchTemplate) *BatchTemplateStats {
	stats := &BatchTemplateStats{}
	if len(items) == 0 {
		return stats
	}
