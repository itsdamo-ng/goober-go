...
type StreamingAlertFilter struct {
	IDs       []int64   `json:"ids"`
...
type StreamingAlertListResponse struct {
	Data       []*StreamingAlert `json:"data"`
...
type StreamingAlertStats struct {
...
func NewStreamingAlert(name string) *StreamingAlert {
	now := time.Now()
...
func (m *StreamingAlert) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("streaming_alert: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("streaming_alert: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("streaming_alert: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("streaming_alert: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("streaming_alert: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("streaming_alert: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("streaming_alert: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("streaming_alert: version must be at least 1")
	}
	return nil
}

func (m *StreamingAlert) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("streaming_alert: marshal failed: %w", err)
	}
	return data, nil
}

func (m *StreamingAlert) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("streaming_alert: unmarshal failed: %w", err)
	}
	return nil
}

func (m *StreamingAlert) Clone() *StreamingAlert {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *StreamingAlert) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("streaming_alert: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("streaming_alert: transition from %q to %q not allowed", m.Status, status)
}

func (m *StreamingAlert) Sanitize() {
...
func (m *StreamingAlert) Merge(other *StreamingAlert) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *StreamingAlert) String() string {
	return fmt.Sprintf("StreamingAlert[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *StreamingAlert) MatchesFilter(f *StreamingAlertFilter) bool {
...
func FilterStreamingAlertList(items []*StreamingAlert, f *StreamingAlertFilter) *StreamingAlertListResponse {
	var filtered []*StreamingAlert
...
func CalculateStreamingAlertStats(items []*StreamingAlert) *StreamingAlertStats {
	stats := &StreamingAlertStats{}
	if len(items) == 0 {
		return stats
	}
