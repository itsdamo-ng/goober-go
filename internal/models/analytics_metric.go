...
type AnalyticsMetricListResponse struct {
	Data       []*AnalyticsMetric `json:"data"`
...
type AnalyticsMetricStats struct {
...
func NewAnalyticsMetric(name string) *AnalyticsMetric {
	now := time.Now()
...
func (m *AnalyticsMetric) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("analytics_metric: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("analytics_metric: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("analytics_metric: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("analytics_metric: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("analytics_metric: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("analytics_metric: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("analytics_metric: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("analytics_metric: version must be at least 1")
	}
	return nil
}

func (m *AnalyticsMetric) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("analytics_metric: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AnalyticsMetric) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("analytics_metric: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AnalyticsMetric) Clone() *AnalyticsMetric {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AnalyticsMetric) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("analytics_metric: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("analytics_metric: transition from %q to %q not allowed", m.Status, status)
}

func (m *AnalyticsMetric) Sanitize() {
...
func (m *AnalyticsMetric) Merge(other *AnalyticsMetric) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AnalyticsMetric) String() string {
	return fmt.Sprintf("AnalyticsMetric[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AnalyticsMetric) MatchesFilter(f *AnalyticsMetricFilter) bool {
...
func FilterAnalyticsMetricList(items []*AnalyticsMetric, f *AnalyticsMetricFilter) *AnalyticsMetricListResponse {
...
func CalculateAnalyticsMetricStats(items []*AnalyticsMetric) *AnalyticsMetricStats {
	stats := &AnalyticsMetricStats{}
	if len(items) == 0 {
		return stats
	}
