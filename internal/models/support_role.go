...
type SupportRoleListResponse struct {
	Data       []*SupportRole `json:"data"`
...
func NewSupportRole(name string) *SupportRole {
	now := time.Now()
	return &SupportRole{
		Name:      name,
		Status:    "pending",
...
func (m *SupportRole) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("support_role: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("support_role: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("support_role: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("support_role: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("support_role: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("support_role: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("support_role: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("support_role: version must be at least 1")
	}
	return nil
}

func (m *SupportRole) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("support_role: marshal failed: %w", err)
	}
	return data, nil
}

func (m *SupportRole) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("support_role: unmarshal failed: %w", err)
	}
	return nil
}

func (m *SupportRole) Clone() *SupportRole {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *SupportRole) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("support_role: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("support_role: transition from %q to %q not allowed", m.Status, status)
}

func (m *SupportRole) Sanitize() {
...
func (m *SupportRole) Merge(other *SupportRole) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *SupportRole) String() string {
	return fmt.Sprintf("SupportRole[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *SupportRole) MatchesFilter(f *SupportRoleFilter) bool {
...
func FilterSupportRoleList(items []*SupportRole, f *SupportRoleFilter) *SupportRoleListResponse {
	var filtered []*SupportRole
	...
	return &SupportRoleListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateSupportRoleStats(items []*SupportRole) *SupportRoleStats {
	stats := &SupportRoleStats{}
	if len(items) == 0 {
		return stats
	}
