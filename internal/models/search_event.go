...
type SearchEventListResponse struct {
	Data       []*SearchEvent `json:"data"`
...
func NewSearchEvent(name string) *SearchEvent {
	now := time.Now()
	return &SearchEvent{
		Name:      name,
		Status:    "pending",
...
func (m *SearchEvent) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("search_event: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("search_event: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("search_event: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("search_event: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("search_event: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("search_event: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("search_event: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("search_event: version must be at least 1")
	}
	return nil
}

func (m *SearchEvent) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("search_event: marshal failed: %w", err)
	}
	return data, nil
}

func (m *SearchEvent) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("search_event: unmarshal failed: %w", err)
	}
	return nil
}

func (m *SearchEvent) Clone() *SearchEvent {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *SearchEvent) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("search_event: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("search_event: transition from %q to %q not allowed", m.Status, status)
}

func (m *SearchEvent) Sanitize() {
...
func (m *SearchEvent) Merge(other *SearchEvent) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *SearchEvent) String() string {
	return fmt.Sprintf("SearchEvent[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *SearchEvent) MatchesFilter(f *SearchEventFilter) bool {
...
func FilterSearchEventList(items []*SearchEvent, f *SearchEventFilter) *SearchEventListResponse {
	var filtered []*SearchEvent
	...
	return &SearchEventListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateSearchEventStats(items []*SearchEvent) *SearchEventStats {
