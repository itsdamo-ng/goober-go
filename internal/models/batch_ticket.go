...
type BatchTicketListResponse struct {
	Data       []*BatchTicket `json:"data"`
...
func NewBatchTicket(name string) *BatchTicket {
	now := time.Now()
	return &BatchTicket{
		Name:      name,
		Status:    "pending",
...
func (m *BatchTicket) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("batch_ticket: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("batch_ticket: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("batch_ticket: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("batch_ticket: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("batch_ticket: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("batch_ticket: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("batch_ticket: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("batch_ticket: version must be at least 1")
	}
	return nil
}

func (m *BatchTicket) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("batch_ticket: marshal failed: %w", err)
	}
	return data, nil
}

func (m *BatchTicket) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("batch_ticket: unmarshal failed: %w", err)
	}
	return nil
}

func (m *BatchTicket) Clone() *BatchTicket {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *BatchTicket) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("batch_ticket: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("batch_ticket: transition from %q to %q not allowed", m.Status, status)
}

func (m *BatchTicket) Sanitize() {
...
func (m *BatchTicket) Merge(other *BatchTicket) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *BatchTicket) String() string {
	return fmt.Sprintf("BatchTicket[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *BatchTicket) MatchesFilter(f *BatchTicketFilter) bool {
...
func FilterBatchTicketList(items []*BatchTicket, f *BatchTicketFilter) *BatchTicketListResponse {
	var filtered []*BatchTicket
	...
	return &BatchTicketListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateBatchTicketStats(items []*BatchTicket) *BatchTicketStats {
	stats := &BatchTicketStats{}
	if len(items) == 0 {
		return stats
	}
