...
type AnalyticsSessionListResponse struct {
	Data       []*AnalyticsSession `json:"data"`
...
func NewAnalyticsSession(name string) *AnalyticsSession {
	now := time.Now()
...
func (m *AnalyticsSession) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("analytics_session: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("analytics_session: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("analytics_session: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("analytics_session: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("analytics_session: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("analytics_session: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("analytics_session: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("analytics_session: version must be at least 1")
	}
	return nil
}

func (m *AnalyticsSession) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("analytics_session: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AnalyticsSession) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("analytics_session: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AnalyticsSession) Clone() *AnalyticsSession {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AnalyticsSession) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("analytics_session: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("analytics_session: transition from %q to %q not allowed", m.Status, status)
}

func (m *AnalyticsSession) Sanitize() {
...
func (m *AnalyticsSession) Merge(other *AnalyticsSession) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AnalyticsSession) String() string {
	return fmt.Sprintf("AnalyticsSession[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AnalyticsSession) MatchesFilter(f *AnalyticsSessionFilter) bool {
...
func FilterAnalyticsSessionList(items []*AnalyticsSession, f *AnalyticsSessionFilter) *AnalyticsSessionListResponse {
	var filtered []*AnalyticsSession
	...
	return &AnalyticsSessionListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateAnalyticsSessionStats(items []*AnalyticsSession) *AnalyticsSessionStats {
