...
type AdminDeploymentListResponse struct {
...
type AdminDeploymentStats struct {
...
func NewAdminDeployment(name string) *AdminDeployment {
	now := time.Now()
...
func (m *AdminDeployment) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("admin_deployment: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_deployment: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_deployment: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_deployment: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_deployment: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_deployment: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_deployment: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_deployment: version must be at least 1")
	}
	return nil
}

func (m *AdminDeployment) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_deployment: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminDeployment) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_deployment: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminDeployment) Clone() *AdminDeployment {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminDeployment) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_deployment: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_deployment: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminDeployment) Sanitize() {
...
func (m *AdminDeployment) Merge(other *AdminDeployment) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminDeployment) String() string {
	return fmt.Sprintf("AdminDeployment[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminDeployment) MatchesFilter(f *AdminDeploymentFilter) bool {
...
func FilterAdminDeploymentList(items []*AdminDeployment, f *AdminDeploymentFilter) *AdminDeploymentListResponse {
	var filtered []*AdminDeployment
...
func CalculateAdminDeploymentStats(items []*AdminDeployment) *AdminDeploymentStats {
