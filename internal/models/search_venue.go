...
type SearchVenueListResponse struct {
	Data       []*SearchVenue `json:"data"`
...
func NewSearchVenue(name string) *SearchVenue {
	now := time.Now()
	return &SearchVenue{
		Name:      name,
		Status:    "pending",
...
func (m *SearchVenue) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("search_venue: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("search_venue: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("search_venue: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("search_venue: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("search_venue: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("search_venue: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("search_venue: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("search_venue: version must be at least 1")
	}
	return nil
}

func (m *SearchVenue) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("search_venue: marshal failed: %w", err)
	}
	return data, nil
}

func (m *SearchVenue) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("search_venue: unmarshal failed: %w", err)
	}
	return nil
}

func (m *SearchVenue) Clone() *SearchVenue {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *SearchVenue) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("search_venue: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("search_venue: transition from %q to %q not allowed", m.Status, status)
}

func (m *SearchVenue) Sanitize() {
...
func (m *SearchVenue) Merge(other *SearchVenue) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *SearchVenue) String() string {
	return fmt.Sprintf("SearchVenue[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *SearchVenue) MatchesFilter(f *SearchVenueFilter) bool {
...
func FilterSearchVenueList(items []*SearchVenue, f *SearchVenueFilter) *SearchVenueListResponse {
	var filtered []*SearchVenue
	...
	return &SearchVenueListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateSearchVenueStats(items []*SearchVenue) *SearchVenueStats {
	stats := &SearchVenueStats{}
	if len(items) == 0 {
		return stats
	}
