...
func (m *Project) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("project: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("project: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("project: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("project: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("project: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("project: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("project: version must be at least 1")
	}
	return nil
}

func (m *Project) ToJSON() ([]byte, error) {
...
func (m *Project) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("project: unmarshal failed: %w", err)
	}
	return nil
}

func (m *Project) Clone() *Project {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *Project) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("project: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("project: transition from %q to %q not allowed", m.Status, status)
}

func (m *Project) Sanitize() {
...
func (m *Project) Merge(other *Project) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Project) String() string {
	return fmt.Sprintf("Project[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Project) MatchesFilter(f *ProjectFilter) bool {
...
func FilterProjectList(items []*Project, f *ProjectFilter) *ProjectListResponse {
...
func CalculateProjectStats(items []*Project) *ProjectStats {
	stats := &ProjectStats{}
	if len(items) == 0 {
		return stats
	}
