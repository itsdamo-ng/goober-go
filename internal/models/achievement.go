...
func (m *Achievement) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("achievement: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("achievement: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("achievement: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("achievement: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("achievement: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("achievement: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("achievement: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("achievement: version must be at least 1")
	}
	return nil
}

func (m *Achievement) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("achievement: marshal failed: %w", err)
	}
	return data, nil
}

func (m *Achievement) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("achievement: unmarshal failed: %w", err)
	}
	return nil
}
...
func (m *Achievement) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("achievement: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("achievement: transition from %q to %q not allowed", m.Status, status)
}

func (m *Achievement) Sanitize() {
...
func (m *Achievement) Merge(other *Achievement) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Achievement) String() string {
	return fmt.Sprintf("Achievement[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Achievement) MatchesFilter(f *AchievementFilter) bool {
...
func FilterAchievementList(items []*Achievement, f *AchievementFilter) *AchievementListResponse {
...
func CalculateAchievementStats(items []*Achievement) *AchievementStats {
