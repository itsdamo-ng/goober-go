...
func (m *Endpoint) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("endpoint: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("endpoint: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("endpoint: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("endpoint: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("endpoint: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("endpoint: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("endpoint: version must be at least 1")
	}
	return nil
}

func (m *Endpoint) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("endpoint: marshal failed: %w", err)
	}
	return data, nil
}

func (m *Endpoint) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("endpoint: unmarshal failed: %w", err)
	}
	return nil
}

func (m *Endpoint) Clone() *Endpoint {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *Endpoint) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("endpoint: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("endpoint: transition from %q to %q not allowed", m.Status, status)
}

func (m *Endpoint) Sanitize() {
...
func (m *Endpoint) Merge(other *Endpoint) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Endpoint) String() string {
	return fmt.Sprintf("Endpoint[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Endpoint) MatchesFilter(f *EndpointFilter) bool {
...
func FilterEndpointList(items []*Endpoint, f *EndpointFilter) *EndpointListResponse {
...
func CalculateEndpointStats(items []*Endpoint) *EndpointStats {
	stats := &EndpointStats{}
	if len(items) == 0 {
		return stats
	}
