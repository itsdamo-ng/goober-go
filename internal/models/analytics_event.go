...
type AnalyticsEventListResponse struct {
...
type AnalyticsEventStats struct {
...
func NewAnalyticsEvent(name string) *AnalyticsEvent {
	now := time.Now()
...
func (m *AnalyticsEvent) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("analytics_event: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("analytics_event: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("analytics_event: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("analytics_event: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("analytics_event: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("analytics_event: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("analytics_event: version must be at least 1")
	}
	return nil
}

func (m *AnalyticsEvent) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("analytics_event: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AnalyticsEvent) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("analytics_event: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AnalyticsEvent) Clone() *AnalyticsEvent {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AnalyticsEvent) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("analytics_event: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("analytics_event: transition from %q to %q not allowed", m.Status, status)
}

func (m *AnalyticsEvent) Sanitize() {
...
func (m *AnalyticsEvent) Merge(other *AnalyticsEvent) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AnalyticsEvent) String() string {
	return fmt.Sprintf("AnalyticsEvent[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AnalyticsEvent) MatchesFilter(f *AnalyticsEventFilter) bool {
...
func FilterAnalyticsEventList(items []*AnalyticsEvent, f *AnalyticsEventFilter) *AnalyticsEventListResponse {
...
func CalculateAnalyticsEventStats(items []*AnalyticsEvent) *AnalyticsEventStats {
	stats := &AnalyticsEventStats{}
	if len(items) == 0 {
		return stats
	}
