...
type SearchProductListResponse struct {
...
type SearchProductStats struct {
...
func NewSearchProduct(name string) *SearchProduct {
	now := time.Now()
...
func (m *SearchProduct) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("search_product: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("search_product: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("search_product: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("search_product: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("search_product: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("search_product: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("search_product: version must be at least 1")
	}
	return nil
}

func (m *SearchProduct) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("search_product: marshal failed: %w", err)
	}
	return data, nil
}

func (m *SearchProduct) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("search_product: unmarshal failed: %w", err)
	}
	return nil
}

func (m *SearchProduct) Clone() *SearchProduct {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *SearchProduct) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("search_product: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("search_product: transition from %q to %q not allowed", m.Status, status)
}

func (m *SearchProduct) Sanitize() {
...
func (m *SearchProduct) Merge(other *SearchProduct) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *SearchProduct) String() string {
	return fmt.Sprintf("SearchProduct[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *SearchProduct) MatchesFilter(f *SearchProductFilter) bool {
...
func FilterSearchProductList(items []*SearchProduct, f *SearchProductFilter) *SearchProductListResponse {
...
func CalculateSearchProductStats(items []*SearchProduct) *SearchProductStats {
