...
type AuditMessageFilter struct {
	IDs       []int64   `json:"ids"`
...
type AuditMessageListResponse struct {
	Data       []*AuditMessage `json:"data"`
...
func NewAuditMessage(name string) *AuditMessage {
	now := time.Now()
	return &AuditMessage{
		Name:      name,
		Status:    "pending",
...
func (m *AuditMessage) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("audit_message: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("audit_message: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("audit_message: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("audit_message: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("audit_message: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("audit_message: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("audit_message: version must be at least 1")
	}
	return nil
}

func (m *AuditMessage) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("audit_message: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuditMessage) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("audit_message: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuditMessage) Clone() *AuditMessage {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuditMessage) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("audit_message: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("audit_message: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuditMessage) Sanitize() {
...
func (m *AuditMessage) Merge(other *AuditMessage) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuditMessage) String() string {
	return fmt.Sprintf("AuditMessage[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuditMessage) MatchesFilter(f *AuditMessageFilter) bool {
...
func FilterAuditMessageList(items []*AuditMessage, f *AuditMessageFilter) *AuditMessageListResponse {
	var filtered []*AuditMessage
...
func CalculateAuditMessageStats(items []*AuditMessage) *AuditMessageStats {
	stats := &AuditMessageStats{}
	if len(items) == 0 {
		return stats
	}
