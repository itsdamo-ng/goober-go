...
type InventoryCategoryFilter struct {
	IDs       []int64   `json:"ids"`
...
type InventoryCategoryListResponse struct {
...
func NewInventoryCategory(name string) *InventoryCategory {
	now := time.Now()
...
func (m *InventoryCategory) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("inventory_category: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("inventory_category: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("inventory_category: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("inventory_category: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("inventory_category: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("inventory_category: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("inventory_category: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("inventory_category: version must be at least 1")
	}
	return nil
}

func (m *InventoryCategory) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("inventory_category: marshal failed: %w", err)
	}
	return data, nil
}

func (m *InventoryCategory) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("inventory_category: unmarshal failed: %w", err)
	}
	return nil
}

func (m *InventoryCategory) Clone() *InventoryCategory {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *InventoryCategory) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("inventory_category: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("inventory_category: transition from %q to %q not allowed", m.Status, status)
}

func (m *InventoryCategory) Sanitize() {
...
func (m *InventoryCategory) Merge(other *InventoryCategory) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *InventoryCategory) String() string {
	return fmt.Sprintf("InventoryCategory[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *InventoryCategory) MatchesFilter(f *InventoryCategoryFilter) bool {
...
func FilterInventoryCategoryList(items []*InventoryCategory, f *InventoryCategoryFilter) *InventoryCategoryListResponse {
	var filtered []*InventoryCategory
...
func CalculateInventoryCategoryStats(items []*InventoryCategory) *InventoryCategoryStats {
	stats := &InventoryCategoryStats{}
	if len(items) == 0 {
		return stats
	}
