...
type SearchTeamListResponse struct {
	Data       []*SearchTeam `json:"data"`
...
func NewSearchTeam(name string) *SearchTeam {
	now := time.Now()
	return &SearchTeam{
		Name:      name,
		Status:    "pending",
...
func (m *SearchTeam) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("search_team: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("search_team: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("search_team: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("search_team: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("search_team: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("search_team: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("search_team: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("search_team: version must be at least 1")
	}
	return nil
}

func (m *SearchTeam) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("search_team: marshal failed: %w", err)
	}
	return data, nil
}

func (m *SearchTeam) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("search_team: unmarshal failed: %w", err)
	}
	return nil
}

func (m *SearchTeam) Clone() *SearchTeam {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *SearchTeam) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("search_team: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("search_team: transition from %q to %q not allowed", m.Status, status)
}

func (m *SearchTeam) Sanitize() {
...
func (m *SearchTeam) Merge(other *SearchTeam) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *SearchTeam) String() string {
	return fmt.Sprintf("SearchTeam[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *SearchTeam) MatchesFilter(f *SearchTeamFilter) bool {
...
func FilterSearchTeamList(items []*SearchTeam, f *SearchTeamFilter) *SearchTeamListResponse {
	var filtered []*SearchTeam
...
func CalculateSearchTeamStats(items []*SearchTeam) *SearchTeamStats {
	stats := &SearchTeamStats{}
	if len(items) == 0 {
		return stats
	}
