...
type ContentCommentListResponse struct {
	Data       []*ContentComment `json:"data"`
...
type ContentCommentStats struct {
...
func NewContentComment(name string) *ContentComment {
	now := time.Now()
...
func (m *ContentComment) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("content_comment: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("content_comment: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("content_comment: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("content_comment: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("content_comment: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("content_comment: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("content_comment: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("content_comment: version must be at least 1")
	}
	return nil
}

func (m *ContentComment) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("content_comment: marshal failed: %w", err)
	}
	return data, nil
}

func (m *ContentComment) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("content_comment: unmarshal failed: %w", err)
	}
	return nil
}

func (m *ContentComment) Clone() *ContentComment {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *ContentComment) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("content_comment: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("content_comment: transition from %q to %q not allowed", m.Status, status)
}

func (m *ContentComment) Sanitize() {
...
func (m *ContentComment) Merge(other *ContentComment) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *ContentComment) String() string {
	return fmt.Sprintf("ContentComment[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *ContentComment) MatchesFilter(f *ContentCommentFilter) bool {
...
func FilterContentCommentList(items []*ContentComment, f *ContentCommentFilter) *ContentCommentListResponse {
	var filtered []*ContentComment
...
func CalculateContentCommentStats(items []*ContentComment) *ContentCommentStats {
	stats := &ContentCommentStats{}
	if len(items) == 0 {
		return stats
	}
