...
type CoreDocumentFilter struct {
	IDs       []int64   `json:"ids"`
...
type CoreDocumentListResponse struct {
	Data       []*CoreDocument `json:"data"`
...
func NewCoreDocument(name string) *CoreDocument {
	now := time.Now()
	return &CoreDocument{
		Name:      name,
		Status:    "pending",
...
func (m *CoreDocument) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("core_document: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("core_document: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("core_document: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("core_document: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("core_document: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("core_document: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("core_document: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("core_document: version must be at least 1")
	}
	return nil
}

func (m *CoreDocument) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("core_document: marshal failed: %w", err)
	}
	return data, nil
}

func (m *CoreDocument) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("core_document: unmarshal failed: %w", err)
	}
	return nil
}

func (m *CoreDocument) Clone() *CoreDocument {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *CoreDocument) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("core_document: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("core_document: transition from %q to %q not allowed", m.Status, status)
}

func (m *CoreDocument) Sanitize() {
...
func (m *CoreDocument) Merge(other *CoreDocument) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *CoreDocument) String() string {
	return fmt.Sprintf("CoreDocument[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *CoreDocument) MatchesFilter(f *CoreDocumentFilter) bool {
...
func FilterCoreDocumentList(items []*CoreDocument, f *CoreDocumentFilter) *CoreDocumentListResponse {
	var filtered []*CoreDocument
	...
	return &CoreDocumentListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateCoreDocumentStats(items []*CoreDocument) *CoreDocumentStats {
	stats := &CoreDocumentStats{}
	if len(items) == 0 {
		return stats
	}
