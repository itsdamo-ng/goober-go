...
type BillingTransactionListResponse struct {
	Data       []*BillingTransaction `json:"data"`
...
type BillingTransactionStats struct {
...
func NewBillingTransaction(name string) *BillingTransaction {
	now := time.Now()
...
func (m *BillingTransaction) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("billing_transaction: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("billing_transaction: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("billing_transaction: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("billing_transaction: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("billing_transaction: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("billing_transaction: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("billing_transaction: version must be at least 1")
	}
	return nil
}

func (m *BillingTransaction) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("billing_transaction: marshal failed: %w", err)
	}
	return data, nil
}

func (m *BillingTransaction) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("billing_transaction: unmarshal failed: %w", err)
	}
	return nil
}

func (m *BillingTransaction) Clone() *BillingTransaction {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *BillingTransaction) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("billing_transaction: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("billing_transaction: transition from %q to %q not allowed", m.Status, status)
}

func (m *BillingTransaction) Sanitize() {
...
func (m *BillingTransaction) Merge(other *BillingTransaction) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *BillingTransaction) String() string {
	return fmt.Sprintf("BillingTransaction[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *BillingTransaction) MatchesFilter(f *BillingTransactionFilter) bool {
...
func FilterBillingTransactionList(items []*BillingTransaction, f *BillingTransactionFilter) *BillingTransactionListResponse {
	var filtered []*BillingTransaction
...
func CalculateBillingTransactionStats(items []*BillingTransaction) *BillingTransactionStats {
	stats := &BillingTransactionStats{}
	if len(items) == 0 {
		return stats
	}
