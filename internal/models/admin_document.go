...
type AdminDocumentStats struct {
...
func NewAdminDocument(name string) *AdminDocument {
	now := time.Now()
...
func (m *AdminDocument) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("admin_document: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_document: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_document: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_document: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_document: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_document: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_document: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_document: version must be at least 1")
	}
	return nil
}

func (m *AdminDocument) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_document: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminDocument) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_document: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminDocument) Clone() *AdminDocument {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminDocument) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_document: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_document: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminDocument) Sanitize() {
...
func (m *AdminDocument) Merge(other *AdminDocument) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminDocument) String() string {
	return fmt.Sprintf("AdminDocument[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminDocument) MatchesFilter(f *AdminDocumentFilter) bool {
...
func FilterAdminDocumentList(items []*AdminDocument, f *AdminDocumentFilter) *AdminDocumentListResponse {
	var filtered []*AdminDocument
...
func CalculateAdminDocumentStats(items []*AdminDocument) *AdminDocumentStats {
