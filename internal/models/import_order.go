...
func NewImportOrder(name string) *ImportOrder {
	now := time.Now()
	return &ImportOrder{
		Name:      name,
		Status:    "pending",
...
func (m *ImportOrder) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("import_order: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("import_order: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("import_order: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("import_order: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("import_order: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("import_order: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("import_order: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("import_order: version must be at least 1")
	}
	return nil
}

func (m *ImportOrder) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("import_order: marshal failed: %w", err)
	}
	return data, nil
}

func (m *ImportOrder) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("import_order: unmarshal failed: %w", err)
	}
	return nil
}

func (m *ImportOrder) Clone() *ImportOrder {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *ImportOrder) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("import_order: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("import_order: transition from %q to %q not allowed", m.Status, status)
}

func (m *ImportOrder) Sanitize() {
...
func (m *ImportOrder) Merge(other *ImportOrder) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *ImportOrder) String() string {
	return fmt.Sprintf("ImportOrder[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *ImportOrder) MatchesFilter(f *ImportOrderFilter) bool {
...
func FilterImportOrderList(items []*ImportOrder, f *ImportOrderFilter) *ImportOrderListResponse {
	var filtered []*ImportOrder
	...
	return &ImportOrderListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateImportOrderStats(items []*ImportOrder) *ImportOrderStats {
