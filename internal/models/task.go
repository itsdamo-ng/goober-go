...
func (m *Task) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("task: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("task: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("task: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("task: amount cannot be negative")
	}
	...
	if m.Version < 1 {
		return fmt.Errorf("task: version must be at least 1")
	}
	return nil
}
...
func (m *Task) ToJSON() ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("task: marshal failed: %w", err)
	}
	return data, nil
}
...
func (m *Task) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("task: unmarshal failed: %w", err)
	}
	return nil
}
...
func (m *Task) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("task: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("task: transition from %q to %q not allowed", m.Status, status)
}

func (m *Task) Sanitize() {
...
func (m *Task) String() string {
	return fmt.Sprintf("Task[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Task) MatchesFilter(f *TaskFilter) bool {
...
func FilterTaskList(items []*Task, f *TaskFilter) *TaskListResponse {
	var filtered []*Task
...
func CalculateTaskStats(items []*Task) *TaskStats {
	stats := &TaskStats{}
	if len(items) == 0 {
		return stats
	}
