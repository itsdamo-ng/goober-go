...
func (m *Product) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("product: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("product: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("product: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("product: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("product: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("product: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("product: version must be at least 1")
	}
	return nil
}
...
func (m *Product) ToJSON() ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("product: marshal failed: %w", err)
	}
	return data, nil
}
...
func (m *Product) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("product: unmarshal failed: %w", err)
	}
	return nil
}
...
func (m *Product) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("product: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("product: transition from %q to %q not allowed", m.Status, status)
}
...
func (m *Product) Merge(other *Product) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Product) String() string {
	return fmt.Sprintf("Product[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Product) MatchesFilter(f *ProductFilter) bool {
...
func FilterProductList(items []*Product, f *ProductFilter) *ProductListResponse {
...
func CalculateProductStats(items []*Product) *ProductStats {
	stats := &ProductStats{}
	if len(items) == 0 {
		return stats
	}
