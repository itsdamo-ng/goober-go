...
type ExportTemplateListResponse struct {
...
type ExportTemplateStats struct {
...
func NewExportTemplate(name string) *ExportTemplate {
	now := time.Now()
...
func (m *ExportTemplate) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("export_template: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("export_template: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("export_template: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("export_template: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("export_template: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("export_template: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("export_template: version must be at least 1")
	}
	return nil
}

func (m *ExportTemplate) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("export_template: marshal failed: %w", err)
	}
	return data, nil
}

func (m *ExportTemplate) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("export_template: unmarshal failed: %w", err)
	}
	return nil
}

func (m *ExportTemplate) Clone() *ExportTemplate {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *ExportTemplate) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("export_template: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("export_template: transition from %q to %q not allowed", m.Status, status)
}

func (m *ExportTemplate) Sanitize() {
...
func (m *ExportTemplate) Merge(other *ExportTemplate) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *ExportTemplate) String() string {
	return fmt.Sprintf("ExportTemplate[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *ExportTemplate) MatchesFilter(f *ExportTemplateFilter) bool {
...
func FilterExportTemplateList(items []*ExportTemplate, f *ExportTemplateFilter) *ExportTemplateListResponse {
	var filtered []*ExportTemplate
...
func CalculateExportTemplateStats(items []*ExportTemplate) *ExportTemplateStats {
	stats := &ExportTemplateStats{}
	if len(items) == 0 {
		return stats
	}
