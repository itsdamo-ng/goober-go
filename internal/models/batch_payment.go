...
type BatchPaymentFilter struct {
	IDs       []int64   `json:"ids"`
...
type BatchPaymentListResponse struct {
	Data       []*BatchPayment `json:"data"`
...
func NewBatchPayment(name string) *BatchPayment {
	now := time.Now()
	return &BatchPayment{
		Name:      name,
		Status:    "pending",
...
func (m *BatchPayment) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("batch_payment: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("batch_payment: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("batch_payment: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("batch_payment: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("batch_payment: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("batch_payment: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("batch_payment: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("batch_payment: version must be at least 1")
	}
	return nil
}

func (m *BatchPayment) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("batch_payment: marshal failed: %w", err)
	}
	return data, nil
}

func (m *BatchPayment) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("batch_payment: unmarshal failed: %w", err)
	}
	return nil
}

func (m *BatchPayment) Clone() *BatchPayment {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *BatchPayment) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("batch_payment: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("batch_payment: transition from %q to %q not allowed", m.Status, status)
}

func (m *BatchPayment) Sanitize() {
...
func (m *BatchPayment) Merge(other *BatchPayment) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *BatchPayment) String() string {
	return fmt.Sprintf("BatchPayment[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *BatchPayment) MatchesFilter(f *BatchPaymentFilter) bool {
...
func FilterBatchPaymentList(items []*BatchPayment, f *BatchPaymentFilter) *BatchPaymentListResponse {
	var filtered []*BatchPayment
	...
	return &BatchPaymentListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateBatchPaymentStats(items []*BatchPayment) *BatchPaymentStats {
