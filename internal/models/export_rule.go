...
func NewExportRule(name string) *ExportRule {
	now := time.Now()
	return &ExportRule{
		Name:      name,
		Status:    "pending",
...
func (m *ExportRule) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("export_rule: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("export_rule: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("export_rule: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("export_rule: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("export_rule: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("export_rule: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("export_rule: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("export_rule: version must be at least 1")
	}
	return nil
}

func (m *ExportRule) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("export_rule: marshal failed: %w", err)
	}
	return data, nil
}

func (m *ExportRule) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("export_rule: unmarshal failed: %w", err)
	}
	return nil
}

func (m *ExportRule) Clone() *ExportRule {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *ExportRule) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("export_rule: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("export_rule: transition from %q to %q not allowed", m.Status, status)
}

func (m *ExportRule) Sanitize() {
...
func (m *ExportRule) Merge(other *ExportRule) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *ExportRule) String() string {
	return fmt.Sprintf("ExportRule[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *ExportRule) MatchesFilter(f *ExportRuleFilter) bool {
...
func FilterExportRuleList(items []*ExportRule, f *ExportRuleFilter) *ExportRuleListResponse {
	var filtered []*ExportRule
...
func CalculateExportRuleStats(items []*ExportRule) *ExportRuleStats {
	stats := &ExportRuleStats{}
	if len(items) == 0 {
		return stats
	}
