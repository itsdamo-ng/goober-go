...
type CoreSurveyListResponse struct {
	Data       []*CoreSurvey `json:"data"`
...
func NewCoreSurvey(name string) *CoreSurvey {
	now := time.Now()
	return &CoreSurvey{
		Name:      name,
		Status:    "pending",
...
func (m *CoreSurvey) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("core_survey: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("core_survey: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("core_survey: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("core_survey: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("core_survey: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("core_survey: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("core_survey: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("core_survey: version must be at least 1")
	}
	return nil
}

func (m *CoreSurvey) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("core_survey: marshal failed: %w", err)
	}
	return data, nil
}

func (m *CoreSurvey) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("core_survey: unmarshal failed: %w", err)
	}
	return nil
}

func (m *CoreSurvey) Clone() *CoreSurvey {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *CoreSurvey) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("core_survey: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("core_survey: transition from %q to %q not allowed", m.Status, status)
}

func (m *CoreSurvey) Sanitize() {
...
func (m *CoreSurvey) Merge(other *CoreSurvey) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *CoreSurvey) String() string {
	return fmt.Sprintf("CoreSurvey[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *CoreSurvey) MatchesFilter(f *CoreSurveyFilter) bool {
...
func FilterCoreSurveyList(items []*CoreSurvey, f *CoreSurveyFilter) *CoreSurveyListResponse {
	var filtered []*CoreSurvey
...
func CalculateCoreSurveyStats(items []*CoreSurvey) *CoreSurveyStats {
	stats := &CoreSurveyStats{}
	if len(items) == 0 {
		return stats
	}
