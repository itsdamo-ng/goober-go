...
type CoreInvoiceListResponse struct {
...
func NewCoreInvoice(name string) *CoreInvoice {
	now := time.Now()
	return &CoreInvoice{
		Name:      name,
		Status:    "pending",
...
func (m *CoreInvoice) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("core_invoice: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("core_invoice: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("core_invoice: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("core_invoice: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("core_invoice: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("core_invoice: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("core_invoice: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("core_invoice: version must be at least 1")
	}
	return nil
}

func (m *CoreInvoice) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("core_invoice: marshal failed: %w", err)
	}
	return data, nil
}

func (m *CoreInvoice) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("core_invoice: unmarshal failed: %w", err)
	}
	return nil
}

func (m *CoreInvoice) Clone() *CoreInvoice {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *CoreInvoice) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("core_invoice: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("core_invoice: transition from %q to %q not allowed", m.Status, status)
}

func (m *CoreInvoice) Sanitize() {
...
func (m *CoreInvoice) Merge(other *CoreInvoice) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *CoreInvoice) String() string {
	return fmt.Sprintf("CoreInvoice[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *CoreInvoice) MatchesFilter(f *CoreInvoiceFilter) bool {
...
func FilterCoreInvoiceList(items []*CoreInvoice, f *CoreInvoiceFilter) *CoreInvoiceListResponse {
	var filtered []*CoreInvoice
	...
	return &CoreInvoiceListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateCoreInvoiceStats(items []*CoreInvoice) *CoreInvoiceStats {
	stats := &CoreInvoiceStats{}
	if len(items) == 0 {
		return stats
	}
