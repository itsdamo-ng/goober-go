...
type AdminAudienceListResponse struct {
	Data       []*AdminAudience `json:"data"`
...
type AdminAudienceStats struct {
...
func NewAdminAudience(name string) *AdminAudience {
	now := time.Now()
...
func (m *AdminAudience) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("admin_audience: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_audience: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_audience: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_audience: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_audience: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_audience: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_audience: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_audience: version must be at least 1")
	}
	return nil
}

func (m *AdminAudience) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_audience: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminAudience) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_audience: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminAudience) Clone() *AdminAudience {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminAudience) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_audience: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_audience: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminAudience) Sanitize() {
...
func (m *AdminAudience) Merge(other *AdminAudience) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminAudience) String() string {
	return fmt.Sprintf("AdminAudience[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminAudience) MatchesFilter(f *AdminAudienceFilter) bool {
...
func FilterAdminAudienceList(items []*AdminAudience, f *AdminAudienceFilter) *AdminAudienceListResponse {
	var filtered []*AdminAudience
	...
	return &AdminAudienceListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateAdminAudienceStats(items []*AdminAudience) *AdminAudienceStats {
