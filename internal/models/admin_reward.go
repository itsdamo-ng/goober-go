...
func NewAdminReward(name string) *AdminReward {
	now := time.Now()
	return &AdminReward{
		Name:      name,
		Status:    "pending",
...
func (m *AdminReward) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("admin_reward: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_reward: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_reward: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_reward: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_reward: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_reward: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_reward: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_reward: version must be at least 1")
	}
	return nil
}

func (m *AdminReward) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_reward: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminReward) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_reward: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminReward) Clone() *AdminReward {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminReward) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_reward: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_reward: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminReward) Sanitize() {
...
func (m *AdminReward) Merge(other *AdminReward) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminReward) String() string {
	return fmt.Sprintf("AdminReward[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminReward) MatchesFilter(f *AdminRewardFilter) bool {
...
func FilterAdminRewardList(items []*AdminReward, f *AdminRewardFilter) *AdminRewardListResponse {
	var filtered []*AdminReward
...
func CalculateAdminRewardStats(items []*AdminReward) *AdminRewardStats {
