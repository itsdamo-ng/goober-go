...
type AdminSnapshotFilter struct {
	IDs       []int64   `json:"ids"`
...
type AdminSnapshotListResponse struct {
	Data       []*AdminSnapshot `json:"data"`
...
func NewAdminSnapshot(name string) *AdminSnapshot {
	now := time.Now()
...
func (m *AdminSnapshot) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("admin_snapshot: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_snapshot: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_snapshot: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_snapshot: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_snapshot: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_snapshot: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_snapshot: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_snapshot: version must be at least 1")
	}
	return nil
}

func (m *AdminSnapshot) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_snapshot: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminSnapshot) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_snapshot: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminSnapshot) Clone() *AdminSnapshot {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminSnapshot) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_snapshot: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_snapshot: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminSnapshot) Sanitize() {
...
func (m *AdminSnapshot) Merge(other *AdminSnapshot) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminSnapshot) String() string {
	return fmt.Sprintf("AdminSnapshot[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminSnapshot) MatchesFilter(f *AdminSnapshotFilter) bool {
...
func FilterAdminSnapshotList(items []*AdminSnapshot, f *AdminSnapshotFilter) *AdminSnapshotListResponse {
	var filtered []*AdminSnapshot
...
func CalculateAdminSnapshotStats(items []*AdminSnapshot) *AdminSnapshotStats {
	stats := &AdminSnapshotStats{}
	if len(items) == 0 {
		return stats
	}
