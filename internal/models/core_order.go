...
type CoreOrderListResponse struct {
	Data       []*CoreOrder `json:"data"`
...
func NewCoreOrder(name string) *CoreOrder {
	now := time.Now()
	return &CoreOrder{
		Name:      name,
		Status:    "pending",
...
func (m *CoreOrder) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("core_order: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("core_order: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("core_order: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("core_order: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("core_order: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("core_order: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("core_order: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("core_order: version must be at least 1")
	}
	return nil
}

func (m *CoreOrder) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("core_order: marshal failed: %w", err)
	}
	return data, nil
}

func (m *CoreOrder) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("core_order: unmarshal failed: %w", err)
	}
	return nil
}

func (m *CoreOrder) Clone() *CoreOrder {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *CoreOrder) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("core_order: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("core_order: transition from %q to %q not allowed", m.Status, status)
}

func (m *CoreOrder) Sanitize() {
...
func (m *CoreOrder) Merge(other *CoreOrder) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *CoreOrder) String() string {
	return fmt.Sprintf("CoreOrder[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *CoreOrder) MatchesFilter(f *CoreOrderFilter) bool {
...
func FilterCoreOrderList(items []*CoreOrder, f *CoreOrderFilter) *CoreOrderListResponse {
	var filtered []*CoreOrder
...
func CalculateCoreOrderStats(items []*CoreOrder) *CoreOrderStats {
	stats := &CoreOrderStats{}
	if len(items) == 0 {
		return stats
	}
