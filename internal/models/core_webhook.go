...
type CoreWebhookListResponse struct {
	Data       []*CoreWebhook `json:"data"`
...
func NewCoreWebhook(name string) *CoreWebhook {
	now := time.Now()
	return &CoreWebhook{
		Name:      name,
		Status:    "pending",
...
func (m *CoreWebhook) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("core_webhook: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("core_webhook: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("core_webhook: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("core_webhook: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("core_webhook: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("core_webhook: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("core_webhook: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("core_webhook: version must be at least 1")
	}
	return nil
}

func (m *CoreWebhook) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("core_webhook: marshal failed: %w", err)
	}
	return data, nil
}

func (m *CoreWebhook) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("core_webhook: unmarshal failed: %w", err)
	}
	return nil
}

func (m *CoreWebhook) Clone() *CoreWebhook {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *CoreWebhook) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("core_webhook: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("core_webhook: transition from %q to %q not allowed", m.Status, status)
}

func (m *CoreWebhook) Sanitize() {
...
func (m *CoreWebhook) Merge(other *CoreWebhook) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *CoreWebhook) String() string {
	return fmt.Sprintf("CoreWebhook[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *CoreWebhook) MatchesFilter(f *CoreWebhookFilter) bool {
...
func FilterCoreWebhookList(items []*CoreWebhook, f *CoreWebhookFilter) *CoreWebhookListResponse {
	var filtered []*CoreWebhook
	...
	return &CoreWebhookListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateCoreWebhookStats(items []*CoreWebhook) *CoreWebhookStats {
	stats := &CoreWebhookStats{}
	if len(items) == 0 {
		return stats
	}
