...
func NewExportTask(name string) *ExportTask {
	now := time.Now()
	return &ExportTask{
		Name:      name,
		Status:    "pending",
...
func (m *ExportTask) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("export_task: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("export_task: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("export_task: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("export_task: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("export_task: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("export_task: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("export_task: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("export_task: version must be at least 1")
	}
	return nil
}

func (m *ExportTask) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("export_task: marshal failed: %w", err)
	}
	return data, nil
}

func (m *ExportTask) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("export_task: unmarshal failed: %w", err)
	}
	return nil
}

func (m *ExportTask) Clone() *ExportTask {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *ExportTask) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("export_task: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("export_task: transition from %q to %q not allowed", m.Status, status)
}

func (m *ExportTask) Sanitize() {
...
func (m *ExportTask) Merge(other *ExportTask) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *ExportTask) String() string {
	return fmt.Sprintf("ExportTask[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *ExportTask) MatchesFilter(f *ExportTaskFilter) bool {
...
func FilterExportTaskList(items []*ExportTask, f *ExportTaskFilter) *ExportTaskListResponse {
	var filtered []*ExportTask
...
func CalculateExportTaskStats(items []*ExportTask) *ExportTaskStats {
	stats := &ExportTaskStats{}
	if len(items) == 0 {
		return stats
	}
