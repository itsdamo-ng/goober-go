...
func (m *User) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("user: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("user: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("user: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("user: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("user: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("user: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("user: version must be at least 1")
	}
	return nil
}
...
func (m *User) ToJSON() ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("user: marshal failed: %w", err)
	}
	return data, nil
}
...
func (m *User) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("user: unmarshal failed: %w", err)
	}
	return nil
}
...
func (m *User) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("user: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("user: transition from %q to %q not allowed", m.Status, status)
}
...
func (m *User) String() string {
	return fmt.Sprintf("User[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *User) MatchesFilter(f *UserFilter) bool {
...
func FilterUserList(items []*User, f *UserFilter) *UserListResponse {
	var filtered []*User
...
func CalculateUserStats(items []*User) *UserStats {
	stats := &UserStats{}
	if len(items) == 0 {
		return stats
	}
