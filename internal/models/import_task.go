...
func (m *ImportTask) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("import_task: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("import_task: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("import_task: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("import_task: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("import_task: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("import_task: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("import_task: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("import_task: version must be at least 1")
	}
	return nil
}

func (m *ImportTask) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("import_task: marshal failed: %w", err)
	}
	return data, nil
}

func (m *ImportTask) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("import_task: unmarshal failed: %w", err)
	}
	return nil
}

func (m *ImportTask) Clone() *ImportTask {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *ImportTask) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("import_task: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("import_task: transition from %q to %q not allowed", m.Status, status)
}

func (m *ImportTask) Sanitize() {
...
func (m *ImportTask) Merge(other *ImportTask) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *ImportTask) String() string {
	return fmt.Sprintf("ImportTask[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *ImportTask) MatchesFilter(f *ImportTaskFilter) bool {
...
func FilterImportTaskList(items []*ImportTask, f *ImportTaskFilter) *ImportTaskListResponse {
	var filtered []*ImportTask
...
func CalculateImportTaskStats(items []*ImportTask) *ImportTaskStats {
