...
type BatchVenueListResponse struct {
	Data       []*BatchVenue `json:"data"`
...
func NewBatchVenue(name string) *BatchVenue {
	now := time.Now()
	return &BatchVenue{
		Name:      name,
		Status:    "pending",
...
func (m *BatchVenue) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("batch_venue: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("batch_venue: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("batch_venue: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("batch_venue: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("batch_venue: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("batch_venue: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("batch_venue: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("batch_venue: version must be at least 1")
	}
	return nil
}

func (m *BatchVenue) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("batch_venue: marshal failed: %w", err)
	}
	return data, nil
}

func (m *BatchVenue) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("batch_venue: unmarshal failed: %w", err)
	}
	return nil
}

func (m *BatchVenue) Clone() *BatchVenue {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *BatchVenue) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("batch_venue: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("batch_venue: transition from %q to %q not allowed", m.Status, status)
}

func (m *BatchVenue) Sanitize() {
...
func (m *BatchVenue) Merge(other *BatchVenue) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *BatchVenue) String() string {
	return fmt.Sprintf("BatchVenue[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *BatchVenue) MatchesFilter(f *BatchVenueFilter) bool {
...
func FilterBatchVenueList(items []*BatchVenue, f *BatchVenueFilter) *BatchVenueListResponse {
	var filtered []*BatchVenue
...
func CalculateBatchVenueStats(items []*BatchVenue) *BatchVenueStats {
	stats := &BatchVenueStats{}
	if len(items) == 0 {
		return stats
	}
