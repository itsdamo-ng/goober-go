...
type InventoryEventListResponse struct {
	Data       []*InventoryEvent `json:"data"`
...
type InventoryEventStats struct {
...
func NewInventoryEvent(name string) *InventoryEvent {
	now := time.Now()
...
func (m *InventoryEvent) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("inventory_event: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("inventory_event: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("inventory_event: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("inventory_event: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("inventory_event: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("inventory_event: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("inventory_event: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("inventory_event: version must be at least 1")
	}
	return nil
}

func (m *InventoryEvent) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("inventory_event: marshal failed: %w", err)
	}
	return data, nil
}

func (m *InventoryEvent) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("inventory_event: unmarshal failed: %w", err)
	}
	return nil
}

func (m *InventoryEvent) Clone() *InventoryEvent {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *InventoryEvent) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("inventory_event: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("inventory_event: transition from %q to %q not allowed", m.Status, status)
}

func (m *InventoryEvent) Sanitize() {
...
func (m *InventoryEvent) Merge(other *InventoryEvent) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *InventoryEvent) String() string {
	return fmt.Sprintf("InventoryEvent[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *InventoryEvent) MatchesFilter(f *InventoryEventFilter) bool {
...
func FilterInventoryEventList(items []*InventoryEvent, f *InventoryEventFilter) *InventoryEventListResponse {
	var filtered []*InventoryEvent
...
func CalculateInventoryEventStats(items []*InventoryEvent) *InventoryEventStats {
	stats := &InventoryEventStats{}
	if len(items) == 0 {
		return stats
	}
