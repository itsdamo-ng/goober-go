...
type CorePolicyListResponse struct {
	Data       []*CorePolicy `json:"data"`
...
func NewCorePolicy(name string) *CorePolicy {
	now := time.Now()
	return &CorePolicy{
		Name:      name,
		Status:    "pending",
...
func (m *CorePolicy) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("core_policy: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("core_policy: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("core_policy: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("core_policy: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("core_policy: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("core_policy: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("core_policy: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("core_policy: version must be at least 1")
	}
	return nil
}

func (m *CorePolicy) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("core_policy: marshal failed: %w", err)
	}
	return data, nil
}

func (m *CorePolicy) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("core_policy: unmarshal failed: %w", err)
	}
	return nil
}

func (m *CorePolicy) Clone() *CorePolicy {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *CorePolicy) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("core_policy: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("core_policy: transition from %q to %q not allowed", m.Status, status)
}

func (m *CorePolicy) Sanitize() {
...
func (m *CorePolicy) Merge(other *CorePolicy) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *CorePolicy) String() string {
	return fmt.Sprintf("CorePolicy[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *CorePolicy) MatchesFilter(f *CorePolicyFilter) bool {
...
func FilterCorePolicyList(items []*CorePolicy, f *CorePolicyFilter) *CorePolicyListResponse {
	var filtered []*CorePolicy
...
func CalculateCorePolicyStats(items []*CorePolicy) *CorePolicyStats {
	stats := &CorePolicyStats{}
	if len(items) == 0 {
		return stats
	}
