...
type SearchMetricFilter struct {
	IDs       []int64   `json:"ids"`
...
func NewSearchMetric(name string) *SearchMetric {
	now := time.Now()
	return &SearchMetric{
		Name:      name,
		Status:    "pending",
...
func (m *SearchMetric) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("search_metric: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("search_metric: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("search_metric: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("search_metric: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("search_metric: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("search_metric: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("search_metric: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("search_metric: version must be at least 1")
	}
	return nil
}

func (m *SearchMetric) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("search_metric: marshal failed: %w", err)
	}
	return data, nil
}

func (m *SearchMetric) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("search_metric: unmarshal failed: %w", err)
	}
	return nil
}

func (m *SearchMetric) Clone() *SearchMetric {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *SearchMetric) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("search_metric: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("search_metric: transition from %q to %q not allowed", m.Status, status)
}

func (m *SearchMetric) Sanitize() {
...
func (m *SearchMetric) Merge(other *SearchMetric) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *SearchMetric) String() string {
	return fmt.Sprintf("SearchMetric[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *SearchMetric) MatchesFilter(f *SearchMetricFilter) bool {
...
func FilterSearchMetricList(items []*SearchMetric, f *SearchMetricFilter) *SearchMetricListResponse {
	var filtered []*SearchMetric
	...
	return &SearchMetricListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateSearchMetricStats(items []*SearchMetric) *SearchMetricStats {
	stats := &SearchMetricStats{}
	if len(items) == 0 {
		return stats
	}
