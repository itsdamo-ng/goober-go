...
func (m *Schedule) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("schedule: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("schedule: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("schedule: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("schedule: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("schedule: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("schedule: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("schedule: version must be at least 1")
	}
	return nil
}
...
func (m *Schedule) ToJSON() ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("schedule: marshal failed: %w", err)
	}
	return data, nil
}
...
func (m *Schedule) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("schedule: unmarshal failed: %w", err)
	}
	return nil
}
...
func (m *Schedule) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("schedule: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("schedule: transition from %q to %q not allowed", m.Status, status)
}

func (m *Schedule) Sanitize() {
...
func (m *Schedule) Merge(other *Schedule) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Schedule) String() string {
	return fmt.Sprintf("Schedule[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Schedule) MatchesFilter(f *ScheduleFilter) bool {
...
func FilterScheduleList(items []*Schedule, f *ScheduleFilter) *ScheduleListResponse {
...
func CalculateScheduleStats(items []*Schedule) *ScheduleStats {
	stats := &ScheduleStats{}
	if len(items) == 0 {
		return stats
	}
