...
func NewAdminOrder(name string) *AdminOrder {
	now := time.Now()
	return &AdminOrder{
		Name:      name,
		Status:    "pending",
...
func (m *AdminOrder) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("admin_order: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_order: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_order: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_order: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_order: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_order: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_order: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_order: version must be at least 1")
	}
	return nil
}

func (m *AdminOrder) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_order: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminOrder) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_order: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminOrder) Clone() *AdminOrder {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminOrder) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_order: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_order: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminOrder) Sanitize() {
...
func (m *AdminOrder) Merge(other *AdminOrder) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminOrder) String() string {
	return fmt.Sprintf("AdminOrder[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminOrder) MatchesFilter(f *AdminOrderFilter) bool {
...
func FilterAdminOrderList(items []*AdminOrder, f *AdminOrderFilter) *AdminOrderListResponse {
	var filtered []*AdminOrder
...
func CalculateAdminOrderStats(items []*AdminOrder) *AdminOrderStats {
