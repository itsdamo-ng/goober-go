...
func NewAdminBooking(name string) *AdminBooking {
	now := time.Now()
	return &AdminBooking{
		Name:      name,
		Status:    "pending",
...
func (m *AdminBooking) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("admin_booking: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_booking: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_booking: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_booking: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_booking: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_booking: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_booking: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_booking: version must be at least 1")
	}
	return nil
}

func (m *AdminBooking) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_booking: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminBooking) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_booking: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminBooking) Clone() *AdminBooking {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminBooking) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_booking: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_booking: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminBooking) Sanitize() {
...
func (m *AdminBooking) Merge(other *AdminBooking) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminBooking) String() string {
	return fmt.Sprintf("AdminBooking[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminBooking) MatchesFilter(f *AdminBookingFilter) bool {
...
func FilterAdminBookingList(items []*AdminBooking, f *AdminBookingFilter) *AdminBookingListResponse {
	var filtered []*AdminBooking
...
func CalculateAdminBookingStats(items []*AdminBooking) *AdminBookingStats {
