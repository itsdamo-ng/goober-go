...
func NewAdminTask(name string) *AdminTask {
	now := time.Now()
	return &AdminTask{
		Name:      name,
		Status:    "pending",
...
func (m *AdminTask) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("admin_task: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_task: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_task: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_task: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_task: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_task: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_task: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_task: version must be at least 1")
	}
	return nil
}

func (m *AdminTask) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_task: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminTask) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_task: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminTask) Clone() *AdminTask {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminTask) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_task: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_task: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminTask) Sanitize() {
...
func (m *AdminTask) Merge(other *AdminTask) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminTask) String() string {
	return fmt.Sprintf("AdminTask[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminTask) MatchesFilter(f *AdminTaskFilter) bool {
...
func FilterAdminTaskList(items []*AdminTask, f *AdminTaskFilter) *AdminTaskListResponse {
	var filtered []*AdminTask
...
func CalculateAdminTaskStats(items []*AdminTask) *AdminTaskStats {
