...
func (m *Ticket) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("ticket: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("ticket: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("ticket: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("ticket: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("ticket: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("ticket: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("ticket: version must be at least 1")
	}
	return nil
}

func (m *Ticket) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("ticket: marshal failed: %w", err)
	}
	return data, nil
}

func (m *Ticket) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("ticket: unmarshal failed: %w", err)
	}
	return nil
}

func (m *Ticket) Clone() *Ticket {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *Ticket) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("ticket: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("ticket: transition from %q to %q not allowed", m.Status, status)
}

func (m *Ticket) Sanitize() {
...
func (m *Ticket) Merge(other *Ticket) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Ticket) String() string {
	return fmt.Sprintf("Ticket[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Ticket) MatchesFilter(f *TicketFilter) bool {
...
func FilterTicketList(items []*Ticket, f *TicketFilter) *TicketListResponse {
	var filtered []*Ticket
...
func CalculateTicketStats(items []*Ticket) *TicketStats {
	stats := &TicketStats{}
	if len(items) == 0 {
		return stats
	}
