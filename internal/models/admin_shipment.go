...
type AdminShipmentStats struct {
...
func NewAdminShipment(name string) *AdminShipment {
	now := time.Now()
...
func (m *AdminShipment) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("admin_shipment: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_shipment: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_shipment: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_shipment: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_shipment: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_shipment: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_shipment: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_shipment: version must be at least 1")
	}
	return nil
}

func (m *AdminShipment) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_shipment: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminShipment) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_shipment: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminShipment) Clone() *AdminShipment {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminShipment) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_shipment: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_shipment: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminShipment) Sanitize() {
...
func (m *AdminShipment) Merge(other *AdminShipment) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminShipment) String() string {
	return fmt.Sprintf("AdminShipment[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminShipment) MatchesFilter(f *AdminShipmentFilter) bool {
...
func FilterAdminShipmentList(items []*AdminShipment, f *AdminShipmentFilter) *AdminShipmentListResponse {
	var filtered []*AdminShipment
...
func CalculateAdminShipmentStats(items []*AdminShipment) *AdminShipmentStats {
	stats := &AdminShipmentStats{}
	if len(items) == 0 {
		return stats
	}
