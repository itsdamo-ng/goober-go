...
type BatchChannelFilter struct {
	IDs       []int64   `json:"ids"`
...
type BatchChannelListResponse struct {
	Data       []*BatchChannel `json:"data"`
...
func NewBatchChannel(name string) *BatchChannel {
	now := time.Now()
	return &BatchChannel{
		Name:      name,
		Status:    "pending",
...
func (m *BatchChannel) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("batch_channel: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("batch_channel: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("batch_channel: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("batch_channel: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("batch_channel: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("batch_channel: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("batch_channel: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("batch_channel: version must be at least 1")
	}
	return nil
}

func (m *BatchChannel) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("batch_channel: marshal failed: %w", err)
	}
	return data, nil
}

func (m *BatchChannel) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("batch_channel: unmarshal failed: %w", err)
	}
	return nil
}

func (m *BatchChannel) Clone() *BatchChannel {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *BatchChannel) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("batch_channel: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("batch_channel: transition from %q to %q not allowed", m.Status, status)
}

func (m *BatchChannel) Sanitize() {
...
func (m *BatchChannel) Merge(other *BatchChannel) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *BatchChannel) String() string {
	return fmt.Sprintf("BatchChannel[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *BatchChannel) MatchesFilter(f *BatchChannelFilter) bool {
...
func FilterBatchChannelList(items []*BatchChannel, f *BatchChannelFilter) *BatchChannelListResponse {
	var filtered []*BatchChannel
	...
	return &BatchChannelListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateBatchChannelStats(items []*BatchChannel) *BatchChannelStats {
