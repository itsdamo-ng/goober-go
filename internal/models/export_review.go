...
type ExportReviewFilter struct {
	IDs       []int64   `json:"ids"`
...
type ExportReviewListResponse struct {
	Data       []*ExportReview `json:"data"`
...
func NewExportReview(name string) *ExportReview {
	now := time.Now()
	return &ExportReview{
		Name:      name,
		Status:    "pending",
...
func (m *ExportReview) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("export_review: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("export_review: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("export_review: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("export_review: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("export_review: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("export_review: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("export_review: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("export_review: version must be at least 1")
	}
	return nil
}

func (m *ExportReview) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("export_review: marshal failed: %w", err)
	}
	return data, nil
}

func (m *ExportReview) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("export_review: unmarshal failed: %w", err)
	}
	return nil
}

func (m *ExportReview) Clone() *ExportReview {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *ExportReview) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("export_review: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("export_review: transition from %q to %q not allowed", m.Status, status)
}

func (m *ExportReview) Sanitize() {
...
func (m *ExportReview) Merge(other *ExportReview) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *ExportReview) String() string {
	return fmt.Sprintf("ExportReview[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *ExportReview) MatchesFilter(f *ExportReviewFilter) bool {
...
func FilterExportReviewList(items []*ExportReview, f *ExportReviewFilter) *ExportReviewListResponse {
	var filtered []*ExportReview
	...
	return &ExportReviewListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateExportReviewStats(items []*ExportReview) *ExportReviewStats {
	stats := &ExportReviewStats{}
	if len(items) == 0 {
		return stats
	}
