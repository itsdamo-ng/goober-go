...
func NewAuthUser(name string) *AuthUser {
	now := time.Now()
	return &AuthUser{
		Name:      name,
		Status:    "pending",
...
func (m *AuthUser) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("auth_user: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("auth_user: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("auth_user: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("auth_user: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("auth_user: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("auth_user: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("auth_user: version must be at least 1")
	}
	return nil
}

func (m *AuthUser) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("auth_user: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuthUser) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("auth_user: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuthUser) Clone() *AuthUser {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuthUser) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("auth_user: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("auth_user: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuthUser) Sanitize() {
...
func (m *AuthUser) Merge(other *AuthUser) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuthUser) String() string {
	return fmt.Sprintf("AuthUser[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuthUser) MatchesFilter(f *AuthUserFilter) bool {
...
func FilterAuthUserList(items []*AuthUser, f *AuthUserFilter) *AuthUserListResponse {
	var filtered []*AuthUser
...
func CalculateAuthUserStats(items []*AuthUser) *AuthUserStats {
	stats := &AuthUserStats{}
	if len(items) == 0 {
		return stats
	}
