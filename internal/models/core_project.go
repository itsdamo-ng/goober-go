...
type CoreProjectListResponse struct {
	Data       []*CoreProject `json:"data"`
...
func NewCoreProject(name string) *CoreProject {
	now := time.Now()
	return &CoreProject{
		Name:      name,
		Status:    "pending",
...
func (m *CoreProject) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("core_project: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("core_project: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("core_project: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("core_project: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("core_project: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("core_project: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("core_project: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("core_project: version must be at least 1")
	}
	return nil
}

func (m *CoreProject) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("core_project: marshal failed: %w", err)
	}
	return data, nil
}

func (m *CoreProject) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("core_project: unmarshal failed: %w", err)
	}
	return nil
}

func (m *CoreProject) Clone() *CoreProject {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *CoreProject) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("core_project: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("core_project: transition from %q to %q not allowed", m.Status, status)
}

func (m *CoreProject) Sanitize() {
...
func (m *CoreProject) Merge(other *CoreProject) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *CoreProject) String() string {
	return fmt.Sprintf("CoreProject[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *CoreProject) MatchesFilter(f *CoreProjectFilter) bool {
...
func FilterCoreProjectList(items []*CoreProject, f *CoreProjectFilter) *CoreProjectListResponse {
	var filtered []*CoreProject
	...
	return &CoreProjectListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateCoreProjectStats(items []*CoreProject) *CoreProjectStats {
	stats := &CoreProjectStats{}
	if len(items) == 0 {
		return stats
	}
