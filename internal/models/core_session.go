...
type CoreSessionListResponse struct {
	Data       []*CoreSession `json:"data"`
...
func NewCoreSession(name string) *CoreSession {
	now := time.Now()
	return &CoreSession{
		Name:      name,
		Status:    "pending",
...
func (m *CoreSession) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("core_session: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("core_session: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("core_session: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("core_session: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("core_session: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("core_session: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("core_session: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("core_session: version must be at least 1")
	}
	return nil
}

func (m *CoreSession) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("core_session: marshal failed: %w", err)
	}
	return data, nil
}

func (m *CoreSession) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("core_session: unmarshal failed: %w", err)
	}
	return nil
}

func (m *CoreSession) Clone() *CoreSession {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *CoreSession) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("core_session: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("core_session: transition from %q to %q not allowed", m.Status, status)
}

func (m *CoreSession) Sanitize() {
...
func (m *CoreSession) Merge(other *CoreSession) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *CoreSession) String() string {
	return fmt.Sprintf("CoreSession[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *CoreSession) MatchesFilter(f *CoreSessionFilter) bool {
...
func FilterCoreSessionList(items []*CoreSession, f *CoreSessionFilter) *CoreSessionListResponse {
	var filtered []*CoreSession
	...
	return &CoreSessionListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateCoreSessionStats(items []*CoreSession) *CoreSessionStats {
	stats := &CoreSessionStats{}
	if len(items) == 0 {
		return stats
	}
