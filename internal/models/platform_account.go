...
type PlatformAccountStats struct {
...
func NewPlatformAccount(name string) *PlatformAccount {
	now := time.Now()
...
func (m *PlatformAccount) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("platform_account: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("platform_account: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("platform_account: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("platform_account: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("platform_account: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("platform_account: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("platform_account: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("platform_account: version must be at least 1")
	}
	return nil
}

func (m *PlatformAccount) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("platform_account: marshal failed: %w", err)
	}
	return data, nil
}

func (m *PlatformAccount) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("platform_account: unmarshal failed: %w", err)
	}
	return nil
}

func (m *PlatformAccount) Clone() *PlatformAccount {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *PlatformAccount) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("platform_account: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("platform_account: transition from %q to %q not allowed", m.Status, status)
}

func (m *PlatformAccount) Sanitize() {
...
func (m *PlatformAccount) Merge(other *PlatformAccount) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *PlatformAccount) String() string {
	return fmt.Sprintf("PlatformAccount[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *PlatformAccount) MatchesFilter(f *PlatformAccountFilter) bool {
...
func FilterPlatformAccountList(items []*PlatformAccount, f *PlatformAccountFilter) *PlatformAccountListResponse {
	var filtered []*PlatformAccount
...
func CalculatePlatformAccountStats(items []*PlatformAccount) *PlatformAccountStats {
