...
type ContentCampaignFilter struct {
	IDs       []int64   `json:"ids"`
...
type ContentCampaignListResponse struct {
	Data       []*ContentCampaign `json:"data"`
...
type ContentCampaignStats struct {
...
func NewContentCampaign(name string) *ContentCampaign {
	now := time.Now()
...
func (m *ContentCampaign) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("content_campaign: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("content_campaign: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("content_campaign: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("content_campaign: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("content_campaign: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("content_campaign: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("content_campaign: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("content_campaign: version must be at least 1")
	}
	return nil
}

func (m *ContentCampaign) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("content_campaign: marshal failed: %w", err)
	}
	return data, nil
}

func (m *ContentCampaign) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("content_campaign: unmarshal failed: %w", err)
	}
	return nil
}

func (m *ContentCampaign) Clone() *ContentCampaign {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *ContentCampaign) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("content_campaign: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("content_campaign: transition from %q to %q not allowed", m.Status, status)
}

func (m *ContentCampaign) Sanitize() {
...
func (m *ContentCampaign) Merge(other *ContentCampaign) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *ContentCampaign) String() string {
	return fmt.Sprintf("ContentCampaign[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *ContentCampaign) MatchesFilter(f *ContentCampaignFilter) bool {
...
func FilterContentCampaignList(items []*ContentCampaign, f *ContentCampaignFilter) *ContentCampaignListResponse {
	var filtered []*ContentCampaign
...
func CalculateContentCampaignStats(items []*ContentCampaign) *ContentCampaignStats {
