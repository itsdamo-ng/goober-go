...
type CoreAlertListResponse struct {
	Data       []*CoreAlert `json:"data"`
...
func NewCoreAlert(name string) *CoreAlert {
	now := time.Now()
	return &CoreAlert{
		Name:      name,
		Status:    "pending",
...
func (m *CoreAlert) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("core_alert: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("core_alert: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("core_alert: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("core_alert: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("core_alert: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("core_alert: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("core_alert: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("core_alert: version must be at least 1")
	}
	return nil
}

func (m *CoreAlert) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("core_alert: marshal failed: %w", err)
	}
	return data, nil
}

func (m *CoreAlert) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("core_alert: unmarshal failed: %w", err)
	}
	return nil
}

func (m *CoreAlert) Clone() *CoreAlert {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *CoreAlert) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("core_alert: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("core_alert: transition from %q to %q not allowed", m.Status, status)
}

func (m *CoreAlert) Sanitize() {
...
func (m *CoreAlert) Merge(other *CoreAlert) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *CoreAlert) String() string {
	return fmt.Sprintf("CoreAlert[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *CoreAlert) MatchesFilter(f *CoreAlertFilter) bool {
...
func FilterCoreAlertList(items []*CoreAlert, f *CoreAlertFilter) *CoreAlertListResponse {
	var filtered []*CoreAlert
...
func CalculateCoreAlertStats(items []*CoreAlert) *CoreAlertStats {
	stats := &CoreAlertStats{}
	if len(items) == 0 {
		return stats
	}
