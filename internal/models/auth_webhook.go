...
type AuthWebhookListResponse struct {
	Data       []*AuthWebhook `json:"data"`
...
func NewAuthWebhook(name string) *AuthWebhook {
	now := time.Now()
	return &AuthWebhook{
		Name:      name,
		Status:    "pending",
...
func (m *AuthWebhook) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("auth_webhook: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("auth_webhook: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("auth_webhook: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("auth_webhook: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("auth_webhook: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("auth_webhook: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("auth_webhook: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("auth_webhook: version must be at least 1")
	}
	return nil
}

func (m *AuthWebhook) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("auth_webhook: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuthWebhook) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("auth_webhook: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuthWebhook) Clone() *AuthWebhook {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuthWebhook) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("auth_webhook: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("auth_webhook: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuthWebhook) Sanitize() {
...
func (m *AuthWebhook) Merge(other *AuthWebhook) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuthWebhook) String() string {
	return fmt.Sprintf("AuthWebhook[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuthWebhook) MatchesFilter(f *AuthWebhookFilter) bool {
...
func FilterAuthWebhookList(items []*AuthWebhook, f *AuthWebhookFilter) *AuthWebhookListResponse {
	var filtered []*AuthWebhook
	...
	return &AuthWebhookListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateAuthWebhookStats(items []*AuthWebhook) *AuthWebhookStats {
	stats := &AuthWebhookStats{}
	if len(items) == 0 {
		return stats
	}
