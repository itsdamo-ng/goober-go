...
type BatchRewardListResponse struct {
	Data       []*BatchReward `json:"data"`
...
func NewBatchReward(name string) *BatchReward {
	now := time.Now()
	return &BatchReward{
		Name:      name,
		Status:    "pending",
...
func (m *BatchReward) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("batch_reward: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("batch_reward: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("batch_reward: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("batch_reward: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("batch_reward: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("batch_reward: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("batch_reward: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("batch_reward: version must be at least 1")
	}
	return nil
}

func (m *BatchReward) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("batch_reward: marshal failed: %w", err)
	}
	return data, nil
}

func (m *BatchReward) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("batch_reward: unmarshal failed: %w", err)
	}
	return nil
}

func (m *BatchReward) Clone() *BatchReward {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *BatchReward) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("batch_reward: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("batch_reward: transition from %q to %q not allowed", m.Status, status)
}

func (m *BatchReward) Sanitize() {
...
func (m *BatchReward) Merge(other *BatchReward) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *BatchReward) String() string {
	return fmt.Sprintf("BatchReward[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *BatchReward) MatchesFilter(f *BatchRewardFilter) bool {
...
func FilterBatchRewardList(items []*BatchReward, f *BatchRewardFilter) *BatchRewardListResponse {
	var filtered []*BatchReward
	...
	return &BatchRewardListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateBatchRewardStats(items []*BatchReward) *BatchRewardStats {
	stats := &BatchRewardStats{}
	if len(items) == 0 {
		return stats
	}
