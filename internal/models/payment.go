...
func (m *Payment) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("payment: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("payment: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("payment: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("payment: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("payment: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("payment: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("payment: version must be at least 1")
	}
	return nil
}

func (m *Payment) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("payment: marshal failed: %w", err)
	}
	return data, nil
}

func (m *Payment) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("payment: unmarshal failed: %w", err)
	}
	return nil
}

func (m *Payment) Clone() *Payment {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *Payment) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("payment: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("payment: transition from %q to %q not allowed", m.Status, status)
}

func (m *Payment) Sanitize() {
...
func (m *Payment) Merge(other *Payment) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Payment) String() string {
	return fmt.Sprintf("Payment[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Payment) MatchesFilter(f *PaymentFilter) bool {
...
func FilterPaymentList(items []*Payment, f *PaymentFilter) *PaymentListResponse {
...
func CalculatePaymentStats(items []*Payment) *PaymentStats {
	stats := &PaymentStats{}
	if len(items) == 0 {
		return stats
	}
