...
func NewAuthProject(name string) *AuthProject {
	now := time.Now()
	return &AuthProject{
		Name:      name,
		Status:    "pending",
...
func (m *AuthProject) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("auth_project: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("auth_project: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("auth_project: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("auth_project: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("auth_project: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("auth_project: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("auth_project: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("auth_project: version must be at least 1")
	}
	return nil
}

func (m *AuthProject) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("auth_project: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuthProject) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("auth_project: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuthProject) Clone() *AuthProject {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuthProject) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("auth_project: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("auth_project: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuthProject) Sanitize() {
...
func (m *AuthProject) Merge(other *AuthProject) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuthProject) String() string {
	return fmt.Sprintf("AuthProject[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuthProject) MatchesFilter(f *AuthProjectFilter) bool {
...
func FilterAuthProjectList(items []*AuthProject, f *AuthProjectFilter) *AuthProjectListResponse {
	var filtered []*AuthProject
	...
	return &AuthProjectListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateAuthProjectStats(items []*AuthProject) *AuthProjectStats {
	stats := &AuthProjectStats{}
	if len(items) == 0 {
		return stats
	}
