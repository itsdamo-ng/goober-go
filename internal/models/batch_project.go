...
type BatchProjectFilter struct {
	IDs       []int64   `json:"ids"`
...
type BatchProjectListResponse struct {
	Data       []*BatchProject `json:"data"`
...
func NewBatchProject(name string) *BatchProject {
	now := time.Now()
	return &BatchProject{
		Name:      name,
		Status:    "pending",
...
func (m *BatchProject) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("batch_project: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("batch_project: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("batch_project: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("batch_project: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("batch_project: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("batch_project: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("batch_project: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("batch_project: version must be at least 1")
	}
	return nil
}

func (m *BatchProject) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("batch_project: marshal failed: %w", err)
	}
	return data, nil
}

func (m *BatchProject) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("batch_project: unmarshal failed: %w", err)
	}
	return nil
}

func (m *BatchProject) Clone() *BatchProject {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *BatchProject) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("batch_project: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("batch_project: transition from %q to %q not allowed", m.Status, status)
}

func (m *BatchProject) Sanitize() {
...
func (m *BatchProject) Merge(other *BatchProject) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *BatchProject) String() string {
	return fmt.Sprintf("BatchProject[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *BatchProject) MatchesFilter(f *BatchProjectFilter) bool {
...
func FilterBatchProjectList(items []*BatchProject, f *BatchProjectFilter) *BatchProjectListResponse {
	var filtered []*BatchProject
...
func CalculateBatchProjectStats(items []*BatchProject) *BatchProjectStats {
	stats := &BatchProjectStats{}
	if len(items) == 0 {
		return stats
	}
