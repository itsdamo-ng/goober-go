...
type SearchFeatureFilter struct {
	IDs       []int64   `json:"ids"`
...
type SearchFeatureListResponse struct {
...
type SearchFeatureStats struct {
...
func NewSearchFeature(name string) *SearchFeature {
	now := time.Now()
...
func (m *SearchFeature) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("search_feature: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("search_feature: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("search_feature: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("search_feature: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("search_feature: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("search_feature: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("search_feature: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("search_feature: version must be at least 1")
	}
	return nil
}

func (m *SearchFeature) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("search_feature: marshal failed: %w", err)
	}
	return data, nil
}

func (m *SearchFeature) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("search_feature: unmarshal failed: %w", err)
	}
	return nil
}

func (m *SearchFeature) Clone() *SearchFeature {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *SearchFeature) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("search_feature: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("search_feature: transition from %q to %q not allowed", m.Status, status)
}

func (m *SearchFeature) Sanitize() {
...
func (m *SearchFeature) Merge(other *SearchFeature) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *SearchFeature) String() string {
	return fmt.Sprintf("SearchFeature[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *SearchFeature) MatchesFilter(f *SearchFeatureFilter) bool {
...
func FilterSearchFeatureList(items []*SearchFeature, f *SearchFeatureFilter) *SearchFeatureListResponse {
	var filtered []*SearchFeature
	...
	return &SearchFeatureListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateSearchFeatureStats(items []*SearchFeature) *SearchFeatureStats {
	stats := &SearchFeatureStats{}
	if len(items) == 0 {
		return stats
	}
