...
type SupportLeadListResponse struct {
	Data       []*SupportLead `json:"data"`
...
func NewSupportLead(name string) *SupportLead {
	now := time.Now()
	return &SupportLead{
		Name:      name,
		Status:    "pending",
...
func (m *SupportLead) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("support_lead: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("support_lead: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("support_lead: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("support_lead: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("support_lead: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("support_lead: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("support_lead: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("support_lead: version must be at least 1")
	}
	return nil
}

func (m *SupportLead) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("support_lead: marshal failed: %w", err)
	}
	return data, nil
}

func (m *SupportLead) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("support_lead: unmarshal failed: %w", err)
	}
	return nil
}

func (m *SupportLead) Clone() *SupportLead {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *SupportLead) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("support_lead: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("support_lead: transition from %q to %q not allowed", m.Status, status)
}

func (m *SupportLead) Sanitize() {
...
func (m *SupportLead) Merge(other *SupportLead) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *SupportLead) String() string {
	return fmt.Sprintf("SupportLead[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *SupportLead) MatchesFilter(f *SupportLeadFilter) bool {
...
func FilterSupportLeadList(items []*SupportLead, f *SupportLeadFilter) *SupportLeadListResponse {
	var filtered []*SupportLead
	...
	return &SupportLeadListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateSupportLeadStats(items []*SupportLead) *SupportLeadStats {
	stats := &SupportLeadStats{}
	if len(items) == 0 {
		return stats
	}
