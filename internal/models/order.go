...
func (m *Order) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("order: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("order: description exceeds maximum length of 5000 characters")
	}
...
func (m *Order) ToJSON() ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("order: marshal failed: %w", err)
	}
	return data, nil
}
...
func (m *Order) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("order: unmarshal failed: %w", err)
	}
	return nil
}
...
func (m *Order) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("order: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("order: transition from %q to %q not allowed", m.Status, status)
}

func (m *Order) Sanitize() {
...
func (m *Order) Merge(other *Order) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Order) String() string {
	return fmt.Sprintf("Order[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Order) MatchesFilter(f *OrderFilter) bool {
...
func FilterOrderList(items []*Order, f *OrderFilter) *OrderListResponse {
	var filtered []*Order
...
func CalculateOrderStats(items []*Order) *OrderStats {
	stats := &OrderStats{}
	if len(items) == 0 {
		return stats
	}
