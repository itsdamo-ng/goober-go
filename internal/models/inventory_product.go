...
type InventoryProductListResponse struct {
...
func NewInventoryProduct(name string) *InventoryProduct {
	now := time.Now()
...
func (m *InventoryProduct) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("inventory_product: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("inventory_product: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("inventory_product: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("inventory_product: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("inventory_product: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("inventory_product: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("inventory_product: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("inventory_product: version must be at least 1")
	}
	return nil
}

func (m *InventoryProduct) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("inventory_product: marshal failed: %w", err)
	}
	return data, nil
}

func (m *InventoryProduct) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("inventory_product: unmarshal failed: %w", err)
	}
	return nil
}

func (m *InventoryProduct) Clone() *InventoryProduct {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *InventoryProduct) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("inventory_product: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("inventory_product: transition from %q to %q not allowed", m.Status, status)
}

func (m *InventoryProduct) Sanitize() {
...
func (m *InventoryProduct) Merge(other *InventoryProduct) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *InventoryProduct) String() string {
	return fmt.Sprintf("InventoryProduct[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *InventoryProduct) MatchesFilter(f *InventoryProductFilter) bool {
...
func FilterInventoryProductList(items []*InventoryProduct, f *InventoryProductFilter) *InventoryProductListResponse {
...
func CalculateInventoryProductStats(items []*InventoryProduct) *InventoryProductStats {
	stats := &InventoryProductStats{}
	if len(items) == 0 {
		return stats
	}
