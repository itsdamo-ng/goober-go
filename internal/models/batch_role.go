...
type BatchRoleListResponse struct {
	Data       []*BatchRole `json:"data"`
...
func NewBatchRole(name string) *BatchRole {
	now := time.Now()
	return &BatchRole{
		Name:      name,
		Status:    "pending",
...
func (m *BatchRole) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("batch_role: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("batch_role: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("batch_role: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("batch_role: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("batch_role: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("batch_role: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("batch_role: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("batch_role: version must be at least 1")
	}
	return nil
}

func (m *BatchRole) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("batch_role: marshal failed: %w", err)
	}
	return data, nil
}

func (m *BatchRole) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("batch_role: unmarshal failed: %w", err)
	}
	return nil
}

func (m *BatchRole) Clone() *BatchRole {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *BatchRole) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("batch_role: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("batch_role: transition from %q to %q not allowed", m.Status, status)
}

func (m *BatchRole) Sanitize() {
...
func (m *BatchRole) Merge(other *BatchRole) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *BatchRole) String() string {
	return fmt.Sprintf("BatchRole[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *BatchRole) MatchesFilter(f *BatchRoleFilter) bool {
...
func FilterBatchRoleList(items []*BatchRole, f *BatchRoleFilter) *BatchRoleListResponse {
	var filtered []*BatchRole
...
func CalculateBatchRoleStats(items []*BatchRole) *BatchRoleStats {
	stats := &BatchRoleStats{}
	if len(items) == 0 {
		return stats
	}
