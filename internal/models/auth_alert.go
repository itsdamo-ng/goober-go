...
type AuthAlertListResponse struct {
	Data       []*AuthAlert `json:"data"`
...
func NewAuthAlert(name string) *AuthAlert {
	now := time.Now()
	return &AuthAlert{
		Name:      name,
		Status:    "pending",
...
func (m *AuthAlert) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("auth_alert: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("auth_alert: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("auth_alert: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("auth_alert: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("auth_alert: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("auth_alert: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("auth_alert: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("auth_alert: version must be at least 1")
	}
	return nil
}

func (m *AuthAlert) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("auth_alert: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuthAlert) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("auth_alert: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuthAlert) Clone() *AuthAlert {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuthAlert) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("auth_alert: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("auth_alert: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuthAlert) Sanitize() {
...
func (m *AuthAlert) Merge(other *AuthAlert) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuthAlert) String() string {
	return fmt.Sprintf("AuthAlert[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuthAlert) MatchesFilter(f *AuthAlertFilter) bool {
...
func FilterAuthAlertList(items []*AuthAlert, f *AuthAlertFilter) *AuthAlertListResponse {
	var filtered []*AuthAlert
...
func CalculateAuthAlertStats(items []*AuthAlert) *AuthAlertStats {
	stats := &AuthAlertStats{}
	if len(items) == 0 {
		return stats
	}
