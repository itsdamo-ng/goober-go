...
func NewAdminFeedback(name string) *AdminFeedback {
	now := time.Now()
...
func (m *AdminFeedback) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("admin_feedback: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_feedback: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_feedback: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_feedback: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_feedback: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_feedback: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_feedback: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_feedback: version must be at least 1")
	}
	return nil
}

func (m *AdminFeedback) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_feedback: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminFeedback) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_feedback: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminFeedback) Clone() *AdminFeedback {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminFeedback) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_feedback: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_feedback: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminFeedback) Sanitize() {
...
func (m *AdminFeedback) Merge(other *AdminFeedback) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminFeedback) String() string {
	return fmt.Sprintf("AdminFeedback[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminFeedback) MatchesFilter(f *AdminFeedbackFilter) bool {
...
func FilterAdminFeedbackList(items []*AdminFeedback, f *AdminFeedbackFilter) *AdminFeedbackListResponse {
	var filtered []*AdminFeedback
...
func CalculateAdminFeedbackStats(items []*AdminFeedback) *AdminFeedbackStats {
