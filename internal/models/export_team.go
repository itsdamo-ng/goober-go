...
type ExportTeamListResponse struct {
	Data       []*ExportTeam `json:"data"`
...
func NewExportTeam(name string) *ExportTeam {
	now := time.Now()
	return &ExportTeam{
		Name:      name,
		Status:    "pending",
...
func (m *ExportTeam) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("export_team: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("export_team: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("export_team: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("export_team: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("export_team: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("export_team: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("export_team: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("export_team: version must be at least 1")
	}
	return nil
}

func (m *ExportTeam) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("export_team: marshal failed: %w", err)
	}
	return data, nil
}

func (m *ExportTeam) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("export_team: unmarshal failed: %w", err)
	}
	return nil
}

func (m *ExportTeam) Clone() *ExportTeam {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *ExportTeam) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("export_team: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("export_team: transition from %q to %q not allowed", m.Status, status)
}

func (m *ExportTeam) Sanitize() {
...
func (m *ExportTeam) Merge(other *ExportTeam) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *ExportTeam) String() string {
	return fmt.Sprintf("ExportTeam[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *ExportTeam) MatchesFilter(f *ExportTeamFilter) bool {
...
func FilterExportTeamList(items []*ExportTeam, f *ExportTeamFilter) *ExportTeamListResponse {
	var filtered []*ExportTeam
...
func CalculateExportTeamStats(items []*ExportTeam) *ExportTeamStats {
	stats := &ExportTeamStats{}
	if len(items) == 0 {
		return stats
	}
