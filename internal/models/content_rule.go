...
type ContentRuleListResponse struct {
	Data       []*ContentRule `json:"data"`
...
func NewContentRule(name string) *ContentRule {
	now := time.Now()
	return &ContentRule{
		Name:      name,
		Status:    "pending",
...
func (m *ContentRule) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("content_rule: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("content_rule: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("content_rule: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("content_rule: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("content_rule: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("content_rule: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("content_rule: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("content_rule: version must be at least 1")
	}
	return nil
}

func (m *ContentRule) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("content_rule: marshal failed: %w", err)
	}
	return data, nil
}

func (m *ContentRule) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("content_rule: unmarshal failed: %w", err)
	}
	return nil
}

func (m *ContentRule) Clone() *ContentRule {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *ContentRule) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("content_rule: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("content_rule: transition from %q to %q not allowed", m.Status, status)
}

func (m *ContentRule) Sanitize() {
...
func (m *ContentRule) Merge(other *ContentRule) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *ContentRule) String() string {
	return fmt.Sprintf("ContentRule[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *ContentRule) MatchesFilter(f *ContentRuleFilter) bool {
...
func FilterContentRuleList(items []*ContentRule, f *ContentRuleFilter) *ContentRuleListResponse {
	var filtered []*ContentRule
	...
	return &ContentRuleListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateContentRuleStats(items []*ContentRule) *ContentRuleStats {
	stats := &ContentRuleStats{}
	if len(items) == 0 {
		return stats
	}
