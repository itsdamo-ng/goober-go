...
type AdminTemplateFilter struct {
	IDs       []int64   `json:"ids"`
...
type AdminTemplateStats struct {
...
func NewAdminTemplate(name string) *AdminTemplate {
	now := time.Now()
...
func (m *AdminTemplate) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("admin_template: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_template: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_template: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_template: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_template: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_template: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_template: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_template: version must be at least 1")
	}
	return nil
}

func (m *AdminTemplate) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_template: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminTemplate) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_template: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminTemplate) Clone() *AdminTemplate {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminTemplate) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_template: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_template: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminTemplate) Sanitize() {
...
func (m *AdminTemplate) Merge(other *AdminTemplate) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminTemplate) String() string {
	return fmt.Sprintf("AdminTemplate[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminTemplate) MatchesFilter(f *AdminTemplateFilter) bool {
...
func FilterAdminTemplateList(items []*AdminTemplate, f *AdminTemplateFilter) *AdminTemplateListResponse {
	var filtered []*AdminTemplate
...
func CalculateAdminTemplateStats(items []*AdminTemplate) *AdminTemplateStats {
