...
func NewAdminPayment(name string) *AdminPayment {
	now := time.Now()
	return &AdminPayment{
		Name:      name,
		Status:    "pending",
...
func (m *AdminPayment) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("admin_payment: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_payment: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_payment: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_payment: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_payment: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_payment: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_payment: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_payment: version must be at least 1")
	}
	return nil
}

func (m *AdminPayment) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_payment: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminPayment) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_payment: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminPayment) Clone() *AdminPayment {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminPayment) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_payment: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_payment: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminPayment) Sanitize() {
...
func (m *AdminPayment) Merge(other *AdminPayment) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminPayment) String() string {
	return fmt.Sprintf("AdminPayment[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminPayment) MatchesFilter(f *AdminPaymentFilter) bool {
...
func FilterAdminPaymentList(items []*AdminPayment, f *AdminPaymentFilter) *AdminPaymentListResponse {
	var filtered []*AdminPayment
...
func CalculateAdminPaymentStats(items []*AdminPayment) *AdminPaymentStats {
