...
type BatchShipmentFilter struct {
	IDs       []int64   `json:"ids"`
...
type BatchShipmentListResponse struct {
	Data       []*BatchShipment `json:"data"`
...
type BatchShipmentStats struct {
...
func NewBatchShipment(name string) *BatchShipment {
	now := time.Now()
...
func (m *BatchShipment) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("batch_shipment: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("batch_shipment: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("batch_shipment: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("batch_shipment: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("batch_shipment: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("batch_shipment: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("batch_shipment: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("batch_shipment: version must be at least 1")
	}
	return nil
}

func (m *BatchShipment) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("batch_shipment: marshal failed: %w", err)
	}
	return data, nil
}

func (m *BatchShipment) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("batch_shipment: unmarshal failed: %w", err)
	}
	return nil
}

func (m *BatchShipment) Clone() *BatchShipment {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *BatchShipment) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("batch_shipment: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("batch_shipment: transition from %q to %q not allowed", m.Status, status)
}

func (m *BatchShipment) Sanitize() {
...
func (m *BatchShipment) Merge(other *BatchShipment) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *BatchShipment) String() string {
	return fmt.Sprintf("BatchShipment[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *BatchShipment) MatchesFilter(f *BatchShipmentFilter) bool {
...
func FilterBatchShipmentList(items []*BatchShipment, f *BatchShipmentFilter) *BatchShipmentListResponse {
	var filtered []*BatchShipment
...
func CalculateBatchShipmentStats(items []*BatchShipment) *BatchShipmentStats {
	stats := &BatchShipmentStats{}
	if len(items) == 0 {
		return stats
	}
