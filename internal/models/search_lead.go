...
type SearchLeadListResponse struct {
	Data       []*SearchLead `json:"data"`
...
func NewSearchLead(name string) *SearchLead {
	now := time.Now()
	return &SearchLead{
		Name:      name,
		Status:    "pending",
...
func (m *SearchLead) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("search_lead: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("search_lead: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("search_lead: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("search_lead: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("search_lead: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("search_lead: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("search_lead: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("search_lead: version must be at least 1")
	}
	return nil
}

func (m *SearchLead) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("search_lead: marshal failed: %w", err)
	}
	return data, nil
}

func (m *SearchLead) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("search_lead: unmarshal failed: %w", err)
	}
	return nil
}

func (m *SearchLead) Clone() *SearchLead {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *SearchLead) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("search_lead: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("search_lead: transition from %q to %q not allowed", m.Status, status)
}

func (m *SearchLead) Sanitize() {
...
func (m *SearchLead) Merge(other *SearchLead) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *SearchLead) String() string {
	return fmt.Sprintf("SearchLead[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *SearchLead) MatchesFilter(f *SearchLeadFilter) bool {
...
func FilterSearchLeadList(items []*SearchLead, f *SearchLeadFilter) *SearchLeadListResponse {
	var filtered []*SearchLead
...
func CalculateSearchLeadStats(items []*SearchLead) *SearchLeadStats {
	stats := &SearchLeadStats{}
	if len(items) == 0 {
		return stats
	}
