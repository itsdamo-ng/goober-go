...
func NewBillingAccount(name string) *BillingAccount {
	now := time.Now()
...
func (m *BillingAccount) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("billing_account: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("billing_account: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("billing_account: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("billing_account: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("billing_account: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("billing_account: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("billing_account: version must be at least 1")
	}
	return nil
}

func (m *BillingAccount) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("billing_account: marshal failed: %w", err)
	}
	return data, nil
}

func (m *BillingAccount) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("billing_account: unmarshal failed: %w", err)
	}
	return nil
}

func (m *BillingAccount) Clone() *BillingAccount {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *BillingAccount) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("billing_account: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("billing_account: transition from %q to %q not allowed", m.Status, status)
}

func (m *BillingAccount) Sanitize() {
...
func (m *BillingAccount) Merge(other *BillingAccount) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *BillingAccount) String() string {
	return fmt.Sprintf("BillingAccount[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *BillingAccount) MatchesFilter(f *BillingAccountFilter) bool {
...
func FilterBillingAccountList(items []*BillingAccount, f *BillingAccountFilter) *BillingAccountListResponse {
	var filtered []*BillingAccount
...
func CalculateBillingAccountStats(items []*BillingAccount) *BillingAccountStats {
	stats := &BillingAccountStats{}
	if len(items) == 0 {
		return stats
	}
