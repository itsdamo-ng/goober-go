...
func (m *Document) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("document: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("document: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("document: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("document: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("document: priority must be between 0 and 10")
	}
...
func (m *Document) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("document: marshal failed: %w", err)
	}
	return data, nil
}

func (m *Document) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("document: unmarshal failed: %w", err)
	}
	return nil
}

func (m *Document) Clone() *Document {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *Document) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("document: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("document: transition from %q to %q not allowed", m.Status, status)
}

func (m *Document) Sanitize() {
...
func (m *Document) Merge(other *Document) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Document) String() string {
	return fmt.Sprintf("Document[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Document) MatchesFilter(f *DocumentFilter) bool {
...
func FilterDocumentList(items []*Document, f *DocumentFilter) *DocumentListResponse {
...
func CalculateDocumentStats(items []*Document) *DocumentStats {
	stats := &DocumentStats{}
	if len(items) == 0 {
		return stats
	}
