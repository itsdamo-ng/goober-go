...
func NewAdminMessage(name string) *AdminMessage {
	now := time.Now()
	return &AdminMessage{
		Name:      name,
		Status:    "pending",
...
func (m *AdminMessage) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("admin_message: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_message: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_message: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_message: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_message: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_message: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_message: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_message: version must be at least 1")
	}
	return nil
}

func (m *AdminMessage) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_message: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminMessage) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_message: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminMessage) Clone() *AdminMessage {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminMessage) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_message: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_message: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminMessage) Sanitize() {
...
func (m *AdminMessage) Merge(other *AdminMessage) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminMessage) String() string {
	return fmt.Sprintf("AdminMessage[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminMessage) MatchesFilter(f *AdminMessageFilter) bool {
...
func FilterAdminMessageList(items []*AdminMessage, f *AdminMessageFilter) *AdminMessageListResponse {
	var filtered []*AdminMessage
...
func CalculateAdminMessageStats(items []*AdminMessage) *AdminMessageStats {
