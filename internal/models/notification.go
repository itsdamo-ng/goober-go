...
func (m *Notification) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("notification: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("notification: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("notification: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("notification: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("notification: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("notification: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("notification: version must be at least 1")
	}
	return nil
}
...
func (m *Notification) ToJSON() ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("notification: marshal failed: %w", err)
	}
	return data, nil
}

func (m *Notification) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("notification: unmarshal failed: %w", err)
	}
	return nil
}
...
func (m *Notification) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("notification: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("notification: transition from %q to %q not allowed", m.Status, status)
}

func (m *Notification) Sanitize() {
...
func (m *Notification) Merge(other *Notification) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Notification) String() string {
	return fmt.Sprintf("Notification[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Notification) MatchesFilter(f *NotificationFilter) bool {
...
func FilterNotificationList(items []*Notification, f *NotificationFilter) *NotificationListResponse {
...
func CalculateNotificationStats(items []*Notification) *NotificationStats {
