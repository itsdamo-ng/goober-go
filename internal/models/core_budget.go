...
type CoreBudgetListResponse struct {
	Data       []*CoreBudget `json:"data"`
...
func NewCoreBudget(name string) *CoreBudget {
	now := time.Now()
	return &CoreBudget{
		Name:      name,
		Status:    "pending",
...
func (m *CoreBudget) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("core_budget: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("core_budget: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("core_budget: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("core_budget: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("core_budget: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("core_budget: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("core_budget: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("core_budget: version must be at least 1")
	}
	return nil
}

func (m *CoreBudget) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("core_budget: marshal failed: %w", err)
	}
	return data, nil
}

func (m *CoreBudget) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("core_budget: unmarshal failed: %w", err)
	}
	return nil
}

func (m *CoreBudget) Clone() *CoreBudget {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *CoreBudget) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("core_budget: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("core_budget: transition from %q to %q not allowed", m.Status, status)
}

func (m *CoreBudget) Sanitize() {
...
func (m *CoreBudget) Merge(other *CoreBudget) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *CoreBudget) String() string {
	return fmt.Sprintf("CoreBudget[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *CoreBudget) MatchesFilter(f *CoreBudgetFilter) bool {
...
func FilterCoreBudgetList(items []*CoreBudget, f *CoreBudgetFilter) *CoreBudgetListResponse {
	var filtered []*CoreBudget
...
func CalculateCoreBudgetStats(items []*CoreBudget) *CoreBudgetStats {
	stats := &CoreBudgetStats{}
	if len(items) == 0 {
		return stats
	}
