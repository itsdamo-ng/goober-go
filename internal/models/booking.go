...
func NewBooking(name string) *Booking {
	now := time.Now()
	return &Booking{
		Name:      name,
		Status:    "pending",
...
func (m *Booking) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("booking: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("booking: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("booking: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("booking: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("booking: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("booking: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("booking: version must be at least 1")
	}
	return nil
}

func (m *Booking) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("booking: marshal failed: %w", err)
	}
	return data, nil
}

func (m *Booking) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("booking: unmarshal failed: %w", err)
	}
	return nil
}

func (m *Booking) Clone() *Booking {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *Booking) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("booking: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("booking: transition from %q to %q not allowed", m.Status, status)
}

func (m *Booking) Sanitize() {
...
func (m *Booking) Merge(other *Booking) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Booking) String() string {
	return fmt.Sprintf("Booking[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Booking) MatchesFilter(f *BookingFilter) bool {
...
func FilterBookingList(items []*Booking, f *BookingFilter) *BookingListResponse {
...
func CalculateBookingStats(items []*Booking) *BookingStats {
	stats := &BookingStats{}
	if len(items) == 0 {
		return stats
	}
