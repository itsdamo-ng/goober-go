...
type AdminRuleListResponse struct {
...
func NewAdminRule(name string) *AdminRule {
	now := time.Now()
	return &AdminRule{
		Name:      name,
		Status:    "pending",
...
func (m *AdminRule) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("admin_rule: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_rule: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_rule: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_rule: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_rule: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_rule: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_rule: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_rule: version must be at least 1")
	}
	return nil
}

func (m *AdminRule) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_rule: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminRule) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_rule: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminRule) Clone() *AdminRule {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminRule) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_rule: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_rule: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminRule) Sanitize() {
...
func (m *AdminRule) Merge(other *AdminRule) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminRule) String() string {
	return fmt.Sprintf("AdminRule[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminRule) MatchesFilter(f *AdminRuleFilter) bool {
...
func FilterAdminRuleList(items []*AdminRule, f *AdminRuleFilter) *AdminRuleListResponse {
	var filtered []*AdminRule
...
func CalculateAdminRuleStats(items []*AdminRule) *AdminRuleStats {
	stats := &AdminRuleStats{}
	if len(items) == 0 {
		return stats
	}
