...
type ExportEventListResponse struct {
	Data       []*ExportEvent `json:"data"`
...
func NewExportEvent(name string) *ExportEvent {
	now := time.Now()
	return &ExportEvent{
		Name:      name,
		Status:    "pending",
...
func (m *ExportEvent) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("export_event: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("export_event: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("export_event: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("export_event: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("export_event: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("export_event: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("export_event: version must be at least 1")
	}
	return nil
}

func (m *ExportEvent) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("export_event: marshal failed: %w", err)
	}
	return data, nil
}

func (m *ExportEvent) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("export_event: unmarshal failed: %w", err)
	}
	return nil
}

func (m *ExportEvent) Clone() *ExportEvent {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *ExportEvent) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("export_event: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("export_event: transition from %q to %q not allowed", m.Status, status)
}

func (m *ExportEvent) Sanitize() {
...
func (m *ExportEvent) Merge(other *ExportEvent) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *ExportEvent) String() string {
	return fmt.Sprintf("ExportEvent[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *ExportEvent) MatchesFilter(f *ExportEventFilter) bool {
...
func FilterExportEventList(items []*ExportEvent, f *ExportEventFilter) *ExportEventListResponse {
	var filtered []*ExportEvent
	...
	return &ExportEventListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateExportEventStats(items []*ExportEvent) *ExportEventStats {
	stats := &ExportEventStats{}
	if len(items) == 0 {
		return stats
	}
