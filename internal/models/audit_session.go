...
type AuditSessionFilter struct {
	IDs       []int64   `json:"ids"`
...
type AuditSessionListResponse struct {
	Data       []*AuditSession `json:"data"`
...
func NewAuditSession(name string) *AuditSession {
	now := time.Now()
	return &AuditSession{
		Name:      name,
		Status:    "pending",
...
func (m *AuditSession) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("audit_session: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("audit_session: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("audit_session: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("audit_session: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("audit_session: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("audit_session: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("audit_session: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("audit_session: version must be at least 1")
	}
	return nil
}

func (m *AuditSession) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("audit_session: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuditSession) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("audit_session: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuditSession) Clone() *AuditSession {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuditSession) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("audit_session: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("audit_session: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuditSession) Sanitize() {
...
func (m *AuditSession) Merge(other *AuditSession) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuditSession) String() string {
	return fmt.Sprintf("AuditSession[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuditSession) MatchesFilter(f *AuditSessionFilter) bool {
...
func FilterAuditSessionList(items []*AuditSession, f *AuditSessionFilter) *AuditSessionListResponse {
	var filtered []*AuditSession
...
func CalculateAuditSessionStats(items []*AuditSession) *AuditSessionStats {
