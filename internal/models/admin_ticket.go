...
func NewAdminTicket(name string) *AdminTicket {
	now := time.Now()
	return &AdminTicket{
		Name:      name,
		Status:    "pending",
...
func (m *AdminTicket) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("admin_ticket: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_ticket: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_ticket: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_ticket: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_ticket: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_ticket: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_ticket: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_ticket: version must be at least 1")
	}
	return nil
}

func (m *AdminTicket) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_ticket: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminTicket) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_ticket: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminTicket) Clone() *AdminTicket {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminTicket) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_ticket: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_ticket: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminTicket) Sanitize() {
...
func (m *AdminTicket) Merge(other *AdminTicket) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminTicket) String() string {
	return fmt.Sprintf("AdminTicket[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminTicket) MatchesFilter(f *AdminTicketFilter) bool {
...
func FilterAdminTicketList(items []*AdminTicket, f *AdminTicketFilter) *AdminTicketListResponse {
	var filtered []*AdminTicket
...
func CalculateAdminTicketStats(items []*AdminTicket) *AdminTicketStats {
