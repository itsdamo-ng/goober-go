...
func (m *Role) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("role: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("role: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("role: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("role: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("role: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("role: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("role: version must be at least 1")
	}
	return nil
}

func (m *Role) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("role: marshal failed: %w", err)
	}
	return data, nil
}

func (m *Role) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("role: unmarshal failed: %w", err)
	}
	return nil
}
...
func (m *Role) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("role: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("role: transition from %q to %q not allowed", m.Status, status)
}

func (m *Role) Sanitize() {
...
func (m *Role) String() string {
	return fmt.Sprintf("Role[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Role) MatchesFilter(f *RoleFilter) bool {
...
func FilterRoleList(items []*Role, f *RoleFilter) *RoleListResponse {
	var filtered []*Role
...
func CalculateRoleStats(items []*Role) *RoleStats {
	stats := &RoleStats{}
	if len(items) == 0 {
		return stats
	}
