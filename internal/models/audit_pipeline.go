...
type AuditPipelineFilter struct {
	IDs       []int64   `json:"ids"`
...
type AuditPipelineListResponse struct {
	Data       []*AuditPipeline `json:"data"`
...
func NewAuditPipeline(name string) *AuditPipeline {
	now := time.Now()
...
func (m *AuditPipeline) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("audit_pipeline: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("audit_pipeline: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("audit_pipeline: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("audit_pipeline: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("audit_pipeline: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("audit_pipeline: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("audit_pipeline: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("audit_pipeline: version must be at least 1")
	}
	return nil
}

func (m *AuditPipeline) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("audit_pipeline: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuditPipeline) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("audit_pipeline: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuditPipeline) Clone() *AuditPipeline {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuditPipeline) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("audit_pipeline: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("audit_pipeline: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuditPipeline) Sanitize() {
...
func (m *AuditPipeline) Merge(other *AuditPipeline) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuditPipeline) String() string {
	return fmt.Sprintf("AuditPipeline[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuditPipeline) MatchesFilter(f *AuditPipelineFilter) bool {
...
func FilterAuditPipelineList(items []*AuditPipeline, f *AuditPipelineFilter) *AuditPipelineListResponse {
	var filtered []*AuditPipeline
	...
	return &AuditPipelineListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateAuditPipelineStats(items []*AuditPipeline) *AuditPipelineStats {
