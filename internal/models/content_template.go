...
type ContentTemplateListResponse struct {
	Data       []*ContentTemplate `json:"data"`
...
type ContentTemplateStats struct {
...
func NewContentTemplate(name string) *ContentTemplate {
	now := time.Now()
...
func (m *ContentTemplate) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("content_template: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("content_template: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("content_template: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("content_template: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("content_template: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("content_template: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("content_template: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("content_template: version must be at least 1")
	}
	return nil
}

func (m *ContentTemplate) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("content_template: marshal failed: %w", err)
	}
	return data, nil
}

func (m *ContentTemplate) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("content_template: unmarshal failed: %w", err)
	}
	return nil
}

func (m *ContentTemplate) Clone() *ContentTemplate {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *ContentTemplate) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("content_template: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("content_template: transition from %q to %q not allowed", m.Status, status)
}

func (m *ContentTemplate) Sanitize() {
...
func (m *ContentTemplate) Merge(other *ContentTemplate) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *ContentTemplate) String() string {
	return fmt.Sprintf("ContentTemplate[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *ContentTemplate) MatchesFilter(f *ContentTemplateFilter) bool {
...
func FilterContentTemplateList(items []*ContentTemplate, f *ContentTemplateFilter) *ContentTemplateListResponse {
	var filtered []*ContentTemplate
...
func CalculateContentTemplateStats(items []*ContentTemplate) *ContentTemplateStats {
	stats := &ContentTemplateStats{}
	if len(items) == 0 {
		return stats
	}
