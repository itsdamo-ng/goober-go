...
func (m *Department) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("department: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("department: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("department: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("department: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("department: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("department: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("department: version must be at least 1")
	}
	return nil
}

func (m *Department) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("department: marshal failed: %w", err)
	}
	return data, nil
}

func (m *Department) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("department: unmarshal failed: %w", err)
	}
	return nil
}

func (m *Department) Clone() *Department {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *Department) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("department: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("department: transition from %q to %q not allowed", m.Status, status)
}

func (m *Department) Sanitize() {
...
func (m *Department) Merge(other *Department) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Department) String() string {
	return fmt.Sprintf("Department[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Department) MatchesFilter(f *DepartmentFilter) bool {
...
func FilterDepartmentList(items []*Department, f *DepartmentFilter) *DepartmentListResponse {
...
func CalculateDepartmentStats(items []*Department) *DepartmentStats {
