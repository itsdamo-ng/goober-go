...
type MarketingPaymentFilter struct {
	IDs       []int64   `json:"ids"`
...
type MarketingPaymentListResponse struct {
	Data       []*MarketingPayment `json:"data"`
...
type MarketingPaymentStats struct {
...
func NewMarketingPayment(name string) *MarketingPayment {
	now := time.Now()
...
func (m *MarketingPayment) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("marketing_payment: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("marketing_payment: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("marketing_payment: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("marketing_payment: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("marketing_payment: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("marketing_payment: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("marketing_payment: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("marketing_payment: version must be at least 1")
	}
	return nil
}

func (m *MarketingPayment) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("marketing_payment: marshal failed: %w", err)
	}
	return data, nil
}

func (m *MarketingPayment) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("marketing_payment: unmarshal failed: %w", err)
	}
	return nil
}

func (m *MarketingPayment) Clone() *MarketingPayment {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *MarketingPayment) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("marketing_payment: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("marketing_payment: transition from %q to %q not allowed", m.Status, status)
}

func (m *MarketingPayment) Sanitize() {
...
func (m *MarketingPayment) Merge(other *MarketingPayment) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *MarketingPayment) String() string {
	return fmt.Sprintf("MarketingPayment[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *MarketingPayment) MatchesFilter(f *MarketingPaymentFilter) bool {
...
func FilterMarketingPaymentList(items []*MarketingPayment, f *MarketingPaymentFilter) *MarketingPaymentListResponse {
	var filtered []*MarketingPayment
	...
	return &MarketingPaymentListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateMarketingPaymentStats(items []*MarketingPayment) *MarketingPaymentStats {
	stats := &MarketingPaymentStats{}
	if len(items) == 0 {
		return stats
	}
