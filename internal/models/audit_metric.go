...
type AuditMetricListResponse struct {
	Data       []*AuditMetric `json:"data"`
...
func NewAuditMetric(name string) *AuditMetric {
	now := time.Now()
	return &AuditMetric{
		Name:      name,
		Status:    "pending",
...
func (m *AuditMetric) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("audit_metric: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("audit_metric: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("audit_metric: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("audit_metric: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("audit_metric: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("audit_metric: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("audit_metric: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("audit_metric: version must be at least 1")
	}
	return nil
}

func (m *AuditMetric) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("audit_metric: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuditMetric) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("audit_metric: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuditMetric) Clone() *AuditMetric {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuditMetric) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("audit_metric: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("audit_metric: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuditMetric) Sanitize() {
...
func (m *AuditMetric) Merge(other *AuditMetric) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuditMetric) String() string {
	return fmt.Sprintf("AuditMetric[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuditMetric) MatchesFilter(f *AuditMetricFilter) bool {
...
func FilterAuditMetricList(items []*AuditMetric, f *AuditMetricFilter) *AuditMetricListResponse {
	var filtered []*AuditMetric
	...
	return &AuditMetricListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateAuditMetricStats(items []*AuditMetric) *AuditMetricStats {
	stats := &AuditMetricStats{}
	if len(items) == 0 {
		return stats
	}
