...
func (m *Team) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("team: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("team: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("team: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("team: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("team: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("team: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("team: version must be at least 1")
	}
	return nil
}

func (m *Team) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("team: marshal failed: %w", err)
	}
	return data, nil
}

func (m *Team) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("team: unmarshal failed: %w", err)
	}
	return nil
}
...
func (m *Team) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("team: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("team: transition from %q to %q not allowed", m.Status, status)
}

func (m *Team) Sanitize() {
...
func (m *Team) String() string {
	return fmt.Sprintf("Team[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Team) MatchesFilter(f *TeamFilter) bool {
...
func FilterTeamList(items []*Team, f *TeamFilter) *TeamListResponse {
	var filtered []*Team
...
func CalculateTeamStats(items []*Team) *TeamStats {
	stats := &TeamStats{}
	if len(items) == 0 {
		return stats
	}
