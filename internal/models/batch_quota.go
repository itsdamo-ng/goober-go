...
type BatchQuotaListResponse struct {
	Data       []*BatchQuota `json:"data"`
...
func NewBatchQuota(name string) *BatchQuota {
	now := time.Now()
	return &BatchQuota{
		Name:      name,
		Status:    "pending",
...
func (m *BatchQuota) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("batch_quota: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("batch_quota: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("batch_quota: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("batch_quota: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("batch_quota: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("batch_quota: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("batch_quota: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("batch_quota: version must be at least 1")
	}
	return nil
}

func (m *BatchQuota) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("batch_quota: marshal failed: %w", err)
	}
	return data, nil
}

func (m *BatchQuota) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("batch_quota: unmarshal failed: %w", err)
	}
	return nil
}

func (m *BatchQuota) Clone() *BatchQuota {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *BatchQuota) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("batch_quota: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("batch_quota: transition from %q to %q not allowed", m.Status, status)
}

func (m *BatchQuota) Sanitize() {
...
func (m *BatchQuota) Merge(other *BatchQuota) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *BatchQuota) String() string {
	return fmt.Sprintf("BatchQuota[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *BatchQuota) MatchesFilter(f *BatchQuotaFilter) bool {
...
func FilterBatchQuotaList(items []*BatchQuota, f *BatchQuotaFilter) *BatchQuotaListResponse {
	var filtered []*BatchQuota
...
func CalculateBatchQuotaStats(items []*BatchQuota) *BatchQuotaStats {
	stats := &BatchQuotaStats{}
	if len(items) == 0 {
		return stats
	}
