...
type ShippingTaskFilter struct {
	IDs       []int64   `json:"ids"`
...
type ShippingTaskListResponse struct {
	Data       []*ShippingTask `json:"data"`
...
func NewShippingTask(name string) *ShippingTask {
	now := time.Now()
	return &ShippingTask{
		Name:      name,
		Status:    "pending",
...
func (m *ShippingTask) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("shipping_task: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("shipping_task: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("shipping_task: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("shipping_task: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("shipping_task: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("shipping_task: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("shipping_task: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("shipping_task: version must be at least 1")
	}
	return nil
}

func (m *ShippingTask) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("shipping_task: marshal failed: %w", err)
	}
	return data, nil
}

func (m *ShippingTask) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("shipping_task: unmarshal failed: %w", err)
	}
	return nil
}

func (m *ShippingTask) Clone() *ShippingTask {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *ShippingTask) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("shipping_task: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("shipping_task: transition from %q to %q not allowed", m.Status, status)
}

func (m *ShippingTask) Sanitize() {
...
func (m *ShippingTask) Merge(other *ShippingTask) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *ShippingTask) String() string {
	return fmt.Sprintf("ShippingTask[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *ShippingTask) MatchesFilter(f *ShippingTaskFilter) bool {
...
func FilterShippingTaskList(items []*ShippingTask, f *ShippingTaskFilter) *ShippingTaskListResponse {
	var filtered []*ShippingTask
	...
	return &ShippingTaskListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateShippingTaskStats(items []*ShippingTask) *ShippingTaskStats {
