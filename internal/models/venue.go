...
func (m *Venue) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("venue: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("venue: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("venue: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("venue: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("venue: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("venue: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("venue: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("venue: version must be at least 1")
	}
	return nil
}

func (m *Venue) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("venue: marshal failed: %w", err)
	}
	return data, nil
}

func (m *Venue) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("venue: unmarshal failed: %w", err)
	}
	return nil
}

func (m *Venue) Clone() *Venue {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *Venue) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("venue: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("venue: transition from %q to %q not allowed", m.Status, status)
}

func (m *Venue) Sanitize() {
...
func (m *Venue) Merge(other *Venue) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Venue) String() string {
	return fmt.Sprintf("Venue[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Venue) MatchesFilter(f *VenueFilter) bool {
...
func FilterVenueList(items []*Venue, f *VenueFilter) *VenueListResponse {
	var filtered []*Venue
...
func CalculateVenueStats(items []*Venue) *VenueStats {
	stats := &VenueStats{}
	if len(items) == 0 {
		return stats
	}
