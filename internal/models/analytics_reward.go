...
type AnalyticsRewardFilter struct {
	IDs       []int64   `json:"ids"`
...
type AnalyticsRewardListResponse struct {
	Data       []*AnalyticsReward `json:"data"`
...
type AnalyticsRewardStats struct {
...
func NewAnalyticsReward(name string) *AnalyticsReward {
	now := time.Now()
	return &AnalyticsReward{
		Name:      name,
		Status:    "pending",
		IsActive:  true,
		Version:   1,
...
func (m *AnalyticsReward) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("analytics_reward: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("analytics_reward: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("analytics_reward: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("analytics_reward: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("analytics_reward: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("analytics_reward: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("analytics_reward: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("analytics_reward: version must be at least 1")
	}
	return nil
}

func (m *AnalyticsReward) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("analytics_reward: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AnalyticsReward) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("analytics_reward: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AnalyticsReward) Clone() *AnalyticsReward {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AnalyticsReward) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("analytics_reward: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("analytics_reward: transition from %q to %q not allowed", m.Status, status)
}

func (m *AnalyticsReward) Sanitize() {
...
func (m *AnalyticsReward) Merge(other *AnalyticsReward) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AnalyticsReward) String() string {
	return fmt.Sprintf("AnalyticsReward[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AnalyticsReward) MatchesFilter(f *AnalyticsRewardFilter) bool {
...
func FilterAnalyticsRewardList(items []*AnalyticsReward, f *AnalyticsRewardFilter) *AnalyticsRewardListResponse {
	var filtered []*AnalyticsReward
	...
	return &AnalyticsRewardListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateAnalyticsRewardStats(items []*AnalyticsReward) *AnalyticsRewardStats {
	stats := &AnalyticsRewardStats{}
	if len(items) == 0 {
		return stats
	}
