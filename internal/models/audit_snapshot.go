...
type AuditSnapshotListResponse struct {
	Data       []*AuditSnapshot `json:"data"`
...
func NewAuditSnapshot(name string) *AuditSnapshot {
	now := time.Now()
...
func (m *AuditSnapshot) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("audit_snapshot: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("audit_snapshot: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("audit_snapshot: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("audit_snapshot: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("audit_snapshot: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("audit_snapshot: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("audit_snapshot: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("audit_snapshot: version must be at least 1")
	}
	return nil
}

func (m *AuditSnapshot) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("audit_snapshot: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuditSnapshot) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("audit_snapshot: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuditSnapshot) Clone() *AuditSnapshot {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuditSnapshot) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("audit_snapshot: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("audit_snapshot: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuditSnapshot) Sanitize() {
...
func (m *AuditSnapshot) Merge(other *AuditSnapshot) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuditSnapshot) String() string {
	return fmt.Sprintf("AuditSnapshot[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuditSnapshot) MatchesFilter(f *AuditSnapshotFilter) bool {
...
func FilterAuditSnapshotList(items []*AuditSnapshot, f *AuditSnapshotFilter) *AuditSnapshotListResponse {
	var filtered []*AuditSnapshot
	...
	return &AuditSnapshotListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateAuditSnapshotStats(items []*AuditSnapshot) *AuditSnapshotStats {
