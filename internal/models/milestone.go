...
func (m *Milestone) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("milestone: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("milestone: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("milestone: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("milestone: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("milestone: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("milestone: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("milestone: version must be at least 1")
	}
	return nil
}
...
func (m *Milestone) ToJSON() ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("milestone: marshal failed: %w", err)
	}
	return data, nil
}

func (m *Milestone) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("milestone: unmarshal failed: %w", err)
	}
	return nil
}
...
func (m *Milestone) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("milestone: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("milestone: transition from %q to %q not allowed", m.Status, status)
}

func (m *Milestone) Sanitize() {
...
func (m *Milestone) Merge(other *Milestone) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Milestone) String() string {
	return fmt.Sprintf("Milestone[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Milestone) MatchesFilter(f *MilestoneFilter) bool {
...
func FilterMilestoneList(items []*Milestone, f *MilestoneFilter) *MilestoneListResponse {
...
func CalculateMilestoneStats(items []*Milestone) *MilestoneStats {
