...
type SupportUserListResponse struct {
	Data       []*SupportUser `json:"data"`
...
func NewSupportUser(name string) *SupportUser {
	now := time.Now()
	return &SupportUser{
		Name:      name,
		Status:    "pending",
...
func (m *SupportUser) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("support_user: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("support_user: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("support_user: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("support_user: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("support_user: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("support_user: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("support_user: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("support_user: version must be at least 1")
	}
	return nil
}

func (m *SupportUser) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("support_user: marshal failed: %w", err)
	}
	return data, nil
}

func (m *SupportUser) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("support_user: unmarshal failed: %w", err)
	}
	return nil
}

func (m *SupportUser) Clone() *SupportUser {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *SupportUser) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("support_user: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("support_user: transition from %q to %q not allowed", m.Status, status)
}

func (m *SupportUser) Sanitize() {
...
func (m *SupportUser) Merge(other *SupportUser) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *SupportUser) String() string {
	return fmt.Sprintf("SupportUser[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *SupportUser) MatchesFilter(f *SupportUserFilter) bool {
...
func FilterSupportUserList(items []*SupportUser, f *SupportUserFilter) *SupportUserListResponse {
	var filtered []*SupportUser
	...
	return &SupportUserListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateSupportUserStats(items []*SupportUser) *SupportUserStats {
