...
type ContentReviewFilter struct {
	IDs       []int64   `json:"ids"`
...
type ContentReviewListResponse struct {
...
type ContentReviewStats struct {
...
func NewContentReview(name string) *ContentReview {
	now := time.Now()
...
func (m *ContentReview) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("content_review: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("content_review: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("content_review: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("content_review: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("content_review: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("content_review: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("content_review: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("content_review: version must be at least 1")
	}
	return nil
}

func (m *ContentReview) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("content_review: marshal failed: %w", err)
	}
	return data, nil
}

func (m *ContentReview) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("content_review: unmarshal failed: %w", err)
	}
	return nil
}

func (m *ContentReview) Clone() *ContentReview {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *ContentReview) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("content_review: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("content_review: transition from %q to %q not allowed", m.Status, status)
}

func (m *ContentReview) Sanitize() {
...
func (m *ContentReview) Merge(other *ContentReview) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *ContentReview) String() string {
	return fmt.Sprintf("ContentReview[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *ContentReview) MatchesFilter(f *ContentReviewFilter) bool {
...
func FilterContentReviewList(items []*ContentReview, f *ContentReviewFilter) *ContentReviewListResponse {
	var filtered []*ContentReview
...
func CalculateContentReviewStats(items []*ContentReview) *ContentReviewStats {
	stats := &ContentReviewStats{}
	if len(items) == 0 {
		return stats
	}
