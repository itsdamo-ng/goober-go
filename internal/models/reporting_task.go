...
type ReportingTaskFilter struct {
	IDs       []int64   `json:"ids"`
...
type ReportingTaskListResponse struct {
	Data       []*ReportingTask `json:"data"`
...
type ReportingTaskStats struct {
...
func NewReportingTask(name string) *ReportingTask {
	now := time.Now()
...
func (m *ReportingTask) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("reporting_task: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("reporting_task: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("reporting_task: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("reporting_task: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("reporting_task: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("reporting_task: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("reporting_task: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("reporting_task: version must be at least 1")
	}
	return nil
}

func (m *ReportingTask) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("reporting_task: marshal failed: %w", err)
	}
	return data, nil
}

func (m *ReportingTask) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("reporting_task: unmarshal failed: %w", err)
	}
	return nil
}

func (m *ReportingTask) Clone() *ReportingTask {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *ReportingTask) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("reporting_task: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("reporting_task: transition from %q to %q not allowed", m.Status, status)
}

func (m *ReportingTask) Sanitize() {
...
func (m *ReportingTask) Merge(other *ReportingTask) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *ReportingTask) String() string {
	return fmt.Sprintf("ReportingTask[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *ReportingTask) MatchesFilter(f *ReportingTaskFilter) bool {
...
func FilterReportingTaskList(items []*ReportingTask, f *ReportingTaskFilter) *ReportingTaskListResponse {
	var filtered []*ReportingTask
...
func CalculateReportingTaskStats(items []*ReportingTask) *ReportingTaskStats {
	stats := &ReportingTaskStats{}
	if len(items) == 0 {
		return stats
	}
