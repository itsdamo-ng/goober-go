...
type ContentTaskListResponse struct {
...
func NewContentTask(name string) *ContentTask {
	now := time.Now()
	return &ContentTask{
		Name:      name,
		Status:    "pending",
...
func (m *ContentTask) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("content_task: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("content_task: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("content_task: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("content_task: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("content_task: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("content_task: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("content_task: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("content_task: version must be at least 1")
	}
	return nil
}

func (m *ContentTask) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("content_task: marshal failed: %w", err)
	}
	return data, nil
}

func (m *ContentTask) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("content_task: unmarshal failed: %w", err)
	}
	return nil
}

func (m *ContentTask) Clone() *ContentTask {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *ContentTask) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("content_task: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("content_task: transition from %q to %q not allowed", m.Status, status)
}

func (m *ContentTask) Sanitize() {
...
func (m *ContentTask) Merge(other *ContentTask) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *ContentTask) String() string {
	return fmt.Sprintf("ContentTask[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *ContentTask) MatchesFilter(f *ContentTaskFilter) bool {
...
func FilterContentTaskList(items []*ContentTask, f *ContentTaskFilter) *ContentTaskListResponse {
	var filtered []*ContentTask
	...
	return &ContentTaskListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateContentTaskStats(items []*ContentTask) *ContentTaskStats {
	stats := &ContentTaskStats{}
	if len(items) == 0 {
		return stats
	}
