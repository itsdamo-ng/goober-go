...
type RealtimeEventListResponse struct {
	Data       []*RealtimeEvent `json:"data"`
...
type RealtimeEventStats struct {
...
func NewRealtimeEvent(name string) *RealtimeEvent {
	now := time.Now()
...
func (m *RealtimeEvent) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("realtime_event: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("realtime_event: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("realtime_event: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("realtime_event: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("realtime_event: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("realtime_event: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("realtime_event: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("realtime_event: version must be at least 1")
	}
	return nil
}

func (m *RealtimeEvent) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("realtime_event: marshal failed: %w", err)
	}
	return data, nil
}

func (m *RealtimeEvent) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("realtime_event: unmarshal failed: %w", err)
	}
	return nil
}

func (m *RealtimeEvent) Clone() *RealtimeEvent {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *RealtimeEvent) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("realtime_event: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("realtime_event: transition from %q to %q not allowed", m.Status, status)
}

func (m *RealtimeEvent) Sanitize() {
...
func (m *RealtimeEvent) Merge(other *RealtimeEvent) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *RealtimeEvent) String() string {
	return fmt.Sprintf("RealtimeEvent[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *RealtimeEvent) MatchesFilter(f *RealtimeEventFilter) bool {
...
func FilterRealtimeEventList(items []*RealtimeEvent, f *RealtimeEventFilter) *RealtimeEventListResponse {
	var filtered []*RealtimeEvent
...
func CalculateRealtimeEventStats(items []*RealtimeEvent) *RealtimeEventStats {
	stats := &RealtimeEventStats{}
	if len(items) == 0 {
		return stats
	}
