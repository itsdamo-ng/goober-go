...
func NewAdminReview(name string) *AdminReview {
	now := time.Now()
	return &AdminReview{
		Name:      name,
		Status:    "pending",
...
func (m *AdminReview) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("admin_review: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_review: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_review: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_review: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_review: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_review: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_review: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_review: version must be at least 1")
	}
	return nil
}

func (m *AdminReview) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_review: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminReview) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_review: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminReview) Clone() *AdminReview {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminReview) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_review: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_review: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminReview) Sanitize() {
...
func (m *AdminReview) Merge(other *AdminReview) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminReview) String() string {
	return fmt.Sprintf("AdminReview[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminReview) MatchesFilter(f *AdminReviewFilter) bool {
...
func FilterAdminReviewList(items []*AdminReview, f *AdminReviewFilter) *AdminReviewListResponse {
	var filtered []*AdminReview
...
func CalculateAdminReviewStats(items []*AdminReview) *AdminReviewStats {
