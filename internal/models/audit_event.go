...
func NewAuditEvent(name string) *AuditEvent {
	now := time.Now()
	return &AuditEvent{
		Name:      name,
		Status:    "pending",
...
func (m *AuditEvent) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("audit_event: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("audit_event: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("audit_event: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("audit_event: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("audit_event: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("audit_event: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("audit_event: version must be at least 1")
	}
	return nil
}
...
func (m *AuditEvent) ToJSON() ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("audit_event: marshal failed: %w", err)
	}
	return data, nil
}
...
func (m *AuditEvent) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("audit_event: unmarshal failed: %w", err)
	}
	return nil
}
...
func (m *AuditEvent) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("audit_event: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("audit_event: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuditEvent) Sanitize() {
...
func (m *AuditEvent) Merge(other *AuditEvent) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuditEvent) String() string {
	return fmt.Sprintf("AuditEvent[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuditEvent) MatchesFilter(f *AuditEventFilter) bool {
...
func FilterAuditEventList(items []*AuditEvent, f *AuditEventFilter) *AuditEventListResponse {
...
func CalculateAuditEventStats(items []*AuditEvent) *AuditEventStats {
