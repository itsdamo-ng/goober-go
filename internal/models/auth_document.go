...
type AuthDocumentFilter struct {
	IDs       []int64   `json:"ids"`
...
type AuthDocumentListResponse struct {
	Data       []*AuthDocument `json:"data"`
...
func NewAuthDocument(name string) *AuthDocument {
	now := time.Now()
	return &AuthDocument{
		Name:      name,
		Status:    "pending",
...
func (m *AuthDocument) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("auth_document: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("auth_document: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("auth_document: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("auth_document: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("auth_document: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("auth_document: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("auth_document: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("auth_document: version must be at least 1")
	}
	return nil
}

func (m *AuthDocument) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("auth_document: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuthDocument) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("auth_document: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuthDocument) Clone() *AuthDocument {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuthDocument) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("auth_document: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("auth_document: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuthDocument) Sanitize() {
...
func (m *AuthDocument) Merge(other *AuthDocument) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuthDocument) String() string {
	return fmt.Sprintf("AuthDocument[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuthDocument) MatchesFilter(f *AuthDocumentFilter) bool {
...
func FilterAuthDocumentList(items []*AuthDocument, f *AuthDocumentFilter) *AuthDocumentListResponse {
	var filtered []*AuthDocument
...
func CalculateAuthDocumentStats(items []*AuthDocument) *AuthDocumentStats {
	stats := &AuthDocumentStats{}
	if len(items) == 0 {
		return stats
	}
