...
func NewBillingAlert(name string) *BillingAlert {
	now := time.Now()
	return &BillingAlert{
		Name:      name,
		Status:    "pending",
...
func (m *BillingAlert) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("billing_alert: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("billing_alert: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("billing_alert: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("billing_alert: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("billing_alert: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("billing_alert: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("billing_alert: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("billing_alert: version must be at least 1")
	}
	return nil
}

func (m *BillingAlert) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("billing_alert: marshal failed: %w", err)
	}
	return data, nil
}

func (m *BillingAlert) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("billing_alert: unmarshal failed: %w", err)
	}
	return nil
}

func (m *BillingAlert) Clone() *BillingAlert {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *BillingAlert) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("billing_alert: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("billing_alert: transition from %q to %q not allowed", m.Status, status)
}

func (m *BillingAlert) Sanitize() {
...
func (m *BillingAlert) Merge(other *BillingAlert) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *BillingAlert) String() string {
	return fmt.Sprintf("BillingAlert[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *BillingAlert) MatchesFilter(f *BillingAlertFilter) bool {
...
func FilterBillingAlertList(items []*BillingAlert, f *BillingAlertFilter) *BillingAlertListResponse {
	var filtered []*BillingAlert
...
func CalculateBillingAlertStats(items []*BillingAlert) *BillingAlertStats {
	stats := &BillingAlertStats{}
	if len(items) == 0 {
		return stats
	}
