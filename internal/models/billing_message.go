...
type BillingMessageFilter struct {
	IDs       []int64   `json:"ids"`
...
type BillingMessageListResponse struct {
	Data       []*BillingMessage `json:"data"`
...
type BillingMessageStats struct {
...
func NewBillingMessage(name string) *BillingMessage {
	now := time.Now()
...
func (m *BillingMessage) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("billing_message: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("billing_message: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("billing_message: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("billing_message: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("billing_message: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("billing_message: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("billing_message: version must be at least 1")
	}
	return nil
}

func (m *BillingMessage) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("billing_message: marshal failed: %w", err)
	}
	return data, nil
}

func (m *BillingMessage) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("billing_message: unmarshal failed: %w", err)
	}
	return nil
}

func (m *BillingMessage) Clone() *BillingMessage {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *BillingMessage) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("billing_message: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("billing_message: transition from %q to %q not allowed", m.Status, status)
}

func (m *BillingMessage) Sanitize() {
...
func (m *BillingMessage) Merge(other *BillingMessage) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *BillingMessage) String() string {
	return fmt.Sprintf("BillingMessage[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *BillingMessage) MatchesFilter(f *BillingMessageFilter) bool {
...
func FilterBillingMessageList(items []*BillingMessage, f *BillingMessageFilter) *BillingMessageListResponse {
	var filtered []*BillingMessage
...
func CalculateBillingMessageStats(items []*BillingMessage) *BillingMessageStats {
	stats := &BillingMessageStats{}
	if len(items) == 0 {
		return stats
	}
