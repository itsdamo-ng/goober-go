...
func NewAlert(name string) *Alert {
	now := time.Now()
	return &Alert{
		Name:      name,
		Status:    "pending",
...
func (m *Alert) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("alert: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("alert: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("alert: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("alert: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("alert: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("alert: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("alert: version must be at least 1")
	}
	return nil
}

func (m *Alert) ToJSON() ([]byte, error) {
...
func (m *Alert) FromJSON(data []byte) error {
...
func (m *Alert) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("alert: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("alert: transition from %q to %q not allowed", m.Status, status)
}

func (m *Alert) Sanitize() {
...
func (m *Alert) Merge(other *Alert) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Alert) String() string {
	return fmt.Sprintf("Alert[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Alert) MatchesFilter(f *AlertFilter) bool {
...
func FilterAlertList(items []*Alert, f *AlertFilter) *AlertListResponse {
	var filtered []*Alert
...
func CalculateAlertStats(items []*Alert) *AlertStats {
	stats := &AlertStats{}
	if len(items) == 0 {
		return stats
	}
