...
type BillingRoleListResponse struct {
	Data       []*BillingRole `json:"data"`
...
func NewBillingRole(name string) *BillingRole {
	now := time.Now()
	return &BillingRole{
		Name:      name,
		Status:    "pending",
...
func (m *BillingRole) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("billing_role: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("billing_role: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("billing_role: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("billing_role: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("billing_role: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("billing_role: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("billing_role: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("billing_role: version must be at least 1")
	}
	return nil
}

func (m *BillingRole) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("billing_role: marshal failed: %w", err)
	}
	return data, nil
}

func (m *BillingRole) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("billing_role: unmarshal failed: %w", err)
	}
	return nil
}

func (m *BillingRole) Clone() *BillingRole {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *BillingRole) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("billing_role: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("billing_role: transition from %q to %q not allowed", m.Status, status)
}

func (m *BillingRole) Sanitize() {
...
func (m *BillingRole) Merge(other *BillingRole) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *BillingRole) String() string {
	return fmt.Sprintf("BillingRole[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *BillingRole) MatchesFilter(f *BillingRoleFilter) bool {
...
func FilterBillingRoleList(items []*BillingRole, f *BillingRoleFilter) *BillingRoleListResponse {
	var filtered []*BillingRole
	...
	return &BillingRoleListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateBillingRoleStats(items []*BillingRole) *BillingRoleStats {
	stats := &BillingRoleStats{}
	if len(items) == 0 {
		return stats
	}
