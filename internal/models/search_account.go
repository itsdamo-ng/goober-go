...
type SearchAccountFilter struct {
	IDs       []int64   `json:"ids"`
...
type SearchAccountStats struct {
...
func NewSearchAccount(name string) *SearchAccount {
	now := time.Now()
...
func (m *SearchAccount) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("search_account: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("search_account: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("search_account: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("search_account: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("search_account: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("search_account: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("search_account: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("search_account: version must be at least 1")
	}
	return nil
}

func (m *SearchAccount) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("search_account: marshal failed: %w", err)
	}
	return data, nil
}

func (m *SearchAccount) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("search_account: unmarshal failed: %w", err)
	}
	return nil
}

func (m *SearchAccount) Clone() *SearchAccount {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *SearchAccount) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("search_account: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("search_account: transition from %q to %q not allowed", m.Status, status)
}

func (m *SearchAccount) Sanitize() {
...
func (m *SearchAccount) Merge(other *SearchAccount) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *SearchAccount) String() string {
	return fmt.Sprintf("SearchAccount[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *SearchAccount) MatchesFilter(f *SearchAccountFilter) bool {
...
func FilterSearchAccountList(items []*SearchAccount, f *SearchAccountFilter) *SearchAccountListResponse {
	var filtered []*SearchAccount
	...
	return &SearchAccountListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateSearchAccountStats(items []*SearchAccount) *SearchAccountStats {
	stats := &SearchAccountStats{}
	if len(items) == 0 {
		return stats
	}
