...
type SearchCampaignFilter struct {
	IDs       []int64   `json:"ids"`
...
type SearchCampaignStats struct {
...
func NewSearchCampaign(name string) *SearchCampaign {
	now := time.Now()
...
func (m *SearchCampaign) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("search_campaign: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("search_campaign: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("search_campaign: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("search_campaign: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("search_campaign: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("search_campaign: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("search_campaign: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("search_campaign: version must be at least 1")
	}
	return nil
}

func (m *SearchCampaign) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("search_campaign: marshal failed: %w", err)
	}
	return data, nil
}

func (m *SearchCampaign) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("search_campaign: unmarshal failed: %w", err)
	}
	return nil
}

func (m *SearchCampaign) Clone() *SearchCampaign {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *SearchCampaign) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("search_campaign: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("search_campaign: transition from %q to %q not allowed", m.Status, status)
}

func (m *SearchCampaign) Sanitize() {
...
func (m *SearchCampaign) Merge(other *SearchCampaign) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *SearchCampaign) String() string {
	return fmt.Sprintf("SearchCampaign[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *SearchCampaign) MatchesFilter(f *SearchCampaignFilter) bool {
...
func FilterSearchCampaignList(items []*SearchCampaign, f *SearchCampaignFilter) *SearchCampaignListResponse {
	var filtered []*SearchCampaign
	...
	return &SearchCampaignListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateSearchCampaignStats(items []*SearchCampaign) *SearchCampaignStats {
	stats := &SearchCampaignStats{}
	if len(items) == 0 {
		return stats
	}
