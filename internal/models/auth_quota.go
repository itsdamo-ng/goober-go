...
type AuthQuotaListResponse struct {
	Data       []*AuthQuota `json:"data"`
...
func NewAuthQuota(name string) *AuthQuota {
	now := time.Now()
	return &AuthQuota{
		Name:      name,
		Status:    "pending",
...
func (m *AuthQuota) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("auth_quota: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("auth_quota: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("auth_quota: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("auth_quota: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("auth_quota: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("auth_quota: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("auth_quota: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("auth_quota: version must be at least 1")
	}
	return nil
}

func (m *AuthQuota) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("auth_quota: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuthQuota) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("auth_quota: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuthQuota) Clone() *AuthQuota {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuthQuota) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("auth_quota: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("auth_quota: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuthQuota) Sanitize() {
...
func (m *AuthQuota) Merge(other *AuthQuota) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuthQuota) String() string {
	return fmt.Sprintf("AuthQuota[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuthQuota) MatchesFilter(f *AuthQuotaFilter) bool {
...
func FilterAuthQuotaList(items []*AuthQuota, f *AuthQuotaFilter) *AuthQuotaListResponse {
	var filtered []*AuthQuota
...
func CalculateAuthQuotaStats(items []*AuthQuota) *AuthQuotaStats {
	stats := &AuthQuotaStats{}
	if len(items) == 0 {
		return stats
	}
