...
type AuthReviewListResponse struct {
	Data       []*AuthReview `json:"data"`
...
func NewAuthReview(name string) *AuthReview {
	now := time.Now()
	return &AuthReview{
		Name:      name,
		Status:    "pending",
...
func (m *AuthReview) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("auth_review: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("auth_review: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("auth_review: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("auth_review: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("auth_review: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("auth_review: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("auth_review: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("auth_review: version must be at least 1")
	}
	return nil
}

func (m *AuthReview) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("auth_review: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuthReview) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("auth_review: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuthReview) Clone() *AuthReview {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuthReview) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("auth_review: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("auth_review: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuthReview) Sanitize() {
...
func (m *AuthReview) Merge(other *AuthReview) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuthReview) String() string {
	return fmt.Sprintf("AuthReview[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuthReview) MatchesFilter(f *AuthReviewFilter) bool {
...
func FilterAuthReviewList(items []*AuthReview, f *AuthReviewFilter) *AuthReviewListResponse {
	var filtered []*AuthReview
...
func CalculateAuthReviewStats(items []*AuthReview) *AuthReviewStats {
	stats := &AuthReviewStats{}
	if len(items) == 0 {
		return stats
	}
