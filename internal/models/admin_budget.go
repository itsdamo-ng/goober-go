...
type AdminBudgetListResponse struct {
	Data       []*AdminBudget `json:"data"`
...
func NewAdminBudget(name string) *AdminBudget {
	now := time.Now()
	return &AdminBudget{
		Name:      name,
		Status:    "pending",
...
func (m *AdminBudget) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("admin_budget: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_budget: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_budget: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_budget: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_budget: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_budget: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_budget: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_budget: version must be at least 1")
	}
	return nil
}

func (m *AdminBudget) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_budget: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminBudget) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_budget: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminBudget) Clone() *AdminBudget {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminBudget) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_budget: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_budget: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminBudget) Sanitize() {
...
func (m *AdminBudget) Merge(other *AdminBudget) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminBudget) String() string {
	return fmt.Sprintf("AdminBudget[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminBudget) MatchesFilter(f *AdminBudgetFilter) bool {
...
func FilterAdminBudgetList(items []*AdminBudget, f *AdminBudgetFilter) *AdminBudgetListResponse {
	var filtered []*AdminBudget
	...
	return &AdminBudgetListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateAdminBudgetStats(items []*AdminBudget) *AdminBudgetStats {
	stats := &AdminBudgetStats{}
	if len(items) == 0 {
		return stats
	}
