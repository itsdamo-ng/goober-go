...
type AuthTeamListResponse struct {
	Data       []*AuthTeam `json:"data"`
...
func NewAuthTeam(name string) *AuthTeam {
	now := time.Now()
	return &AuthTeam{
		Name:      name,
		Status:    "pending",
...
func (m *AuthTeam) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("auth_team: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("auth_team: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("auth_team: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("auth_team: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("auth_team: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("auth_team: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("auth_team: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("auth_team: version must be at least 1")
	}
	return nil
}

func (m *AuthTeam) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("auth_team: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuthTeam) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("auth_team: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuthTeam) Clone() *AuthTeam {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuthTeam) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("auth_team: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("auth_team: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuthTeam) Sanitize() {
...
func (m *AuthTeam) Merge(other *AuthTeam) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuthTeam) String() string {
	return fmt.Sprintf("AuthTeam[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuthTeam) MatchesFilter(f *AuthTeamFilter) bool {
...
func FilterAuthTeamList(items []*AuthTeam, f *AuthTeamFilter) *AuthTeamListResponse {
	var filtered []*AuthTeam
...
func CalculateAuthTeamStats(items []*AuthTeam) *AuthTeamStats {
	stats := &AuthTeamStats{}
	if len(items) == 0 {
		return stats
	}
