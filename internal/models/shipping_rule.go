...
type ShippingRuleListResponse struct {
	Data       []*ShippingRule `json:"data"`
...
func NewShippingRule(name string) *ShippingRule {
	now := time.Now()
	return &ShippingRule{
		Name:      name,
		Status:    "pending",
...
func (m *ShippingRule) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("shipping_rule: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("shipping_rule: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("shipping_rule: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("shipping_rule: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("shipping_rule: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("shipping_rule: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("shipping_rule: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("shipping_rule: version must be at least 1")
	}
	return nil
}

func (m *ShippingRule) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("shipping_rule: marshal failed: %w", err)
	}
	return data, nil
}

func (m *ShippingRule) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("shipping_rule: unmarshal failed: %w", err)
	}
	return nil
}

func (m *ShippingRule) Clone() *ShippingRule {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *ShippingRule) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("shipping_rule: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("shipping_rule: transition from %q to %q not allowed", m.Status, status)
}

func (m *ShippingRule) Sanitize() {
...
func (m *ShippingRule) Merge(other *ShippingRule) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *ShippingRule) String() string {
	return fmt.Sprintf("ShippingRule[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *ShippingRule) MatchesFilter(f *ShippingRuleFilter) bool {
...
func FilterShippingRuleList(items []*ShippingRule, f *ShippingRuleFilter) *ShippingRuleListResponse {
	var filtered []*ShippingRule
...
func CalculateShippingRuleStats(items []*ShippingRule) *ShippingRuleStats {
	stats := &ShippingRuleStats{}
	if len(items) == 0 {
		return stats
	}
