...
type AuthSnapshotFilter struct {
	IDs       []int64   `json:"ids"`
...
func NewAuthSnapshot(name string) *AuthSnapshot {
	now := time.Now()
	return &AuthSnapshot{
		Name:      name,
		Status:    "pending",
...
func (m *AuthSnapshot) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("auth_snapshot: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("auth_snapshot: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("auth_snapshot: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("auth_snapshot: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("auth_snapshot: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("auth_snapshot: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("auth_snapshot: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("auth_snapshot: version must be at least 1")
	}
	return nil
}

func (m *AuthSnapshot) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("auth_snapshot: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuthSnapshot) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("auth_snapshot: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuthSnapshot) Clone() *AuthSnapshot {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuthSnapshot) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("auth_snapshot: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("auth_snapshot: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuthSnapshot) Sanitize() {
...
func (m *AuthSnapshot) Merge(other *AuthSnapshot) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuthSnapshot) String() string {
	return fmt.Sprintf("AuthSnapshot[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuthSnapshot) MatchesFilter(f *AuthSnapshotFilter) bool {
...
func FilterAuthSnapshotList(items []*AuthSnapshot, f *AuthSnapshotFilter) *AuthSnapshotListResponse {
	var filtered []*AuthSnapshot
	...
	return &AuthSnapshotListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateAuthSnapshotStats(items []*AuthSnapshot) *AuthSnapshotStats {
