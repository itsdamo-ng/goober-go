...
func (m *Subscription) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("subscription: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("subscription: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("subscription: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("subscription: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("subscription: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("subscription: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("subscription: version must be at least 1")
	}
	return nil
}
...
func (m *Subscription) ToJSON() ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("subscription: marshal failed: %w", err)
	}
	return data, nil
}
...
func (m *Subscription) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("subscription: unmarshal failed: %w", err)
	}
	return nil
}
...
func (m *Subscription) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("subscription: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("subscription: transition from %q to %q not allowed", m.Status, status)
}
...
func (m *Subscription) Merge(other *Subscription) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Subscription) String() string {
	return fmt.Sprintf("Subscription[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Subscription) MatchesFilter(f *SubscriptionFilter) bool {
...
func FilterSubscriptionList(items []*Subscription, f *SubscriptionFilter) *SubscriptionListResponse {
...
func CalculateSubscriptionStats(items []*Subscription) *SubscriptionStats {
