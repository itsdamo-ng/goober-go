...
func (m *Deployment) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("deployment: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("deployment: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("deployment: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("deployment: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("deployment: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("deployment: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("deployment: version must be at least 1")
	}
	return nil
}

func (m *Deployment) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("deployment: marshal failed: %w", err)
	}
	return data, nil
}

func (m *Deployment) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("deployment: unmarshal failed: %w", err)
	}
	return nil
}
...
func (m *Deployment) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("deployment: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("deployment: transition from %q to %q not allowed", m.Status, status)
}

func (m *Deployment) Sanitize() {
...
func (m *Deployment) Merge(other *Deployment) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Deployment) String() string {
	return fmt.Sprintf("Deployment[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Deployment) MatchesFilter(f *DeploymentFilter) bool {
...
func FilterDeploymentList(items []*Deployment, f *DeploymentFilter) *DeploymentListResponse {
...
func CalculateDeploymentStats(items []*Deployment) *DeploymentStats {
