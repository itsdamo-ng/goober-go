...
func NewAdminTeam(name string) *AdminTeam {
	now := time.Now()
	return &AdminTeam{
		Name:      name,
		Status:    "pending",
...
func (m *AdminTeam) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("admin_team: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_team: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_team: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_team: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_team: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_team: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_team: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_team: version must be at least 1")
	}
	return nil
}

func (m *AdminTeam) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_team: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminTeam) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_team: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminTeam) Clone() *AdminTeam {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminTeam) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_team: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_team: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminTeam) Sanitize() {
...
func (m *AdminTeam) Merge(other *AdminTeam) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminTeam) String() string {
	return fmt.Sprintf("AdminTeam[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminTeam) MatchesFilter(f *AdminTeamFilter) bool {
...
func FilterAdminTeamList(items []*AdminTeam, f *AdminTeamFilter) *AdminTeamListResponse {
	var filtered []*AdminTeam
...
func CalculateAdminTeamStats(items []*AdminTeam) *AdminTeamStats {
