...
type ContentPolicyListResponse struct {
	Data       []*ContentPolicy `json:"data"`
...
func NewContentPolicy(name string) *ContentPolicy {
	now := time.Now()
...
func (m *ContentPolicy) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("content_policy: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("content_policy: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("content_policy: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("content_policy: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("content_policy: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("content_policy: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("content_policy: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("content_policy: version must be at least 1")
	}
	return nil
}

func (m *ContentPolicy) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("content_policy: marshal failed: %w", err)
	}
	return data, nil
}

func (m *ContentPolicy) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("content_policy: unmarshal failed: %w", err)
	}
	return nil
}

func (m *ContentPolicy) Clone() *ContentPolicy {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *ContentPolicy) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("content_policy: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("content_policy: transition from %q to %q not allowed", m.Status, status)
}

func (m *ContentPolicy) Sanitize() {
...
func (m *ContentPolicy) Merge(other *ContentPolicy) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *ContentPolicy) String() string {
	return fmt.Sprintf("ContentPolicy[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *ContentPolicy) MatchesFilter(f *ContentPolicyFilter) bool {
...
func FilterContentPolicyList(items []*ContentPolicy, f *ContentPolicyFilter) *ContentPolicyListResponse {
	var filtered []*ContentPolicy
...
func CalculateContentPolicyStats(items []*ContentPolicy) *ContentPolicyStats {
