...
type SearchRoleListResponse struct {
	Data       []*SearchRole `json:"data"`
...
func NewSearchRole(name string) *SearchRole {
	now := time.Now()
	return &SearchRole{
		Name:      name,
		Status:    "pending",
...
func (m *SearchRole) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("search_role: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("search_role: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("search_role: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("search_role: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("search_role: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("search_role: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("search_role: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("search_role: version must be at least 1")
	}
	return nil
}

func (m *SearchRole) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("search_role: marshal failed: %w", err)
	}
	return data, nil
}

func (m *SearchRole) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("search_role: unmarshal failed: %w", err)
	}
	return nil
}

func (m *SearchRole) Clone() *SearchRole {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *SearchRole) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("search_role: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("search_role: transition from %q to %q not allowed", m.Status, status)
}

func (m *SearchRole) Sanitize() {
...
func (m *SearchRole) Merge(other *SearchRole) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *SearchRole) String() string {
	return fmt.Sprintf("SearchRole[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *SearchRole) MatchesFilter(f *SearchRoleFilter) bool {
...
func FilterSearchRoleList(items []*SearchRole, f *SearchRoleFilter) *SearchRoleListResponse {
	var filtered []*SearchRole
...
func CalculateSearchRoleStats(items []*SearchRole) *SearchRoleStats {
	stats := &SearchRoleStats{}
	if len(items) == 0 {
		return stats
	}
