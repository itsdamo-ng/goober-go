...
type PlatformTaskFilter struct {
	IDs       []int64   `json:"ids"`
...
type PlatformTaskListResponse struct {
	Data       []*PlatformTask `json:"data"`
...
func NewPlatformTask(name string) *PlatformTask {
	now := time.Now()
	return &PlatformTask{
		Name:      name,
		Status:    "pending",
...
func (m *PlatformTask) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("platform_task: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("platform_task: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("platform_task: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("platform_task: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("platform_task: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("platform_task: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("platform_task: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("platform_task: version must be at least 1")
	}
	return nil
}

func (m *PlatformTask) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("platform_task: marshal failed: %w", err)
	}
	return data, nil
}

func (m *PlatformTask) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("platform_task: unmarshal failed: %w", err)
	}
	return nil
}

func (m *PlatformTask) Clone() *PlatformTask {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *PlatformTask) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("platform_task: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("platform_task: transition from %q to %q not allowed", m.Status, status)
}

func (m *PlatformTask) Sanitize() {
...
func (m *PlatformTask) Merge(other *PlatformTask) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *PlatformTask) String() string {
	return fmt.Sprintf("PlatformTask[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *PlatformTask) MatchesFilter(f *PlatformTaskFilter) bool {
...
func FilterPlatformTaskList(items []*PlatformTask, f *PlatformTaskFilter) *PlatformTaskListResponse {
	var filtered []*PlatformTask
...
func CalculatePlatformTaskStats(items []*PlatformTask) *PlatformTaskStats {
