...
type AuthFeatureListResponse struct {
	Data       []*AuthFeature `json:"data"`
...
func NewAuthFeature(name string) *AuthFeature {
	now := time.Now()
	return &AuthFeature{
		Name:      name,
		Status:    "pending",
...
func (m *AuthFeature) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("auth_feature: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("auth_feature: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("auth_feature: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("auth_feature: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("auth_feature: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("auth_feature: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("auth_feature: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("auth_feature: version must be at least 1")
	}
	return nil
}

func (m *AuthFeature) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("auth_feature: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuthFeature) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("auth_feature: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuthFeature) Clone() *AuthFeature {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuthFeature) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("auth_feature: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("auth_feature: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuthFeature) Sanitize() {
...
func (m *AuthFeature) Merge(other *AuthFeature) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuthFeature) String() string {
	return fmt.Sprintf("AuthFeature[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuthFeature) MatchesFilter(f *AuthFeatureFilter) bool {
...
func FilterAuthFeatureList(items []*AuthFeature, f *AuthFeatureFilter) *AuthFeatureListResponse {
	var filtered []*AuthFeature
	...
	return &AuthFeatureListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateAuthFeatureStats(items []*AuthFeature) *AuthFeatureStats {
	stats := &AuthFeatureStats{}
	if len(items) == 0 {
		return stats
	}
