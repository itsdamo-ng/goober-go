...
type BatchLeadListResponse struct {
	Data       []*BatchLead `json:"data"`
...
func NewBatchLead(name string) *BatchLead {
	now := time.Now()
	return &BatchLead{
		Name:      name,
		Status:    "pending",
...
func (m *BatchLead) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("batch_lead: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("batch_lead: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("batch_lead: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("batch_lead: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("batch_lead: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("batch_lead: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("batch_lead: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("batch_lead: version must be at least 1")
	}
	return nil
}

func (m *BatchLead) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("batch_lead: marshal failed: %w", err)
	}
	return data, nil
}

func (m *BatchLead) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("batch_lead: unmarshal failed: %w", err)
	}
	return nil
}

func (m *BatchLead) Clone() *BatchLead {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *BatchLead) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("batch_lead: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("batch_lead: transition from %q to %q not allowed", m.Status, status)
}

func (m *BatchLead) Sanitize() {
...
func (m *BatchLead) Merge(other *BatchLead) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *BatchLead) String() string {
	return fmt.Sprintf("BatchLead[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *BatchLead) MatchesFilter(f *BatchLeadFilter) bool {
...
func FilterBatchLeadList(items []*BatchLead, f *BatchLeadFilter) *BatchLeadListResponse {
	var filtered []*BatchLead
...
func CalculateBatchLeadStats(items []*BatchLead) *BatchLeadStats {
