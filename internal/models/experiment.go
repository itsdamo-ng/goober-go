...
func (m *Experiment) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("experiment: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("experiment: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("experiment: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("experiment: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("experiment: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("experiment: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("experiment: version must be at least 1")
	}
	return nil
}

func (m *Experiment) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("experiment: marshal failed: %w", err)
	}
	return data, nil
}

func (m *Experiment) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("experiment: unmarshal failed: %w", err)
	}
	return nil
}

func (m *Experiment) Clone() *Experiment {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *Experiment) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("experiment: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("experiment: transition from %q to %q not allowed", m.Status, status)
}

func (m *Experiment) Sanitize() {
...
func (m *Experiment) Merge(other *Experiment) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Experiment) String() string {
	return fmt.Sprintf("Experiment[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Experiment) MatchesFilter(f *ExperimentFilter) bool {
...
func FilterExperimentList(items []*Experiment, f *ExperimentFilter) *ExperimentListResponse {
...
func CalculateExperimentStats(items []*Experiment) *ExperimentStats {
