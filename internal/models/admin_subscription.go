...
type AdminSubscriptionListResponse struct {
	Data       []*AdminSubscription `json:"data"`
...
func NewAdminSubscription(name string) *AdminSubscription {
	now := time.Now()
...
func (m *AdminSubscription) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("admin_subscription: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_subscription: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_subscription: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_subscription: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_subscription: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_subscription: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_subscription: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_subscription: version must be at least 1")
	}
	return nil
}

func (m *AdminSubscription) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_subscription: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminSubscription) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_subscription: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminSubscription) Clone() *AdminSubscription {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminSubscription) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_subscription: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_subscription: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminSubscription) Sanitize() {
...
func (m *AdminSubscription) Merge(other *AdminSubscription) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminSubscription) String() string {
	return fmt.Sprintf("AdminSubscription[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminSubscription) MatchesFilter(f *AdminSubscriptionFilter) bool {
...
func FilterAdminSubscriptionList(items []*AdminSubscription, f *AdminSubscriptionFilter) *AdminSubscriptionListResponse {
...
func CalculateAdminSubscriptionStats(items []*AdminSubscription) *AdminSubscriptionStats {
