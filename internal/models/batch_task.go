...
func (m *BatchTask) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("batch_task: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("batch_task: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("batch_task: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("batch_task: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("batch_task: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("batch_task: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("batch_task: version must be at least 1")
	}
	return nil
}

func (m *BatchTask) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("batch_task: marshal failed: %w", err)
	}
	return data, nil
}

func (m *BatchTask) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("batch_task: unmarshal failed: %w", err)
	}
	return nil
}

func (m *BatchTask) Clone() *BatchTask {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *BatchTask) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("batch_task: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("batch_task: transition from %q to %q not allowed", m.Status, status)
}

func (m *BatchTask) Sanitize() {
...
func (m *BatchTask) Merge(other *BatchTask) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *BatchTask) String() string {
	return fmt.Sprintf("BatchTask[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *BatchTask) MatchesFilter(f *BatchTaskFilter) bool {
...
func FilterBatchTaskList(items []*BatchTask, f *BatchTaskFilter) *BatchTaskListResponse {
	var filtered []*BatchTask
...
func CalculateBatchTaskStats(items []*BatchTask) *BatchTaskStats {
