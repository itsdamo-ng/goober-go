...
func NewFeedback(name string) *Feedback {
	now := time.Now()
	return &Feedback{
		Name:      name,
		Status:    "pending",
...
func (m *Feedback) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("feedback: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("feedback: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("feedback: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("feedback: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("feedback: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("feedback: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("feedback: version must be at least 1")
	}
	return nil
}

func (m *Feedback) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("feedback: marshal failed: %w", err)
	}
	return data, nil
}

func (m *Feedback) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("feedback: unmarshal failed: %w", err)
	}
	return nil
}

func (m *Feedback) Clone() *Feedback {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *Feedback) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("feedback: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("feedback: transition from %q to %q not allowed", m.Status, status)
}

func (m *Feedback) Sanitize() {
...
func (m *Feedback) Merge(other *Feedback) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Feedback) String() string {
	return fmt.Sprintf("Feedback[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Feedback) MatchesFilter(f *FeedbackFilter) bool {
...
func FilterFeedbackList(items []*Feedback, f *FeedbackFilter) *FeedbackListResponse {
...
func CalculateFeedbackStats(items []*Feedback) *FeedbackStats {
	stats := &FeedbackStats{}
	if len(items) == 0 {
		return stats
	}
