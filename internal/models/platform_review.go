...
type PlatformReviewListResponse struct {
	Data       []*PlatformReview `json:"data"`
...
func NewPlatformReview(name string) *PlatformReview {
	now := time.Now()
...
func (m *PlatformReview) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("platform_review: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("platform_review: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("platform_review: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("platform_review: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("platform_review: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("platform_review: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("platform_review: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("platform_review: version must be at least 1")
	}
	return nil
}

func (m *PlatformReview) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("platform_review: marshal failed: %w", err)
	}
	return data, nil
}

func (m *PlatformReview) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("platform_review: unmarshal failed: %w", err)
	}
	return nil
}

func (m *PlatformReview) Clone() *PlatformReview {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *PlatformReview) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("platform_review: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("platform_review: transition from %q to %q not allowed", m.Status, status)
}

func (m *PlatformReview) Sanitize() {
...
func (m *PlatformReview) Merge(other *PlatformReview) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *PlatformReview) String() string {
	return fmt.Sprintf("PlatformReview[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *PlatformReview) MatchesFilter(f *PlatformReviewFilter) bool {
...
func FilterPlatformReviewList(items []*PlatformReview, f *PlatformReviewFilter) *PlatformReviewListResponse {
	var filtered []*PlatformReview
	...
	return &PlatformReviewListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculatePlatformReviewStats(items []*PlatformReview) *PlatformReviewStats {
