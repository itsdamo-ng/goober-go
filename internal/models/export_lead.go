...
type ExportLeadListResponse struct {
	Data       []*ExportLead `json:"data"`
...
func NewExportLead(name string) *ExportLead {
	now := time.Now()
	return &ExportLead{
		Name:      name,
		Status:    "pending",
...
func (m *ExportLead) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("export_lead: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("export_lead: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("export_lead: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("export_lead: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("export_lead: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("export_lead: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("export_lead: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("export_lead: version must be at least 1")
	}
	return nil
}

func (m *ExportLead) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("export_lead: marshal failed: %w", err)
	}
	return data, nil
}

func (m *ExportLead) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("export_lead: unmarshal failed: %w", err)
	}
	return nil
}

func (m *ExportLead) Clone() *ExportLead {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *ExportLead) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("export_lead: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("export_lead: transition from %q to %q not allowed", m.Status, status)
}

func (m *ExportLead) Sanitize() {
...
func (m *ExportLead) Merge(other *ExportLead) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *ExportLead) String() string {
	return fmt.Sprintf("ExportLead[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *ExportLead) MatchesFilter(f *ExportLeadFilter) bool {
...
func FilterExportLeadList(items []*ExportLead, f *ExportLeadFilter) *ExportLeadListResponse {
	var filtered []*ExportLead
...
func CalculateExportLeadStats(items []*ExportLead) *ExportLeadStats {
	stats := &ExportLeadStats{}
	if len(items) == 0 {
		return stats
	}
