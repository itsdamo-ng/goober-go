...
func NewAdminProduct(name string) *AdminProduct {
	now := time.Now()
	return &AdminProduct{
		Name:      name,
		Status:    "pending",
...
func (m *AdminProduct) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("admin_product: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_product: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_product: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_product: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_product: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_product: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_product: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_product: version must be at least 1")
	}
	return nil
}

func (m *AdminProduct) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_product: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminProduct) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_product: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminProduct) Clone() *AdminProduct {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminProduct) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_product: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_product: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminProduct) Sanitize() {
...
func (m *AdminProduct) Merge(other *AdminProduct) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminProduct) String() string {
	return fmt.Sprintf("AdminProduct[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminProduct) MatchesFilter(f *AdminProductFilter) bool {
...
func FilterAdminProductList(items []*AdminProduct, f *AdminProductFilter) *AdminProductListResponse {
	var filtered []*AdminProduct
...
func CalculateAdminProductStats(items []*AdminProduct) *AdminProductStats {
