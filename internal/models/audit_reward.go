...
type AuditRewardListResponse struct {
	Data       []*AuditReward `json:"data"`
...
func NewAuditReward(name string) *AuditReward {
	now := time.Now()
	return &AuditReward{
		Name:      name,
		Status:    "pending",
...
func (m *AuditReward) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("audit_reward: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("audit_reward: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("audit_reward: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("audit_reward: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("audit_reward: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("audit_reward: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("audit_reward: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("audit_reward: version must be at least 1")
	}
	return nil
}

func (m *AuditReward) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("audit_reward: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuditReward) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("audit_reward: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuditReward) Clone() *AuditReward {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuditReward) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("audit_reward: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("audit_reward: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuditReward) Sanitize() {
...
func (m *AuditReward) Merge(other *AuditReward) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuditReward) String() string {
	return fmt.Sprintf("AuditReward[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuditReward) MatchesFilter(f *AuditRewardFilter) bool {
...
func FilterAuditRewardList(items []*AuditReward, f *AuditRewardFilter) *AuditRewardListResponse {
	var filtered []*AuditReward
	...
	return &AuditRewardListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateAuditRewardStats(items []*AuditReward) *AuditRewardStats {
	stats := &AuditRewardStats{}
	if len(items) == 0 {
		return stats
	}
