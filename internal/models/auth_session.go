...
func NewAuthSession(name string) *AuthSession {
	now := time.Now()
	return &AuthSession{
		Name:      name,
		Status:    "pending",
...
func (m *AuthSession) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("auth_session: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("auth_session: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("auth_session: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("auth_session: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("auth_session: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("auth_session: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("auth_session: version must be at least 1")
	}
	return nil
}

func (m *AuthSession) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("auth_session: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuthSession) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("auth_session: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuthSession) Clone() *AuthSession {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuthSession) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("auth_session: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("auth_session: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuthSession) Sanitize() {
...
func (m *AuthSession) Merge(other *AuthSession) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuthSession) String() string {
	return fmt.Sprintf("AuthSession[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuthSession) MatchesFilter(f *AuthSessionFilter) bool {
...
func FilterAuthSessionList(items []*AuthSession, f *AuthSessionFilter) *AuthSessionListResponse {
	var filtered []*AuthSession
...
func CalculateAuthSessionStats(items []*AuthSession) *AuthSessionStats {
