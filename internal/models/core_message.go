...
type CoreMessageListResponse struct {
	Data       []*CoreMessage `json:"data"`
...
func NewCoreMessage(name string) *CoreMessage {
	now := time.Now()
	return &CoreMessage{
		Name:      name,
		Status:    "pending",
...
func (m *CoreMessage) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("core_message: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("core_message: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("core_message: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("core_message: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("core_message: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("core_message: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("core_message: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("core_message: version must be at least 1")
	}
	return nil
}

func (m *CoreMessage) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("core_message: marshal failed: %w", err)
	}
	return data, nil
}

func (m *CoreMessage) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("core_message: unmarshal failed: %w", err)
	}
	return nil
}

func (m *CoreMessage) Clone() *CoreMessage {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *CoreMessage) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("core_message: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("core_message: transition from %q to %q not allowed", m.Status, status)
}

func (m *CoreMessage) Sanitize() {
...
func (m *CoreMessage) Merge(other *CoreMessage) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *CoreMessage) String() string {
	return fmt.Sprintf("CoreMessage[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *CoreMessage) MatchesFilter(f *CoreMessageFilter) bool {
...
func FilterCoreMessageList(items []*CoreMessage, f *CoreMessageFilter) *CoreMessageListResponse {
	var filtered []*CoreMessage
	...
	return &CoreMessageListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateCoreMessageStats(items []*CoreMessage) *CoreMessageStats {
	stats := &CoreMessageStats{}
	if len(items) == 0 {
		return stats
	}
