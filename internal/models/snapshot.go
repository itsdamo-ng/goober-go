...
func (m *Snapshot) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("snapshot: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("snapshot: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("snapshot: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("snapshot: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("snapshot: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("snapshot: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("snapshot: version must be at least 1")
	}
	return nil
}

func (m *Snapshot) ToJSON() ([]byte, error) {
...
func (m *Snapshot) FromJSON(data []byte) error {
...
func (m *Snapshot) Clone() *Snapshot {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *Snapshot) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("snapshot: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("snapshot: transition from %q to %q not allowed", m.Status, status)
}

func (m *Snapshot) Sanitize() {
...
func (m *Snapshot) Merge(other *Snapshot) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Snapshot) String() string {
	return fmt.Sprintf("Snapshot[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Snapshot) MatchesFilter(f *SnapshotFilter) bool {
...
func FilterSnapshotList(items []*Snapshot, f *SnapshotFilter) *SnapshotListResponse {
...
func CalculateSnapshotStats(items []*Snapshot) *SnapshotStats {
	stats := &SnapshotStats{}
	if len(items) == 0 {
		return stats
	}
