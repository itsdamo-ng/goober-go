...
type CoreProductListResponse struct {
	Data       []*CoreProduct `json:"data"`
...
func NewCoreProduct(name string) *CoreProduct {
	now := time.Now()
	return &CoreProduct{
		Name:      name,
		Status:    "pending",
...
func (m *CoreProduct) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("core_product: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("core_product: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("core_product: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("core_product: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("core_product: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("core_product: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("core_product: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("core_product: version must be at least 1")
	}
	return nil
}

func (m *CoreProduct) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("core_product: marshal failed: %w", err)
	}
	return data, nil
}

func (m *CoreProduct) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("core_product: unmarshal failed: %w", err)
	}
	return nil
}

func (m *CoreProduct) Clone() *CoreProduct {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *CoreProduct) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("core_product: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("core_product: transition from %q to %q not allowed", m.Status, status)
}

func (m *CoreProduct) Sanitize() {
...
func (m *CoreProduct) Merge(other *CoreProduct) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *CoreProduct) String() string {
	return fmt.Sprintf("CoreProduct[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *CoreProduct) MatchesFilter(f *CoreProductFilter) bool {
...
func FilterCoreProductList(items []*CoreProduct, f *CoreProductFilter) *CoreProductListResponse {
	var filtered []*CoreProduct
...
func CalculateCoreProductStats(items []*CoreProduct) *CoreProductStats {
	stats := &CoreProductStats{}
	if len(items) == 0 {
		return stats
	}
