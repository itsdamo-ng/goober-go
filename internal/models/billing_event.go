...
type BillingEventListResponse struct {
...
func NewBillingEvent(name string) *BillingEvent {
	now := time.Now()
	return &BillingEvent{
		Name:      name,
		Status:    "pending",
...
func (m *BillingEvent) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("billing_event: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("billing_event: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("billing_event: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("billing_event: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("billing_event: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("billing_event: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("billing_event: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("billing_event: version must be at least 1")
	}
	return nil
}

func (m *BillingEvent) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("billing_event: marshal failed: %w", err)
	}
	return data, nil
}

func (m *BillingEvent) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("billing_event: unmarshal failed: %w", err)
	}
	return nil
}

func (m *BillingEvent) Clone() *BillingEvent {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *BillingEvent) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("billing_event: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("billing_event: transition from %q to %q not allowed", m.Status, status)
}

func (m *BillingEvent) Sanitize() {
...
func (m *BillingEvent) Merge(other *BillingEvent) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *BillingEvent) String() string {
	return fmt.Sprintf("BillingEvent[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *BillingEvent) MatchesFilter(f *BillingEventFilter) bool {
...
func FilterBillingEventList(items []*BillingEvent, f *BillingEventFilter) *BillingEventListResponse {
	var filtered []*BillingEvent
...
func CalculateBillingEventStats(items []*BillingEvent) *BillingEventStats {
	stats := &BillingEventStats{}
	if len(items) == 0 {
		return stats
	}
