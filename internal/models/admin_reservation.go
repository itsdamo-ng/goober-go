...
type AdminReservationListResponse struct {
	Data       []*AdminReservation `json:"data"`
...
type AdminReservationStats struct {
...
func NewAdminReservation(name string) *AdminReservation {
	now := time.Now()
...
func (m *AdminReservation) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("admin_reservation: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_reservation: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_reservation: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_reservation: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_reservation: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_reservation: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_reservation: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_reservation: version must be at least 1")
	}
	return nil
}

func (m *AdminReservation) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_reservation: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminReservation) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_reservation: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminReservation) Clone() *AdminReservation {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminReservation) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_reservation: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_reservation: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminReservation) Sanitize() {
...
func (m *AdminReservation) Merge(other *AdminReservation) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminReservation) String() string {
	return fmt.Sprintf("AdminReservation[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminReservation) MatchesFilter(f *AdminReservationFilter) bool {
...
func FilterAdminReservationList(items []*AdminReservation, f *AdminReservationFilter) *AdminReservationListResponse {
	var filtered []*AdminReservation
...
func CalculateAdminReservationStats(items []*AdminReservation) *AdminReservationStats {
