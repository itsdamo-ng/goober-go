...
func NewBillingOrder(name string) *BillingOrder {
	now := time.Now()
	return &BillingOrder{
		Name:      name,
		Status:    "pending",
...
func (m *BillingOrder) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("billing_order: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("billing_order: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("billing_order: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("billing_order: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("billing_order: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("billing_order: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("billing_order: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("billing_order: version must be at least 1")
	}
	return nil
}

func (m *BillingOrder) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("billing_order: marshal failed: %w", err)
	}
	return data, nil
}

func (m *BillingOrder) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("billing_order: unmarshal failed: %w", err)
	}
	return nil
}

func (m *BillingOrder) Clone() *BillingOrder {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *BillingOrder) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("billing_order: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("billing_order: transition from %q to %q not allowed", m.Status, status)
}

func (m *BillingOrder) Sanitize() {
...
func (m *BillingOrder) Merge(other *BillingOrder) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *BillingOrder) String() string {
	return fmt.Sprintf("BillingOrder[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *BillingOrder) MatchesFilter(f *BillingOrderFilter) bool {
...
func FilterBillingOrderList(items []*BillingOrder, f *BillingOrderFilter) *BillingOrderListResponse {
	var filtered []*BillingOrder
	...
	return &BillingOrderListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateBillingOrderStats(items []*BillingOrder) *BillingOrderStats {
	stats := &BillingOrderStats{}
	if len(items) == 0 {
		return stats
	}
