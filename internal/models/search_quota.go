...
type SearchQuotaListResponse struct {
	Data       []*SearchQuota `json:"data"`
...
func NewSearchQuota(name string) *SearchQuota {
	now := time.Now()
	return &SearchQuota{
		Name:      name,
		Status:    "pending",
...
func (m *SearchQuota) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("search_quota: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("search_quota: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("search_quota: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("search_quota: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("search_quota: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("search_quota: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("search_quota: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("search_quota: version must be at least 1")
	}
	return nil
}

func (m *SearchQuota) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("search_quota: marshal failed: %w", err)
	}
	return data, nil
}

func (m *SearchQuota) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("search_quota: unmarshal failed: %w", err)
	}
	return nil
}

func (m *SearchQuota) Clone() *SearchQuota {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *SearchQuota) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("search_quota: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("search_quota: transition from %q to %q not allowed", m.Status, status)
}

func (m *SearchQuota) Sanitize() {
...
func (m *SearchQuota) Merge(other *SearchQuota) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *SearchQuota) String() string {
	return fmt.Sprintf("SearchQuota[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *SearchQuota) MatchesFilter(f *SearchQuotaFilter) bool {
...
func FilterSearchQuotaList(items []*SearchQuota, f *SearchQuotaFilter) *SearchQuotaListResponse {
	var filtered []*SearchQuota
	...
	return &SearchQuotaListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateSearchQuotaStats(items []*SearchQuota) *SearchQuotaStats {
	stats := &SearchQuotaStats{}
	if len(items) == 0 {
		return stats
	}
