...
func (m *Feature) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("feature: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("feature: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("feature: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("feature: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("feature: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("feature: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("feature: version must be at least 1")
	}
	return nil
}

func (m *Feature) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("feature: marshal failed: %w", err)
	}
	return data, nil
}

func (m *Feature) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("feature: unmarshal failed: %w", err)
	}
	return nil
}
...
func (m *Feature) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("feature: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("feature: transition from %q to %q not allowed", m.Status, status)
}

func (m *Feature) Sanitize() {
...
func (m *Feature) Merge(other *Feature) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Feature) String() string {
	return fmt.Sprintf("Feature[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Feature) MatchesFilter(f *FeatureFilter) bool {
...
func FilterFeatureList(items []*Feature, f *FeatureFilter) *FeatureListResponse {
...
func CalculateFeatureStats(items []*Feature) *FeatureStats {
	stats := &FeatureStats{}
	if len(items) == 0 {
		return stats
	}
