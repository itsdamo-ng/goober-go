...
type AdminContractFilter struct {
	IDs       []int64   `json:"ids"`
...
type AdminContractListResponse struct {
	Data       []*AdminContract `json:"data"`
...
type AdminContractStats struct {
...
func NewAdminContract(name string) *AdminContract {
	now := time.Now()
...
func (m *AdminContract) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("admin_contract: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_contract: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_contract: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_contract: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_contract: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_contract: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_contract: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_contract: version must be at least 1")
	}
	return nil
}

func (m *AdminContract) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_contract: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminContract) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_contract: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminContract) Clone() *AdminContract {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminContract) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_contract: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_contract: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminContract) Sanitize() {
...
func (m *AdminContract) Merge(other *AdminContract) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminContract) String() string {
	return fmt.Sprintf("AdminContract[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminContract) MatchesFilter(f *AdminContractFilter) bool {
...
func FilterAdminContractList(items []*AdminContract, f *AdminContractFilter) *AdminContractListResponse {
	var filtered []*AdminContract
...
func CalculateAdminContractStats(items []*AdminContract) *AdminContractStats {
	stats := &AdminContractStats{}
	if len(items) == 0 {
		return stats
	}
