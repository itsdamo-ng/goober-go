...
type SearchSegmentFilter struct {
	IDs       []int64   `json:"ids"`
...
type SearchSegmentListResponse struct {
	Data       []*SearchSegment `json:"data"`
...
type SearchSegmentStats struct {
...
func NewSearchSegment(name string) *SearchSegment {
	now := time.Now()
...
func (m *SearchSegment) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("search_segment: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("search_segment: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("search_segment: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("search_segment: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("search_segment: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("search_segment: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("search_segment: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("search_segment: version must be at least 1")
	}
	return nil
}

func (m *SearchSegment) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("search_segment: marshal failed: %w", err)
	}
	return data, nil
}

func (m *SearchSegment) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("search_segment: unmarshal failed: %w", err)
	}
	return nil
}

func (m *SearchSegment) Clone() *SearchSegment {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *SearchSegment) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("search_segment: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("search_segment: transition from %q to %q not allowed", m.Status, status)
}

func (m *SearchSegment) Sanitize() {
...
func (m *SearchSegment) Merge(other *SearchSegment) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *SearchSegment) String() string {
	return fmt.Sprintf("SearchSegment[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *SearchSegment) MatchesFilter(f *SearchSegmentFilter) bool {
...
func FilterSearchSegmentList(items []*SearchSegment, f *SearchSegmentFilter) *SearchSegmentListResponse {
	var filtered []*SearchSegment
	...
	return &SearchSegmentListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateSearchSegmentStats(items []*SearchSegment) *SearchSegmentStats {
