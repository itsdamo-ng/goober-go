...
func NewQuota(name string) *Quota {
	now := time.Now()
	return &Quota{
		Name:      name,
		Status:    "pending",
...
func (m *Quota) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("quota: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("quota: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("quota: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("quota: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("quota: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("quota: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("quota: version must be at least 1")
	}
	return nil
}

func (m *Quota) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("quota: marshal failed: %w", err)
	}
	return data, nil
}

func (m *Quota) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("quota: unmarshal failed: %w", err)
	}
	return nil
}

func (m *Quota) Clone() *Quota {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *Quota) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("quota: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("quota: transition from %q to %q not allowed", m.Status, status)
}

func (m *Quota) Sanitize() {
...
func (m *Quota) Merge(other *Quota) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Quota) String() string {
	return fmt.Sprintf("Quota[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Quota) MatchesFilter(f *QuotaFilter) bool {
...
func FilterQuotaList(items []*Quota, f *QuotaFilter) *QuotaListResponse {
	var filtered []*Quota
...
func CalculateQuotaStats(items []*Quota) *QuotaStats {
	stats := &QuotaStats{}
	if len(items) == 0 {
		return stats
	}
