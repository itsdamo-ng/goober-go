...
type SearchAlertListResponse struct {
	Data       []*SearchAlert `json:"data"`
...
func NewSearchAlert(name string) *SearchAlert {
	now := time.Now()
	return &SearchAlert{
		Name:      name,
		Status:    "pending",
...
func (m *SearchAlert) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("search_alert: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("search_alert: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("search_alert: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("search_alert: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("search_alert: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("search_alert: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("search_alert: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("search_alert: version must be at least 1")
	}
	return nil
}

func (m *SearchAlert) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("search_alert: marshal failed: %w", err)
	}
	return data, nil
}

func (m *SearchAlert) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("search_alert: unmarshal failed: %w", err)
	}
	return nil
}

func (m *SearchAlert) Clone() *SearchAlert {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *SearchAlert) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("search_alert: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("search_alert: transition from %q to %q not allowed", m.Status, status)
}

func (m *SearchAlert) Sanitize() {
...
func (m *SearchAlert) Merge(other *SearchAlert) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *SearchAlert) String() string {
	return fmt.Sprintf("SearchAlert[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *SearchAlert) MatchesFilter(f *SearchAlertFilter) bool {
...
func FilterSearchAlertList(items []*SearchAlert, f *SearchAlertFilter) *SearchAlertListResponse {
	var filtered []*SearchAlert
	...
	return &SearchAlertListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateSearchAlertStats(items []*SearchAlert) *SearchAlertStats {
	stats := &SearchAlertStats{}
	if len(items) == 0 {
		return stats
	}
