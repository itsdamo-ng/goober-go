...
type AuthCommentListResponse struct {
	Data       []*AuthComment `json:"data"`
...
func NewAuthComment(name string) *AuthComment {
	now := time.Now()
	return &AuthComment{
		Name:      name,
		Status:    "pending",
...
func (m *AuthComment) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("auth_comment: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("auth_comment: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("auth_comment: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("auth_comment: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("auth_comment: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("auth_comment: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("auth_comment: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("auth_comment: version must be at least 1")
	}
	return nil
}

func (m *AuthComment) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("auth_comment: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuthComment) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("auth_comment: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuthComment) Clone() *AuthComment {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuthComment) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("auth_comment: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("auth_comment: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuthComment) Sanitize() {
...
func (m *AuthComment) Merge(other *AuthComment) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuthComment) String() string {
	return fmt.Sprintf("AuthComment[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuthComment) MatchesFilter(f *AuthCommentFilter) bool {
...
func FilterAuthCommentList(items []*AuthComment, f *AuthCommentFilter) *AuthCommentListResponse {
	var filtered []*AuthComment
	...
	return &AuthCommentListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateAuthCommentStats(items []*AuthComment) *AuthCommentStats {
	stats := &AuthCommentStats{}
	if len(items) == 0 {
		return stats
	}
