...
func NewSearchChannel(name string) *SearchChannel {
	now := time.Now()
...
func (m *SearchChannel) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("search_channel: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("search_channel: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("search_channel: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("search_channel: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("search_channel: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("search_channel: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("search_channel: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("search_channel: version must be at least 1")
	}
	return nil
}

func (m *SearchChannel) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("search_channel: marshal failed: %w", err)
	}
	return data, nil
}

func (m *SearchChannel) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("search_channel: unmarshal failed: %w", err)
	}
	return nil
}

func (m *SearchChannel) Clone() *SearchChannel {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *SearchChannel) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("search_channel: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("search_channel: transition from %q to %q not allowed", m.Status, status)
}

func (m *SearchChannel) Sanitize() {
...
func (m *SearchChannel) Merge(other *SearchChannel) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *SearchChannel) String() string {
	return fmt.Sprintf("SearchChannel[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *SearchChannel) MatchesFilter(f *SearchChannelFilter) bool {
...
func FilterSearchChannelList(items []*SearchChannel, f *SearchChannelFilter) *SearchChannelListResponse {
	var filtered []*SearchChannel
	...
	return &SearchChannelListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateSearchChannelStats(items []*SearchChannel) *SearchChannelStats {
	stats := &SearchChannelStats{}
	if len(items) == 0 {
		return stats
	}
