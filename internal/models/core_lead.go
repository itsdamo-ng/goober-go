...
type CoreLeadListResponse struct {
	Data       []*CoreLead `json:"data"`
...
func NewCoreLead(name string) *CoreLead {
	now := time.Now()
	return &CoreLead{
		Name:      name,
		Status:    "pending",
...
func (m *CoreLead) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("core_lead: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("core_lead: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("core_lead: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("core_lead: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("core_lead: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("core_lead: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("core_lead: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("core_lead: version must be at least 1")
	}
	return nil
}

func (m *CoreLead) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("core_lead: marshal failed: %w", err)
	}
	return data, nil
}

func (m *CoreLead) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("core_lead: unmarshal failed: %w", err)
	}
	return nil
}

func (m *CoreLead) Clone() *CoreLead {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *CoreLead) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("core_lead: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("core_lead: transition from %q to %q not allowed", m.Status, status)
}

func (m *CoreLead) Sanitize() {
...
func (m *CoreLead) Merge(other *CoreLead) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *CoreLead) String() string {
	return fmt.Sprintf("CoreLead[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *CoreLead) MatchesFilter(f *CoreLeadFilter) bool {
...
func FilterCoreLeadList(items []*CoreLead, f *CoreLeadFilter) *CoreLeadListResponse {
	var filtered []*CoreLead
...
func CalculateCoreLeadStats(items []*CoreLead) *CoreLeadStats {
	stats := &CoreLeadStats{}
	if len(items) == 0 {
		return stats
	}
