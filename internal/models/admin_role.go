...
func NewAdminRole(name string) *AdminRole {
	now := time.Now()
	return &AdminRole{
		Name:      name,
		Status:    "pending",
...
func (m *AdminRole) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_role: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_role: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_role: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_role: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_role: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_role: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_role: version must be at least 1")
	}
	return nil
}

func (m *AdminRole) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_role: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminRole) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_role: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminRole) Clone() *AdminRole {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminRole) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_role: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_role: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminRole) Sanitize() {
...
func (m *AdminRole) Merge(other *AdminRole) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminRole) String() string {
	return fmt.Sprintf("AdminRole[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminRole) MatchesFilter(f *AdminRoleFilter) bool {
...
func FilterAdminRoleList(items []*AdminRole, f *AdminRoleFilter) *AdminRoleListResponse {
	var filtered []*AdminRole
...
func CalculateAdminRoleStats(items []*AdminRole) *AdminRoleStats {
	stats := &AdminRoleStats{}
	if len(items) == 0 {
		return stats
	}
