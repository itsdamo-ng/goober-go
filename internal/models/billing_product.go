...
type BillingProductFilter struct {
	IDs       []int64   `json:"ids"`
...
type BillingProductListResponse struct {
	Data       []*BillingProduct `json:"data"`
...
type BillingProductStats struct {
...
func NewBillingProduct(name string) *BillingProduct {
	now := time.Now()
...
func (m *BillingProduct) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("billing_product: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("billing_product: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("billing_product: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("billing_product: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("billing_product: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("billing_product: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("billing_product: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("billing_product: version must be at least 1")
	}
	return nil
}

func (m *BillingProduct) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("billing_product: marshal failed: %w", err)
	}
	return data, nil
}

func (m *BillingProduct) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("billing_product: unmarshal failed: %w", err)
	}
	return nil
}

func (m *BillingProduct) Clone() *BillingProduct {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *BillingProduct) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("billing_product: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("billing_product: transition from %q to %q not allowed", m.Status, status)
}

func (m *BillingProduct) Sanitize() {
...
func (m *BillingProduct) Merge(other *BillingProduct) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *BillingProduct) String() string {
	return fmt.Sprintf("BillingProduct[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *BillingProduct) MatchesFilter(f *BillingProductFilter) bool {
...
func FilterBillingProductList(items []*BillingProduct, f *BillingProductFilter) *BillingProductListResponse {
	var filtered []*BillingProduct
...
func CalculateBillingProductStats(items []*BillingProduct) *BillingProductStats {
	stats := &BillingProductStats{}
	if len(items) == 0 {
		return stats
	}
