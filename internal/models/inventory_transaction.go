...
type InventoryTransactionListResponse struct {
	Data       []*InventoryTransaction `json:"data"`
...
func NewInventoryTransaction(name string) *InventoryTransaction {
	now := time.Now()
...
func (m *InventoryTransaction) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("inventory_transaction: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("inventory_transaction: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("inventory_transaction: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("inventory_transaction: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("inventory_transaction: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("inventory_transaction: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("inventory_transaction: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("inventory_transaction: version must be at least 1")
	}
	return nil
}

func (m *InventoryTransaction) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("inventory_transaction: marshal failed: %w", err)
	}
	return data, nil
}

func (m *InventoryTransaction) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("inventory_transaction: unmarshal failed: %w", err)
	}
	return nil
}

func (m *InventoryTransaction) Clone() *InventoryTransaction {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *InventoryTransaction) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("inventory_transaction: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("inventory_transaction: transition from %q to %q not allowed", m.Status, status)
}

func (m *InventoryTransaction) Sanitize() {
...
func (m *InventoryTransaction) Merge(other *InventoryTransaction) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *InventoryTransaction) String() string {
	return fmt.Sprintf("InventoryTransaction[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *InventoryTransaction) MatchesFilter(f *InventoryTransactionFilter) bool {
...
func FilterInventoryTransactionList(items []*InventoryTransaction, f *InventoryTransactionFilter) *InventoryTransactionListResponse {
	var filtered []*InventoryTransaction
...
func CalculateInventoryTransactionStats(items []*InventoryTransaction) *InventoryTransactionStats {
