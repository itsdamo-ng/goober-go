...
func (m *Segment) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("segment: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("segment: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("segment: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("segment: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("segment: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("segment: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("segment: version must be at least 1")
	}
	return nil
}

func (m *Segment) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("segment: marshal failed: %w", err)
	}
	return data, nil
}

func (m *Segment) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("segment: unmarshal failed: %w", err)
	}
	return nil
}
...
func (m *Segment) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("segment: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("segment: transition from %q to %q not allowed", m.Status, status)
}

func (m *Segment) Sanitize() {
...
func (m *Segment) Merge(other *Segment) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Segment) String() string {
	return fmt.Sprintf("Segment[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Segment) MatchesFilter(f *SegmentFilter) bool {
...
func FilterSegmentList(items []*Segment, f *SegmentFilter) *SegmentListResponse {
...
func CalculateSegmentStats(items []*Segment) *SegmentStats {
	stats := &SegmentStats{}
	if len(items) == 0 {
		return stats
	}
