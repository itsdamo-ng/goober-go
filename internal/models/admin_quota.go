...
func NewAdminQuota(name string) *AdminQuota {
	now := time.Now()
	return &AdminQuota{
		Name:      name,
		Status:    "pending",
...
func (m *AdminQuota) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("admin_quota: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_quota: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_quota: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_quota: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_quota: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_quota: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_quota: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_quota: version must be at least 1")
	}
	return nil
}

func (m *AdminQuota) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_quota: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminQuota) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_quota: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminQuota) Clone() *AdminQuota {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminQuota) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_quota: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_quota: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminQuota) Sanitize() {
...
func (m *AdminQuota) Merge(other *AdminQuota) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminQuota) String() string {
	return fmt.Sprintf("AdminQuota[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminQuota) MatchesFilter(f *AdminQuotaFilter) bool {
...
func FilterAdminQuotaList(items []*AdminQuota, f *AdminQuotaFilter) *AdminQuotaListResponse {
	var filtered []*AdminQuota
...
func CalculateAdminQuotaStats(items []*AdminQuota) *AdminQuotaStats {
	stats := &AdminQuotaStats{}
	if len(items) == 0 {
		return stats
	}
