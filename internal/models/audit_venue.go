...
type AuditVenueListResponse struct {
	Data       []*AuditVenue `json:"data"`
...
func NewAuditVenue(name string) *AuditVenue {
	now := time.Now()
	return &AuditVenue{
		Name:      name,
		Status:    "pending",
...
func (m *AuditVenue) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("audit_venue: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("audit_venue: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("audit_venue: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("audit_venue: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("audit_venue: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("audit_venue: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("audit_venue: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("audit_venue: version must be at least 1")
	}
	return nil
}

func (m *AuditVenue) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("audit_venue: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuditVenue) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("audit_venue: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuditVenue) Clone() *AuditVenue {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuditVenue) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("audit_venue: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("audit_venue: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuditVenue) Sanitize() {
...
func (m *AuditVenue) Merge(other *AuditVenue) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuditVenue) String() string {
	return fmt.Sprintf("AuditVenue[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuditVenue) MatchesFilter(f *AuditVenueFilter) bool {
...
func FilterAuditVenueList(items []*AuditVenue, f *AuditVenueFilter) *AuditVenueListResponse {
	var filtered []*AuditVenue
...
func CalculateAuditVenueStats(items []*AuditVenue) *AuditVenueStats {
	stats := &AuditVenueStats{}
	if len(items) == 0 {
		return stats
	}
