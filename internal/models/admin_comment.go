...
func NewAdminComment(name string) *AdminComment {
	now := time.Now()
	return &AdminComment{
		Name:      name,
		Status:    "pending",
...
func (m *AdminComment) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("admin_comment: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_comment: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_comment: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_comment: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_comment: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_comment: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_comment: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_comment: version must be at least 1")
	}
	return nil
}

func (m *AdminComment) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_comment: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminComment) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_comment: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminComment) Clone() *AdminComment {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminComment) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_comment: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_comment: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminComment) Sanitize() {
...
func (m *AdminComment) Merge(other *AdminComment) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminComment) String() string {
	return fmt.Sprintf("AdminComment[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminComment) MatchesFilter(f *AdminCommentFilter) bool {
...
func FilterAdminCommentList(items []*AdminComment, f *AdminCommentFilter) *AdminCommentListResponse {
	var filtered []*AdminComment
...
func CalculateAdminCommentStats(items []*AdminComment) *AdminCommentStats {
