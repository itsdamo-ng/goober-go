...
type CoreBookingListResponse struct {
	Data       []*CoreBooking `json:"data"`
...
func NewCoreBooking(name string) *CoreBooking {
	now := time.Now()
	return &CoreBooking{
		Name:      name,
		Status:    "pending",
...
func (m *CoreBooking) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("core_booking: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("core_booking: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("core_booking: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("core_booking: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("core_booking: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("core_booking: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("core_booking: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("core_booking: version must be at least 1")
	}
	return nil
}

func (m *CoreBooking) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("core_booking: marshal failed: %w", err)
	}
	return data, nil
}

func (m *CoreBooking) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("core_booking: unmarshal failed: %w", err)
	}
	return nil
}

func (m *CoreBooking) Clone() *CoreBooking {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *CoreBooking) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("core_booking: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("core_booking: transition from %q to %q not allowed", m.Status, status)
}

func (m *CoreBooking) Sanitize() {
...
func (m *CoreBooking) Merge(other *CoreBooking) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *CoreBooking) String() string {
	return fmt.Sprintf("CoreBooking[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *CoreBooking) MatchesFilter(f *CoreBookingFilter) bool {
...
func FilterCoreBookingList(items []*CoreBooking, f *CoreBookingFilter) *CoreBookingListResponse {
	var filtered []*CoreBooking
	...
	return &CoreBookingListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateCoreBookingStats(items []*CoreBooking) *CoreBookingStats {
	stats := &CoreBookingStats{}
	if len(items) == 0 {
		return stats
	}
