...
func (m *Employee) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("employee: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("employee: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("employee: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("employee: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("employee: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("employee: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("employee: version must be at least 1")
	}
	return nil
}

func (m *Employee) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("employee: marshal failed: %w", err)
	}
	return data, nil
}

func (m *Employee) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("employee: unmarshal failed: %w", err)
	}
	return nil
}
...
func (m *Employee) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("employee: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("employee: transition from %q to %q not allowed", m.Status, status)
}

func (m *Employee) Sanitize() {
...
func (m *Employee) Merge(other *Employee) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Employee) String() string {
	return fmt.Sprintf("Employee[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Employee) MatchesFilter(f *EmployeeFilter) bool {
...
func FilterEmployeeList(items []*Employee, f *EmployeeFilter) *EmployeeListResponse {
...
func CalculateEmployeeStats(items []*Employee) *EmployeeStats {
	stats := &EmployeeStats{}
	if len(items) == 0 {
		return stats
	}
