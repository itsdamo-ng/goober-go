...
type BatchRuleListResponse struct {
	Data       []*BatchRule `json:"data"`
...
func NewBatchRule(name string) *BatchRule {
	now := time.Now()
	return &BatchRule{
		Name:      name,
		Status:    "pending",
...
func (m *BatchRule) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("batch_rule: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("batch_rule: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("batch_rule: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("batch_rule: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("batch_rule: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("batch_rule: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("batch_rule: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("batch_rule: version must be at least 1")
	}
	return nil
}

func (m *BatchRule) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("batch_rule: marshal failed: %w", err)
	}
	return data, nil
}

func (m *BatchRule) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("batch_rule: unmarshal failed: %w", err)
	}
	return nil
}

func (m *BatchRule) Clone() *BatchRule {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *BatchRule) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("batch_rule: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("batch_rule: transition from %q to %q not allowed", m.Status, status)
}

func (m *BatchRule) Sanitize() {
...
func (m *BatchRule) Merge(other *BatchRule) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *BatchRule) String() string {
	return fmt.Sprintf("BatchRule[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *BatchRule) MatchesFilter(f *BatchRuleFilter) bool {
...
func FilterBatchRuleList(items []*BatchRule, f *BatchRuleFilter) *BatchRuleListResponse {
	var filtered []*BatchRule
...
func CalculateBatchRuleStats(items []*BatchRule) *BatchRuleStats {
	stats := &BatchRuleStats{}
	if len(items) == 0 {
		return stats
	}
