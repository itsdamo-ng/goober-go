...
type ImportReviewFilter struct {
	IDs       []int64   `json:"ids"`
...
func NewImportReview(name string) *ImportReview {
	now := time.Now()
	return &ImportReview{
		Name:      name,
		Status:    "pending",
...
func (m *ImportReview) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("import_review: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("import_review: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("import_review: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("import_review: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("import_review: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("import_review: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("import_review: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("import_review: version must be at least 1")
	}
	return nil
}

func (m *ImportReview) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("import_review: marshal failed: %w", err)
	}
	return data, nil
}

func (m *ImportReview) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("import_review: unmarshal failed: %w", err)
	}
	return nil
}

func (m *ImportReview) Clone() *ImportReview {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *ImportReview) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("import_review: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("import_review: transition from %q to %q not allowed", m.Status, status)
}

func (m *ImportReview) Sanitize() {
...
func (m *ImportReview) Merge(other *ImportReview) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *ImportReview) String() string {
	return fmt.Sprintf("ImportReview[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *ImportReview) MatchesFilter(f *ImportReviewFilter) bool {
...
func FilterImportReviewList(items []*ImportReview, f *ImportReviewFilter) *ImportReviewListResponse {
	var filtered []*ImportReview
...
func CalculateImportReviewStats(items []*ImportReview) *ImportReviewStats {
