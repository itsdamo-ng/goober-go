...
type ExportReservationFilter struct {
	IDs       []int64   `json:"ids"`
...
type ExportReservationListResponse struct {
	Data       []*ExportReservation `json:"data"`
...
type ExportReservationStats struct {
...
func NewExportReservation(name string) *ExportReservation {
	now := time.Now()
...
func (m *ExportReservation) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("export_reservation: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("export_reservation: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("export_reservation: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("export_reservation: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("export_reservation: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("export_reservation: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("export_reservation: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("export_reservation: version must be at least 1")
	}
	return nil
}

func (m *ExportReservation) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("export_reservation: marshal failed: %w", err)
	}
	return data, nil
}

func (m *ExportReservation) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("export_reservation: unmarshal failed: %w", err)
	}
	return nil
}

func (m *ExportReservation) Clone() *ExportReservation {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *ExportReservation) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("export_reservation: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("export_reservation: transition from %q to %q not allowed", m.Status, status)
}

func (m *ExportReservation) Sanitize() {
...
func (m *ExportReservation) Merge(other *ExportReservation) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *ExportReservation) String() string {
	return fmt.Sprintf("ExportReservation[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *ExportReservation) MatchesFilter(f *ExportReservationFilter) bool {
...
func FilterExportReservationList(items []*ExportReservation, f *ExportReservationFilter) *ExportReservationListResponse {
	var filtered []*ExportReservation
	...
	return &ExportReservationListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateExportReservationStats(items []*ExportReservation) *ExportReservationStats {
	stats := &ExportReservationStats{}
	if len(items) == 0 {
		return stats
	}
