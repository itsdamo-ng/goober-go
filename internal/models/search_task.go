...
func NewSearchTask(name string) *SearchTask {
	now := time.Now()
	return &SearchTask{
		Name:      name,
		Status:    "pending",
...
func (m *SearchTask) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("search_task: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("search_task: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("search_task: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("search_task: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("search_task: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("search_task: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("search_task: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("search_task: version must be at least 1")
	}
	return nil
}

func (m *SearchTask) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("search_task: marshal failed: %w", err)
	}
	return data, nil
}

func (m *SearchTask) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("search_task: unmarshal failed: %w", err)
	}
	return nil
}

func (m *SearchTask) Clone() *SearchTask {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *SearchTask) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("search_task: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("search_task: transition from %q to %q not allowed", m.Status, status)
}

func (m *SearchTask) Sanitize() {
...
func (m *SearchTask) Merge(other *SearchTask) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *SearchTask) String() string {
	return fmt.Sprintf("SearchTask[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *SearchTask) MatchesFilter(f *SearchTaskFilter) bool {
...
func FilterSearchTaskList(items []*SearchTask, f *SearchTaskFilter) *SearchTaskListResponse {
...
func CalculateSearchTaskStats(items []*SearchTask) *SearchTaskStats {
