...
func (m *Lead) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("lead: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("lead: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("lead: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("lead: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("lead: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("lead: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("lead: version must be at least 1")
	}
	return nil
}

func (m *Lead) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("lead: marshal failed: %w", err)
	}
	return data, nil
}

func (m *Lead) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("lead: unmarshal failed: %w", err)
	}
	return nil
}

func (m *Lead) Clone() *Lead {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *Lead) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("lead: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("lead: transition from %q to %q not allowed", m.Status, status)
}

func (m *Lead) Sanitize() {
...
func (m *Lead) String() string {
	return fmt.Sprintf("Lead[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Lead) MatchesFilter(f *LeadFilter) bool {
...
func FilterLeadList(items []*Lead, f *LeadFilter) *LeadListResponse {
	var filtered []*Lead
...
func CalculateLeadStats(items []*Lead) *LeadStats {
	stats := &LeadStats{}
	if len(items) == 0 {
		return stats
	}
