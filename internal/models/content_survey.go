...
type ContentSurveyFilter struct {
	IDs       []int64   `json:"ids"`
...
type ContentSurveyListResponse struct {
	Data       []*ContentSurvey `json:"data"`
...
type ContentSurveyStats struct {
...
func NewContentSurvey(name string) *ContentSurvey {
	now := time.Now()
...
func (m *ContentSurvey) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("content_survey: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("content_survey: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("content_survey: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("content_survey: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("content_survey: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("content_survey: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("content_survey: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("content_survey: version must be at least 1")
	}
	return nil
}

func (m *ContentSurvey) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("content_survey: marshal failed: %w", err)
	}
	return data, nil
}

func (m *ContentSurvey) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("content_survey: unmarshal failed: %w", err)
	}
	return nil
}

func (m *ContentSurvey) Clone() *ContentSurvey {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *ContentSurvey) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("content_survey: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("content_survey: transition from %q to %q not allowed", m.Status, status)
}

func (m *ContentSurvey) Sanitize() {
...
func (m *ContentSurvey) Merge(other *ContentSurvey) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *ContentSurvey) String() string {
	return fmt.Sprintf("ContentSurvey[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *ContentSurvey) MatchesFilter(f *ContentSurveyFilter) bool {
...
func FilterContentSurveyList(items []*ContentSurvey, f *ContentSurveyFilter) *ContentSurveyListResponse {
	var filtered []*ContentSurvey
	...
	return &ContentSurveyListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateContentSurveyStats(items []*ContentSurvey) *ContentSurveyStats {
