...
type BillingInvoiceListResponse struct {
...
type BillingInvoiceStats struct {
...
func NewBillingInvoice(name string) *BillingInvoice {
	now := time.Now()
...
func (m *BillingInvoice) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("billing_invoice: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("billing_invoice: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("billing_invoice: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("billing_invoice: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("billing_invoice: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("billing_invoice: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("billing_invoice: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("billing_invoice: version must be at least 1")
	}
	return nil
}

func (m *BillingInvoice) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("billing_invoice: marshal failed: %w", err)
	}
	return data, nil
}

func (m *BillingInvoice) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("billing_invoice: unmarshal failed: %w", err)
	}
	return nil
}

func (m *BillingInvoice) Clone() *BillingInvoice {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *BillingInvoice) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("billing_invoice: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("billing_invoice: transition from %q to %q not allowed", m.Status, status)
}

func (m *BillingInvoice) Sanitize() {
...
func (m *BillingInvoice) Merge(other *BillingInvoice) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *BillingInvoice) String() string {
	return fmt.Sprintf("BillingInvoice[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *BillingInvoice) MatchesFilter(f *BillingInvoiceFilter) bool {
...
func FilterBillingInvoiceList(items []*BillingInvoice, f *BillingInvoiceFilter) *BillingInvoiceListResponse {
	var filtered []*BillingInvoice
...
func CalculateBillingInvoiceStats(items []*BillingInvoice) *BillingInvoiceStats {
	stats := &BillingInvoiceStats{}
	if len(items) == 0 {
		return stats
	}
