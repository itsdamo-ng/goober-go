...
type CoreEventListResponse struct {
	Data       []*CoreEvent `json:"data"`
...
func NewCoreEvent(name string) *CoreEvent {
	now := time.Now()
	return &CoreEvent{
		Name:      name,
		Status:    "pending",
...
func (m *CoreEvent) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("core_event: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("core_event: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("core_event: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("core_event: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("core_event: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("core_event: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("core_event: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("core_event: version must be at least 1")
	}
	return nil
}

func (m *CoreEvent) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("core_event: marshal failed: %w", err)
	}
	return data, nil
}

func (m *CoreEvent) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("core_event: unmarshal failed: %w", err)
	}
	return nil
}

func (m *CoreEvent) Clone() *CoreEvent {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *CoreEvent) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("core_event: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("core_event: transition from %q to %q not allowed", m.Status, status)
}

func (m *CoreEvent) Sanitize() {
...
func (m *CoreEvent) Merge(other *CoreEvent) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *CoreEvent) String() string {
	return fmt.Sprintf("CoreEvent[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *CoreEvent) MatchesFilter(f *CoreEventFilter) bool {
...
func FilterCoreEventList(items []*CoreEvent, f *CoreEventFilter) *CoreEventListResponse {
	var filtered []*CoreEvent
...
func CalculateCoreEventStats(items []*CoreEvent) *CoreEventStats {
	stats := &CoreEventStats{}
	if len(items) == 0 {
		return stats
	}
