...
type AuditAlertListResponse struct {
	Data       []*AuditAlert `json:"data"`
...
func NewAuditAlert(name string) *AuditAlert {
	now := time.Now()
	return &AuditAlert{
		Name:      name,
		Status:    "pending",
...
func (m *AuditAlert) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("audit_alert: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("audit_alert: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("audit_alert: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("audit_alert: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("audit_alert: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("audit_alert: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("audit_alert: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("audit_alert: version must be at least 1")
	}
	return nil
}

func (m *AuditAlert) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("audit_alert: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuditAlert) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("audit_alert: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuditAlert) Clone() *AuditAlert {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuditAlert) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("audit_alert: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("audit_alert: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuditAlert) Sanitize() {
...
func (m *AuditAlert) Merge(other *AuditAlert) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuditAlert) String() string {
	return fmt.Sprintf("AuditAlert[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuditAlert) MatchesFilter(f *AuditAlertFilter) bool {
...
func FilterAuditAlertList(items []*AuditAlert, f *AuditAlertFilter) *AuditAlertListResponse {
	var filtered []*AuditAlert
...
func CalculateAuditAlertStats(items []*AuditAlert) *AuditAlertStats {
	stats := &AuditAlertStats{}
	if len(items) == 0 {
		return stats
	}
