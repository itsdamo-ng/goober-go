...
type CoreFeatureListResponse struct {
	Data       []*CoreFeature `json:"data"`
...
func NewCoreFeature(name string) *CoreFeature {
	now := time.Now()
	return &CoreFeature{
		Name:      name,
		Status:    "pending",
...
func (m *CoreFeature) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("core_feature: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("core_feature: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("core_feature: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("core_feature: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("core_feature: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("core_feature: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("core_feature: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("core_feature: version must be at least 1")
	}
	return nil
}

func (m *CoreFeature) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("core_feature: marshal failed: %w", err)
	}
	return data, nil
}

func (m *CoreFeature) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("core_feature: unmarshal failed: %w", err)
	}
	return nil
}

func (m *CoreFeature) Clone() *CoreFeature {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *CoreFeature) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("core_feature: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("core_feature: transition from %q to %q not allowed", m.Status, status)
}

func (m *CoreFeature) Sanitize() {
...
func (m *CoreFeature) Merge(other *CoreFeature) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *CoreFeature) String() string {
	return fmt.Sprintf("CoreFeature[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *CoreFeature) MatchesFilter(f *CoreFeatureFilter) bool {
...
func FilterCoreFeatureList(items []*CoreFeature, f *CoreFeatureFilter) *CoreFeatureListResponse {
	var filtered []*CoreFeature
	...
	return &CoreFeatureListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateCoreFeatureStats(items []*CoreFeature) *CoreFeatureStats {
	stats := &CoreFeatureStats{}
	if len(items) == 0 {
		return stats
	}
