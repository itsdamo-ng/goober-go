...
func NewImportLead(name string) *ImportLead {
	now := time.Now()
	return &ImportLead{
		Name:      name,
		Status:    "pending",
...
func (m *ImportLead) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("import_lead: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("import_lead: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("import_lead: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("import_lead: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("import_lead: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("import_lead: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("import_lead: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("import_lead: version must be at least 1")
	}
	return nil
}

func (m *ImportLead) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("import_lead: marshal failed: %w", err)
	}
	return data, nil
}

func (m *ImportLead) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("import_lead: unmarshal failed: %w", err)
	}
	return nil
}

func (m *ImportLead) Clone() *ImportLead {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *ImportLead) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("import_lead: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("import_lead: transition from %q to %q not allowed", m.Status, status)
}

func (m *ImportLead) Sanitize() {
...
func (m *ImportLead) Merge(other *ImportLead) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *ImportLead) String() string {
	return fmt.Sprintf("ImportLead[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *ImportLead) MatchesFilter(f *ImportLeadFilter) bool {
...
func FilterImportLeadList(items []*ImportLead, f *ImportLeadFilter) *ImportLeadListResponse {
	var filtered []*ImportLead
...
func CalculateImportLeadStats(items []*ImportLead) *ImportLeadStats {
	stats := &ImportLeadStats{}
	if len(items) == 0 {
		return stats
	}
