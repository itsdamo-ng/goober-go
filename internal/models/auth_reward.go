...
type AuthRewardListResponse struct {
	Data       []*AuthReward `json:"data"`
...
func NewAuthReward(name string) *AuthReward {
	now := time.Now()
	return &AuthReward{
		Name:      name,
		Status:    "pending",
...
func (m *AuthReward) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("auth_reward: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("auth_reward: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("auth_reward: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("auth_reward: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("auth_reward: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("auth_reward: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("auth_reward: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("auth_reward: version must be at least 1")
	}
	return nil
}

func (m *AuthReward) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("auth_reward: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuthReward) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("auth_reward: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuthReward) Clone() *AuthReward {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuthReward) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("auth_reward: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("auth_reward: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuthReward) Sanitize() {
...
func (m *AuthReward) Merge(other *AuthReward) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuthReward) String() string {
	return fmt.Sprintf("AuthReward[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuthReward) MatchesFilter(f *AuthRewardFilter) bool {
...
func FilterAuthRewardList(items []*AuthReward, f *AuthRewardFilter) *AuthRewardListResponse {
	var filtered []*AuthReward
...
func CalculateAuthRewardStats(items []*AuthReward) *AuthRewardStats {
	stats := &AuthRewardStats{}
	if len(items) == 0 {
		return stats
	}
