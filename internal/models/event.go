...
func (m *Event) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("event: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("event: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("event: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("event: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("event: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("event: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("event: version must be at least 1")
	}
	return nil
}
...
func (m *Event) FromJSON(data []byte) error {
...
func (m *Event) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("event: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("event: transition from %q to %q not allowed", m.Status, status)
}

func (m *Event) Sanitize() {
...
func (m *Event) Merge(other *Event) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Event) String() string {
	return fmt.Sprintf("Event[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Event) MatchesFilter(f *EventFilter) bool {
...
func FilterEventList(items []*Event, f *EventFilter) *EventListResponse {
	var filtered []*Event
...
func CalculateEventStats(items []*Event) *EventStats {
	stats := &EventStats{}
	if len(items) == 0 {
		return stats
	}
