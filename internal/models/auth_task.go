...
type AuthTaskListResponse struct {
	Data       []*AuthTask `json:"data"`
...
func NewAuthTask(name string) *AuthTask {
	now := time.Now()
	return &AuthTask{
		Name:      name,
		Status:    "pending",
...
func (m *AuthTask) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("auth_task: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("auth_task: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("auth_task: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("auth_task: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("auth_task: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("auth_task: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("auth_task: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("auth_task: version must be at least 1")
	}
	return nil
}

func (m *AuthTask) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("auth_task: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuthTask) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("auth_task: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuthTask) Clone() *AuthTask {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuthTask) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("auth_task: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("auth_task: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuthTask) Sanitize() {
...
func (m *AuthTask) Merge(other *AuthTask) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuthTask) String() string {
	return fmt.Sprintf("AuthTask[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuthTask) MatchesFilter(f *AuthTaskFilter) bool {
...
func FilterAuthTaskList(items []*AuthTask, f *AuthTaskFilter) *AuthTaskListResponse {
	var filtered []*AuthTask
...
func CalculateAuthTaskStats(items []*AuthTask) *AuthTaskStats {
	stats := &AuthTaskStats{}
	if len(items) == 0 {
		return stats
	}
