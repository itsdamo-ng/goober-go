...
type ContentEventListResponse struct {
	Data       []*ContentEvent `json:"data"`
...
func NewContentEvent(name string) *ContentEvent {
	now := time.Now()
	return &ContentEvent{
		Name:      name,
		Status:    "pending",
...
func (m *ContentEvent) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("content_event: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("content_event: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("content_event: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("content_event: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("content_event: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("content_event: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("content_event: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("content_event: version must be at least 1")
	}
	return nil
}

func (m *ContentEvent) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("content_event: marshal failed: %w", err)
	}
	return data, nil
}

func (m *ContentEvent) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("content_event: unmarshal failed: %w", err)
	}
	return nil
}

func (m *ContentEvent) Clone() *ContentEvent {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *ContentEvent) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("content_event: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("content_event: transition from %q to %q not allowed", m.Status, status)
}

func (m *ContentEvent) Sanitize() {
...
func (m *ContentEvent) Merge(other *ContentEvent) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *ContentEvent) String() string {
	return fmt.Sprintf("ContentEvent[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *ContentEvent) MatchesFilter(f *ContentEventFilter) bool {
...
func FilterContentEventList(items []*ContentEvent, f *ContentEventFilter) *ContentEventListResponse {
	var filtered []*ContentEvent
	...
	return &ContentEventListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateContentEventStats(items []*ContentEvent) *ContentEventStats {
