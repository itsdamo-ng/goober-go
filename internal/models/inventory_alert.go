...
type InventoryAlertStats struct {
...
func NewInventoryAlert(name string) *InventoryAlert {
	now := time.Now()
...
func (m *InventoryAlert) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("inventory_alert: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("inventory_alert: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("inventory_alert: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("inventory_alert: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("inventory_alert: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("inventory_alert: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("inventory_alert: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("inventory_alert: version must be at least 1")
	}
	return nil
}

func (m *InventoryAlert) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("inventory_alert: marshal failed: %w", err)
	}
	return data, nil
}

func (m *InventoryAlert) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("inventory_alert: unmarshal failed: %w", err)
	}
	return nil
}

func (m *InventoryAlert) Clone() *InventoryAlert {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *InventoryAlert) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("inventory_alert: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("inventory_alert: transition from %q to %q not allowed", m.Status, status)
}

func (m *InventoryAlert) Sanitize() {
...
func (m *InventoryAlert) Merge(other *InventoryAlert) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *InventoryAlert) String() string {
	return fmt.Sprintf("InventoryAlert[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *InventoryAlert) MatchesFilter(f *InventoryAlertFilter) bool {
...
func FilterInventoryAlertList(items []*InventoryAlert, f *InventoryAlertFilter) *InventoryAlertListResponse {
	var filtered []*InventoryAlert
...
func CalculateInventoryAlertStats(items []*InventoryAlert) *InventoryAlertStats {
	stats := &InventoryAlertStats{}
	if len(items) == 0 {
		return stats
	}
