...
type BatchTeamListResponse struct {
	Data       []*BatchTeam `json:"data"`
...
func NewBatchTeam(name string) *BatchTeam {
	now := time.Now()
	return &BatchTeam{
		Name:      name,
		Status:    "pending",
...
func (m *BatchTeam) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("batch_team: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("batch_team: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("batch_team: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("batch_team: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("batch_team: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("batch_team: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("batch_team: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("batch_team: version must be at least 1")
	}
	return nil
}

func (m *BatchTeam) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("batch_team: marshal failed: %w", err)
	}
	return data, nil
}

func (m *BatchTeam) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("batch_team: unmarshal failed: %w", err)
	}
	return nil
}

func (m *BatchTeam) Clone() *BatchTeam {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *BatchTeam) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("batch_team: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("batch_team: transition from %q to %q not allowed", m.Status, status)
}

func (m *BatchTeam) Sanitize() {
...
func (m *BatchTeam) Merge(other *BatchTeam) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *BatchTeam) String() string {
	return fmt.Sprintf("BatchTeam[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *BatchTeam) MatchesFilter(f *BatchTeamFilter) bool {
...
func FilterBatchTeamList(items []*BatchTeam, f *BatchTeamFilter) *BatchTeamListResponse {
	var filtered []*BatchTeam
...
func CalculateBatchTeamStats(items []*BatchTeam) *BatchTeamStats {
	stats := &BatchTeamStats{}
	if len(items) == 0 {
		return stats
	}
