...
type SearchCommentFilter struct {
	IDs       []int64   `json:"ids"`
...
type SearchCommentListResponse struct {
	Data       []*SearchComment `json:"data"`
...
type SearchCommentStats struct {
...
func NewSearchComment(name string) *SearchComment {
	now := time.Now()
...
func (m *SearchComment) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("search_comment: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("search_comment: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("search_comment: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("search_comment: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("search_comment: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("search_comment: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("search_comment: version must be at least 1")
	}
	return nil
}

func (m *SearchComment) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("search_comment: marshal failed: %w", err)
	}
	return data, nil
}

func (m *SearchComment) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("search_comment: unmarshal failed: %w", err)
	}
	return nil
}

func (m *SearchComment) Clone() *SearchComment {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *SearchComment) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("search_comment: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("search_comment: transition from %q to %q not allowed", m.Status, status)
}

func (m *SearchComment) Sanitize() {
...
func (m *SearchComment) Merge(other *SearchComment) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *SearchComment) String() string {
	return fmt.Sprintf("SearchComment[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *SearchComment) MatchesFilter(f *SearchCommentFilter) bool {
...
func FilterSearchCommentList(items []*SearchComment, f *SearchCommentFilter) *SearchCommentListResponse {
	var filtered []*SearchComment
	...
	return &SearchCommentListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateSearchCommentStats(items []*SearchComment) *SearchCommentStats {
	stats := &SearchCommentStats{}
	if len(items) == 0 {
		return stats
	}
