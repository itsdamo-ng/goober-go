...
type CoreVenueListResponse struct {
	Data       []*CoreVenue `json:"data"`
...
func NewCoreVenue(name string) *CoreVenue {
	now := time.Now()
	return &CoreVenue{
		Name:      name,
		Status:    "pending",
...
func (m *CoreVenue) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("core_venue: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("core_venue: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("core_venue: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("core_venue: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("core_venue: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("core_venue: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("core_venue: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("core_venue: version must be at least 1")
	}
	return nil
}

func (m *CoreVenue) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("core_venue: marshal failed: %w", err)
	}
	return data, nil
}

func (m *CoreVenue) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("core_venue: unmarshal failed: %w", err)
	}
	return nil
}

func (m *CoreVenue) Clone() *CoreVenue {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *CoreVenue) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("core_venue: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("core_venue: transition from %q to %q not allowed", m.Status, status)
}

func (m *CoreVenue) Sanitize() {
...
func (m *CoreVenue) Merge(other *CoreVenue) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *CoreVenue) String() string {
	return fmt.Sprintf("CoreVenue[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *CoreVenue) MatchesFilter(f *CoreVenueFilter) bool {
...
func FilterCoreVenueList(items []*CoreVenue, f *CoreVenueFilter) *CoreVenueListResponse {
	var filtered []*CoreVenue
...
func CalculateCoreVenueStats(items []*CoreVenue) *CoreVenueStats {
	stats := &CoreVenueStats{}
	if len(items) == 0 {
		return stats
	}
