...
type AuditOrderListResponse struct {
	Data       []*AuditOrder `json:"data"`
...
func NewAuditOrder(name string) *AuditOrder {
	now := time.Now()
	return &AuditOrder{
		Name:      name,
		Status:    "pending",
...
func (m *AuditOrder) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("audit_order: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("audit_order: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("audit_order: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("audit_order: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("audit_order: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("audit_order: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("audit_order: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("audit_order: version must be at least 1")
	}
	return nil
}

func (m *AuditOrder) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("audit_order: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuditOrder) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("audit_order: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuditOrder) Clone() *AuditOrder {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuditOrder) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("audit_order: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("audit_order: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuditOrder) Sanitize() {
...
func (m *AuditOrder) Merge(other *AuditOrder) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuditOrder) String() string {
	return fmt.Sprintf("AuditOrder[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuditOrder) MatchesFilter(f *AuditOrderFilter) bool {
...
func FilterAuditOrderList(items []*AuditOrder, f *AuditOrderFilter) *AuditOrderListResponse {
	var filtered []*AuditOrder
...
func CalculateAuditOrderStats(items []*AuditOrder) *AuditOrderStats {
	stats := &AuditOrderStats{}
	if len(items) == 0 {
		return stats
	}
