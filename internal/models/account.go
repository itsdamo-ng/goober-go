...
func (m *Account) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("account: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("account: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("account: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("account: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("account: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("account: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("account: version must be at least 1")
	}
	return nil
}
...
func (m *Account) ToJSON() ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("account: marshal failed: %w", err)
	}
	return data, nil
}
...
func (m *Account) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("account: unmarshal failed: %w", err)
	}
	return nil
}
...
func (m *Account) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("account: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("account: transition from %q to %q not allowed", m.Status, status)
}

func (m *Account) Sanitize() {
...
func (m *Account) Merge(other *Account) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Account) String() string {
	return fmt.Sprintf("Account[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Account) MatchesFilter(f *AccountFilter) bool {
...
func FilterAccountList(items []*Account, f *AccountFilter) *AccountListResponse {
...
func CalculateAccountStats(items []*Account) *AccountStats {
	stats := &AccountStats{}
	if len(items) == 0 {
		return stats
	}
