...
func NewAdminSession(name string) *AdminSession {
	now := time.Now()
	return &AdminSession{
		Name:      name,
		Status:    "pending",
...
func (m *AdminSession) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("admin_session: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_session: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_session: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_session: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_session: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_session: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_session: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_session: version must be at least 1")
	}
	return nil
}

func (m *AdminSession) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_session: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminSession) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_session: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminSession) Clone() *AdminSession {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminSession) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_session: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_session: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminSession) Sanitize() {
...
func (m *AdminSession) Merge(other *AdminSession) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminSession) String() string {
	return fmt.Sprintf("AdminSession[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminSession) MatchesFilter(f *AdminSessionFilter) bool {
...
func FilterAdminSessionList(items []*AdminSession, f *AdminSessionFilter) *AdminSessionListResponse {
...
func CalculateAdminSessionStats(items []*AdminSession) *AdminSessionStats {
