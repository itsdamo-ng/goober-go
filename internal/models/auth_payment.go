...
type AuthPaymentListResponse struct {
	Data       []*AuthPayment `json:"data"`
...
func NewAuthPayment(name string) *AuthPayment {
	now := time.Now()
	return &AuthPayment{
		Name:      name,
		Status:    "pending",
...
func (m *AuthPayment) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("auth_payment: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("auth_payment: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("auth_payment: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("auth_payment: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("auth_payment: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("auth_payment: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("auth_payment: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("auth_payment: version must be at least 1")
	}
	return nil
}

func (m *AuthPayment) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("auth_payment: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuthPayment) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("auth_payment: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuthPayment) Clone() *AuthPayment {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuthPayment) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("auth_payment: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("auth_payment: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuthPayment) Sanitize() {
...
func (m *AuthPayment) Merge(other *AuthPayment) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuthPayment) String() string {
	return fmt.Sprintf("AuthPayment[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuthPayment) MatchesFilter(f *AuthPaymentFilter) bool {
...
func FilterAuthPaymentList(items []*AuthPayment, f *AuthPaymentFilter) *AuthPaymentListResponse {
	var filtered []*AuthPayment
	...
	return &AuthPaymentListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateAuthPaymentStats(items []*AuthPayment) *AuthPaymentStats {
	stats := &AuthPaymentStats{}
	if len(items) == 0 {
		return stats
	}
