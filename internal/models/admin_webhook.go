...
func NewAdminWebhook(name string) *AdminWebhook {
	now := time.Now()
	return &AdminWebhook{
		Name:      name,
		Status:    "pending",
...
func (m *AdminWebhook) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("admin_webhook: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_webhook: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_webhook: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_webhook: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_webhook: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_webhook: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_webhook: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_webhook: version must be at least 1")
	}
	return nil
}

func (m *AdminWebhook) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_webhook: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminWebhook) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_webhook: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminWebhook) Clone() *AdminWebhook {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminWebhook) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_webhook: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_webhook: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminWebhook) Sanitize() {
...
func (m *AdminWebhook) Merge(other *AdminWebhook) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminWebhook) String() string {
	return fmt.Sprintf("AdminWebhook[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminWebhook) MatchesFilter(f *AdminWebhookFilter) bool {
...
func FilterAdminWebhookList(items []*AdminWebhook, f *AdminWebhookFilter) *AdminWebhookListResponse {
	var filtered []*AdminWebhook
...
func CalculateAdminWebhookStats(items []*AdminWebhook) *AdminWebhookStats {
