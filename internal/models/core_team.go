...
type CoreTeamListResponse struct {
	Data       []*CoreTeam `json:"data"`
...
func NewCoreTeam(name string) *CoreTeam {
	now := time.Now()
	return &CoreTeam{
		Name:      name,
		Status:    "pending",
...
func (m *CoreTeam) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("core_team: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("core_team: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("core_team: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("core_team: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("core_team: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("core_team: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("core_team: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("core_team: version must be at least 1")
	}
	return nil
}

func (m *CoreTeam) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("core_team: marshal failed: %w", err)
	}
	return data, nil
}

func (m *CoreTeam) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("core_team: unmarshal failed: %w", err)
	}
	return nil
}

func (m *CoreTeam) Clone() *CoreTeam {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *CoreTeam) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("core_team: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("core_team: transition from %q to %q not allowed", m.Status, status)
}

func (m *CoreTeam) Sanitize() {
...
func (m *CoreTeam) Merge(other *CoreTeam) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *CoreTeam) String() string {
	return fmt.Sprintf("CoreTeam[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *CoreTeam) MatchesFilter(f *CoreTeamFilter) bool {
...
func FilterCoreTeamList(items []*CoreTeam, f *CoreTeamFilter) *CoreTeamListResponse {
	var filtered []*CoreTeam
...
func CalculateCoreTeamStats(items []*CoreTeam) *CoreTeamStats {
	stats := &CoreTeamStats{}
	if len(items) == 0 {
		return stats
	}
