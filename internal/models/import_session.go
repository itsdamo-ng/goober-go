...
type ImportSessionFilter struct {
	IDs       []int64   `json:"ids"`
...
type ImportSessionListResponse struct {
	Data       []*ImportSession `json:"data"`
...
type ImportSessionStats struct {
...
func NewImportSession(name string) *ImportSession {
	now := time.Now()
...
func (m *ImportSession) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("import_session: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("import_session: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("import_session: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("import_session: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("import_session: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("import_session: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("import_session: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("import_session: version must be at least 1")
	}
	return nil
}

func (m *ImportSession) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("import_session: marshal failed: %w", err)
	}
	return data, nil
}

func (m *ImportSession) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("import_session: unmarshal failed: %w", err)
	}
	return nil
}

func (m *ImportSession) Clone() *ImportSession {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *ImportSession) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("import_session: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("import_session: transition from %q to %q not allowed", m.Status, status)
}

func (m *ImportSession) Sanitize() {
...
func (m *ImportSession) Merge(other *ImportSession) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *ImportSession) String() string {
	return fmt.Sprintf("ImportSession[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *ImportSession) MatchesFilter(f *ImportSessionFilter) bool {
...
func FilterImportSessionList(items []*ImportSession, f *ImportSessionFilter) *ImportSessionListResponse {
	var filtered []*ImportSession
...
func CalculateImportSessionStats(items []*ImportSession) *ImportSessionStats {
