...
type SearchMessageListResponse struct {
	Data       []*SearchMessage `json:"data"`
...
type SearchMessageStats struct {
...
func NewSearchMessage(name string) *SearchMessage {
	now := time.Now()
...
func (m *SearchMessage) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("search_message: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("search_message: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("search_message: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("search_message: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("search_message: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("search_message: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("search_message: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("search_message: version must be at least 1")
	}
	return nil
}

func (m *SearchMessage) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("search_message: marshal failed: %w", err)
	}
	return data, nil
}

func (m *SearchMessage) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("search_message: unmarshal failed: %w", err)
	}
	return nil
}

func (m *SearchMessage) Clone() *SearchMessage {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *SearchMessage) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("search_message: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("search_message: transition from %q to %q not allowed", m.Status, status)
}

func (m *SearchMessage) Sanitize() {
...
func (m *SearchMessage) Merge(other *SearchMessage) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *SearchMessage) String() string {
	return fmt.Sprintf("SearchMessage[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *SearchMessage) MatchesFilter(f *SearchMessageFilter) bool {
...
func FilterSearchMessageList(items []*SearchMessage, f *SearchMessageFilter) *SearchMessageListResponse {
	var filtered []*SearchMessage
...
func CalculateSearchMessageStats(items []*SearchMessage) *SearchMessageStats {
	stats := &SearchMessageStats{}
	if len(items) == 0 {
		return stats
	}
