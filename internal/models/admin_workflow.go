...
type AdminWorkflowFilter struct {
	IDs       []int64   `json:"ids"`
...
type AdminWorkflowStats struct {
...
func NewAdminWorkflow(name string) *AdminWorkflow {
	now := time.Now()
...
func (m *AdminWorkflow) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("admin_workflow: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_workflow: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_workflow: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_workflow: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_workflow: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_workflow: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_workflow: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_workflow: version must be at least 1")
	}
	return nil
}

func (m *AdminWorkflow) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_workflow: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminWorkflow) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_workflow: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminWorkflow) Clone() *AdminWorkflow {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminWorkflow) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_workflow: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_workflow: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminWorkflow) Sanitize() {
...
func (m *AdminWorkflow) Merge(other *AdminWorkflow) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminWorkflow) String() string {
	return fmt.Sprintf("AdminWorkflow[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminWorkflow) MatchesFilter(f *AdminWorkflowFilter) bool {
...
func FilterAdminWorkflowList(items []*AdminWorkflow, f *AdminWorkflowFilter) *AdminWorkflowListResponse {
	var filtered []*AdminWorkflow
...
func CalculateAdminWorkflowStats(items []*AdminWorkflow) *AdminWorkflowStats {
