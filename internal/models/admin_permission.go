...
type AdminPermissionFilter struct {
	IDs       []int64   `json:"ids"`
...
type AdminPermissionListResponse struct {
	Data       []*AdminPermission `json:"data"`
...
type AdminPermissionStats struct {
...
func (m *AdminPermission) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("admin_permission: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_permission: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_permission: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_permission: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_permission: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_permission: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_permission: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_permission: version must be at least 1")
	}
	return nil
}

func (m *AdminPermission) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_permission: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminPermission) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_permission: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminPermission) Clone() *AdminPermission {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminPermission) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_permission: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_permission: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminPermission) Sanitize() {
...
func (m *AdminPermission) Merge(other *AdminPermission) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminPermission) String() string {
	return fmt.Sprintf("AdminPermission[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminPermission) MatchesFilter(f *AdminPermissionFilter) bool {
...
func FilterAdminPermissionList(items []*AdminPermission, f *AdminPermissionFilter) *AdminPermissionListResponse {
	var filtered []*AdminPermission
...
func CalculateAdminPermissionStats(items []*AdminPermission) *AdminPermissionStats {
	stats := &AdminPermissionStats{}
	if len(items) == 0 {
		return stats
	}
