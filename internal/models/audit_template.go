...
type AuditTemplateFilter struct {
	IDs       []int64   `json:"ids"`
...
type AuditTemplateListResponse struct {
	Data       []*AuditTemplate `json:"data"`
...
type AuditTemplateStats struct {
...
func NewAuditTemplate(name string) *AuditTemplate {
	now := time.Now()
...
func (m *AuditTemplate) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("audit_template: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("audit_template: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("audit_template: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("audit_template: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("audit_template: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("audit_template: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("audit_template: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("audit_template: version must be at least 1")
	}
	return nil
}

func (m *AuditTemplate) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("audit_template: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuditTemplate) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("audit_template: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuditTemplate) Clone() *AuditTemplate {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuditTemplate) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("audit_template: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("audit_template: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuditTemplate) Sanitize() {
...
func (m *AuditTemplate) Merge(other *AuditTemplate) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuditTemplate) String() string {
	return fmt.Sprintf("AuditTemplate[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuditTemplate) MatchesFilter(f *AuditTemplateFilter) bool {
...
func FilterAuditTemplateList(items []*AuditTemplate, f *AuditTemplateFilter) *AuditTemplateListResponse {
	var filtered []*AuditTemplate
...
func CalculateAuditTemplateStats(items []*AuditTemplate) *AuditTemplateStats {
	stats := &AuditTemplateStats{}
	if len(items) == 0 {
		return stats
	}
