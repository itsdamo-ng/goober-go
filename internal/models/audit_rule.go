...
type AuditRuleListResponse struct {
	Data       []*AuditRule `json:"data"`
...
func NewAuditRule(name string) *AuditRule {
	now := time.Now()
	return &AuditRule{
		Name:      name,
		Status:    "pending",
...
func (m *AuditRule) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("audit_rule: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("audit_rule: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("audit_rule: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("audit_rule: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("audit_rule: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("audit_rule: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("audit_rule: version must be at least 1")
	}
	return nil
}

func (m *AuditRule) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("audit_rule: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuditRule) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("audit_rule: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuditRule) Clone() *AuditRule {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuditRule) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("audit_rule: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("audit_rule: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuditRule) Sanitize() {
...
func (m *AuditRule) Merge(other *AuditRule) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuditRule) String() string {
	return fmt.Sprintf("AuditRule[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuditRule) MatchesFilter(f *AuditRuleFilter) bool {
...
func FilterAuditRuleList(items []*AuditRule, f *AuditRuleFilter) *AuditRuleListResponse {
	var filtered []*AuditRule
...
func CalculateAuditRuleStats(items []*AuditRule) *AuditRuleStats {
	stats := &AuditRuleStats{}
	if len(items) == 0 {
		return stats
	}
