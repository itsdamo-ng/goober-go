...
type CoreRuleListResponse struct {
	Data       []*CoreRule `json:"data"`
...
func NewCoreRule(name string) *CoreRule {
	now := time.Now()
	return &CoreRule{
		Name:      name,
		Status:    "pending",
...
func (m *CoreRule) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("core_rule: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("core_rule: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("core_rule: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("core_rule: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("core_rule: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("core_rule: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("core_rule: version must be at least 1")
	}
	return nil
}

func (m *CoreRule) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("core_rule: marshal failed: %w", err)
	}
	return data, nil
}

func (m *CoreRule) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("core_rule: unmarshal failed: %w", err)
	}
	return nil
}

func (m *CoreRule) Clone() *CoreRule {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *CoreRule) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("core_rule: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("core_rule: transition from %q to %q not allowed", m.Status, status)
}

func (m *CoreRule) Sanitize() {
...
func (m *CoreRule) Merge(other *CoreRule) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *CoreRule) String() string {
	return fmt.Sprintf("CoreRule[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *CoreRule) MatchesFilter(f *CoreRuleFilter) bool {
...
func FilterCoreRuleList(items []*CoreRule, f *CoreRuleFilter) *CoreRuleListResponse {
...
func CalculateCoreRuleStats(items []*CoreRule) *CoreRuleStats {
	stats := &CoreRuleStats{}
	if len(items) == 0 {
		return stats
	}
