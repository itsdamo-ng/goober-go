...
type AdminNotificationListResponse struct {
	Data       []*AdminNotification `json:"data"`
...
type AdminNotificationStats struct {
...
func NewAdminNotification(name string) *AdminNotification {
	now := time.Now()
...
func (m *AdminNotification) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("admin_notification: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_notification: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_notification: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_notification: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_notification: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_notification: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_notification: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_notification: version must be at least 1")
	}
	return nil
}

func (m *AdminNotification) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_notification: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminNotification) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_notification: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminNotification) Clone() *AdminNotification {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminNotification) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_notification: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_notification: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminNotification) Sanitize() {
...
func (m *AdminNotification) Merge(other *AdminNotification) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminNotification) String() string {
	return fmt.Sprintf("AdminNotification[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminNotification) MatchesFilter(f *AdminNotificationFilter) bool {
...
func FilterAdminNotificationList(items []*AdminNotification, f *AdminNotificationFilter) *AdminNotificationListResponse {
...
func CalculateAdminNotificationStats(items []*AdminNotification) *AdminNotificationStats {
