...
type AuditUserListResponse struct {
	Data       []*AuditUser `json:"data"`
...
func NewAuditUser(name string) *AuditUser {
	now := time.Now()
	return &AuditUser{
		Name:      name,
		Status:    "pending",
...
func (m *AuditUser) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("audit_user: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("audit_user: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("audit_user: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("audit_user: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("audit_user: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("audit_user: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("audit_user: version must be at least 1")
	}
	return nil
}

func (m *AuditUser) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("audit_user: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuditUser) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("audit_user: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuditUser) Clone() *AuditUser {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuditUser) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("audit_user: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("audit_user: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuditUser) Sanitize() {
...
func (m *AuditUser) Merge(other *AuditUser) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuditUser) String() string {
	return fmt.Sprintf("AuditUser[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuditUser) MatchesFilter(f *AuditUserFilter) bool {
...
func FilterAuditUserList(items []*AuditUser, f *AuditUserFilter) *AuditUserListResponse {
	var filtered []*AuditUser
...
func CalculateAuditUserStats(items []*AuditUser) *AuditUserStats {
	stats := &AuditUserStats{}
	if len(items) == 0 {
		return stats
	}
