...
type AuditPolicyListResponse struct {
	Data       []*AuditPolicy `json:"data"`
...
func NewAuditPolicy(name string) *AuditPolicy {
	now := time.Now()
	return &AuditPolicy{
		Name:      name,
		Status:    "pending",
...
func (m *AuditPolicy) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("audit_policy: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("audit_policy: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("audit_policy: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("audit_policy: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("audit_policy: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("audit_policy: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("audit_policy: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("audit_policy: version must be at least 1")
	}
	return nil
}

func (m *AuditPolicy) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("audit_policy: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuditPolicy) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("audit_policy: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuditPolicy) Clone() *AuditPolicy {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuditPolicy) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("audit_policy: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("audit_policy: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuditPolicy) Sanitize() {
...
func (m *AuditPolicy) Merge(other *AuditPolicy) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuditPolicy) String() string {
	return fmt.Sprintf("AuditPolicy[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuditPolicy) MatchesFilter(f *AuditPolicyFilter) bool {
...
func FilterAuditPolicyList(items []*AuditPolicy, f *AuditPolicyFilter) *AuditPolicyListResponse {
	var filtered []*AuditPolicy
...
func CalculateAuditPolicyStats(items []*AuditPolicy) *AuditPolicyStats {
	stats := &AuditPolicyStats{}
	if len(items) == 0 {
		return stats
	}
