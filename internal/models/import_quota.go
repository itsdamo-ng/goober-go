...
type ImportQuotaListResponse struct {
	Data       []*ImportQuota `json:"data"`
...
func NewImportQuota(name string) *ImportQuota {
	now := time.Now()
	return &ImportQuota{
		Name:      name,
		Status:    "pending",
...
func (m *ImportQuota) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("import_quota: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("import_quota: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("import_quota: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("import_quota: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("import_quota: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("import_quota: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("import_quota: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("import_quota: version must be at least 1")
	}
	return nil
}

func (m *ImportQuota) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("import_quota: marshal failed: %w", err)
	}
	return data, nil
}

func (m *ImportQuota) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("import_quota: unmarshal failed: %w", err)
	}
	return nil
}

func (m *ImportQuota) Clone() *ImportQuota {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *ImportQuota) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("import_quota: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("import_quota: transition from %q to %q not allowed", m.Status, status)
}

func (m *ImportQuota) Sanitize() {
...
func (m *ImportQuota) Merge(other *ImportQuota) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *ImportQuota) String() string {
	return fmt.Sprintf("ImportQuota[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *ImportQuota) MatchesFilter(f *ImportQuotaFilter) bool {
...
func FilterImportQuotaList(items []*ImportQuota, f *ImportQuotaFilter) *ImportQuotaListResponse {
	var filtered []*ImportQuota
	...
	return &ImportQuotaListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateImportQuotaStats(items []*ImportQuota) *ImportQuotaStats {
	stats := &ImportQuotaStats{}
	if len(items) == 0 {
		return stats
	}
