...
type CoreUserListResponse struct {
	Data       []*CoreUser `json:"data"`
...
func NewCoreUser(name string) *CoreUser {
	now := time.Now()
	return &CoreUser{
		Name:      name,
		Status:    "pending",
...
func (m *CoreUser) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("core_user: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("core_user: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("core_user: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("core_user: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("core_user: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("core_user: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("core_user: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("core_user: version must be at least 1")
	}
	return nil
}

func (m *CoreUser) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("core_user: marshal failed: %w", err)
	}
	return data, nil
}

func (m *CoreUser) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("core_user: unmarshal failed: %w", err)
	}
	return nil
}

func (m *CoreUser) Clone() *CoreUser {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *CoreUser) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("core_user: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("core_user: transition from %q to %q not allowed", m.Status, status)
}

func (m *CoreUser) Sanitize() {
...
func (m *CoreUser) Merge(other *CoreUser) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *CoreUser) String() string {
	return fmt.Sprintf("CoreUser[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *CoreUser) MatchesFilter(f *CoreUserFilter) bool {
...
func FilterCoreUserList(items []*CoreUser, f *CoreUserFilter) *CoreUserListResponse {
	var filtered []*CoreUser
...
func CalculateCoreUserStats(items []*CoreUser) *CoreUserStats {
	stats := &CoreUserStats{}
	if len(items) == 0 {
		return stats
	}
