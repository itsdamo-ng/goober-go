...
type BatchMetricListResponse struct {
	Data       []*BatchMetric `json:"data"`
...
func NewBatchMetric(name string) *BatchMetric {
	now := time.Now()
	return &BatchMetric{
		Name:      name,
		Status:    "pending",
...
func (m *BatchMetric) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("batch_metric: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("batch_metric: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("batch_metric: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("batch_metric: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("batch_metric: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("batch_metric: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("batch_metric: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("batch_metric: version must be at least 1")
	}
	return nil
}

func (m *BatchMetric) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("batch_metric: marshal failed: %w", err)
	}
	return data, nil
}

func (m *BatchMetric) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("batch_metric: unmarshal failed: %w", err)
	}
	return nil
}

func (m *BatchMetric) Clone() *BatchMetric {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *BatchMetric) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("batch_metric: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("batch_metric: transition from %q to %q not allowed", m.Status, status)
}

func (m *BatchMetric) Sanitize() {
...
func (m *BatchMetric) Merge(other *BatchMetric) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *BatchMetric) String() string {
	return fmt.Sprintf("BatchMetric[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *BatchMetric) MatchesFilter(f *BatchMetricFilter) bool {
...
func FilterBatchMetricList(items []*BatchMetric, f *BatchMetricFilter) *BatchMetricListResponse {
	var filtered []*BatchMetric
	...
	return &BatchMetricListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateBatchMetricStats(items []*BatchMetric) *BatchMetricStats {
	stats := &BatchMetricStats{}
	if len(items) == 0 {
		return stats
	}
