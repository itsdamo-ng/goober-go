...
type InventoryRuleFilter struct {
	IDs       []int64   `json:"ids"`
...
type InventoryRuleListResponse struct {
	Data       []*InventoryRule `json:"data"`
...
type InventoryRuleStats struct {
...
func NewInventoryRule(name string) *InventoryRule {
	now := time.Now()
...
func (m *InventoryRule) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("inventory_rule: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("inventory_rule: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("inventory_rule: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("inventory_rule: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("inventory_rule: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("inventory_rule: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("inventory_rule: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("inventory_rule: version must be at least 1")
	}
	return nil
}

func (m *InventoryRule) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("inventory_rule: marshal failed: %w", err)
	}
	return data, nil
}

func (m *InventoryRule) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("inventory_rule: unmarshal failed: %w", err)
	}
	return nil
}

func (m *InventoryRule) Clone() *InventoryRule {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *InventoryRule) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("inventory_rule: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("inventory_rule: transition from %q to %q not allowed", m.Status, status)
}

func (m *InventoryRule) Sanitize() {
...
func (m *InventoryRule) Merge(other *InventoryRule) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *InventoryRule) String() string {
	return fmt.Sprintf("InventoryRule[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *InventoryRule) MatchesFilter(f *InventoryRuleFilter) bool {
...
func FilterInventoryRuleList(items []*InventoryRule, f *InventoryRuleFilter) *InventoryRuleListResponse {
	var filtered []*InventoryRule
	...
	return &InventoryRuleListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateInventoryRuleStats(items []*InventoryRule) *InventoryRuleStats {
	stats := &InventoryRuleStats{}
	if len(items) == 0 {
		return stats
	}
