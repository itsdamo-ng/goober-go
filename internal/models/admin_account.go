...
func NewAdminAccount(name string) *AdminAccount {
	now := time.Now()
	return &AdminAccount{
		Name:      name,
		Status:    "pending",
...
func (m *AdminAccount) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("admin_account: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_account: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_account: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_account: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_account: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_account: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_account: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_account: version must be at least 1")
	}
	return nil
}

func (m *AdminAccount) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_account: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminAccount) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_account: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminAccount) Clone() *AdminAccount {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminAccount) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_account: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_account: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminAccount) Sanitize() {
...
func (m *AdminAccount) Merge(other *AdminAccount) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminAccount) String() string {
	return fmt.Sprintf("AdminAccount[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminAccount) MatchesFilter(f *AdminAccountFilter) bool {
...
func FilterAdminAccountList(items []*AdminAccount, f *AdminAccountFilter) *AdminAccountListResponse {
	var filtered []*AdminAccount
...
func CalculateAdminAccountStats(items []*AdminAccount) *AdminAccountStats {
