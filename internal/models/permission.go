...
func (m *Permission) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("permission: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("permission: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("permission: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("permission: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("permission: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("permission: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("permission: version must be at least 1")
	}
	return nil
}

func (m *Permission) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("permission: marshal failed: %w", err)
	}
	return data, nil
}

func (m *Permission) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("permission: unmarshal failed: %w", err)
	}
	return nil
}
...
func (m *Permission) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("permission: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("permission: transition from %q to %q not allowed", m.Status, status)
}

func (m *Permission) Sanitize() {
...
func (m *Permission) Merge(other *Permission) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Permission) String() string {
	return fmt.Sprintf("Permission[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Permission) MatchesFilter(f *PermissionFilter) bool {
...
func FilterPermissionList(items []*Permission, f *PermissionFilter) *PermissionListResponse {
...
func CalculatePermissionStats(items []*Permission) *PermissionStats {
