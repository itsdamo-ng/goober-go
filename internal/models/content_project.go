...
type ContentProjectListResponse struct {
	Data       []*ContentProject `json:"data"`
...
type ContentProjectStats struct {
...
func NewContentProject(name string) *ContentProject {
	now := time.Now()
...
func (m *ContentProject) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("content_project: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("content_project: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("content_project: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("content_project: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("content_project: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("content_project: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("content_project: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("content_project: version must be at least 1")
	}
	return nil
}

func (m *ContentProject) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("content_project: marshal failed: %w", err)
	}
	return data, nil
}

func (m *ContentProject) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("content_project: unmarshal failed: %w", err)
	}
	return nil
}

func (m *ContentProject) Clone() *ContentProject {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *ContentProject) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("content_project: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("content_project: transition from %q to %q not allowed", m.Status, status)
}

func (m *ContentProject) Sanitize() {
...
func (m *ContentProject) Merge(other *ContentProject) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *ContentProject) String() string {
	return fmt.Sprintf("ContentProject[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *ContentProject) MatchesFilter(f *ContentProjectFilter) bool {
...
func FilterContentProjectList(items []*ContentProject, f *ContentProjectFilter) *ContentProjectListResponse {
	var filtered []*ContentProject
...
func CalculateContentProjectStats(items []*ContentProject) *ContentProjectStats {
	stats := &ContentProjectStats{}
	if len(items) == 0 {
		return stats
	}
