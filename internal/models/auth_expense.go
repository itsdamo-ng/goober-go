...
type AuthExpenseListResponse struct {
	Data       []*AuthExpense `json:"data"`
...
func NewAuthExpense(name string) *AuthExpense {
	now := time.Now()
	return &AuthExpense{
		Name:      name,
		Status:    "pending",
...
func (m *AuthExpense) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("auth_expense: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("auth_expense: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("auth_expense: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("auth_expense: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("auth_expense: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("auth_expense: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("auth_expense: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("auth_expense: version must be at least 1")
	}
	return nil
}

func (m *AuthExpense) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("auth_expense: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuthExpense) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("auth_expense: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuthExpense) Clone() *AuthExpense {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuthExpense) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("auth_expense: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("auth_expense: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuthExpense) Sanitize() {
...
func (m *AuthExpense) Merge(other *AuthExpense) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuthExpense) String() string {
	return fmt.Sprintf("AuthExpense[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuthExpense) MatchesFilter(f *AuthExpenseFilter) bool {
...
func FilterAuthExpenseList(items []*AuthExpense, f *AuthExpenseFilter) *AuthExpenseListResponse {
	var filtered []*AuthExpense
	...
	return &AuthExpenseListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateAuthExpenseStats(items []*AuthExpense) *AuthExpenseStats {
	stats := &AuthExpenseStats{}
	if len(items) == 0 {
		return stats
	}
