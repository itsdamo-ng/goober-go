...
type CorePaymentListResponse struct {
	Data       []*CorePayment `json:"data"`
...
func NewCorePayment(name string) *CorePayment {
	now := time.Now()
	return &CorePayment{
		Name:      name,
		Status:    "pending",
...
func (m *CorePayment) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("core_payment: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("core_payment: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("core_payment: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("core_payment: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("core_payment: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("core_payment: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("core_payment: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("core_payment: version must be at least 1")
	}
	return nil
}

func (m *CorePayment) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("core_payment: marshal failed: %w", err)
	}
	return data, nil
}

func (m *CorePayment) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("core_payment: unmarshal failed: %w", err)
	}
	return nil
}

func (m *CorePayment) Clone() *CorePayment {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *CorePayment) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("core_payment: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("core_payment: transition from %q to %q not allowed", m.Status, status)
}

func (m *CorePayment) Sanitize() {
...
func (m *CorePayment) Merge(other *CorePayment) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *CorePayment) String() string {
	return fmt.Sprintf("CorePayment[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *CorePayment) MatchesFilter(f *CorePaymentFilter) bool {
...
func FilterCorePaymentList(items []*CorePayment, f *CorePaymentFilter) *CorePaymentListResponse {
	var filtered []*CorePayment
	...
	return &CorePaymentListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateCorePaymentStats(items []*CorePayment) *CorePaymentStats {
	stats := &CorePaymentStats{}
	if len(items) == 0 {
		return stats
	}
