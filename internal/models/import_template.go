...
type ImportTemplateFilter struct {
	IDs       []int64   `json:"ids"`
...
type ImportTemplateStats struct {
...
func NewImportTemplate(name string) *ImportTemplate {
	now := time.Now()
...
func (m *ImportTemplate) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("import_template: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("import_template: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("import_template: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("import_template: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("import_template: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("import_template: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("import_template: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("import_template: version must be at least 1")
	}
	return nil
}

func (m *ImportTemplate) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("import_template: marshal failed: %w", err)
	}
	return data, nil
}

func (m *ImportTemplate) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("import_template: unmarshal failed: %w", err)
	}
	return nil
}

func (m *ImportTemplate) Clone() *ImportTemplate {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *ImportTemplate) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("import_template: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("import_template: transition from %q to %q not allowed", m.Status, status)
}

func (m *ImportTemplate) Sanitize() {
...
func (m *ImportTemplate) Merge(other *ImportTemplate) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *ImportTemplate) String() string {
	return fmt.Sprintf("ImportTemplate[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *ImportTemplate) MatchesFilter(f *ImportTemplateFilter) bool {
...
func FilterImportTemplateList(items []*ImportTemplate, f *ImportTemplateFilter) *ImportTemplateListResponse {
	var filtered []*ImportTemplate
...
func CalculateImportTemplateStats(items []*ImportTemplate) *ImportTemplateStats {
	stats := &ImportTemplateStats{}
	if len(items) == 0 {
		return stats
	}
