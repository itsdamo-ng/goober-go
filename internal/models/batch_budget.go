...
type BatchBudgetListResponse struct {
	Data       []*BatchBudget `json:"data"`
...
func NewBatchBudget(name string) *BatchBudget {
	now := time.Now()
	return &BatchBudget{
		Name:      name,
		Status:    "pending",
...
func (m *BatchBudget) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("batch_budget: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("batch_budget: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("batch_budget: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("batch_budget: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("batch_budget: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("batch_budget: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("batch_budget: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("batch_budget: version must be at least 1")
	}
	return nil
}

func (m *BatchBudget) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("batch_budget: marshal failed: %w", err)
	}
	return data, nil
}

func (m *BatchBudget) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("batch_budget: unmarshal failed: %w", err)
	}
	return nil
}

func (m *BatchBudget) Clone() *BatchBudget {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *BatchBudget) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("batch_budget: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("batch_budget: transition from %q to %q not allowed", m.Status, status)
}

func (m *BatchBudget) Sanitize() {
...
func (m *BatchBudget) Merge(other *BatchBudget) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *BatchBudget) String() string {
	return fmt.Sprintf("BatchBudget[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *BatchBudget) MatchesFilter(f *BatchBudgetFilter) bool {
...
func FilterBatchBudgetList(items []*BatchBudget, f *BatchBudgetFilter) *BatchBudgetListResponse {
	var filtered []*BatchBudget
	...
	return &BatchBudgetListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateBatchBudgetStats(items []*BatchBudget) *BatchBudgetStats {
	stats := &BatchBudgetStats{}
	if len(items) == 0 {
		return stats
	}
