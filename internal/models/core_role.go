...
type CoreRoleListResponse struct {
	Data       []*CoreRole `json:"data"`
...
func NewCoreRole(name string) *CoreRole {
	now := time.Now()
	return &CoreRole{
		Name:      name,
		Status:    "pending",
...
func (m *CoreRole) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("core_role: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("core_role: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("core_role: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("core_role: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("core_role: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("core_role: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("core_role: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("core_role: version must be at least 1")
	}
	return nil
}

func (m *CoreRole) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("core_role: marshal failed: %w", err)
	}
	return data, nil
}

func (m *CoreRole) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("core_role: unmarshal failed: %w", err)
	}
	return nil
}

func (m *CoreRole) Clone() *CoreRole {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *CoreRole) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("core_role: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("core_role: transition from %q to %q not allowed", m.Status, status)
}

func (m *CoreRole) Sanitize() {
...
func (m *CoreRole) Merge(other *CoreRole) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *CoreRole) String() string {
	return fmt.Sprintf("CoreRole[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *CoreRole) MatchesFilter(f *CoreRoleFilter) bool {
...
func FilterCoreRoleList(items []*CoreRole, f *CoreRoleFilter) *CoreRoleListResponse {
	var filtered []*CoreRole
...
func CalculateCoreRoleStats(items []*CoreRole) *CoreRoleStats {
	stats := &CoreRoleStats{}
	if len(items) == 0 {
		return stats
	}
