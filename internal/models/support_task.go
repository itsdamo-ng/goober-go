...
type SupportTaskListResponse struct {
	Data       []*SupportTask `json:"data"`
...
func NewSupportTask(name string) *SupportTask {
	now := time.Now()
	return &SupportTask{
		Name:      name,
		Status:    "pending",
...
func (m *SupportTask) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("support_task: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("support_task: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("support_task: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("support_task: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("support_task: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("support_task: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("support_task: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("support_task: version must be at least 1")
	}
	return nil
}

func (m *SupportTask) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("support_task: marshal failed: %w", err)
	}
	return data, nil
}

func (m *SupportTask) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("support_task: unmarshal failed: %w", err)
	}
	return nil
}

func (m *SupportTask) Clone() *SupportTask {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *SupportTask) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("support_task: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("support_task: transition from %q to %q not allowed", m.Status, status)
}

func (m *SupportTask) Sanitize() {
...
func (m *SupportTask) Merge(other *SupportTask) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *SupportTask) String() string {
	return fmt.Sprintf("SupportTask[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *SupportTask) MatchesFilter(f *SupportTaskFilter) bool {
...
func FilterSupportTaskList(items []*SupportTask, f *SupportTaskFilter) *SupportTaskListResponse {
	var filtered []*SupportTask
	...
	return &SupportTaskListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateSupportTaskStats(items []*SupportTask) *SupportTaskStats {
	stats := &SupportTaskStats{}
	if len(items) == 0 {
		return stats
	}
