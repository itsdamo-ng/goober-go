...
type ShippingOrderStats struct {
...
func NewShippingOrder(name string) *ShippingOrder {
	now := time.Now()
...
func (m *ShippingOrder) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("shipping_order: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("shipping_order: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("shipping_order: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("shipping_order: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("shipping_order: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("shipping_order: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("shipping_order: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("shipping_order: version must be at least 1")
	}
	return nil
}

func (m *ShippingOrder) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("shipping_order: marshal failed: %w", err)
	}
	return data, nil
}

func (m *ShippingOrder) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("shipping_order: unmarshal failed: %w", err)
	}
	return nil
}

func (m *ShippingOrder) Clone() *ShippingOrder {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *ShippingOrder) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("shipping_order: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("shipping_order: transition from %q to %q not allowed", m.Status, status)
}

func (m *ShippingOrder) Sanitize() {
...
func (m *ShippingOrder) Merge(other *ShippingOrder) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *ShippingOrder) String() string {
	return fmt.Sprintf("ShippingOrder[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *ShippingOrder) MatchesFilter(f *ShippingOrderFilter) bool {
...
func FilterShippingOrderList(items []*ShippingOrder, f *ShippingOrderFilter) *ShippingOrderListResponse {
	var filtered []*ShippingOrder
	...
	return &ShippingOrderListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateShippingOrderStats(items []*ShippingOrder) *ShippingOrderStats {
	stats := &ShippingOrderStats{}
	if len(items) == 0 {
		return stats
	}
