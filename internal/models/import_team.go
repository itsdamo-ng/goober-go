...
type ImportTeamListResponse struct {
	Data       []*ImportTeam `json:"data"`
...
func NewImportTeam(name string) *ImportTeam {
	now := time.Now()
	return &ImportTeam{
		Name:      name,
		Status:    "pending",
...
func (m *ImportTeam) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("import_team: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("import_team: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("import_team: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("import_team: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("import_team: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("import_team: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("import_team: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("import_team: version must be at least 1")
	}
	return nil
}

func (m *ImportTeam) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("import_team: marshal failed: %w", err)
	}
	return data, nil
}

func (m *ImportTeam) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("import_team: unmarshal failed: %w", err)
	}
	return nil
}

func (m *ImportTeam) Clone() *ImportTeam {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *ImportTeam) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("import_team: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("import_team: transition from %q to %q not allowed", m.Status, status)
}

func (m *ImportTeam) Sanitize() {
...
func (m *ImportTeam) Merge(other *ImportTeam) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *ImportTeam) String() string {
	return fmt.Sprintf("ImportTeam[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *ImportTeam) MatchesFilter(f *ImportTeamFilter) bool {
...
func FilterImportTeamList(items []*ImportTeam, f *ImportTeamFilter) *ImportTeamListResponse {
	var filtered []*ImportTeam
...
func CalculateImportTeamStats(items []*ImportTeam) *ImportTeamStats {
	stats := &ImportTeamStats{}
	if len(items) == 0 {
		return stats
	}
