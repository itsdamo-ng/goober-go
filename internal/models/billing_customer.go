...
type BillingCustomerFilter struct {
	IDs       []int64   `json:"ids"`
...
type BillingCustomerStats struct {
...
func NewBillingCustomer(name string) *BillingCustomer {
	now := time.Now()
...
func (m *BillingCustomer) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("billing_customer: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("billing_customer: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("billing_customer: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("billing_customer: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("billing_customer: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("billing_customer: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("billing_customer: version must be at least 1")
	}
	return nil
}

func (m *BillingCustomer) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("billing_customer: marshal failed: %w", err)
	}
	return data, nil
}

func (m *BillingCustomer) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("billing_customer: unmarshal failed: %w", err)
	}
	return nil
}

func (m *BillingCustomer) Clone() *BillingCustomer {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *BillingCustomer) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("billing_customer: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("billing_customer: transition from %q to %q not allowed", m.Status, status)
}

func (m *BillingCustomer) Sanitize() {
...
func (m *BillingCustomer) Merge(other *BillingCustomer) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *BillingCustomer) String() string {
	return fmt.Sprintf("BillingCustomer[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *BillingCustomer) MatchesFilter(f *BillingCustomerFilter) bool {
...
func FilterBillingCustomerList(items []*BillingCustomer, f *BillingCustomerFilter) *BillingCustomerListResponse {
	var filtered []*BillingCustomer
...
func CalculateBillingCustomerStats(items []*BillingCustomer) *BillingCustomerStats {
	stats := &BillingCustomerStats{}
	if len(items) == 0 {
		return stats
	}
