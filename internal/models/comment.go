...
func (m *Comment) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("comment: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("comment: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("comment: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("comment: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("comment: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("comment: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("comment: version must be at least 1")
	}
	return nil
}

func (m *Comment) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("comment: marshal failed: %w", err)
	}
	return data, nil
}

func (m *Comment) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("comment: unmarshal failed: %w", err)
	}
	return nil
}
...
func (m *Comment) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("comment: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("comment: transition from %q to %q not allowed", m.Status, status)
}

func (m *Comment) Sanitize() {
...
func (m *Comment) Merge(other *Comment) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Comment) String() string {
	return fmt.Sprintf("Comment[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Comment) MatchesFilter(f *CommentFilter) bool {
...
func FilterCommentList(items []*Comment, f *CommentFilter) *CommentListResponse {
...
func CalculateCommentStats(items []*Comment) *CommentStats {
	stats := &CommentStats{}
	if len(items) == 0 {
		return stats
	}
