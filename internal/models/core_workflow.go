...
type CoreWorkflowFilter struct {
	IDs       []int64   `json:"ids"`
...
type CoreWorkflowListResponse struct {
	Data       []*CoreWorkflow `json:"data"`
...
func NewCoreWorkflow(name string) *CoreWorkflow {
	now := time.Now()
	return &CoreWorkflow{
		Name:      name,
		Status:    "pending",
...
func (m *CoreWorkflow) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("core_workflow: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("core_workflow: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("core_workflow: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("core_workflow: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("core_workflow: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("core_workflow: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("core_workflow: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("core_workflow: version must be at least 1")
	}
	return nil
}

func (m *CoreWorkflow) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("core_workflow: marshal failed: %w", err)
	}
	return data, nil
}

func (m *CoreWorkflow) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("core_workflow: unmarshal failed: %w", err)
	}
	return nil
}

func (m *CoreWorkflow) Clone() *CoreWorkflow {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *CoreWorkflow) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("core_workflow: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("core_workflow: transition from %q to %q not allowed", m.Status, status)
}

func (m *CoreWorkflow) Sanitize() {
...
func (m *CoreWorkflow) Merge(other *CoreWorkflow) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *CoreWorkflow) String() string {
	return fmt.Sprintf("CoreWorkflow[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *CoreWorkflow) MatchesFilter(f *CoreWorkflowFilter) bool {
...
func FilterCoreWorkflowList(items []*CoreWorkflow, f *CoreWorkflowFilter) *CoreWorkflowListResponse {
	var filtered []*CoreWorkflow
	...
	return &CoreWorkflowListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateCoreWorkflowStats(items []*CoreWorkflow) *CoreWorkflowStats {
