...
type AdminVenueListResponse struct {
	Data       []*AdminVenue `json:"data"`
...
func NewAdminVenue(name string) *AdminVenue {
	now := time.Now()
	return &AdminVenue{
		Name:      name,
		Status:    "pending",
...
func (m *AdminVenue) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("admin_venue: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_venue: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_venue: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_venue: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_venue: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_venue: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_venue: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_venue: version must be at least 1")
	}
	return nil
}

func (m *AdminVenue) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_venue: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminVenue) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_venue: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminVenue) Clone() *AdminVenue {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminVenue) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_venue: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_venue: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminVenue) Sanitize() {
...
func (m *AdminVenue) Merge(other *AdminVenue) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminVenue) String() string {
	return fmt.Sprintf("AdminVenue[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminVenue) MatchesFilter(f *AdminVenueFilter) bool {
...
func FilterAdminVenueList(items []*AdminVenue, f *AdminVenueFilter) *AdminVenueListResponse {
	var filtered []*AdminVenue
...
func CalculateAdminVenueStats(items []*AdminVenue) *AdminVenueStats {
