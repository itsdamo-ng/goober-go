...
func (m *Transaction) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("transaction: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("transaction: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("transaction: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("transaction: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("transaction: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("transaction: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("transaction: version must be at least 1")
	}
	return nil
}

func (m *Transaction) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("transaction: marshal failed: %w", err)
	}
	return data, nil
}

func (m *Transaction) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("transaction: unmarshal failed: %w", err)
	}
	return nil
}
...
func (m *Transaction) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("transaction: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("transaction: transition from %q to %q not allowed", m.Status, status)
}

func (m *Transaction) Sanitize() {
...
func (m *Transaction) Merge(other *Transaction) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Transaction) String() string {
	return fmt.Sprintf("Transaction[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Transaction) MatchesFilter(f *TransactionFilter) bool {
...
func FilterTransactionList(items []*Transaction, f *TransactionFilter) *TransactionListResponse {
...
func CalculateTransactionStats(items []*Transaction) *TransactionStats {
