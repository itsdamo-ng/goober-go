...
type AuditBudgetListResponse struct {
	Data       []*AuditBudget `json:"data"`
...
func NewAuditBudget(name string) *AuditBudget {
	now := time.Now()
	return &AuditBudget{
		Name:      name,
		Status:    "pending",
...
func (m *AuditBudget) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("audit_budget: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("audit_budget: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("audit_budget: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("audit_budget: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("audit_budget: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("audit_budget: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("audit_budget: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("audit_budget: version must be at least 1")
	}
	return nil
}

func (m *AuditBudget) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("audit_budget: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuditBudget) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("audit_budget: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuditBudget) Clone() *AuditBudget {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuditBudget) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("audit_budget: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("audit_budget: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuditBudget) Sanitize() {
...
func (m *AuditBudget) Merge(other *AuditBudget) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuditBudget) String() string {
	return fmt.Sprintf("AuditBudget[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuditBudget) MatchesFilter(f *AuditBudgetFilter) bool {
...
func FilterAuditBudgetList(items []*AuditBudget, f *AuditBudgetFilter) *AuditBudgetListResponse {
	var filtered []*AuditBudget
	...
	return &AuditBudgetListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateAuditBudgetStats(items []*AuditBudget) *AuditBudgetStats {
	stats := &AuditBudgetStats{}
	if len(items) == 0 {
		return stats
	}
