...
type CoreRewardListResponse struct {
	Data       []*CoreReward `json:"data"`
...
func NewCoreReward(name string) *CoreReward {
	now := time.Now()
	return &CoreReward{
		Name:      name,
		Status:    "pending",
...
func (m *CoreReward) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("core_reward: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("core_reward: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("core_reward: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("core_reward: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("core_reward: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("core_reward: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("core_reward: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("core_reward: version must be at least 1")
	}
	return nil
}

func (m *CoreReward) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("core_reward: marshal failed: %w", err)
	}
	return data, nil
}

func (m *CoreReward) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("core_reward: unmarshal failed: %w", err)
	}
	return nil
}

func (m *CoreReward) Clone() *CoreReward {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *CoreReward) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("core_reward: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("core_reward: transition from %q to %q not allowed", m.Status, status)
}

func (m *CoreReward) Sanitize() {
...
func (m *CoreReward) Merge(other *CoreReward) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *CoreReward) String() string {
	return fmt.Sprintf("CoreReward[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *CoreReward) MatchesFilter(f *CoreRewardFilter) bool {
...
func FilterCoreRewardList(items []*CoreReward, f *CoreRewardFilter) *CoreRewardListResponse {
	var filtered []*CoreReward
...
func CalculateCoreRewardStats(items []*CoreReward) *CoreRewardStats {
	stats := &CoreRewardStats{}
	if len(items) == 0 {
		return stats
	}
