...
type AnalyticsRuleFilter struct {
	IDs       []int64   `json:"ids"`
...
func NewAnalyticsRule(name string) *AnalyticsRule {
	now := time.Now()
...
func (m *AnalyticsRule) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("analytics_rule: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("analytics_rule: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("analytics_rule: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("analytics_rule: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("analytics_rule: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("analytics_rule: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("analytics_rule: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("analytics_rule: version must be at least 1")
	}
	return nil
}

func (m *AnalyticsRule) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("analytics_rule: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AnalyticsRule) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("analytics_rule: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AnalyticsRule) Clone() *AnalyticsRule {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AnalyticsRule) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("analytics_rule: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("analytics_rule: transition from %q to %q not allowed", m.Status, status)
}

func (m *AnalyticsRule) Sanitize() {
...
func (m *AnalyticsRule) Merge(other *AnalyticsRule) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AnalyticsRule) String() string {
	return fmt.Sprintf("AnalyticsRule[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AnalyticsRule) MatchesFilter(f *AnalyticsRuleFilter) bool {
...
func FilterAnalyticsRuleList(items []*AnalyticsRule, f *AnalyticsRuleFilter) *AnalyticsRuleListResponse {
	var filtered []*AnalyticsRule
	...
	return &AnalyticsRuleListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateAnalyticsRuleStats(items []*AnalyticsRule) *AnalyticsRuleStats {
	stats := &AnalyticsRuleStats{}
	if len(items) == 0 {
		return stats
	}
