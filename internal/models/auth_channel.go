...
type AuthChannelListResponse struct {
	Data       []*AuthChannel `json:"data"`
...
func NewAuthChannel(name string) *AuthChannel {
	now := time.Now()
	return &AuthChannel{
		Name:      name,
		Status:    "pending",
...
func (m *AuthChannel) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("auth_channel: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("auth_channel: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("auth_channel: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("auth_channel: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("auth_channel: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("auth_channel: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("auth_channel: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("auth_channel: version must be at least 1")
	}
	return nil
}

func (m *AuthChannel) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("auth_channel: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuthChannel) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("auth_channel: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuthChannel) Clone() *AuthChannel {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuthChannel) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("auth_channel: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("auth_channel: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuthChannel) Sanitize() {
...
func (m *AuthChannel) Merge(other *AuthChannel) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuthChannel) String() string {
	return fmt.Sprintf("AuthChannel[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuthChannel) MatchesFilter(f *AuthChannelFilter) bool {
...
func FilterAuthChannelList(items []*AuthChannel, f *AuthChannelFilter) *AuthChannelListResponse {
	var filtered []*AuthChannel
	...
	return &AuthChannelListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateAuthChannelStats(items []*AuthChannel) *AuthChannelStats {
	stats := &AuthChannelStats{}
	if len(items) == 0 {
		return stats
	}
