...
type SupportEventListResponse struct {
	Data       []*SupportEvent `json:"data"`
...
func NewSupportEvent(name string) *SupportEvent {
	now := time.Now()
	return &SupportEvent{
		Name:      name,
		Status:    "pending",
...
func (m *SupportEvent) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("support_event: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("support_event: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("support_event: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("support_event: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("support_event: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("support_event: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("support_event: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("support_event: version must be at least 1")
	}
	return nil
}

func (m *SupportEvent) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("support_event: marshal failed: %w", err)
	}
	return data, nil
}

func (m *SupportEvent) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("support_event: unmarshal failed: %w", err)
	}
	return nil
}

func (m *SupportEvent) Clone() *SupportEvent {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *SupportEvent) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("support_event: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("support_event: transition from %q to %q not allowed", m.Status, status)
}

func (m *SupportEvent) Sanitize() {
...
func (m *SupportEvent) Merge(other *SupportEvent) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *SupportEvent) String() string {
	return fmt.Sprintf("SupportEvent[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *SupportEvent) MatchesFilter(f *SupportEventFilter) bool {
...
func FilterSupportEventList(items []*SupportEvent, f *SupportEventFilter) *SupportEventListResponse {
	var filtered []*SupportEvent
	...
	return &SupportEventListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateSupportEventStats(items []*SupportEvent) *SupportEventStats {
	stats := &SupportEventStats{}
	if len(items) == 0 {
		return stats
	}
