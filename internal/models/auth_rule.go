...
func NewAuthRule(name string) *AuthRule {
	now := time.Now()
	return &AuthRule{
		Name:      name,
		Status:    "pending",
...
func (m *AuthRule) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("auth_rule: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("auth_rule: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("auth_rule: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("auth_rule: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("auth_rule: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("auth_rule: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("auth_rule: version must be at least 1")
	}
	return nil
}

func (m *AuthRule) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("auth_rule: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuthRule) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("auth_rule: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuthRule) Clone() *AuthRule {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuthRule) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("auth_rule: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("auth_rule: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuthRule) Sanitize() {
...
func (m *AuthRule) Merge(other *AuthRule) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuthRule) String() string {
	return fmt.Sprintf("AuthRule[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuthRule) MatchesFilter(f *AuthRuleFilter) bool {
...
func FilterAuthRuleList(items []*AuthRule, f *AuthRuleFilter) *AuthRuleListResponse {
	var filtered []*AuthRule
...
func CalculateAuthRuleStats(items []*AuthRule) *AuthRuleStats {
	stats := &AuthRuleStats{}
	if len(items) == 0 {
		return stats
	}
