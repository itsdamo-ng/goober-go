...
func (m *Message) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("message: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("message: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("message: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("message: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("message: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("message: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("message: version must be at least 1")
	}
	return nil
}
...
func (m *Message) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("message: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("message: transition from %q to %q not allowed", m.Status, status)
}
...
func (m *Message) String() string {
	return fmt.Sprintf("Message[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Message) MatchesFilter(f *MessageFilter) bool {
...
func FilterMessageList(items []*Message, f *MessageFilter) *MessageListResponse {
...
func CalculateMessageStats(items []*Message) *MessageStats {
	stats := &MessageStats{}
	if len(items) == 0 {
		return stats
	}
