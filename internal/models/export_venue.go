...
type ExportVenueListResponse struct {
	Data       []*ExportVenue `json:"data"`
...
func NewExportVenue(name string) *ExportVenue {
	now := time.Now()
	return &ExportVenue{
		Name:      name,
		Status:    "pending",
...
func (m *ExportVenue) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("export_venue: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("export_venue: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("export_venue: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("export_venue: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("export_venue: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("export_venue: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("export_venue: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("export_venue: version must be at least 1")
	}
	return nil
}

func (m *ExportVenue) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("export_venue: marshal failed: %w", err)
	}
	return data, nil
}

func (m *ExportVenue) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("export_venue: unmarshal failed: %w", err)
	}
	return nil
}

func (m *ExportVenue) Clone() *ExportVenue {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *ExportVenue) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("export_venue: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("export_venue: transition from %q to %q not allowed", m.Status, status)
}

func (m *ExportVenue) Sanitize() {
...
func (m *ExportVenue) Merge(other *ExportVenue) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *ExportVenue) String() string {
	return fmt.Sprintf("ExportVenue[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *ExportVenue) MatchesFilter(f *ExportVenueFilter) bool {
...
func FilterExportVenueList(items []*ExportVenue, f *ExportVenueFilter) *ExportVenueListResponse {
	var filtered []*ExportVenue
	...
	return &ExportVenueListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateExportVenueStats(items []*ExportVenue) *ExportVenueStats {
	stats := &ExportVenueStats{}
	if len(items) == 0 {
		return stats
	}
