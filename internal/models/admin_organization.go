...
type AdminOrganizationFilter struct {
	IDs       []int64   `json:"ids"`
...
type AdminOrganizationListResponse struct {
	Data       []*AdminOrganization `json:"data"`
...
func NewAdminOrganization(name string) *AdminOrganization {
	now := time.Now()
...
func (m *AdminOrganization) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("admin_organization: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_organization: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_organization: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_organization: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_organization: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_organization: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_organization: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_organization: version must be at least 1")
	}
	return nil
}

func (m *AdminOrganization) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_organization: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminOrganization) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_organization: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminOrganization) Clone() *AdminOrganization {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminOrganization) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_organization: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_organization: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminOrganization) Sanitize() {
...
func (m *AdminOrganization) Merge(other *AdminOrganization) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminOrganization) String() string {
	return fmt.Sprintf("AdminOrganization[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminOrganization) MatchesFilter(f *AdminOrganizationFilter) bool {
...
func FilterAdminOrganizationList(items []*AdminOrganization, f *AdminOrganizationFilter) *AdminOrganizationListResponse {
	var filtered []*AdminOrganization
...
func CalculateAdminOrganizationStats(items []*AdminOrganization) *AdminOrganizationStats {
