...
type AuthBudgetListResponse struct {
	Data       []*AuthBudget `json:"data"`
...
func NewAuthBudget(name string) *AuthBudget {
	now := time.Now()
	return &AuthBudget{
		Name:      name,
		Status:    "pending",
...
func (m *AuthBudget) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("auth_budget: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("auth_budget: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("auth_budget: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("auth_budget: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("auth_budget: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("auth_budget: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("auth_budget: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("auth_budget: version must be at least 1")
	}
	return nil
}

func (m *AuthBudget) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("auth_budget: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AuthBudget) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("auth_budget: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AuthBudget) Clone() *AuthBudget {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AuthBudget) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("auth_budget: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("auth_budget: transition from %q to %q not allowed", m.Status, status)
}

func (m *AuthBudget) Sanitize() {
...
func (m *AuthBudget) Merge(other *AuthBudget) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AuthBudget) String() string {
	return fmt.Sprintf("AuthBudget[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AuthBudget) MatchesFilter(f *AuthBudgetFilter) bool {
...
func FilterAuthBudgetList(items []*AuthBudget, f *AuthBudgetFilter) *AuthBudgetListResponse {
	var filtered []*AuthBudget
...
func CalculateAuthBudgetStats(items []*AuthBudget) *AuthBudgetStats {
	stats := &AuthBudgetStats{}
	if len(items) == 0 {
		return stats
	}
