...
type SupportSessionListResponse struct {
...
type SupportSessionStats struct {
...
func NewSupportSession(name string) *SupportSession {
	now := time.Now()
...
func (m *SupportSession) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("support_session: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("support_session: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("support_session: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("support_session: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("support_session: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("support_session: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("support_session: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("support_session: version must be at least 1")
	}
	return nil
}

func (m *SupportSession) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("support_session: marshal failed: %w", err)
	}
	return data, nil
}

func (m *SupportSession) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("support_session: unmarshal failed: %w", err)
	}
	return nil
}

func (m *SupportSession) Clone() *SupportSession {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *SupportSession) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("support_session: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("support_session: transition from %q to %q not allowed", m.Status, status)
}

func (m *SupportSession) Sanitize() {
...
func (m *SupportSession) Merge(other *SupportSession) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *SupportSession) String() string {
	return fmt.Sprintf("SupportSession[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *SupportSession) MatchesFilter(f *SupportSessionFilter) bool {
...
func FilterSupportSessionList(items []*SupportSession, f *SupportSessionFilter) *SupportSessionListResponse {
	var filtered []*SupportSession
...
func CalculateSupportSessionStats(items []*SupportSession) *SupportSessionStats {
	stats := &SupportSessionStats{}
	if len(items) == 0 {
		return stats
	}
