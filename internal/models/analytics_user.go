...
type AnalyticsUserListResponse struct {
...
func NewAnalyticsUser(name string) *AnalyticsUser {
	now := time.Now()
...
func (m *AnalyticsUser) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("analytics_user: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("analytics_user: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("analytics_user: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("analytics_user: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("analytics_user: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("analytics_user: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("analytics_user: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("analytics_user: version must be at least 1")
	}
	return nil
}

func (m *AnalyticsUser) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("analytics_user: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AnalyticsUser) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("analytics_user: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AnalyticsUser) Clone() *AnalyticsUser {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AnalyticsUser) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("analytics_user: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("analytics_user: transition from %q to %q not allowed", m.Status, status)
}

func (m *AnalyticsUser) Sanitize() {
...
func (m *AnalyticsUser) Merge(other *AnalyticsUser) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AnalyticsUser) String() string {
	return fmt.Sprintf("AnalyticsUser[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AnalyticsUser) MatchesFilter(f *AnalyticsUserFilter) bool {
...
func FilterAnalyticsUserList(items []*AnalyticsUser, f *AnalyticsUserFilter) *AnalyticsUserListResponse {
	var filtered []*AnalyticsUser
...
func CalculateAnalyticsUserStats(items []*AnalyticsUser) *AnalyticsUserStats {
