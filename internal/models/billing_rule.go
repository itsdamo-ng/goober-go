...
type BillingRuleListResponse struct {
	Data       []*BillingRule `json:"data"`
...
func NewBillingRule(name string) *BillingRule {
	now := time.Now()
	return &BillingRule{
		Name:      name,
		Status:    "pending",
...
func (m *BillingRule) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("billing_rule: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("billing_rule: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("billing_rule: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("billing_rule: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("billing_rule: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("billing_rule: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("billing_rule: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("billing_rule: version must be at least 1")
	}
	return nil
}

func (m *BillingRule) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("billing_rule: marshal failed: %w", err)
	}
	return data, nil
}

func (m *BillingRule) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("billing_rule: unmarshal failed: %w", err)
	}
	return nil
}

func (m *BillingRule) Clone() *BillingRule {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *BillingRule) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("billing_rule: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("billing_rule: transition from %q to %q not allowed", m.Status, status)
}

func (m *BillingRule) Sanitize() {
...
func (m *BillingRule) Merge(other *BillingRule) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *BillingRule) String() string {
	return fmt.Sprintf("BillingRule[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *BillingRule) MatchesFilter(f *BillingRuleFilter) bool {
...
func FilterBillingRuleList(items []*BillingRule, f *BillingRuleFilter) *BillingRuleListResponse {
	var filtered []*BillingRule
	...
	return &BillingRuleListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateBillingRuleStats(items []*BillingRule) *BillingRuleStats {
	stats := &BillingRuleStats{}
	if len(items) == 0 {
		return stats
	}
