...
type SearchOrderListResponse struct {
	Data       []*SearchOrder `json:"data"`
...
func NewSearchOrder(name string) *SearchOrder {
	now := time.Now()
	return &SearchOrder{
		Name:      name,
		Status:    "pending",
...
func (m *SearchOrder) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("search_order: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("search_order: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("search_order: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("search_order: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("search_order: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("search_order: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("search_order: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("search_order: version must be at least 1")
	}
	return nil
}

func (m *SearchOrder) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("search_order: marshal failed: %w", err)
	}
	return data, nil
}

func (m *SearchOrder) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("search_order: unmarshal failed: %w", err)
	}
	return nil
}

func (m *SearchOrder) Clone() *SearchOrder {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *SearchOrder) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("search_order: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("search_order: transition from %q to %q not allowed", m.Status, status)
}

func (m *SearchOrder) Sanitize() {
...
func (m *SearchOrder) Merge(other *SearchOrder) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *SearchOrder) String() string {
	return fmt.Sprintf("SearchOrder[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *SearchOrder) MatchesFilter(f *SearchOrderFilter) bool {
...
func FilterSearchOrderList(items []*SearchOrder, f *SearchOrderFilter) *SearchOrderListResponse {
	var filtered []*SearchOrder
	...
	return &SearchOrderListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateSearchOrderStats(items []*SearchOrder) *SearchOrderStats {
	stats := &SearchOrderStats{}
	if len(items) == 0 {
		return stats
	}
