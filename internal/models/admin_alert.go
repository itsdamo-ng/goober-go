...
func NewAdminAlert(name string) *AdminAlert {
	now := time.Now()
	return &AdminAlert{
		Name:      name,
		Status:    "pending",
...
func (m *AdminAlert) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("admin_alert: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_alert: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_alert: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_alert: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_alert: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_alert: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_alert: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_alert: version must be at least 1")
	}
	return nil
}

func (m *AdminAlert) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_alert: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminAlert) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_alert: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminAlert) Clone() *AdminAlert {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminAlert) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_alert: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_alert: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminAlert) Sanitize() {
...
func (m *AdminAlert) Merge(other *AdminAlert) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminAlert) String() string {
	return fmt.Sprintf("AdminAlert[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminAlert) MatchesFilter(f *AdminAlertFilter) bool {
...
func FilterAdminAlertList(items []*AdminAlert, f *AdminAlertFilter) *AdminAlertListResponse {
	var filtered []*AdminAlert
...
func CalculateAdminAlertStats(items []*AdminAlert) *AdminAlertStats {
