...
func (m *AdminUser) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_user: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_user: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_user: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_user: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_user: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_user: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_user: version must be at least 1")
	}
	return nil
}

func (m *AdminUser) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_user: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminUser) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_user: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminUser) Clone() *AdminUser {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminUser) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_user: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_user: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminUser) Sanitize() {
...
func (m *AdminUser) Merge(other *AdminUser) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminUser) String() string {
	return fmt.Sprintf("AdminUser[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminUser) MatchesFilter(f *AdminUserFilter) bool {
...
func FilterAdminUserList(items []*AdminUser, f *AdminUserFilter) *AdminUserListResponse {
...
func CalculateAdminUserStats(items []*AdminUser) *AdminUserStats {
