...
func (m *Discount) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("discount: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("discount: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("discount: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("discount: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("discount: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("discount: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("discount: version must be at least 1")
	}
	return nil
}
...
func (m *Discount) ToJSON() ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("discount: marshal failed: %w", err)
	}
	return data, nil
}
...
func (m *Discount) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("discount: unmarshal failed: %w", err)
	}
	return nil
}

func (m *Discount) Clone() *Discount {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *Discount) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("discount: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("discount: transition from %q to %q not allowed", m.Status, status)
}

func (m *Discount) Sanitize() {
...
func (m *Discount) Merge(other *Discount) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *Discount) String() string {
	return fmt.Sprintf("Discount[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *Discount) MatchesFilter(f *DiscountFilter) bool {
...
func FilterDiscountList(items []*Discount, f *DiscountFilter) *DiscountListResponse {
...
func CalculateDiscountStats(items []*Discount) *DiscountStats {
	stats := &DiscountStats{}
	if len(items) == 0 {
		return stats
	}
