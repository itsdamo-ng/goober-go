...
type ContentTeamListResponse struct {
	Data       []*ContentTeam `json:"data"`
...
func NewContentTeam(name string) *ContentTeam {
	now := time.Now()
	return &ContentTeam{
		Name:      name,
		Status:    "pending",
...
func (m *ContentTeam) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("content_team: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("content_team: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("content_team: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("content_team: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("content_team: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("content_team: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("content_team: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("content_team: version must be at least 1")
	}
	return nil
}

func (m *ContentTeam) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("content_team: marshal failed: %w", err)
	}
	return data, nil
}

func (m *ContentTeam) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("content_team: unmarshal failed: %w", err)
	}
	return nil
}

func (m *ContentTeam) Clone() *ContentTeam {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *ContentTeam) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("content_team: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("content_team: transition from %q to %q not allowed", m.Status, status)
}

func (m *ContentTeam) Sanitize() {
...
func (m *ContentTeam) Merge(other *ContentTeam) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *ContentTeam) String() string {
	return fmt.Sprintf("ContentTeam[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *ContentTeam) MatchesFilter(f *ContentTeamFilter) bool {
...
func FilterContentTeamList(items []*ContentTeam, f *ContentTeamFilter) *ContentTeamListResponse {
	var filtered []*ContentTeam
	...
	return &ContentTeamListResponse{
		Data:       filtered,
		Total:      int64(total),
		Page:       page,
		PageSize:   pageSize,
...
func CalculateContentTeamStats(items []*ContentTeam) *ContentTeamStats {
	stats := &ContentTeamStats{}
	if len(items) == 0 {
		return stats
	}
