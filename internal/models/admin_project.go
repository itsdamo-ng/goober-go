...
func NewAdminProject(name string) *AdminProject {
	now := time.Now()
	return &AdminProject{
		Name:      name,
		Status:    "pending",
...
func (m *AdminProject) Validate() error {
	if strings.TrimSpace(m.Name) == "" {
		return fmt.Errorf("admin_project: name is required")
	}
	if len(m.Name) > 500 {
		return fmt.Errorf("admin_project: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("admin_project: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("admin_project: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("admin_project: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("admin_project: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("admin_project: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("admin_project: version must be at least 1")
	}
	return nil
}

func (m *AdminProject) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("admin_project: marshal failed: %w", err)
	}
	return data, nil
}

func (m *AdminProject) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("admin_project: unmarshal failed: %w", err)
	}
	return nil
}

func (m *AdminProject) Clone() *AdminProject {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *AdminProject) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("admin_project: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("admin_project: transition from %q to %q not allowed", m.Status, status)
}

func (m *AdminProject) Sanitize() {
...
func (m *AdminProject) Merge(other *AdminProject) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *AdminProject) String() string {
	return fmt.Sprintf("AdminProject[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *AdminProject) MatchesFilter(f *AdminProjectFilter) bool {
...
func FilterAdminProjectList(items []*AdminProject, f *AdminProjectFilter) *AdminProjectListResponse {
...
func CalculateAdminProjectStats(items []*AdminProject) *AdminProjectStats {
