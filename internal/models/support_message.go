...
type SupportMessageStats struct {
...
func NewSupportMessage(name string) *SupportMessage {
	now := time.Now()
...
func (m *SupportMessage) Validate() error {
	...
	if len(m.Name) > 500 {
		return fmt.Errorf("support_message: name exceeds maximum length of 500 characters")
	}
	if len(m.Description) > 5000 {
		return fmt.Errorf("support_message: description exceeds maximum length of 5000 characters")
	}
	...
	if m.Status != "" && !validStatuses[m.Status] {
		return fmt.Errorf("support_message: invalid status %q", m.Status)
	}
	if m.Amount < 0 {
		return fmt.Errorf("support_message: amount cannot be negative")
	}
	if m.Priority < 0 || m.Priority > 10 {
		return fmt.Errorf("support_message: priority must be between 0 and 10")
	}
	if m.Email != "" && !strings.Contains(m.Email, "@") {
		return fmt.Errorf("support_message: invalid email format")
	}
	if m.Version < 1 {
		return fmt.Errorf("support_message: version must be at least 1")
	}
	return nil
}

func (m *SupportMessage) ToJSON() ([]byte, error) {
	data, err := json.Marshal(m)
	if err != nil {
		return nil, fmt.Errorf("support_message: marshal failed: %w", err)
	}
	return data, nil
}

func (m *SupportMessage) FromJSON(data []byte) error {
	if err := json.Unmarshal(data, m); err != nil {
		return fmt.Errorf("support_message: unmarshal failed: %w", err)
	}
	return nil
}

func (m *SupportMessage) Clone() *SupportMessage {
	clone := *m
	clone.ID = 0
	clone.Version = 1
...
func (m *SupportMessage) SetStatus(status string) error {
	...
	if !ok {
		return fmt.Errorf("support_message: no transitions from status %q", m.Status)
	}
	...
	return fmt.Errorf("support_message: transition from %q to %q not allowed", m.Status, status)
}

func (m *SupportMessage) Sanitize() {
...
func (m *SupportMessage) Merge(other *SupportMessage) {
	if other.Name != "" {
		m.Name = other.Name
	}
...
func (m *SupportMessage) String() string {
	return fmt.Sprintf("SupportMessage[id=%d name=%q status=%s amount=%.2f]", m.ID, m.Name, m.Status, m.Amount)
}

func (m *SupportMessage) MatchesFilter(f *SupportMessageFilter) bool {
...
func FilterSupportMessageList(items []*SupportMessage, f *SupportMessageFilter) *SupportMessageListResponse {
	var filtered []*SupportMessage
...
func CalculateSupportMessageStats(items []*SupportMessage) *SupportMessageStats {
