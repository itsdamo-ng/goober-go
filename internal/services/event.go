...
func (s *EventService) Create(input *CreateEventInput) (*EventRecord, error) {
	if strings.TrimSpace(input.Name) == "" {
		return nil, fmt.Errorf("event: name is required")
	}
	if input.Amount < 0 {
		return nil, fmt.Errorf("event: amount cannot be negative")
	}
	if input.Email != "" && !strings.Contains(input.Email, "@") {
		return nil, fmt.Errorf("event: invalid email format")
	}
	now := time.Now()
	record := &EventRecord{
		ID:          s.nextID,
...
func (s *EventService) GetByID(id int64) (*EventRecord, error) {
	record, ok := s.records[id]
	if !ok {
		return nil, fmt.Errorf("event: record %d not found", id)
	}
	return record, nil
}

func (s *EventService) Update(id int64, input *UpdateEventInput) (*EventRecord, error) {
	record, ok := s.records[id]
	if !ok {
		return nil, fmt.Errorf("event: record %d not found", id)
	}
...
func (s *EventService) Delete(id int64) error {
	if _, ok := s.records[id]; !ok {
		return fmt.Errorf("event: record %d not found", id)
	}
	delete(s.records, id)
	return nil
}

func (s *EventService) List(params *EventSearchParams) ([]*EventRecord, int) {
...
func (s *EventService) BulkCreate(inputs []*CreateEventInput) *EventProcessResult {
	start := time.Now()
...
func (s *EventService) Archive(id int64) error {
	record, ok := s.records[id]
	if !ok {
		return fmt.Errorf("event: record %d not found", id)
	}
	if record.Status == "archived" {
		return fmt.Errorf("event: record %d is already archived", id)
	}
...
func (s *EventService) Restore(id int64) error {
	record, ok := s.records[id]
	if !ok {
		return fmt.Errorf("event: record %d not found", id)
	}
	if record.Status != "archived" {
		return fmt.Errorf("event: record %d is not archived", id)
	}
...
func (s *EventService) Duplicate(id int64) (*EventRecord, error) {
...
func (s *EventService) Count() int {
	return len(s.records)
}

func (s *EventService) CountByStatus(status string) int {
	count := 0
	for _, r := range s.records {
		if r.Status == status {
			count++
