...
type AlertService struct {
	records map[int64]*AlertRecord
	nextID  int64
}

func NewAlertService() *AlertService {
	return &AlertService{
		records: make(map[int64]*AlertRecord),
		nextID:  1,
	}
}

func (s *AlertService) Create(input *CreateAlertInput) (*AlertRecord, error) {
	if strings.TrimSpace(input.Name) == "" {
		return nil, fmt.Errorf("alert: name is required")
	}
	if input.Amount < 0 {
		return nil, fmt.Errorf("alert: amount cannot be negative")
	}
	if input.Email != "" && !strings.Contains(input.Email, "@") {
		return nil, fmt.Errorf("alert: invalid email format")
	}
	now := time.Now()
	record := &AlertRecord{
		ID:          s.nextID,
...
func (s *AlertService) GetByID(id int64) (*AlertRecord, error) {
	record, ok := s.records[id]
	if !ok {
		return nil, fmt.Errorf("alert: record %d not found", id)
	}
	return record, nil
}

func (s *AlertService) Update(id int64, input *UpdateAlertInput) (*AlertRecord, error) {
	record, ok := s.records[id]
	if !ok {
		return nil, fmt.Errorf("alert: record %d not found", id)
	}
...
func (s *AlertService) Delete(id int64) error {
	if _, ok := s.records[id]; !ok {
		return fmt.Errorf("alert: record %d not found", id)
	}
	delete(s.records, id)
	return nil
}

func (s *AlertService) List(params *AlertSearchParams) ([]*AlertRecord, int) {
...
func (s *AlertService) BulkCreate(inputs []*CreateAlertInput) *AlertProcessResult {
	start := time.Now()
...
func (s *AlertService) Archive(id int64) error {
	record, ok := s.records[id]
	if !ok {
		return fmt.Errorf("alert: record %d not found", id)
	}
	if record.Status == "archived" {
		return fmt.Errorf("alert: record %d is already archived", id)
	}
...
func (s *AlertService) Restore(id int64) error {
	record, ok := s.records[id]
	if !ok {
		return fmt.Errorf("alert: record %d not found", id)
	}
	if record.Status != "archived" {
		return fmt.Errorf("alert: record %d is not archived", id)
	}
...
func (s *AlertService) Duplicate(id int64) (*AlertRecord, error) {
...
func (s *AlertService) CountByStatus(status string) int {
	count := 0
	for _, r := range s.records {
		if r.Status == status {
			count++
