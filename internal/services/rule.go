...
func (s *RuleService) Create(input *CreateRuleInput) (*RuleRecord, error) {
	if strings.TrimSpace(input.Name) == "" {
		return nil, fmt.Errorf("rule: name is required")
	}
	if input.Amount < 0 {
		return nil, fmt.Errorf("rule: amount cannot be negative")
	}
	if input.Email != "" && !strings.Contains(input.Email, "@") {
		return nil, fmt.Errorf("rule: invalid email format")
	}
	now := time.Now()
	record := &RuleRecord{
		ID:          s.nextID,
...
func (s *RuleService) GetByID(id int64) (*RuleRecord, error) {
	record, ok := s.records[id]
	if !ok {
		return nil, fmt.Errorf("rule: record %d not found", id)
	}
	return record, nil
}

func (s *RuleService) Update(id int64, input *UpdateRuleInput) (*RuleRecord, error) {
	record, ok := s.records[id]
	if !ok {
		return nil, fmt.Errorf("rule: record %d not found", id)
	}
...
func (s *RuleService) Delete(id int64) error {
	if _, ok := s.records[id]; !ok {
		return fmt.Errorf("rule: record %d not found", id)
	}
	delete(s.records, id)
	return nil
}

func (s *RuleService) List(params *RuleSearchParams) ([]*RuleRecord, int) {
	var results []*RuleRecord
...
func (s *RuleService) BulkCreate(inputs []*CreateRuleInput) *RuleProcessResult {
	start := time.Now()
...
func (s *RuleService) Archive(id int64) error {
	record, ok := s.records[id]
	if !ok {
		return fmt.Errorf("rule: record %d not found", id)
	}
	if record.Status == "archived" {
		return fmt.Errorf("rule: record %d is already archived", id)
	}
...
func (s *RuleService) Restore(id int64) error {
	record, ok := s.records[id]
	if !ok {
		return fmt.Errorf("rule: record %d not found", id)
	}
	if record.Status != "archived" {
		return fmt.Errorf("rule: record %d is not archived", id)
	}
...
func (s *RuleService) Duplicate(id int64) (*RuleRecord, error) {
...
func (s *RuleService) Count() int {
	return len(s.records)
}

func (s *RuleService) CountByStatus(status string) int {
	count := 0
	for _, r := range s.records {
		if r.Status == status {
			count++
