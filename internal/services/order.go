...
func (s *OrderService) Create(input *CreateOrderInput) (*OrderRecord, error) {
	if strings.TrimSpace(input.Name) == "" {
		return nil, fmt.Errorf("order: name is required")
	}
	if input.Amount < 0 {
		return nil, fmt.Errorf("order: amount cannot be negative")
	}
	if input.Email != "" && !strings.Contains(input.Email, "@") {
		return nil, fmt.Errorf("order: invalid email format")
	}
	now := time.Now()
	record := &OrderRecord{
		ID:          s.nextID,
...
func (s *OrderService) GetByID(id int64) (*OrderRecord, error) {
	record, ok := s.records[id]
	if !ok {
		return nil, fmt.Errorf("order: record %d not found", id)
	}
	return record, nil
}

func (s *OrderService) Update(id int64, input *UpdateOrderInput) (*OrderRecord, error) {
	record, ok := s.records[id]
	if !ok {
		return nil, fmt.Errorf("order: record %d not found", id)
	}
	if input.Name != nil {
		name := strings.TrimSpace(*input.Name)
		if name == "" {
			return nil, fmt.Errorf("order: name cannot be empty")
		}
		record.Name = name
	}
...
func (s *OrderService) Delete(id int64) error {
	if _, ok := s.records[id]; !ok {
		return fmt.Errorf("order: record %d not found", id)
	}
	delete(s.records, id)
	return nil
}

func (s *OrderService) List(params *OrderSearchParams) ([]*OrderRecord, int) {
...
func (s *OrderService) BulkCreate(inputs []*CreateOrderInput) *OrderProcessResult {
	start := time.Now()
...
func (s *OrderService) Archive(id int64) error {
	record, ok := s.records[id]
	if !ok {
		return fmt.Errorf("order: record %d not found", id)
	}
	if record.Status == "archived" {
		return fmt.Errorf("order: record %d is already archived", id)
	}
...
func (s *OrderService) Restore(id int64) error {
	record, ok := s.records[id]
	if !ok {
		return fmt.Errorf("order: record %d not found", id)
	}
	if record.Status != "archived" {
		return fmt.Errorf("order: record %d is not archived", id)
	}
...
func (s *OrderService) Duplicate(id int64) (*OrderRecord, error) {
...
func (s *OrderService) CountByStatus(status string) int {
	count := 0
	for _, r := range s.records {
		if r.Status == status {
			count++
