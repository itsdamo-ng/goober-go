...
func (s *TaskService) Create(input *CreateTaskInput) (*TaskRecord, error) {
	if strings.TrimSpace(input.Name) == "" {
		return nil, fmt.Errorf("task: name is required")
	}
	if input.Amount < 0 {
		return nil, fmt.Errorf("task: amount cannot be negative")
	}
	if input.Email != "" && !strings.Contains(input.Email, "@") {
		return nil, fmt.Errorf("task: invalid email format")
	}
	now := time.Now()
	record := &TaskRecord{
		ID:          s.nextID,
...
func (s *TaskService) GetByID(id int64) (*TaskRecord, error) {
	record, ok := s.records[id]
	if !ok {
		return nil, fmt.Errorf("task: record %d not found", id)
	}
	return record, nil
}

func (s *TaskService) Update(id int64, input *UpdateTaskInput) (*TaskRecord, error) {
	record, ok := s.records[id]
	if !ok {
		return nil, fmt.Errorf("task: record %d not found", id)
	}
...
func (s *TaskService) Delete(id int64) error {
	if _, ok := s.records[id]; !ok {
		return fmt.Errorf("task: record %d not found", id)
	}
	delete(s.records, id)
	return nil
}

func (s *TaskService) List(params *TaskSearchParams) ([]*TaskRecord, int) {
...
func (s *TaskService) BulkCreate(inputs []*CreateTaskInput) *TaskProcessResult {
	start := time.Now()
...
func (s *TaskService) Archive(id int64) error {
	record, ok := s.records[id]
	if !ok {
		return fmt.Errorf("task: record %d not found", id)
	}
	if record.Status == "archived" {
		return fmt.Errorf("task: record %d is already archived", id)
	}
...
func (s *TaskService) Restore(id int64) error {
	...
	if record.Status != "archived" {
		return fmt.Errorf("task: record %d is not archived", id)
	}
...
func (s *TaskService) Duplicate(id int64) (*TaskRecord, error) {
...
func (s *TaskService) CountByStatus(status string) int {
	count := 0
	for _, r := range s.records {
		if r.Status == status {
			count++
