...
type UserService struct {
	records map[int64]*UserRecord
	nextID  int64
}

func NewUserService() *UserService {
	return &UserService{
		records: make(map[int64]*UserRecord),
		nextID:  1,
	}
}

func (s *UserService) Create(input *CreateUserInput) (*UserRecord, error) {
	if strings.TrimSpace(input.Name) == "" {
		return nil, fmt.Errorf("user: name is required")
	}
	if input.Amount < 0 {
		return nil, fmt.Errorf("user: amount cannot be negative")
	}
...
func (s *UserService) GetByID(id int64) (*UserRecord, error) {
	record, ok := s.records[id]
	if !ok {
		return nil, fmt.Errorf("user: record %d not found", id)
	}
	return record, nil
}

func (s *UserService) Update(id int64, input *UpdateUserInput) (*UserRecord, error) {
	record, ok := s.records[id]
	if !ok {
		return nil, fmt.Errorf("user: record %d not found", id)
	}
...
func (s *UserService) Delete(id int64) error {
	if _, ok := s.records[id]; !ok {
		return fmt.Errorf("user: record %d not found", id)
	}
	delete(s.records, id)
	return nil
}

func (s *UserService) List(params *UserSearchParams) ([]*UserRecord, int) {
...
func (s *UserService) BulkCreate(inputs []*CreateUserInput) *UserProcessResult {
	start := time.Now()
	result := &UserProcessResult{}
...
func (s *UserService) Archive(id int64) error {
	record, ok := s.records[id]
	if !ok {
		return fmt.Errorf("user: record %d not found", id)
	}
	if record.Status == "archived" {
		return fmt.Errorf("user: record %d is already archived", id)
	}
...
func (s *UserService) Restore(id int64) error {
	...
	if record.Status != "archived" {
		return fmt.Errorf("user: record %d is not archived", id)
	}
...
func (s *UserService) Duplicate(id int64) (*UserRecord, error) {
...
func (s *UserService) CountByStatus(status string) int {
	count := 0
	for _, r := range s.records {
		if r.Status == status {
			count++
