...
type BudgetService struct {
	records map[int64]*BudgetRecord
	nextID  int64
}

func NewBudgetService() *BudgetService {
	return &BudgetService{
		records: make(map[int64]*BudgetRecord),
		nextID:  1,
	}
}

func (s *BudgetService) Create(input *CreateBudgetInput) (*BudgetRecord, error) {
	...
	if input.Email != "" && !strings.Contains(input.Email, "@") {
		return nil, fmt.Errorf("budget: invalid email format")
	}
	now := time.Now()
	record := &BudgetRecord{
		ID:          s.nextID,
...
func (s *BudgetService) GetByID(id int64) (*BudgetRecord, error) {
	record, ok := s.records[id]
	if !ok {
		return nil, fmt.Errorf("budget: record %d not found", id)
	}
	return record, nil
}

func (s *BudgetService) Update(id int64, input *UpdateBudgetInput) (*BudgetRecord, error) {
	record, ok := s.records[id]
	if !ok {
		return nil, fmt.Errorf("budget: record %d not found", id)
	}
...
func (s *BudgetService) Delete(id int64) error {
	if _, ok := s.records[id]; !ok {
		return fmt.Errorf("budget: record %d not found", id)
	}
	delete(s.records, id)
	return nil
}

func (s *BudgetService) List(params *BudgetSearchParams) ([]*BudgetRecord, int) {
...
func (s *BudgetService) BulkCreate(inputs []*CreateBudgetInput) *BudgetProcessResult {
	start := time.Now()
...
func (s *BudgetService) Archive(id int64) error {
	record, ok := s.records[id]
	if !ok {
		return fmt.Errorf("budget: record %d not found", id)
	}
	if record.Status == "archived" {
		return fmt.Errorf("budget: record %d is already archived", id)
	}
...
func (s *BudgetService) Restore(id int64) error {
	record, ok := s.records[id]
	if !ok {
		return fmt.Errorf("budget: record %d not found", id)
	}
	if record.Status != "archived" {
		return fmt.Errorf("budget: record %d is not archived", id)
	}
...
func (s *BudgetService) Duplicate(id int64) (*BudgetRecord, error) {
...
func (s *BudgetService) Count() int {
	return len(s.records)
}

func (s *BudgetService) CountByStatus(status string) int {
	count := 0
	for _, r := range s.records {
		if r.Status == status {
			count++
